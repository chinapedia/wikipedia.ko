...
[수학](https://ko.wikipedia.org/wiki/수학 "wikilink")에서, **초구면 좌표계**(Hyperspherical Coordinates)란 [구면좌표계](https://ko.wikipedia.org/wiki/구면좌표계 "wikilink")의 임의 차원 [유클리드 공간에](../Page/유클리드_공간.md "wikilink") 대한 일반화이다. 3차원보다 높은 차원의 문제에서는 가능한 좌표계의 수가 지나치게 많고 다루기도 복잡하여 보통 [직교 좌표계를](https://ko.wikipedia.org/wiki/직교_좌표계 "wikilink") 사용한다. 그러나 일반적으로 초구면 좌표계는 정의하기가 다른 고차원 좌표계들에 비해 상대적으로 쉬우며, 원점에 어느 정도의 대칭성을 가진 문제들은 직교 좌표계로 다루기가 오히려 복잡할 수 있으므로 이 좌표계는 직교 좌표계 다음으로 종종 사용된다. 특히 [초구체](https://ko.wikipedia.org/wiki/초구체 "wikilink")와 같은 특수한 경우들을 다룰 때에는 아주 유용하다.
