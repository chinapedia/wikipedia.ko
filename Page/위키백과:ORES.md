...
문제가 있는 편집은 [되돌려지고](https://ko.wikipedia.org/wiki/백:되돌리기_기능 "wikilink"), 문제가 없는 편집은 되돌려지지 않으리라는 추측을 기반으로, 위키에서 되돌려진 편집을 가져와 분석할 수 있습니다. 이 방식은 쉽고 빠르게 처리할 수 있지만, 모든 문서의 되돌림이 문서 훼손 때문은 아니므로 이 기능으로 감지할 수 있는 문서 훼손에는 제한이 있습니다. 이러한 문제를 해결하기 위해, ORES는 되돌려진 판의 내용에서 "나쁜 말"을 찾습니다.
