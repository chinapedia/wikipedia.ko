...
local p = {} local function checkPortalExists(portal)

`   return not (mw.title.makeTitle(100, portal).id == 0)`

end local getImageName = require( 'Module:Portal' ).image local yesno = require( 'Module:Yesno' )

local trackingInThisNamespace = false
...
`               if not (trackingInThisNamespace) then`
`                   nav:wikitext('`[`분류:깨진``   ``포털이``   ``있는``   ``포털``   ``틀`](https://ko.wikipedia.org/wiki/분류:깨진_포털이_있는_포털_틀 "wikilink")`') -- Category:Portal templates with redlinked portals`
`               end`
`               break`
`           end`
...
`       if #portals == 0 then`
`           if not (trackingInThisNamespace) then`
`               return '`[`분류:깨진``   ``포털이``   ``있는``   ``포털``   ``틀`](https://ko.wikipedia.org/wiki/분류:깨진_포털이_있는_포털_틀 "wikilink")`' -- Category:Portal templates with all redlinked portals`
`           else`
`               return ""`
`           end`
`       end`
...
`                   :tag( 'span' )`
`                       :css( 'margin', '0 0.5em' )`
`                       :wikitext( string.format( '`[`24x21px`](https://ko.wikipedia.org/wiki/파일:%s "wikilink")`', getImageName{ portal } ) )`
`                       :done()`
`                   :wikitext( string.format( '`[`%s``   ``포털`](https://ko.wikipedia.org/wiki/포털:%s "wikilink")`', portal, portal ) )`
`   end`
`   `
`   nav`
`       :node( list )`
...
`   -- passed explicitly, for example ``. The behaviour of ipairs is undefined if nil values are present, so we`
