...
[미적분학](https://ko.wikipedia.org/wiki/미적분학 "wikilink")에서, **헤세 행렬**(Hesse行列, )은 어떤 함수의 [이계도함수](https://ko.wikipedia.org/wiki/이계도함수 "wikilink")를 행렬로 표현한 것이다. 헤세 행렬은 독일의 수학자 [루트비히 오토 헤세의](../Page/루트비히_오토_헤세.md "wikilink") 이름을 따서 명명되었다. 헤세 행렬은 다변수함수가 극값을 가질 때, 그것이 극대인지, 극소인지 판정할 때 사용한다.
...
\end{bmatrix}\] 헤세 행렬은, 함수의 기울기 벡터 \(\nabla f\)에 대한 [야코비 행렬로도](https://ko.wikipedia.org/wiki/야코비_행렬 "wikilink") 설명이 가능하다.

함수 \(f\)의 이계도함수가 [연속이라면](https://ko.wikipedia.org/wiki/연속함수 "wikilink") 혼합 편미분은 같다. 그 때 이 행렬은 [대칭행렬](https://ko.wikipedia.org/wiki/대칭행렬 "wikilink")이다.

## 테일러 급수와 헤세 행렬

함수 \(f:U\sub\mathbb{R}^n\to\mathbb{R}\)의 \(n=2\)인 [테일러 급수는](https://ko.wikipedia.org/wiki/테일러_급수 "wikilink") 헤세 행렬을 이용해서 나타낼 수 있다.

\[\mathbf{h}\in\mathbb{R}^n\]에 대해 \(\Delta f:= f\left(\mathbf{x}_0+\mathbf{h}\right) -f\left(\mathbf{x}_0\right) \approx J\left(\mathbf{x}_0\right)\mathbf{h}+\frac{1}{2}\mathbf{h}^TH\left( f\right)\left(\mathbf{x}_0\right)\left(\mathbf{h}\right)\) (여기서 \(\mathbf{h}^T\)는 \(\mathbf{h}\)가 열벡터라고 할때 그 [전치행렬](https://ko.wikipedia.org/wiki/전치행렬 "wikilink")인 행벡터를 의미한다.) 만약 \(\mathbf{x}_0\)가 [임계점이라면](https://ko.wikipedia.org/wiki/임계점_\(수학\) "wikilink") \(\mathbf{D}f\left(\mathbf{x}_0\right) =0\)이므로 \(\mathbf{h}\in\mathbb{R}^n\)에 대해 \(\Delta f \approx \frac{1}{2}\mathbf{h}^TH\left( f\right)\left(\mathbf{x}_0\right)\left(\mathbf{h}\right)\) 이다. 즉, 상수가 아닌 가장 첫 번째 항이 바로 헤세 행렬이 되는 셈이다.
