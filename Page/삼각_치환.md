...
[미적분학](https://ko.wikipedia.org/wiki/미적분학 "wikilink")에서, **삼각 치환**(三角置換, )은 변수를 [삼각 함수로](https://ko.wikipedia.org/wiki/삼각_함수 "wikilink") [치환하여](https://ko.wikipedia.org/wiki/치환_적분 "wikilink") [적분](https://ko.wikipedia.org/wiki/적분 "wikilink")하는 기법이다.

## 정의

**삼각 치환**은 다음과 같은 꼴의 함수의 적분을 구하는 데 사용된다.\[1\]

\[R(x,\sqrt{a^2-x^2}),\;R(x,\sqrt{a^2+x^2}),\;R(x,\sqrt{x^2-a^2})\] 여기서 \(R\)는 [유리 함수이며](https://ko.wikipedia.org/wiki/유리_함수 "wikilink") \(a>0\)이다. 이는 \(R(x,\sqrt{ax^2+bx+c})\)의 완전 제곱꼴의 분류이다. 삼각 치환은 \(x\)를 새 변수에 대한 삼각 함수(의 상수배)로 치환한 뒤 삼각 항등식을 통해 제곱근식을 소거한다. 각 경우에 사용되는 치환은 다음과 같다.\[2\]\[3\]
...
새 변수 \(\theta\)의 범위를 각각 [아크사인](https://ko.wikipedia.org/wiki/아크사인 "wikilink"), [아크탄젠트](https://ko.wikipedia.org/wiki/아크탄젠트 "wikilink"), [아크시컨트](https://ko.wikipedia.org/wiki/아크시컨트 "wikilink")의 치역으로 정한 것은 각 치환을 [단사로](https://ko.wikipedia.org/wiki/단사_함수 "wikilink") 만들기 위함이다.\[4\] [쌍곡 치환은](https://ko.wikipedia.org/wiki/쌍곡_치환 "wikilink") 삼각 치환 대신에 쓰일 수 있다.\[5\]\[6\]
