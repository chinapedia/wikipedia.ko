...
Color Matrix 프리미티브는 [RGB](https://ko.wikipedia.org/wiki/RGB "wikilink") 값과 [알파값을](../Page/알파_채널.md "wikilink") 새로운 값으로 매핑하게 한다. 변환은 하단 행이 고정된 5x5 행렬로 설명되며 따라서 일반 변환은 5x4 행렬로 설명된다. 다섯 번째 열은 RGB 또는 알파와 독립적인 값을 추가하여 비선형색 보정을 가능하게 한다.  4 가지 유형의 변환이 정의되며 그 중 3 가지가 첫 번째 클래스의 특수 클래스이다.\[2\]\[3\]
