...
**사이트 간 요청 위조**(또는 **크로스 사이트 요청 위조**, , **CSRF**, **XSRF**)는 [웹사이트](https://ko.wikipedia.org/wiki/웹사이트 "wikilink") [취약점 공격의](https://ko.wikipedia.org/wiki/취약점_공격 "wikilink") 하나로, 사용자가 자신의 의지와는 무관하게 공격자가 의도한 행위(수정, 삭제, 등록 등)를 특정 웹사이트에 요청하게 하는 공격을 말한다.

유명 경매 사이트인 [옥션에서](../Page/옥션_\(웹사이트\).md "wikilink") 발생한 개인정보 유출 사건에서 사용된 공격 방식 중 하나다.

[사이트 간 스크립팅](../Page/사이트_간_스크립팅.md "wikilink")(XSS)을 이용한 공격이 사용자가 특정 웹사이트를 신용하는 점을 노린 것이라면, 사이트간 요청 위조는 특정 웹사이트가 사용자의 [웹 브라우저를](https://ko.wikipedia.org/wiki/웹_브라우저 "wikilink") 신용하는 상태를 노린 것이다. 일단 사용자가 웹사이트에 [로그인](../Page/로그인.md "wikilink")한 상태에서 사이트간 요청 위조 공격 코드가 삽입된 페이지를 열면, 공격 대상이 되는 웹사이트는 위조된 공격 명령이 믿을 수 있는 사용자로부터 발송된 것으로 판단하게 되어 공격에 노출된다.

## 공격 과정

1.  이용자는 웹사이트에 로그인하여 정상적인 [쿠키를](https://ko.wikipedia.org/wiki/HTTP_쿠키 "wikilink") 발급받는다
2.  공격자는 다음과 같은 [링크를](https://ko.wikipedia.org/wiki/하이퍼링크 "wikilink") 이메일이나 게시판 등의 경로를 통해 이용자에게 전달한다.
      -
        http://www.geocities.com/attacker
3.  공격용 [HTML](https://ko.wikipedia.org/wiki/HTML "wikilink") 페이지는 다음과 같은 이미지태그를 가진다.
      -
