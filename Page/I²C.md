...
I²C 는 [풀업 저항이](https://ko.wikipedia.org/wiki/풀업_저항 "wikilink") 연결된 직렬 데이터(SDA)와 직렬 클럭(SCL)이라는 두 개의 양 방향 [오픈 컬렉터](https://ko.wikipedia.org/wiki/오픈_컬렉터 "wikilink") 라인을 사용한다. 최대 전압은 +5 V이며, 일반적으로 +3.3 V 시스템이 사용되지만 다른 전압도 가능하다.
