...
주변 기기들은 늘 핵심 처리 장치들보다 속도가 훨씬 느렸다. 이는 특히 초기 [메인프레임](https://ko.wikipedia.org/wiki/메인프레임 "wikilink")에게 심각한 문제였다. 이를테면, 천공 카드나 인쇄된 출력물을 직접 읽는 잡(job)이 느린 기계 장치들의 속도에 맞추어 강제 실행되었다. IBM의 "스풀 시스템"(7070-IO-076)과 같은 최초의 스풀링 프로그램들은 데이터를 천공 카드로부터 자기 테이프로 복사하거나 테이프에서 거꾸로 천공 카드 및 프린터로 복사하였다. 훨씬 더 빠르면서 [랜덤 액세스를](https://ko.wikipedia.org/wiki/랜덤_액세스 "wikilink") 지원하는 [하드 디스크들은](https://ko.wikipedia.org/wiki/하드_디스크_드라이브 "wikilink") 1960년대 중반에 자기 테이프를 대체하기 시작했으며 1970년대까지 이러한 테이프 이용은 근절되었다.

1960년대 초 IBM 메인프레임의 유닛 레코드 장비가 너무 느렸기 때문에 스풀링 대신 1401과 같은 소형 오프라인 머신을 사용하는 것이 일반적이었다.

스풀(spool)이라는 용어는 Simultaneous Peripheral Operations On-Line(SPOOL) 소프트웨어에서 기원한 것으로 짐작된다.\[1\] 어원 자체는 확실하지 않다. *Simultaneous peripheral operations on-line*는 [배크로님](https://ko.wikipedia.org/wiki/배크로님 "wikilink")일 수 있다.\[2\] 다른 설명으로는 이것이 자기 테이프의 스풀, 즉 [릴](https://ko.wikipedia.org/wiki/릴 "wikilink")을 의미하는 것이기도 하다.
