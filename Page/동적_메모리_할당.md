...
**동적 메모리 할당** 또는 **메모리 동적 할당**은 [컴퓨터 프로그래밍에서](https://ko.wikipedia.org/wiki/컴퓨터_프로그래밍 "wikilink") [실행 시간](https://ko.wikipedia.org/wiki/실행_시간_\(컴퓨터_프로그래밍\) "wikilink") 동안 사용할 [메모리](https://ko.wikipedia.org/wiki/메모리 "wikilink") 공간을 할당하는 것을 말한다. 사용이 끝나면 운영체제가 쓸 수 있도록 반납하고 다음에 요구가 오면 재 할당을 받을 수 있다. 이것은 프로그램이 실행하는 순간 프로그램이 사용할 메모리 크기를 고려하여 메모리의 할당이 이루어지는 [정적 메모리 할당과](https://ko.wikipedia.org/wiki/정적_메모리_할당 "wikilink") 대조적이다.

동적으로 할당된 메모리 공간은 프로그래머가 명시적으로 해제하거나 [쓰레기 수집이](../Page/쓰레기_수집.md "wikilink") 일어나기 전 까지 그대로 유지된다. C/C++와 같이 쓰레기 수집이 없는 언어의 경우, 동적 할당하면 사용자가 해제하기 전까지는 메모리 공간이 계속 유지된다. 동적 할당은 [프로세스](../Page/프로세스.md "wikilink")의 [힙영역에서](https://ko.wikipedia.org/wiki/힙_\(프로그래밍\) "wikilink") 할당하므로 프로세스가 종료되면 [운영 체제에](https://ko.wikipedia.org/wiki/운영_체제 "wikilink") 메모리 리소스가 반납되므로 해제된다. 그러나 프로세스가 계속 실행될 때에는 동적할당 된 영역은 유지되므로 프로그램이 정해진 힙 영역의 크기를 넘는 메모리 할당을 요구하면 할당되지 않는다. 따라서 사용이 완료된 영역은 반납하는 것이 유리한데, 프로그래머가 함수를 사용해서 해제해야 한다.

동적 할당은 [함수가](../Page/함수_\(프로그래밍\).md "wikilink") 종료되거나 [변수 영역을](../Page/변수_영역.md "wikilink") 벗어나면 자동으로 공간 해제가 이루어지는 [스택](https://ko.wikipedia.org/wiki/스택 "wikilink")을 사용한 [자동 변수와](../Page/자동_변수.md "wikilink") 대조적이다. 프로세스의 [정적 메모리 할당은](https://ko.wikipedia.org/wiki/정적_메모리_할당 "wikilink") 프로세스가 시작할 때 이미 정해진 메모리량으로 한정되어있기 때문에, 프로세스가 시작할 때부터 끝날 때까지 유지되는데 반해, 동적 할당은 프로세스의 실행 과정 중에 필요한 메모리를 운영체제에 요구해 할당받고 해제하는 것이 가능하다.

## 용어

### 힙 영역

**힙**()은 [C 언어나](https://ko.wikipedia.org/wiki/C_언어 "wikilink") [자바와](https://ko.wikipedia.org/wiki/자바_\(프로그래밍_언어\) "wikilink") 같은 프로그래밍 환경에서 [원시 자료형이](https://ko.wikipedia.org/wiki/원시_자료형 "wikilink") 아닌 보다 큰 크기의 데이터를 담고자 동적으로 할당하는 메모리 공간을 지칭한다. 프로그램 코드에서 원하는 크기의 메모리 할당을 요청하면 힙을 관리하는 라이브러리 혹은 모듈이 지정된 크기의 힙 공간 안에서 사용 가능한 곳을 찾아 그곳을 다른 [스레드](https://ko.wikipedia.org/wiki/스레드 "wikilink")나 프로그램이 사용하지 못하도록 예약 상태로 만들고 접근 가능한 [핸들](https://ko.wikipedia.org/wiki/핸들 "wikilink")이나 [포인터를](https://ko.wikipedia.org/wiki/포인터_\(프로그래밍\) "wikilink") 반환하는 식으로 사용할 수 있다.

물론 [구조체](https://ko.wikipedia.org/wiki/구조체 "wikilink")나 [객체를](https://ko.wikipedia.org/wiki/객체_\(컴퓨터_과학\) "wikilink") [스택](https://ko.wikipedia.org/wiki/스택 "wikilink")에 선언하여 사용하는 것도 가능하지만, [실행 시간에](https://ko.wikipedia.org/wiki/실행_시간_\(컴퓨터_프로그래밍\) "wikilink") 크기가 결정되는 동적 [배열](https://ko.wikipedia.org/wiki/배열 "wikilink") 및 리스트와 같은 경우는 힙을 사용하는 것이 보다 공간을 효율적으로 활용할 수 있다. 대신에 [C 언어와](https://ko.wikipedia.org/wiki/C_언어 "wikilink") 같은 경우 사용한 공간을 명시적으로 반환해주어야 하므로 이 과정을 빠뜨림으로써 [메모리 누수와](../Page/메모리_누수.md "wikilink") 같은 버그의 원인이 되기도 한다. [메모리 관리를](../Page/메모리_관리.md "wikilink") 자동으로 해 주는 언어도 있지만, 이 경우 [쓰레기 수집기로](../Page/쓰레기_수집_\(컴퓨터_과학\).md "wikilink") 인해 성능이 저하되는 경우도 있다.
...
언어 차원에서 new, delete 연산자를 통해 동적 할당을 제공한다. 재할당 연산자는 현재까지 없으며, 여전히 C 함수를 사용할 수 있다. [new와 delete](https://ko.wikipedia.org/wiki/new와_delete_\(C++\) "wikilink") 연산자는 내부적으로 malloc()과 free()와 유사한 함수를 사용하여 [메모리 맵상의](https://ko.wikipedia.org/wiki/메모리_맵 "wikilink") 힙영역으로부터 저장공간을 할당한다.
...
## 함께 보기
  ...
  - [정적 메모리 할당](https://ko.wikipedia.org/wiki/정적_메모리_할당 "wikilink")
