...
표준 [유닉스](https://ko.wikipedia.org/wiki/유닉스 "wikilink") 명령어 **`who`**는 현재 컴퓨터에 로그인 한 사용자 목록을 보여준다.
...
다른 유닉스와 [유닉스 계열](../Page/유닉스_계열.md "wikilink") 운영 체제는 추가 옵션을 가지고 있을 수 있다. [GNU](https://ko.wikipedia.org/wiki/GNU "wikilink") `who`는 등록된 사용자가 메시지를 받았는지 여부를 보여주는 `-u`와 `-w`옵션과 유사함을 지닌 `-i`옵션을 포함한다. ((SUS는 `-T`옵션이 실행될때 이것을 보여준다.), 그러나 [GNU](https://ko.wikipedia.org/wiki/GNU "wikilink") `who`와 [BSD](https://ko.wikipedia.org/wiki/BSD "wikilink") `who` 둘다 다수의 위 옵션들(`-a`, `-b`, `-d`, 나머지들)을 허용한다.; 대신 [GNU](https://ko.wikipedia.org/wiki/GNU "wikilink") `who`는 등록된 hostname상의 DNS lookup들을 실행하기 위해 `-l`옵션을 사용한다.
...
## 매뉴얼 페이지
  ...
  - [`who`](http://www.gnu.org/software/coreutils/manual/html_node/who-invocation.html) — [GNU](https://ko.wikipedia.org/wiki/GNU "wikilink") [coreutils](https://ko.wikipedia.org/wiki/coreutils "wikilink")의 매뉴얼
