...
**펄 모듈**(Perl module)은 [펄](https://ko.wikipedia.org/wiki/펄 "wikilink") 언어의 소프트웨어 구성 요소이다.

모듈은 마치 [자바 패키지처럼](https://ko.wikipedia.org/wiki/자바_패키지 "wikilink") 소스 코드가 *CGI*, *Net:FTP*, *XML::Parser*와 같은 [이름공간](https://ko.wikipedia.org/wiki/이름공간 "wikilink")을 정의하기 위한 펄 매커니즘인 패키지 안에 있을 수 있도록 정의한다. 즉, 파일 구조는 이름공간 구조를 미러링한다. (예: Net::FTP를 위한 [소스 코드는](https://ko.wikipedia.org/wiki/소스_코드 "wikilink") Net/FTP.pm에 속한다)

## 예제

["Hello, World\!"의](https://ko.wikipedia.org/wiki/"Hello,_World!"_프로그램 "wikilink") 예는 여러 스타일로 구현할 수 있다. 펄에서는 모듈이 반드시 필요한 것이 아니다. 함수와 코드는 어디에서든 정의하여 사용할 수 있다. 아래는 단지 예를 들기 위한 부분이다. 클래스가 꼭 필요한 [자바와](https://ko.wikipedia.org/wiki/자바_\(프로그래밍_언어\) "wikilink") 다르다. "Hello, World"의 함수는 다음과 같이 작성할 수 있다:
...
## 같이 보기
  ...
  - [CPAN](https://ko.wikipedia.org/wiki/CPAN "wikilink")
