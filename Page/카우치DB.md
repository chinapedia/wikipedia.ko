...
**아파치 카우치DB**(Apache CouchDB)는 스케일러블 아키텍처를 쉽게 이용하고 보유하는데 초점을 둔 [오픈 소스](https://ko.wikipedia.org/wiki/오픈_소스_소프트웨어 "wikilink") 데이터베이스 소프트웨어이다. [도큐먼트 지향](https://ko.wikipedia.org/wiki/도큐먼트_지향_데이터베이스 "wikilink") [NoSQL](../Page/NoSQL.md "wikilink") 데이터베이스 구조를 갖추고 있으며 [얼랭](https://ko.wikipedia.org/wiki/얼랭 "wikilink")으로 구현되어 있다. [JSON](https://ko.wikipedia.org/wiki/JSON "wikilink")을 사용하여 데이터를 저장하고, [자바스크립트](https://ko.wikipedia.org/wiki/자바스크립트 "wikilink")를 쿼리 언어로 사용([맵리듀스](https://ko.wikipedia.org/wiki/맵리듀스 "wikilink") 사용)하며 [API](https://ko.wikipedia.org/wiki/API "wikilink")를 위해 [HTTP](https://ko.wikipedia.org/wiki/HTTP "wikilink")를 사용한다.\[1\]

카우치DB는 2005년에 첫 출시되었으며 나중에 2008년에 [아파치 소프트웨어 재단](https://ko.wikipedia.org/wiki/아파치_소프트웨어_재단 "wikilink") 프로젝트로 되었다.
...
그 밖의 기능들로는 [결과 무결성을](https://ko.wikipedia.org/wiki/결과_무결성 "wikilink") 포함한 도큐먼트 수준의 [ACID](https://ko.wikipedia.org/wiki/ACID "wikilink") 시맨틱, (증강) [맵리듀스](https://ko.wikipedia.org/wiki/맵리듀스 "wikilink"), (증강) 복제를 포함한다. 카우치DB의 독특한 기능 중 하나는 [멀티 마스터 복제](https://ko.wikipedia.org/wiki/멀티_마스터_복제 "wikilink")(multi-master replication)이며 여러 머신들로 하여금 스케일할 수 있게 함으로써 고성능 시스템을 구축할 수 있게 한다. Fauxton(과거 이름은 Futon)라는 이름의 내장된 웹 애플리케이션은 관리를 돕는다.
