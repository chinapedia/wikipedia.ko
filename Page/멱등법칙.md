...
**멱등법칙**(冪等法則) 또는 **멱등성**(冪等性, )은 [수학](https://ko.wikipedia.org/wiki/수학 "wikilink")이나 [전산학](https://ko.wikipedia.org/wiki/전산학 "wikilink")에서 연산의 한 성질을 나타내는 것으로, 연산을 여러 번 적용하더라도 결과가 달라지지 않는 성질을 의미한다. 멱등법칙의 개념은 [추상대수학](https://ko.wikipedia.org/wiki/추상대수학 "wikilink")(특히, [사영작용소](../Page/사영작용소.md "wikilink")·[폐포연산자](https://ko.wikipedia.org/wiki/폐포연산자 "wikilink") 이론)과 [함수형 프로그래밍](https://ko.wikipedia.org/wiki/함수형_프로그래밍 "wikilink")([참조 투명성의](../Page/참조_투명성.md "wikilink") 성질과 관련된)의 여러 부분에서 사용하고 있다.

멱등성의 개념은 적용되는 곳에 따라 여러 의미를 가진다.

  - 어떤 [단항연산](https://ko.wikipedia.org/wiki/단항연산 "wikilink")(또는 [함수](https://ko.wikipedia.org/wiki/함수 "wikilink"))은, 어느 값에라도 두 번 적용되었을 때, 한 번 적용했을 때와 같은 결과를 주는 경우, 즉 인 경우 멱등법칙을 만족한다고 한다. 예를 들어, [절댓값](https://ko.wikipedia.org/wiki/절댓값 "wikilink") 함수는 멱등법칙을 만족한다: .
  ...
  - 어떤 이항 연산이 주어지고, 두 같은 값을 피연산자로 할 때 그 값이 결과로 나오는 경우 그 값을 이 연산에 대한 [멱등원](../Page/멱등원.md "wikilink")이라고 한다. 예를 들어, 수 1은 [곱셈](https://ko.wikipedia.org/wiki/곱셈 "wikilink")의 멱등원이다: .
...
[항등 함수](https://ko.wikipedia.org/wiki/항등_함수 "wikilink")
...
[벡터 공간](https://ko.wikipedia.org/wiki/벡터_공간 "wikilink") 위의 [사영작용소](../Page/사영작용소.md "wikilink")는 멱등 함수의 중요한 부류이다. 3차원 공간의 점을 *xy*-평면으로 투영시키는 함수 *π<sub>xy</sub>*(*x*, *y*, *z*) = (*x*, *y*, 0)이 그 예이다.
...
여기서 \(\textstyle{n \choose k}\)는 [조합](https://ko.wikipedia.org/wiki/조합 "wikilink")으로, *n* 원소 집합에서 *k* 개의 부동점을 고른 것이다. *k<sup>n-k</sup>*은, *n* 원소 집합을 부동점들의 집합 *A*와 아닌 원소들의 집합 *B*로 [분할했을](../Page/집합의_분할.md "wikilink") 때 *B*에서 *A*로 가는 함수의 총수이다.
...
멱등원의 예로, ★의 [항등원](https://ko.wikipedia.org/wiki/항등원 "wikilink")이 존재한다면, 그는 멱등원이다.
...
[합집합](https://ko.wikipedia.org/wiki/합집합 "wikilink")·[교집합](https://ko.wikipedia.org/wiki/교집합 "wikilink") 연산, [논리곱](https://ko.wikipedia.org/wiki/논리곱 "wikilink")·[논리합](https://ko.wikipedia.org/wiki/논리합 "wikilink") 연산, 그리고 일반적으로 [격자의](../Page/격자_\(순서론\).md "wikilink") [이음과](https://ko.wikipedia.org/wiki/이음_\(수학\) "wikilink") [만남](https://ko.wikipedia.org/wiki/만남_\(수학\) "wikilink") 연산은 모두 이항연산으로서 멱등성을 갖는다.
...
[실수](https://ko.wikipedia.org/wiki/실수 "wikilink") 또는 [복소수](https://ko.wikipedia.org/wiki/복소수 "wikilink") 변수의 [절댓값](https://ko.wikipedia.org/wiki/절댓값 "wikilink") 함수, 실변수의 [바닥 함수는](https://ko.wikipedia.org/wiki/바닥_함수 "wikilink") 모두 멱등 함수이다.

위상 공간 *X*의 부분집합 *U*를 *U*의 [폐포로](https://ko.wikipedia.org/wiki/폐포_\(위상수학\) "wikilink") 대응시키는 함수는 *X*의 [멱집합](../Page/멱집합.md "wikilink") *P*(*X*)에 정의된 함수로 멱등성을 가진다. 이러한 함수는 [폐포연산](../Page/폐포연산.md "wikilink")의 한 예이다, 모든 폐포연산은 멱등성을 만족한다.

[통계학](https://ko.wikipedia.org/wiki/통계학 "wikilink")에서의 [분산](https://ko.wikipedia.org/wiki/분산 "wikilink")과 비슷한 다음과 같은 함수는 멱등성을 만족한다.
...
[선형대수학](https://ko.wikipedia.org/wiki/선형대수학 "wikilink")에서 [사영작용소](../Page/사영작용소.md "wikilink")는 멱등법칙을 만족한다. 벡터 공간의 사영작용소들은 벡터 공간 위의 [선형변환](https://ko.wikipedia.org/wiki/선형변환 "wikilink")이 이루는 환의 멱등원이다.
