> This article is converted from Wikipedia: [모듈:Chessboard](https://ko.wikipedia.org/wiki/모듈:Chessboard).
...
`   return string.format( '`[`%dx%dpx`](https://ko.wikipedia.org/wiki/File:Chess_%s%st45.svg "fig:%dx%dpx")`', piece, color, size, size, alt, alt )`
...
`       :wikitext(string.format( '`[`%dx%dpx`](https://ko.wikipedia.org/wiki/File:Chessboard480.svg "fig:%dx%dpx")`', 8 * size, 8 * size ))`
`   `
`   for trow = 1,8 do`
