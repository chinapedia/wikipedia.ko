...
[섬네일](https://ko.wikipedia.org/wiki/파일:Manhattan_distance.svg "wikilink") **맨해튼 거리**(Manhattan distance, 혹은 **택시 거리**, ***L*<sub>1</sub> 거리**, **시가지 거리**,Taxicab geometry)는 [19세기](https://ko.wikipedia.org/wiki/19세기 "wikilink")의 수학자 [헤르만 민코프스키가](https://ko.wikipedia.org/wiki/헤르만_민코프스키 "wikilink") 고안한 용어로, 보통 [유클리드 기하학의](https://ko.wikipedia.org/wiki/유클리드_기하학 "wikilink") [거리 공간을](https://ko.wikipedia.org/wiki/거리_공간 "wikilink") 좌표에 표시된 두 점 사이의 거리(절댓값)의 차이에 따른 새로운 거리 공간으로 대신하기도 한다.

## 정의

맨해튼 거리는 \(d_1\)과 벡터 \(\mathbf{p}, \mathbf{q}\) 사이에 차원 [실수](https://ko.wikipedia.org/wiki/실수 "wikilink")를 [직교 좌표계에](https://ko.wikipedia.org/wiki/직교_좌표계 "wikilink") 일정한 좌표축의 점 위에 투영한 [선분](https://ko.wikipedia.org/wiki/선분 "wikilink") 길이의 합을 말하는데, 이를 공식으로 표현하면 다음과 같다.
...
예를 들어 [평면](../Page/평면.md "wikilink") 위의 맨해튼 거리가 \((p_1,p_2)\)과 \((q_1,q_2)\) 사이이면 \(| p_1 - q_1 | + | p_2 - q_2 |\)이다. 맨해튼 거리는 좌표계의 회전에 의존하지만, 좌표의 축을 [반사하거나](../Page/반사_\(기하학\).md "wikilink") [평행이동](https://ko.wikipedia.org/wiki/평행이동 "wikilink")을 하는 경우는 그렇지 않다. 맨해튼 거리는 [SAS 합동](https://ko.wikipedia.org/wiki/합동_\(기하학\) "wikilink") (두 개의 변과 그 사이의 각이 같은 두 개의 삼각형을 만들 수 있으나, 합동이 아니다.)인 경우를 제외하면 모든 [힐베르트 공리계](../Page/힐베르트_공리계.md "wikilink") ([유클리드 기하학의](https://ko.wikipedia.org/wiki/유클리드_기하학 "wikilink") 의식화)와 일치한다.
