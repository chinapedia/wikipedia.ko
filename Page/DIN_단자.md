> This article is converted from Wikipedia: [DIN ](https://ko.wikipedia.org/wiki/DIN_).


[섬네일](https://ko.wikipedia.org/wiki/파일:5pin180dinplug.jpg "wikilink") 자판의 5핀 180도 DIN 단자 (수).\]\] **DIN 단자**는 독일의 국가 표준 단체 [독일 공업 규격 위원회](https://ko.wikipedia.org/wiki/독일_공업_규격_위원회 "wikilink")(DIN)가 표준화한 전기적 단자이다. 각기 다른 수많은 단자들을 위한 DIN 표준들이 있으므로 "DIN 단자" 그 자체만 기술하면 정확하지 않다. 일부 DIN 단자 표준들은 다음을 포함한다:

  - DIN 41524: 소리 신호에 주로 쓰이는 원형 단자
  - [DIN 41612](https://ko.wikipedia.org/wiki/DIN_41612 "wikilink"): 뒷면이나 메인보드의 플러그인 카드 연결에 쓰이는 직사각형 단자
  - DIN 41652: 컴퓨터 데이터 및 비디오를 위한 [D-sub](../Page/D-sub.md "wikilink") 단자

## 원형 단자

[60px](https://ko.wikipedia.org/wiki/파일:DIN-3_Diagram.svg "wikilink") [60px](https://ko.wikipedia.org/wiki/파일:DIN-4_Diagram.svg "wikilink") [60px](https://ko.wikipedia.org/wiki/파일:DIN-5b_Diagram.svg "wikilink") [60px](https://ko.wikipedia.org/wiki/파일:DIN-5c_Diagram.svg "wikilink") [60px](https://ko.wikipedia.org/wiki/파일:DIN-5_Diagram.svg "wikilink") [60px](https://ko.wikipedia.org/wiki/파일:DIN-6_Diagram.svg "wikilink") [60px](https://ko.wikipedia.org/wiki/파일:DIN-7_Diagram.svg "wikilink") [60px](https://ko.wikipedia.org/wiki/파일:DIN-8_Diagram.svg "wikilink") [60px](https://ko.wikipedia.org/wiki/파일:DIN-8_262_Diagram.svg "wikilink")

## 응용

### 아날로그 오디오

| 응용      | 단자    | 핀 기능                                                                         |
| ------- | ----- | ---------------------------------------------------------------------------- |
| 1       | 4     | 2                                                                            |
| 증폭기     | 모노폰   | [60px](https://ko.wikipedia.org/wiki/파일:DIN-5_Diagram.svg "wikilink") 5/180° |
| 스테레오폰   | 좌측 출력 | 우측 출력                                                                        |
| 테이프 레코더 | 모노폰   | 오디오 입력                                                                       |
| 스테레오폰   | 좌측 입력 | 우측 입력                                                                        |

## 같이 보기

  - [PS/2 단자](https://ko.wikipedia.org/wiki/PS/2_단자 "wikilink")
  - [XLR 단자](../Page/XLR_단자.md "wikilink")

## 참고문헌

  - [DIN Connectors](http://www.electronics2000.co.uk/pin-out/dincon.php)
  - IEC 60574-3: Audiovisual, video and television equipment and systems — Part 3: Specification for connectors for the interconnection of equipment in audiovisual systems.

[분류:시청각 연결 단자](https://ko.wikipedia.org/wiki/분류:시청각_연결_단자 "wikilink")