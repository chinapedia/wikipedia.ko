...
[수론에서의](https://ko.wikipedia.org/wiki/정수론 "wikilink") **뫼비우스 반전 공식**(Möbius inversion formula)은 19세기 수학자 [아우구스트 페르디난트 뫼비우스의](https://ko.wikipedia.org/wiki/아우구스트_페르디난트_뫼비우스 "wikilink") 이름을 딴 공식이다.
...
\[f(n)=\sum_{d\mid n}g(d)\mu(n/d)\] 여기서 \(\mu\)는 [뫼비우스 함수](https://ko.wikipedia.org/wiki/뫼비우스_함수 "wikilink")(Möbius function)이고, 덧셈은 *n*의 양의 약수 *d* 전체에 대해 이루어진다.

수론적 함수 \(g\)는 \(f\)의 누적으로 이루어지는데, 역으로 \(g\)를 통해 \(f\)를 꺼내는 공식이므로 **반전** 공식이라 불린다.

*f*와 *g*가 자연수에서 어떤 [아벨 군으로의](https://ko.wikipedia.org/wiki/아벨_군 "wikilink") 함수일 때에도 공식은 성립한다.

## 디리클레 합성곱과의 관계

[디리클레 합성곱](https://ko.wikipedia.org/wiki/디리클레_합성곱 "wikilink")(Dirichlet convolution)을 사용하여 공식을 써 보면 다음과 같다.
