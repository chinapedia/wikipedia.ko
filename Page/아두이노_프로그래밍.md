...
**[아두이노](https://ko.wikipedia.org/wiki/아두이노 "wikilink")**(Arduino)는 [마이크로컨트롤러](https://ko.wikipedia.org/wiki/마이크로컨트롤러 "wikilink")이므로 원하는 기능을 먼저 설정하고 이에 맞는 프로그래밍을 통해 기능을 구현한다. 사용 언어는 C/C++을 사용하고 [컴파일러](https://ko.wikipedia.org/wiki/컴파일러 "wikilink") 및 [라이브러리](https://ko.wikipedia.org/wiki/라이브러리 "wikilink")는 [AVR-GCC](https://ko.wikipedia.org/wiki/AVR-GCC "wikilink")을 사용한다.\[1\]

아두이노는 공식적으로 오픈소스로 이러한 [통합개발환경](https://ko.wikipedia.org/wiki/통합개발환경 "wikilink")(IDE)인 [아두이노 소프트웨어를](https://ko.wikipedia.org/wiki/아두이노_IDE "wikilink") 제작하여 무료로 배포하고있다.
...
2.  듀티값 (value) : 펄스파의 HIGH와 LOW의 시간 비이다. 최댓값이 8비트 타이머는 비교레지스터도 8비트를 사용하므로 0 \~255 이다. 0 은 항상 LOW가 출력되고 이 값의 비에 따라 펄스폭이 결정된다.
...
### 외부 [인터럽트](https://ko.wikipedia.org/wiki/인터럽트 "wikilink") 함수

  - attachInterrupt(interrupt, function, mode)\[6\] : 외부 인터럽트가 발생했을 때, 처리를 하기위해 설정하는 함수이다. 여기서 function은 인터럽트가 발생했을 때 기능을 처리하기 위한 [인터럽트 핸들러이다](https://ko.wikipedia.org/wiki/인터럽트_핸들러 "wikilink").
...
2.  function : [인터럽트 핸들러로](https://ko.wikipedia.org/wiki/인터럽트_핸들러 "wikilink") void 타입의 함수로 정의해야 한다. 인터럽트가 걸리면 이 함수가 자동 호출된다.
...
### [인터럽트](https://ko.wikipedia.org/wiki/인터럽트 "wikilink") 함수
...
마이크로컨트롤러의 포트를 조작할 때, 하나의 핀을 사용할 경우 입출력 모드 함수로 설정하고 입출력 함수를 데이터를 입출력 할 수 있다. 그러나 포트를 동시에 한 묶음으로 조작 하거나 분산된 입출력 이라면 [마이크로컨트롤러](https://ko.wikipedia.org/wiki/마이크로컨트롤러 "wikilink")의 레지스터를 직접 조작하여 사용할 수 있다.\[8\]
...
AVR-gcc는 gcc를 Atmel AVR적용하기 위해 만들어 졌다. 일반 컴퓨터와 마이크로컨트롤러의 개발 특성이 다른 부분이 있기 때문에, AVR 특성에 맞는 몇가지가 추가되거나 변형 되었다.\[9\] 각 마이크로컨트롤러의 레지스터, 사용하는 마이크로프로세서 체계 등에 적합하도록 변형 및 추가 되었다. 예를 들어 사용하는 AVR의 SFR 레지스터가 정의되어 있다.
...
AVR을 기반으로 하는 아두이노 프로그래밍은 AVR-gcc를 아두이노 [통합 개발 환경에](https://ko.wikipedia.org/wiki/통합_개발_환경 "wikilink") 결합하여, 개발 환경을 설치하면 별도의 추가 설치없이 사용할 수 있다.
...
` sei(); //  CPU는 인터럽트 요청을 허용하여 `[`인터럽트``   ``핸들러를`](https://ko.wikipedia.org/wiki/인터럽트_핸들러 "wikilink")` 실행한다.`

delayMicroseconds()나 시리얼 통신는 인터럽트의 cli()에 의한 비활성화에 영향을 받는다.

### 시간 지연

delayMicroseconds()는 가장 작은 시간 지연이 2μs 정도이다. 이것보다 짧은 시간을 지연할 때는 [어셈블리어](https://ko.wikipedia.org/wiki/어셈블리어 "wikilink")의 'NOP' 명령을 사용할 수 있다. 'NOP' 명령 실행에 걸리는 시간은 16 Mhz을 사용할 경우 62.5ns 정도의 지연 시간을 얻을 수 있다.
...
이미 언급 했드시, AVR 포트 제어 시, 레지스터를 직접 제어하여 빠른 포트 제어를 실현 할 수 있다.
...
## 같이 보기
  ...
  - [아두이노 라이브러리](https://ko.wikipedia.org/wiki/아두이노_라이브러리 "wikilink")
...
4.
5.  [analogReference()함수](https://www.arduino.cc/reference/ko/language/functions/analog-io/analogreference/)
6.
