...
**XpressEngine**(eXpress+press+Engine, 구 [제로보드](https://ko.wikipedia.org/wiki/제로보드 "wikilink") XE)은 고영수가 여러 자원 봉사자들과 함께 개발한 LGPL 기반 오픈 프로젝트로, [제로보드](https://ko.wikipedia.org/wiki/제로보드 "wikilink") 4나 zb5와는 별개로 완전히 새로 개발한 웹 프레임워크이다. 제로보드 4와는 달리 BBS, 블로그, 쇼핑몰, 위키 등 웹 사이트에 필요한 모든 것을 모듈로 구현해, 종합적인 웹 빌더로 사용할 수 있는 프레임워크를 목표로 개발이 진행 중이다. 이전 명칭은 '[제로보드](https://ko.wikipedia.org/wiki/제로보드 "wikilink") XE'였으나, 정식으로 CMS 기능을 갖춘 1.1.0 버전 안내를 공지하면서 '보드'의 개념과 상이하다며 명칭을 변경하였다. 현재는 네이버 산하 오픈소스 프로젝트로 개발이 진행되고 있다.

[제로보드](https://ko.wikipedia.org/wiki/제로보드 "wikilink")의 명칭을 유지한 1.0.6 버전까지는 다양한 사이트와 블로그를 운영할 수 있는 홈빌더의 형태였으나, XpressEngine으로 명칭을 변경한 1.1.0 버전부터는 마이크로 블로그(플래닛) 패키지, 가상 사이트를 이용한 분양 시스템 등 더 폭넓고 다양한 기능이 탑재되어 있다.
...
호환성이나 개발 난이도 때문에 그누보드와 비교되지만, [HHVM](https://ko.wikipedia.org/wiki/HHVM "wikilink")이나 [라라벨](../Page/라라벨.md "wikilink") 같은 최신 PHP 프레임워크를 적극적으로 받아들이고 있으며, MVC 패턴 적용, RESTful API, 프론트엔드의 컴포넌트화 등등 최신 웹개발 트렌드를 따라잡는 행보는 높이 평가할만 하다.
...
## 같이 보기
  ...
  - [그누보드](https://ko.wikipedia.org/wiki/그누보드 "wikilink")
