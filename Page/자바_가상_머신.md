...
자바 바이트코드는 주로 [자바7로](../Page/자바_\(프로그래밍_언어\).md "wikilink") 작성된 [소스 코드를](../Page/소스_코드.md "wikilink") 컴파일하여 생성하며 현재 자바 가상 머신의 구조는 자바 언어의 구조를 거의 일대일로 반영하고 있다. 하지만 [스칼라](../Page/스칼라_\(프로그래밍_언어\).md "wikilink") 같은 다른 컴파일 언어도 자바 바이트코드를 생성할 수 있으며, 자바로 작성된 [자바스크립트](https://ko.wikipedia.org/wiki/자바스크립트 "wikilink") 엔진인 [라이노](https://ko.wikipedia.org/wiki/라이노_\(프로그래밍_언어\) "wikilink")() 같은 경우는 옵션에 따라 자바 가상 머신 위에서 [인터프리터](../Page/인터프리터.md "wikilink")를 실행하기도 하고 자바 바이트코드를 동적으로 생성하기도 한다. 특히 자바 가상 머신 위에서 동적인 [스크립트 언어가](../Page/스크립트_언어.md "wikilink") 널리 쓰이게 되면서 [자바(프로그래밍 언어)에서는](https://ko.wikipedia.org/wiki/자바_7 "wikilink") 동적 타입 언어()를 지원하기 위한 추가 명령과 API가 추가되었다.\[5\] 이러한 다양한 언어 구현이 가능한 것은 다음 특성 때문이라고 할 수 있다.
