> This article is converted from Wikipedia: [모듈:ParameterCount](https://ko.wikipedia.org/wiki/모듈:ParameterCount).
...
\-- Trim a string local function trim(s)

`   return s:match('^%s*(.-)%s*$')`

end

\-- Test whether a string is blank local function isBlank(s)
