...
**PyPy**는 유연함과 쉬운 실험을 위해 [파이썬](../Page/파이썬.md "wikilink") 프로그래밍 언어 자체로 작성된 파이썬 구현체이다. 이 프로젝트의 목적 중 하나는 최적화된 PyPy의 파이썬 구현이 현재의 [C](../Page/C_\(프로그래밍_언어\).md "wikilink") 구현보다 빠르도록 하는 것이다. PyPy 자체는 파이썬 언어의 부분집합인 [RPython](../Page/RPython.md "wikilink")으로 구현되어 있다.

PyPy는 파이썬 코드를 [기계어](../Page/기계어.md "wikilink")나 다른 [저급 언어로](https://ko.wikipedia.org/wiki/저급_언어 "wikilink") 자동 번역하는 [저스트 인 타임 컴파일러](https://ko.wikipedia.org/wiki/저스트_인_타임_컴파일러 "wikilink") 기능을 포함하고 있는데, 특이한 점은 JIT 컴파일러 자체가 JIT 컴파일러 생성기로부터 — PyPy RPython 코드를 분석하여 — 자동생성된다는 점이다.
