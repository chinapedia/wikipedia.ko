...
**저장 프로시저** 또는 **스토어드 프로시저**(stored procedure)는 일련의 쿼리를 마치 하나의 함수처럼 실행하기 위한 쿼리의 집합이다. [데이터베이스](https://ko.wikipedia.org/wiki/데이터베이스 "wikilink")에 대한 일련의 작업을 정리한 절차를 [관계형 데이터베이스 관리 시스템에](https://ko.wikipedia.org/wiki/관계형_데이터베이스_관리_시스템 "wikilink") 저장한(지속성) 것으로, **영구저장모듈**(Persistent Storage Module)이라고도 불린다.

## 개요

데이터베이스 언어 표준 [SQL](https://ko.wikipedia.org/wiki/SQL "wikilink")에서는 SQL / PSM 기준으로 책정되어 있다. 벤더(제조사) 각사 모두 정적, 동적 SQL에 커서 처리 및 제어 구문, 예외 처리 등을 포함한 사양의 확장 언어로 절차를 설명할 수 있는 [DBMS](https://ko.wikipedia.org/wiki/DBMS "wikilink")를 제공하는 경우가 많다. 또한 [C 언어로](https://ko.wikipedia.org/wiki/C_언어 "wikilink") 작성된 [컴파일](https://ko.wikipedia.org/wiki/컴파일 "wikilink")한 외부 모듈(공유 라이브러리) 및 Java 클래스 라이브러리에서 함수나 [클래스 메소드를](https://ko.wikipedia.org/wiki/클래스_메소드 "wikilink") 호출하는 것으로 실현하는 ‘외부 프로시저’ 기능을 구현하는 것도 있다. 저장프로시저를 사용하여 다음과 같은 장점이 있다.
...
다음은 PL/SQL로 구현한 오라클 [저장 프로시저의](https://ko.wikipedia.org/wiki/저장_프로시저 "wikilink") 예이다.
