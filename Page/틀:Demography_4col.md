> This article is converted from Wikipedia: [틀:Demography 4col](https://ko.wikipedia.org/wiki/틀:Demography_4col).


<onlyinclude>{| border="0" width= cellspacing="2" cellpadding="2" | colspan="4" bgcolor="\#eeeeee" align="center" | <strong>부터 까지 <span style="color:#0000D0;"></span>의 인구 변화</strong> |- bgcolor="\#eeeeee" | align="center" |<strong></strong> | align="center" |<strong></strong> | align="center" |<strong></strong> | align="center" |<strong></strong>

|- bgcolor="\#eeeeee" | align="center" | | align="center" | | align="center" | | align="center" | |- | |}</onlyinclude>

**이 틀의 번역판**은 인구 데이터를 다른 언어판들 사이에 옮기기 쉽게 하기 위해 만들어졌습니다.

이 틀의 첫 번째 버전은 프랑스어로 만들어졌습니다. [:fr:Modèle:Demography 4col](https://ko.wikipedia.org/wiki/:fr:Modèle:Demography_4col "wikilink")

이 틀은 도시나 지방의 인구 변화 모습을 나타내는 4칸짜리 표입니다.

## 사용법

이 틀을 다음과 같이 크기(510px 대신), 연도, 그리고 데이터를 인자로 주어 사용하십시오 :

`{{Demography 4col|510px|[[1975|1975]]|[[1982|1982]]|[[1990|1990]]|[[1999|1999]]`
`|3,363|3,457|3,898|3,748}}`

## 다른 틀 선언

[틀:Demography 4col](../Page/틀:Demography_4col.md "wikilink") | [틀:Demography 5col](../Page/틀:Demography_5col.md "wikilink") | [틀:Demography 6col](../Page/틀:Demography_6col.md "wikilink") | [틀:Demography 7col](../Page/틀:Demography_7col.md "wikilink") | [틀:Demography 12col](../Page/틀:Demography_12col.md "wikilink")

[](https://ko.wikipedia.org/wiki/분류:인구에_관한_틀 "wikilink")

<noinclude> [es:Plantilla:Evolución demográfica](https://ko.wikipedia.org/wiki/es:Plantilla:Evolución_demográfica "wikilink") </noinclude>