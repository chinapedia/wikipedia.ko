...
[이차 형식](https://ko.wikipedia.org/wiki/이차_형식 "wikilink") 이론에서, **비트 환**(Witt環, )은 [비퇴화 이차 형식의](https://ko.wikipedia.org/wiki/비퇴화_이차_형식 "wikilink") [동치류](https://ko.wikipedia.org/wiki/동치류 "wikilink")로 구성된 [가환환](https://ko.wikipedia.org/wiki/가환환 "wikilink")이다.

## 정의

### 비트 분해

**비트 분해 정리**(Witt分解定理, )에 따르면, 표수가 2가 아닌 체 \(K\) 위의 유한 차원 벡터 공간 \(V\) 위의 [이차 형식](https://ko.wikipedia.org/wiki/이차_형식 "wikilink") \(Q\)는 다음과 같은 꼴로 표준적으로 분해된다.
...
이 경우 \((V_1,Q_1)\)을 \((V,Q)\)의 **핵심**(核心, )이라고 한다. 또한, \(\dim_K(V_1\oplus V_2)\)를 \(Q\)의 **계수**(階數, )라고 하며, \((\dim_KV_2)/2\)를 \(Q\)의 **비트 지표**()라고 한다.\[1\] 비트 정리에 따라, \(Q|_W=0\)이 되는 부분 벡터 공간들의 포함 관계에 대한 [부분 순서 집합에서](https://ko.wikipedia.org/wiki/부분_순서_집합 "wikilink"), [극대 원소들의](https://ko.wikipedia.org/wiki/극대_원소 "wikilink") 차원은 항상 비트 지표와 같다.

### 비트 환

같은 체 위의 두 이차 공간 \((V,Q)\), \((V',Q')\)의 핵심이 서로 동형이라면, 두 이차 공간이 서로 **비트 동치**()라고 한다. [표수가](https://ko.wikipedia.org/wiki/체의_표수 "wikilink") 2가 아닌 체 \(K\) 위의 비퇴화 유한 차원 이차 공간들의 비트 [동치류](https://ko.wikipedia.org/wiki/동치류 "wikilink")들의 집합 \(\operatorname{Witt}(K)\)을 생각하자. 여기에 다음과 같은 연산을 부여하면, 이는 [가환환](https://ko.wikipedia.org/wiki/가환환 "wikilink")을 이룬다.

  - \((V,Q)+(V',Q')=(V\oplus V',Q\oplus Q')\). \(\oplus\)는 벡터 공간(및 그 위의 함수)의 [직합](https://ko.wikipedia.org/wiki/직합 "wikilink")이다.
  - \(-(V,Q)=(V,-Q)\)
  - \(0=(K^0,0)\). 여기서 \(K^0\)는 \(K\) 위의 0차원 벡터 공간이다.
  - \((V,Q)\cdot(V',Q')=(V\otimes V',Q\otimes Q')\) \(\otimes\)는 벡터 공간 (및 그 위의 함수)의 [텐서곱](https://ko.wikipedia.org/wiki/텐서곱 "wikilink")이다.
  - \(1=(K^1,x\mapsto x^2)\)
...
\[(\dim\bmod2)\colon\dim\colon\operatorname{Witt}(K)\to\mathbb Z/(2)\] 을 갖는다. (곱셈 항등원은 홀수 계수이므로, 이는 \(\mathbb Z/2\)-[등급환](https://ko.wikipedia.org/wiki/등급환 "wikilink")을 이루지 않는다.) 이 준동형의 [핵](https://ko.wikipedia.org/wiki/핵_\(수학\) "wikilink") \(\mathfrak i(K)=\ker(\dim\bmod2)\subseteq\operatorname{Witt}(K)\)을 비트 환의 **기본 아이디얼**()이라고 한다.

표수가 2가 아닌 체 위의 [비퇴화 이차 형식](https://ko.wikipedia.org/wiki/비퇴화_이차_형식 "wikilink") \(Q\)의 **행렬식**() 또는 **판별식**() \(\det Q\in K^\times/(K^\times)^2\)은 \(Q\)를 나타내는 [대칭 행렬](https://ko.wikipedia.org/wiki/대칭_행렬 "wikilink") \(Q(x)=x^\top Mx\)의 [행렬식](https://ko.wikipedia.org/wiki/행렬식 "wikilink") \(\det M\)의 \(K^\times/(K^\times)^2\)에서의 [동치류](https://ko.wikipedia.org/wiki/동치류 "wikilink")이다. 이 경우, 사용하는 [기저를](https://ko.wikipedia.org/wiki/기저_\(선형대수학\) "wikilink") [가역 행렬](https://ko.wikipedia.org/wiki/가역_행렬 "wikilink") \(A\)를 통해 바꾼다면

\[AM'A=M\]

\[(\det M')(\det A)^2=\det M\] 가 되므로, 비퇴화 이차 형식의 행렬식은 \(K^\times/(K^\times)^2\)의 원소로서 잘 정의된다.

표수가 2가 아닌 체 \(K\)에 대하여, 다음과 같은 [가환환](https://ko.wikipedia.org/wiki/가환환 "wikilink") \(\operatorname{GrQExt}(K)\)를 정의하자.
...
\[(V,Q)\mapsto(\det Q,\dim_KV\bmod2)\] 이는 [전사 함수이며](https://ko.wikipedia.org/wiki/전사_함수 "wikilink"), 그 [핵은](https://ko.wikipedia.org/wiki/핵_\(수학\) "wikilink") 기본 아이디얼의 제곱이다.\[3\]

### 하세-비트 불변량

표수가 2가 아닌 체 \(K\) 위의 \(n\)차원 벡터 공간 \(V\) 위의 대각화된 비퇴화 이차 형식 \(Q=\operatorname{diag}(a_1,a_2,\dots,a_n)\)가 주어졌을 때, [사원수형 대수](https://ko.wikipedia.org/wiki/사원수형_대수 "wikilink") (2차원 벡터 공간 위의 [클리퍼드 대수](https://ko.wikipedia.org/wiki/클리퍼드_대수 "wikilink")) \(\left(\tfrac{a_i,a_j}K\right)\)들은 (짝수 차원이므로) [중심 단순 대수를](https://ko.wikipedia.org/wiki/중심_단순_대수 "wikilink") 이루며, 따라서 [브라우어 군](https://ko.wikipedia.org/wiki/브라우어_군 "wikilink") \(\operatorname{Br}(K)\)의 원소들의 대표원들을 이룬다. \(Q\)의 **하세-비트 불변량**()은 이 브라우어 군 원소들의 합이다.
...
[표수가](https://ko.wikipedia.org/wiki/체의_표수 "wikilink") 2가 아닌 체 \(K\)의 비트 환 \(\operatorname{Witt}(K)\)의 기본 아이디얼 \(\mathfrak i(K)\subseteq\operatorname{Witt}(K)\)의 거듭제곱들은 [하강 여과를](https://ko.wikipedia.org/wiki/여과_\(수학\) "wikilink") 이룬다.
...
체 \(K\) 위의 **피스터 이차 형식**()은 다음과 같은 꼴의, \(2^n\)차원 [벡터 공간](https://ko.wikipedia.org/wiki/벡터_공간 "wikilink") 위의 [이차 형식이다](https://ko.wikipedia.org/wiki/이차_형식 "wikilink").
...
\[\operatorname K^{\operatorname M}(K)=\frac{\operatorname T(K^\times;\mathbb Z)}{(a\otimes(1-a))_{a\in K\setminus\{0,1\}}}\] 의 원소를 \(\{a_1,a_2,\dots,a_n\}\in\operatorname K^{\operatorname M}_n(K)\)로 표기하자. 그렇다면, 피스터 형식을 통해 밀너 환에서 위 등급환으로 가는 [등급환](https://ko.wikipedia.org/wiki/등급환 "wikilink") [준동형](https://ko.wikipedia.org/wiki/준동형 "wikilink")을 정의할 수 있다.
...
**이차 형식에 대한 밀너 추측**()에 따르면, 이 준동형은 등급환의 [동형](https://ko.wikipedia.org/wiki/동형 "wikilink")을 이룬다. 이는 [존 밀너가](https://ko.wikipedia.org/wiki/존_밀너 "wikilink") 추측하였으며,\[5\] 2007년에 드미트리 오를로프() · 알렉산드르 비시크() · [블라디미르 보예보츠키가](https://ko.wikipedia.org/wiki/블라디미르_보예보츠키 "wikilink") 증명하였다.\[6\]
...
[대수적으로 닫힌 체의](https://ko.wikipedia.org/wiki/대수적으로_닫힌_체 "wikilink") [브라우어 군은](https://ko.wikipedia.org/wiki/브라우어_군 "wikilink") 자명하므로, 이 경우 하세-비트 불변량 역시 자명하다.
...
[실수체](https://ko.wikipedia.org/wiki/실수체 "wikilink")의 [브라우어 군은](https://ko.wikipedia.org/wiki/브라우어_군 "wikilink") [실수체](https://ko.wikipedia.org/wiki/실수체 "wikilink")와 [사원수환](https://ko.wikipedia.org/wiki/사원수환 "wikilink") \(\mathbb H\)로 구성되며, 2차 [순환군](https://ko.wikipedia.org/wiki/순환군 "wikilink")이다.
...
\[\operatorname{Witt}(K)\cong\begin{cases}\left(\mathbb Z/(2)\right)[\operatorname{Cyc}(2)\oplus\operatorname{Cyc}(2)]&q\equiv1\pmod4\\\left((\mathbb Z/(4)\right)[\operatorname{Cyc}(2)]&q\equiv3\pmod4\end{cases}\] 여기서 \(\operatorname{Cyc}(2)\)는 2차 [순환군](https://ko.wikipedia.org/wiki/순환군 "wikilink")이며, \(R[G]\)는 [군환](https://ko.wikipedia.org/wiki/군환 "wikilink")을 뜻한다.
