...
공식을 그대로 적용하면 O(*N*<sup>2</sup>)의 연산이 필요하지만, [고속 푸리에 변환](https://ko.wikipedia.org/wiki/고속_푸리에_변환 "wikilink")(FFT)과 마찬가지로 계산 과정을 분해하여 O(*N* log *N*)만큼의 연산으로 계산할 수도 있다. O(*N*)만큼의 전처리 및 후처리 과정을 통해 DCT를 FFT로 변환하여 계산할 수도 있다.
