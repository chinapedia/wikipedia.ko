...
\-- Stringifies lists with "and" or "or" function p.andList (...) return conjList("및", ...) end function p.orList (...) return conjList("또는", ...) end

-----

\-- For see -- -- Makes a "For X, see [Y](https://ko.wikipedia.org/wiki/Y "wikilink")." list from raw parameters. Intended for the --  and  templates and their variants.
...
\-- Produces a "For X, see [Y](https://ko.wikipedia.org/wiki/Y "wikilink")" string from arguments. Expects index gaps -- but not blank/whitespace values. Ignores named args and args \< "from". function p._forSee (args, from, options)
