> This article is converted from Wikipedia: [미디어위키:Gadget-charinsert-core.js](https://ko.wikipedia.org/wiki/미디어위키:Gadget-charinsert-core.js).


/\*\*

`* Copied from `[`mw:User:Alex``   ``Smotrov/edittools.js`](https://ko.wikipedia.org/wiki/mw:User:Alex_Smotrov/edittools.js "wikilink")`, modified for use on the English Wikipedia.`
`*`
`* Configuration (to be set from `[`Special:MyPage/common.js`](https://ko.wikipedia.org/wiki/Special:MyPage/common.js "wikilink")`):`
...
`       // Please make any changes to `[`MediaWiki:Edittools`](https://ko.wikipedia.org/wiki/MediaWiki:Edittools "wikilink")` as well, however, instead of using the ␥ symbol, use {{#ifeq:``|``| | }}.`
`       charinsert: {`
`           '기본': '· …  – — ―  | [+] [\[+]] {\{+}}  〈+〉 《+》 ‘+’ “+”  € † ← → ° ′ ″  ≈ ≒ ≠ ≤ ≥ ± − × ÷ ← → · `<sup>`+`</sup>` `<sub>`+`</sub>`  `\(\)`     `<u>`+`</u>` `<s>`+`</s>`  `\[1\]` `

<references />

{\\{풀기:+}} [분류:+](https://ko.wikipedia.org/wiki/분류:+ "wikilink") \#넘겨주기.\[\\\[+\]\]  {\\{토막글|+}} {\\{IPA|+}} {\\{lang||+}} {\\{llang||+}}',

`           '위키문법': '삽입:  · … – — ° ′ ″ ≈ ≠ ≤ ≥ ± − × ÷ ← → · § ␥~~\~~ `\[2\]`  위키문법:  {\{+}}  {\{\{+}}}  |  [+]  [\[+]]  [\[분류:+]]  #넘겨주기.[\[+]]     `<u>`+`</u>`  `<s>`+`</s>`  `<sup>`+`</sup>`  `<sub>`+`</sub>`  +  `
...
\<ref.name="+"_/\> {\\{\#tag:ref|+|group="nb"|name=""}} {\\{각주}} \<references./\> {\\{lang||+}} {\\{llang||+}} <onlyinclude>+</onlyinclude> <includeonly>+</includeonly> <noinclude>+</noinclude> {\\{기본정렬:+}} + \<span.class="plainlinks"\>+</span> {\\{풀기:+}} {\\{첫가끝|+}}',

`           '기호': '~ | ¡¿†‡↔↑↓•¶#∞  〈+〉 《+》  ‘+’ “+” ‹+› «+»  ¤₳฿₵¢₡₢$₫₯€₠₣ƒ₴₭₤ℳ₥₦№₧₰£៛₨₪৳₮₩¥  ♠♣♥♦  ♭♯♮  ©®™ ◌',`
`           '로마자': 'A a Á á À à Â â Ä ä Ǎ ǎ Ă ă Ā ā Ã ã Å å Ą ą Æ æ Ǣ ǣ  B b  C c Ć ć Ċ ċ Ĉ ĉ Č č Ç ç  D d Ď ď Đ đ Ḍ ḍ Ð ð  E e É é È è Ė ė Ê ê Ë ë Ě ě Ĕ ĕ Ē ē Ẽ ẽ Ę ę Ẹ ẹ Ɛ ɛ Ǝ 