...
**`crt0`** (또는 **`c0`**)는 C로 작성된 프로그램의 메인 함수를 호출하기 전에 필요한 초기화를 수행하는 루틴(execution startup routines)의 집합으로 프로그램에 링크되어 있다. 이것은 보통 `crt0.o`라고 불리는 [목적 파일](../Page/목적_파일.md "wikilink") 형태로, 흔히 [어셈블리어](../Page/어셈블리어.md "wikilink")로 작성되며, 빌드 시에 [링커에](../Page/링커_\(컴퓨팅\).md "wikilink") 의해 자동으로 [실행 파일에](../Page/실행_파일.md "wikilink") 포함된다.\[1\]

`crt0`는 [런타임 라이브러리의](../Page/런타임_라이브러리.md "wikilink") 가장 기본적인 부분을 포함한다. 정확히 하는 일은 프로그램의 컴파일러, 운영체제 그리고 [C 표준 라이브러리의](../Page/C_표준_라이브러리.md "wikilink") 구현에 달려 있다.\[2\] 환경과 [툴체인](https://ko.wikipedia.org/wiki/툴체인 "wikilink")에 필요한 초기화 작업 외에도, crt0는 C++의 전역 [생성자](https://ko.wikipedia.org/wiki/생성자 "wikilink")나 [GCC의](../Page/GNU_컴파일러_모음.md "wikilink") ((constructor)) 속성을 포함하는 C 함수를 실행하는 것 같이 프로그래머에 의해 정의된 추가적인 작업을 할 수 있다.\[3\]\[4\]
