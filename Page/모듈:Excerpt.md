> This article is converted from Wikipedia: [모듈:Excerpt](https://ko.wikipedia.org/wiki/모듈:Excerpt).
...
\-- Initialise this big table once to save time local unwantedTemplates = {"\[Ee\]fn", "\[Ee\]fn%-\[lu\]\[arg\]", "\[Ee\]l\[mn\]", "\[Rr\]p?", "\[Ss\]fn\[bmp\]", "\[Ss\]f\[bn\]", "NoteTag", "\#\[Tt\]ag:%s\*\[Rr\]ef", "\[Rr\]efn?",
...
\-- Help gsub to remove unwanted templates and pseudo-templates such as \#tag:ref and DEFAULTSORT local function striptemplate(t)
...
`   -- Replace `` by its text parameter: `` → Bar`
`   noref = mw.ustring.gsub(noref, "^{{%s*[Aa]udio.-|.-|(.-)%f[|}].*", "%1")`

`   -- Replace `` by its text parameter: `` → English`
...
\-- Check image for suitability local function checkimage(image)
...
\-- Attempt to parse [File:...](https://ko.wikipedia.org/wiki/File:... "fig:File:...") or [Image:...](https://ko.wikipedia.org/wiki/File:... "fig:Image:..."), either anywhere (start=false) or at the start only (start=true) local function parseimage(text, start)
...
`   if image then`
`       image = mw.ustring.match(image, "%b[]%s*") -- matching `[`...`](https://ko.wikipedia.org/wiki/... "wikilink")` to handle wikilinks nested in caption`
`   end`
`   return image`
...
\-- Attempt to construct a [File:...](https://ko.wikipedia.org/wiki/File:... "fig:File:...") block from  local function argimage(text)
...
`       if caption then`
`           -- extend caption to parse "| caption = Foo `` Bar\n"`
...
`           -- altText is terminated by }} or |, but first skip any matched `[`...`](https://ko.wikipedia.org/wiki/... "wikilink")` and {{...}}`
...
`            mw.ustring.match(altText, "()|", lookfrom) or len+1)`
`           altText = mw.ustring.sub(altText, 1, aftertext-1) -- chop off |... or }}... which is not part of `[`...`](https://ko.wikipedia.org/wiki/... "wikilink")` or {{...}}`
...
`       local token = parseimage(image, true) -- look for image=`[`File:...`](https://ko.wikipedia.org/wiki/File:... "fig:File:...")` etc.`
`       if not token then`
...
`   local filetext = "" -- output text with concatenated `[`...`](https://ko.wikipedia.org/wiki/File:Foo "fig:...")`\n entries`
...
`       local token = mw.ustring.match(text, "^%b{}%s*") or false -- `
`       if not leadstart and not token then token = mw.ustring.match(text, "^%b<>%s*%b{}%s*") end -- allow `<tag>` before lead has started`
...
`               if not images then`
`                   local image = parseimage(token, false) -- look for embedded `[`File:...`](https://ko.wikipedia.org/wiki/File:... "fig:File:...")`, |image=, etc.`
...
\-- Remove unmatched <tag> or </tag> tags local function fixtags(text, tag)
...
`           pagename = mw.ustring.match(pagename, "%[%[%s*(.-)[%]|]") or pagename -- "`[`Bar`](https://ko.wikipedia.org/wiki/Foo "wikilink")`" → "Foo"`
...
`       text = mw.ustring.sub(text, 1, pos + 2) .. "`[`"``   ``..``   ``mw.ustring.sub(text,``   ``pos``   ``+``   ``3,``   ``pos``   ``+``   ``len``   ``+``   ``2)``   ``..``   ``"`](https://ko.wikipedia.org/wiki/"_.._mw.ustring.sub\(text,_pos_+_3,_pos_+_len_+_2\)_.._" "wikilink")`" .. mw.ustring.sub(text, pos + len + 3, -1) -- link it`
...
`               return "`**[`"``   ``..``   ``b``   ``..``   ``"`](https://ko.wikipedia.org/wiki/"_.._pagename_.._" "wikilink")**`" -- replace `**`Foo`**` by `**[`Foo`](https://ko.wikipedia.org/wiki/pagename "wikilink")**
`           else`
...
`   repeat -- hide matched ``s including nested templates`
`       local t = text`
`       text = mw.ustring.gsub(text, "{(%b{})}", "\27{\27%1\27}\27"); -- `` → E{Esometemplate}E}E where E represents escape`
`       text = mw.ustring.gsub(text, "(< *math[^>]*>[^<]-)}}(.-< */math *>)", "%1}\27}\27%2"); -- `\(\{sqrt\{hat{x}}\)` → `\(\{sqrt\{hat{x}E}E\)
`   until text == t`
`   repeat -- do similar for `[`wikilink`](https://ko.wikipedia.org/wiki/wikilink "wikilink")s
`       local t = text`
`       text = mw.ustring.gsub(text, "%[(%b[])%]", "\27[\27%1\27]\27");`
`   until text == t`

`   text = text.gsub(text, "([{}%[%]])%1[^\27].*", ""); -- remove unmatched {{, }}, `[`or`](https://ko.wikipedia.org/wiki/or "wikilink")` and everything thereafter, avoiding ]E]E etc.`
`   text = text.gsub(text, "([{}%[%]])%1$", ""); -- remove unmatched {{, }}, `[`or`](https://ko.wikipedia.org/wiki/or "wikilink")` at end of text`
`   text = mw.ustring.gsub(text, "\27", ""); -- unhide matched pairs: E{E{ → {{, ]E]E → ]], etc.`

`   -- Ensure div tags match`
`   text = fixtags(text, "div")`

`   if options.more then text = text .. " `**[`"``   ``..``   ``options.more``   ``..``   ``"`](https://ko.wikipedia.org/wiki/"_.._pagename_.._" "wikilink")**`" end -- wikilink to article for more info`
`   return text`
...
`       text = mw.ustring.gsub(text, "{{%s*[Aa]nnotated[ _]link%s*|%s*(.-)%s*}}", "`[`%1`](https://ko.wikipedia.org/wiki/%1 "wikilink")`")`
`       if func == "linked" then`
