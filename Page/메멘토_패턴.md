> This article is converted from Wikipedia: [메멘토 패턴](https://ko.wikipedia.org/wiki/메멘토_패턴).


**메멘토 패턴**(memento pattern)은 객체를 이전 상태로 되돌릴 수 있는 기능을 제공하는 [소프트웨어 디자인 패턴이다](https://ko.wikipedia.org/wiki/소프트웨어_디자인_패턴 "wikilink"). (롤백을 통한 [실행 취소](../Page/실행_취소.md "wikilink"))
...
메멘토 패턴의 전형적인 예로 [유사난수](../Page/유사난수.md "wikilink")의 시드(seed), [유한 상태 기계의](../Page/유한_상태_기계.md "wikilink") 상태(state)를 포함한다.

## 개요

메멘토\[1\] 디자인 패턴은 유연하고 재사용 가능한 객체 지향 소프트웨어를 설계하기 위해 반복되는 디자인 문제를 해결하는 방법, 즉 객체는 구현, 변경, 테스트, 재사용이 쉬워야 한다는 것을 기술하는, 잘 알려진 23가지 [GoF 디자인 패턴들](../Page/디자인_패턴_\(책\).md "wikilink") 중 하나이다. 메멘토 패턴은 초기 HP 제품을 위해 Noah Thompson과 Dr.Drew Clinkenbeard에 의해 제작되었다.
...
다음 [Java](../Page/자바_\(프로그래밍_언어\).md "wikilink") 프로그램은 메멘토 패턴의 "undo" 사용을 보여준다.
...
## 외부 링크
  ...
  - [Description of Memento Pattern](http://adapower.com/index.php?Command=Class&ClassID=Patterns&CID=271) in [Ada](../Page/에이다_\(프로그래밍_언어\).md "wikilink")
  - [Memento UML Class Diagram](https://web.archive.org/web/20140724185307/http://www.dofactory.com/Patterns/PatternMemento.aspx) with C\# and .NET code samples
