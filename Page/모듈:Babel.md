> This article is converted from Wikipedia: [모듈:Babel](https://ko.wikipedia.org/wiki/모듈:Babel).
...
`   end`
`   local args = getArgs(frame, {wrappers = '틀:바벨'})`
...
`                   ['text-align'] = 'center' })`
`           :attr('colspan',col_span)`
`           :wikitext( args.footer or '`[`언어별``   ``사용자``   ``찾기`](https://ko.wikipedia.org/wiki/:분류:언어별_위키백과_사용자 "wikilink")`' )`
`           :done()`
