...
[환론](https://ko.wikipedia.org/wiki/환론 "wikilink")과 [모노이드](https://ko.wikipedia.org/wiki/모노이드 "wikilink") 이론에서, **멱등원**(蓂等元, )은 거듭제곱하여도 변하지 않는 원소이다.

## 정의

### 범주의 멱등 사상

[범주](https://ko.wikipedia.org/wiki/범주_\(수학\) "wikilink") \(\mathcal C\)의 [자기 사상](https://ko.wikipedia.org/wiki/자기_사상 "wikilink") \(e\colon X\to X\)가 \(e\circ e=e\)를 만족시킨다면, \(e\)를 \(\mathcal C\)의 **멱등 사상**()이라고 한다.

만약 \(e=g\circ f\)이며 \(f\circ g=\operatorname{id}_Y\)가 되는 사상 \(f\colon X\to Y\), \(g\colon Y\to X\)가 존재한다면, \(e\)를 **분할 멱등 사상**()이라고 한다.

\(\mathcal C\)의 **카루비 껍질**() \(\operatorname{Split}(\mathcal C)\)는 다음과 같은 [범주이다](https://ko.wikipedia.org/wiki/범주_\(수학\) "wikilink").

  - \(\operatorname{Split}(\mathcal C)\)의 대상 \((X,e)\)는 \(\mathcal C\)의 대상 \(X\in\mathcal C\)과 \(X\) 위의 멱등 사상 \(e\colon X\to X\)의 [순서쌍](https://ko.wikipedia.org/wiki/순서쌍 "wikilink")이다.
...
\[f\mapsto f\] 가 존재한다. 또한, [준층](https://ko.wikipedia.org/wiki/준층 "wikilink") [범주의 동치](https://ko.wikipedia.org/wiki/범주의_동치 "wikilink")
...
[모노이드](https://ko.wikipedia.org/wiki/모노이드 "wikilink") \((R,\cdot)\)의 원소 \(e\in R\)가 \(e^2=e\cdot e=1\)을 만족시킨다면, \(e\)를 \(R\)의 **멱등원**이라고 한다.

[모노이드](https://ko.wikipedia.org/wiki/모노이드 "wikilink") \(R\)의 멱등원들만으로 구성된 집합 \(E\subseteq R\)에서, 만약

\[ee'=e'e\qquad\forall e\in E\] 가 성립한다면, \(E\)가 **직교 멱등원 집합**()이라고 한다.

모든 원소가 멱등원인 모노이드를 **멱등 모노이드**()라고 하며, 그 [모임은](https://ko.wikipedia.org/wiki/모임_\(수학\) "wikilink") [대수 구조 다양체를](../Page/대수_구조_다양체.md "wikilink") 이룬다. \(n\)개의 원소로 생성되는 [자유](../Page/자유_대상.md "wikilink") 멱등 모노이드의 [집합의 크기는](https://ko.wikipedia.org/wiki/집합의_크기 "wikilink") 다음과 같다.

\[\sum_{k=0}^n\binom nk\prod_{i=1}^k(k-i+1)^{2^i}\]

### 환의 멱등원

[환은](https://ko.wikipedia.org/wiki/환_\(수학\) "wikilink") 곱셈 [모노이드](https://ko.wikipedia.org/wiki/모노이드 "wikilink")를 이루며, 환의 멱등원이란 곱셈에 대한 멱등원을 뜻한다.

환 \(R\)의 멱등원 \(e\in R\)에 대하여 다음 세 조건이 서로 [동치](https://ko.wikipedia.org/wiki/동치 "wikilink")이며, 이를 만족시키는 멱등원을 **원시 멱등원**()이라고 한다.
  ...
  - [환](https://ko.wikipedia.org/wiki/환_\(수학\) "wikilink") \(eRe\)의 모든 멱등원은 0 또는 1이다.
  - \(e=e_1+e_2\)이자 \(e_1e_2=e_1e_2\), \(e_1\ne0\ne e_2\)인 멱등원 \(e_1,e_2\in R\)가 존재하지 않는다.

환 \(R\)의 멱등원 \(e\in R\)에 대하여 다음 세 조건이 서로 [동치](https://ko.wikipedia.org/wiki/동치 "wikilink")이며, 이를 만족시키는 멱등원을 **국소 멱등원**()이라고 한다.
