...
**와니어 함수**는 [고체물리학](https://ko.wikipedia.org/wiki/고체물리학 "wikilink")에서 이용되는 완전집합 함수로 물리학자 그레고리 와니어가 도입하였다. 고체에서 서로 다른 격자에 해당하는 와니어 함수는 서로 수직을 이룬다. 와니어 함수는 고체물리학에서 다양하게 이용되고 있는데, 예를 들면 전자의 결합력을 분석하는데 쓰일 수 있다. 부도체에서의 와니어 함수는 공간적으로 국소화된 형태를 가진다는 것이 증명되었다.
