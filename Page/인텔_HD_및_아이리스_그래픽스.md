> This article is converted from Wikipedia: [인텔 HD 및 아이리스 그래픽스](https://ko.wikipedia.org/wiki/인텔_HD_및_아이리스_그래픽스).


[섬네일](https://ko.wikipedia.org/wiki/파일:Intel_core_i5-2500_top_IMGP9336_wp.jpg "wikilink") **인텔 HD 그래픽스**(Intel HD Graphics)는 [2010년](../Page/2010년.md "wikilink")에 발표된 [인텔](../Page/인텔.md "wikilink")의 내장 [GPU](../Page/그래픽_처리_장치.md "wikilink") 시리즈이다. [마더보드](https://ko.wikipedia.org/wiki/마더보드 "wikilink")에 그래픽 프로세서가 탑재된 종전의 [인텔 GMA와](../Page/인텔_GMA.md "wikilink") 달리, 인텔 HD 그래픽스는 [CPU와](../Page/중앙_처리_장치.md "wikilink") 같은 다이에 탑재되어 [APU를](https://ko.wikipedia.org/wiki/가속_처리_장치 "wikilink") 이룬다.

**인텔 아이리스 그래픽스**(Intel Iris Graphics)와 **인텔 아이리스 프로 그래픽스**(Intel Iris Pro Graphics)는 2013년에 고성능 버전의 HD 그래픽스로서 [해스웰](https://ko.wikipedia.org/wiki/해스웰_\(마이크로아키텍처\) "wikilink") 프로세서의 일부 모델에 도입되었다. 아이리스 프로의 경우 [임베디드 DRAM을](https://ko.wikipedia.org/wiki/eDRAM "wikilink") 포함한 첫 시리즈이다.\[1\]

## 세대

### 웨스트미어

  - HD 그래픽스

### 샌디브리지

| 모델 번호            | 티어  | 실행 유닛 | 클럭 속도 (MHz) | GFLOPS |
| ---------------- | --- | ----- | ----------- | ------ |
| HD Graphics      | GT1 | 6     | 1100        |        |
| HD Graphics 2000 | GT1 | 6     | 1350        | 64.8   |
| HD Graphics 3000 | GT2 | 12    | 1350        | 129.6  |

### 아이비브리지

| 모델 번호            | 티어  | 실행 유닛 | 클럭 속도 (MHz) | GFLOPS |
| ---------------- | --- | ----- | ----------- | ------ |
| HD Graphics      | GT1 | 6     |             |        |
| HD Graphics 2500 | GT1 | 6     | 1150        | 110.4  |
| HD Graphics 4000 | GT2 | 16    | 1300        | 332.8  |

### 하스웰

| 시장                     | 모델 번호             | 티어  | 실행 단위 | eDRAM (MB)  | 최대 클럭 속도 (MHz) | GFLOPS    |
| ---------------------- | ----------------- | --- | ----- | ----------- | -------------- | --------- |
| 소비자                    | HD Graphics       | GT1 | 10    | –           | 1150           | 184       |
| HD Graphics 4200       | GT2               | 20  | –     | 850         | 272            |           |
| HD Graphics 4400       | GT2               | 20  | –     | 950 – 1150  | 304 – 368      |           |
| HD Graphics 4600       | GT2               | 20  | –     | 900 – 1350  | 288 – 432      |           |
| HD Graphics 5000       | GT3               | 40  | –     | 1000 – 1100 | 640 – 768      |           |
| Iris Graphics 5100     | GT3               | 40  | –     | 1100 – 1200 | 704 – 768      |           |
| Iris Pro Graphics 5200 | GT3e              | 40  | 128   | 1200        | 832            |           |
| 서버                     | HD Graphics P4600 | GT2 | 20    | –           | 1200 – 1250    | 384 – 400 |
| HD Graphics P4700      | GT2               | 20  | –     | 1250 – 1300 | 400 – 416      |           |

### 브로드웰

| 시장                      | 모델 번호             | 티어  | 실행 단위 | eDRAM (MB) | 클럭 속도 (MHz) | GFLOPS |
| ----------------------- | ----------------- | --- | ----- | ---------- | ----------- | ------ |
| 소비자                     | HD Graphics       | GT1 | 12    | –          | 850         | 163.2  |
| HD Graphics 5300        | GT2               | 24  | –     | 900        | 345.6       |        |
| HD Graphics 5500        | GT2               | 24  | –     | 950        | 364.8       |        |
| HD Graphics 5600        | GT2               | 24  | –     | 1050       | 403.2       |        |
| HD Graphics 6000        | GT3               | 48  | –     | 1000       | 768         |        |
| Iris Graphics 6100      | GT3               | 48  | –     | 1100       | 844.8       |        |
| Iris Pro Graphics 6200  | GT3e              | 48  | 128   | 1150       | 883.2       |        |
| 서버                      | HD Graphics P5700 | GT2 | 24    | –          | 1000        | 384    |
| Iris Pro Graphics P6300 | GT3e              | 48  | 128   | 1150       | 883.2       |        |

### 에어몬트 / 브래스웰

  - HD Graphics 400/405 – GT1 – 12 EU (N30xx/N31xx) / 16 EU (N37xx)
      - N30xx – 320-600 MHz
      - N31xx – 320-640 MHz
      - N37xx – 400-700 MHz

### 스카이레이크

[스카이레이크](../Page/스카이레이크_\(마이크로아키텍처\).md "wikilink") 계열의 프로세서는 [VGA](../Page/VGA_단자.md "wikilink") 지원을 제거하는 한편, HDMI 1.4, 디스플레이포트 1.2, 임베디드 디스플레이포트(eDP) 1.3 인터페이스를 통해 [다중 모니터](../Page/다중_모니터.md "wikilink") 설정을 최대 3대의 모니터까지 지원한다.\[2\]\[3\]

| 시장                     | 모델 번호            | 티어  | 실행 단위 | eDRAM (MB) | 클럭 속도 (MHz) | GFLOPS |
| ---------------------- | ---------------- | --- | ----- | ---------- | ----------- | ------ |
| 소비자                    | HD Graphics 510  | GT1 | 12    | –          | 950         | 182.4  |
| HD Graphics 515        | GT2              | 24  | –     | 1000       | 384         |        |
| HD Graphics 520        | GT2              | 24  | –     | 1050       | 403.2       |        |
| HD Graphics 530        | GT2              | 24  | –     | 1150\[4\]  | 441.6       |        |
| Iris Graphics 540      | GT3e             | 48  | 64    | 1050       | 806.4       |        |
| Iris Graphics 550      | GT3e             | 48  | 64    | 1100       | 844.8       |        |
| Iris Pro Graphics 580  | GT4e             | 72  | 128   | 1000       | 1152        |        |
| 서버                     | HD Graphics P530 | GT2 | 24    | –          | 1150        | 441.6  |
| Iris Pro Graphics P555 | GT3e             | 48  | 128   | 1000\[5\]  | 768         |        |
| Iris Pro Graphics P580 | GT4e             | 72  | 128   | 1000       | 1152        |        |

### 카비레이크

고속 4K UHD, VP9, H.265 가속 등의 신기능이 제공된다.\[6\]

| 시장              | 모델 번호           | 티어          | 실행 단위                        | eDRAM (MB) | 기본 클럭 (MHz) | 부스트 클럭 (MHz) | GFLOPS        | 사용처                       |
| --------------- | --------------- | ----------- | ---------------------------- | ---------- | ----------- | ------------ | ------------- | ------------------------- |
| 소비자             | HD Graphics 615 | GT2         | 24                           | –          | 300         | 900–1050     | 345.6 – 403.2 | m3-7Y30, i5-7Y54, i7-7Y75 |
| HD Graphics 620 | 1000–1050       | 384 – 403.2 | i3-7100U, i5-7200U, i7-7500U |            |             |              |               |                           |

### 커피레이크

| 시장               | 모델 번호            | 티어  | 실행 단위      | eDRAM (MB) | 기본 클럭 (MHz) | 부스트 클럭 (MHz) |
| ---------------- | ---------------- | --- | ---------- | ---------- | ----------- | ------------ |
| 소비자              | UHD Graphics 610 | GT1 | 24         | –          | 350         | 1050\~1150   |
| UHD Graphics 630 | GT2              | 350 | 1050\~1200 |            |             |              |

## 각주

## 외부 링크

  - [Intel® Graphics Performance Analyzers 2013 R4](http://software.intel.com/en-us/vcsource/tools/intel-gpa)

  - (영어) [Latest product version: Intel GPA 2013 R4 P1 (December 19th, 2013)](https://software.intel.com/en-us/forums/graphics-profiling-debugging-and-analysis/topic/377096) : 2018년 5월 20일에 확인함. 상기 링크 깨져서 수정된 링크.

[분류:그래픽 처리 장치](https://ko.wikipedia.org/wiki/분류:그래픽_처리_장치 "wikilink") [분류:인텔의 마이크로프로세서](https://ko.wikipedia.org/wiki/분류:인텔의_마이크로프로세서 "wikilink")

1.
2.
3.
4.
5.
6.  <https://newsroom.intel.com/editorials/new-7th-gen-intel-core-processor-built-immersive-internet/>