...
[선형대수학](https://ko.wikipedia.org/wiki/선형대수학 "wikilink")에서 **슈트라센 알고리즘**은 독일의 수학자 [폴커 슈트라센](https://ko.wikipedia.org/wiki/폴커_슈트라센 "wikilink")(Volker Strassen)이 [1969년](https://ko.wikipedia.org/wiki/1969년 "wikilink")에 개발한 [행렬 곱셈](https://ko.wikipedia.org/wiki/행렬_곱셈 "wikilink") [알고리즘](https://ko.wikipedia.org/wiki/알고리즘 "wikilink")이다. 정의에 따라 *n*×*n* 크기의 두 행렬을 곱하면 [O](https://ko.wikipedia.org/wiki/점근_표기법 "wikilink")(n<sup>3</sup>)의 시간이 소요되지만 이 알고리즘은 대략 O(n<sup>2.807</sup>)의 시간이 소요된다.

## 알고리즘

*A*와 *B*를 [체](https://ko.wikipedia.org/wiki/체_\(수학\) "wikilink") *F*에 대한 [정사각행렬이라고](https://ko.wikipedia.org/wiki/행렬#정사각행렬 "wikilink") 하자. 두 행렬의 곱 *C*는 다음과 같다.
...
슈무엘 위노그라드(Shmuel Winograd)는 [1980년](https://ko.wikipedia.org/wiki/1980년 "wikilink")에 슈트라센 알고리즘을 변형한 **위노그라드 알고리즘**(Winograd algorithm)을 개발하였다. 이 알고리즘은 점근적으로 슈트라센 알고리즘과 동일한 복잡도를 지니지만 덧셈의 횟수를 15번으로 줄인 방법이다. 이 방법에서는 다음과 같은 변수를 쓴다.
...
[돈 코퍼스미스](https://ko.wikipedia.org/wiki/돈_코퍼스미스 "wikilink")(Don Coppersmith)와 슈무엘 위노그라드는 [1987년](https://ko.wikipedia.org/wiki/1987년 "wikilink")에 또 다른 알고리즘인 **코퍼스미스-위노그라드 알고리즘**(Coppersmith–Winograd algorithm)을 개발했다. 이 알고리즘 역시 슈트라센 알고리즘처럼 재귀적 발상에서 나온 것으로, 시간 복잡도가 O(n<sup>2.376</sup>)이고 2010년에 Stother가 O(n<sup>2.3737</sup>)로 개선할 때까지 가장 빠른 알고리즘이었다. 이듬해 월리엄스가 O(n<sup>2.3727</sup>)로 개선한 알고리즘이 현존하는 가장 빠른 알고리즘이다. [2005년](https://ko.wikipedia.org/wiki/2005년 "wikilink")에는 이 알고리즘을 [군론](https://ko.wikipedia.org/wiki/군론 "wikilink")적 구성을 사용해 유도한 논문이 발표되었다.

## 참고 자료 및 각주

  - Strassen, Volker, *Gaussian Elimination is not Optimal*, Numer. Math. 13, p. 354-356, 1969
  - Thomas H. Cormen, Charles E. Leiserson, Ronald L. Rivest, and Clifford Stein. *[Introduction to Algorithms](https://ko.wikipedia.org/wiki/Introduction_to_Algorithms "wikilink")*, Second Edition. MIT Press and McGraw-Hill, 2001. . Chapter 28: Section 28.2: Strassen's algorithm for matrix multiplication, pp.735–741.
