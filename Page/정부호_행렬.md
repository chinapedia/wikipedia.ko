...
실수체에서 정의하는 경우, [에르미트 행렬](../Page/에르미트_행렬.md "wikilink") \(M\) 대신 [대칭행렬](../Page/대칭행렬.md "wikilink") \(M\), [켤레전치](https://ko.wikipedia.org/wiki/켤레전치 "wikilink") \(x^*\)대신 [전치](https://ko.wikipedia.org/wiki/전치행렬 "wikilink") \(x^T\)를 사용한다.
...
\(n \times n\) 복소수 양의 정부호 행렬 \(M\)에 대해, 다음의 성질이 항상 성립한다.
  ...
  - 임의의 두 벡터 \(x, y\)에 대해 \(<x, y> = x^* M y\)로 [내적을](https://ko.wikipedia.org/wiki/내적공간 "wikilink") 정의하는 것이 가능하다. 반대로, 복소수 [벡터 공간](https://ko.wikipedia.org/wiki/벡터_공간 "wikilink") \(\mathbb{C}^n\)에서 정의할 수 있는 내적은 모두 양의 정부호 행렬에 대한 곱으로 표현이 가능하다.
