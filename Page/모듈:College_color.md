> This article is converted from Wikipedia: [모듈:College color](https://ko.wikipedia.org/wiki/모듈:College_color).
...
`   local results = {'Problems in `[`Module:College``   ``color/data`](https://ko.wikipedia.org/wiki/Module:College_color/data "wikilink")`:'}`
