...
[컴퓨터 프로그래밍에서](https://ko.wikipedia.org/wiki/컴퓨터_프로그래밍 "wikilink"), **분기 테이블**() 또는 **점프 테이블**()은 분기나 점프 명령어들을 이용해서, 프로그램의 제어를 프로그램의 다른 부분으로 옮기는 방법이다. 이것은 [다방향 분기의](https://ko.wikipedia.org/wiki/다방향_분기 "wikilink") 형태이다. 분기 테이블 구조는 주로 [Switch 문을](https://ko.wikipedia.org/wiki/Switch_문 "wikilink") 구현하는, [컴파일러](https://ko.wikipedia.org/wiki/컴파일러 "wikilink")에 의해 생성된 [어셈블리어](https://ko.wikipedia.org/wiki/어셈블리어 "wikilink") 프로그램에 사용된다.
...
3.  주소로 분기하는 것은 분기 테이블의 베이스 주소와 방금 생성된 오프셋의 합으로 구성된다. 이것은 가끔 [프로그램 카운터](https://ko.wikipedia.org/wiki/프로그램_카운터 "wikilink") [레지스터](https://ko.wikipedia.org/wiki/프로세서_레지스터 "wikilink") 오프셋에의 덧셈과 관련된다. (몇몇 명령어 집합에서, 분기 명령어가 추가적인 인덱스 레지스터를 허용하지 않는 한) 이 마지막 주소는 보통 무조건적인 분기 명령어들의 결과 또는 자기 바로 다음 명령어(테이블에서 한 엔트리를 절약하며)를 가리킨다.
...
분기 테이블과 [원시 자료의](https://ko.wikipedia.org/wiki/원시_자료 "wikilink") 사용은 메모리가 비싸고 CPU가 느리며, 압축된 데이터 표현과 대안의 효율적인 선택이 중요했던, 컴퓨터의 초기 시절에는 흔했다. 요즘은 이것들이 보통 다음과 같이 사용된다.
  ...
  - [임베디드](https://ko.wikipedia.org/wiki/임베디드_시스템 "wikilink") 프로그래밍
  - [운영 체제](https://ko.wikipedia.org/wiki/운영_체제 "wikilink") 개발. 많은 운영체제에서, [시스템 호출과](https://ko.wikipedia.org/wiki/시스템_호출 "wikilink") [라이브러리](https://ko.wikipedia.org/wiki/라이브러리_\(컴퓨팅\) "wikilink") 함수들은 분기 테이블에 대한 정수 인덱스에 의해 참조된다.
  - [IBM/360](https://ko.wikipedia.org/wiki/IBM_시스템/360 "wikilink") 같은 몇몇 컴퓨터 구조는 분기 테이블을 [인터럽트](https://ko.wikipedia.org/wiki/인터럽트 "wikilink") 디스패치를 위해 사용한다.
...
정수에 의해 참조되는 [라이브러리](https://ko.wikipedia.org/wiki/라이브러리_\(컴퓨팅\) "wikilink") 함수들

  - 하위 소프트웨어 버전과의 호환성 증가. 만약 함수와 [엔트리 포인트의](https://ko.wikipedia.org/wiki/엔트리_포인트 "wikilink") 주소 코드가 바뀌면, 오직 분기 테이블의 분기 명령어만 바꾸면 된다.
...
[해시 테이블도](https://ko.wikipedia.org/wiki/해시_테이블 "wikilink") 몇몇 경우에 사용되기도 한다.

## 기법의 다른 사용들

비록 분기 테이블을 사용하는 분기 기법은 단지 프로그램 흐름 변경을 위해 사용되지만, 이 기법은 다른 목적으로 사용될 수도 있다. 예를 들면 [컴파일러 최적화나](https://ko.wikipedia.org/wiki/컴파일러_최적화 "wikilink") [루프 풀기에](https://ko.wikipedia.org/wiki/루프_풀기 "wikilink") 대한 [JIT](https://ko.wikipedia.org/wiki/JIT_컴파일 "wikilink") 컴파일러에서 사용되기도 한다.

## 같이 보기

  - [디스패치 테이블](../Page/디스패치_테이블.md "wikilink")
  - [순람표](https://ko.wikipedia.org/wiki/순람표 "wikilink")
  - [Switch 문](https://ko.wikipedia.org/wiki/Switch_문 "wikilink")
...
## 외부 링크
  ...
  - [1](http://en.wikibooks.org/wiki/360_Assembly/Branch_Instructions) Example of branch table in [Wikibooks](https://ko.wikipedia.org/wiki/위키책 "wikilink") for [IBM S/360](https://ko.wikipedia.org/wiki/IBM_시스템/360 "wikilink")
  - [2](https://web.archive.org/web/20120212110151/http://www.netrino.com/node/137) Examples of, and arguments for, Jump Tables via Function Pointer Arrays in [C](https://ko.wikipedia.org/wiki/C_\(프로그래밍_언어\) "wikilink")/[C++](https://ko.wikipedia.org/wiki/C++ "wikilink")
