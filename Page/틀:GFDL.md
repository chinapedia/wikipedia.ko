> This article is converted from Wikipedia: [틀:GFDL](https://ko.wikipedia.org/wiki/틀:GFDL).


<onlyinclude>

<div class="licensetpl">

</div>

{{\#if:|

`  {{#ifeq:``|갱신|``|`
`    {{#ifeq:``|부적합|{{#ifeq:``|``|`[`분류:라이선스``   ``갱신:부적합`](https://ko.wikipedia.org/wiki/분류:라이선스_갱신:부적합 "wikilink")`}}|`
`      {{#ifeq:``|불필요|`[`분류:라이선스``   ``갱신:불필요`](https://ko.wikipedia.org/wiki/분류:라이선스_갱신:불필요 "wikilink")`|`
`      }}`
`    }}`
` }}`

|  }} <includeonly>[](https://ko.wikipedia.org/wiki/분류:GFDL_그림 "wikilink")</includeonly></onlyinclude>

[\*](https://ko.wikipedia.org/wiki/분류:GFDL_그림 "wikilink") [](https://ko.wikipedia.org/wiki/분류:위키백과_그림_저작권_틀 "wikilink")

<noinclude> </noinclude>