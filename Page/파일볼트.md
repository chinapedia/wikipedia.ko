...
**파일볼트**(**FileVault**)는 [맥 OS X 팬더서부터](https://ko.wikipedia.org/wiki/맥_OS_X_팬더 "wikilink") 사용된 [디스크 암호화 소프트웨어이다](https://ko.wikipedia.org/wiki/디스크_암호화_소프트웨어 "wikilink"). 파일볼트는 실시간으로 [매킨토시](https://ko.wikipedia.org/wiki/매킨토시 "wikilink")의 볼륨을 암호화 시킨다.

## 버전과 중요 특성

파일볼트는 [맥 OS X 팬더서](https://ko.wikipedia.org/wiki/맥_OS_X_팬더 "wikilink") 등장하였다.\[1\] 그리고 처음 파일 볼트는 시동 볼륨을 암호화 하지 않고 유저의 홈 디렉터리만 암호화 하였다. [맥 OS는](https://ko.wikipedia.org/wiki/맥_OS "wikilink") 홈 디렉터리가 있는 볼륨을 암호화된 디스크 이미지(sparse disk image, 거대한 하나의 파일)를 암호화 시켰다. 그리고 [맥 OS X 레퍼드와](https://ko.wikipedia.org/wiki/맥_OS_X_레퍼드 "wikilink") [맥 OS X 스노 레퍼드에서는](https://ko.wikipedia.org/wiki/맥_OS_X_스노_레퍼드 "wikilink") 더욱 현대화된 sparse 번들 디스크 이미지를 사용했다.\[2\] 이 번들 이미지에는 8 MB 이상의 데이터("bands"라 불러지는)가 흩어져 있었는데, 애플은 이 파일볼트를 가리켜 레거시 파일볼트라 일컫는다.\[3\]
...
[네할렘 (마이크로아키텍처)](https://ko.wikipedia.org/wiki/네할렘_\(마이크로아키텍처\) "wikilink") 아키텍처에 장착된 AES 암호화 셋을 사용하고, 또한 [맥 OS X 라이언에서](https://ko.wikipedia.org/wiki/맥_OS_X_라이언 "wikilink") 파일볼트를 사용시 [I/O](https://ko.wikipedia.org/wiki/I/O "wikilink") 성능은 안 켰을때보다 20\~30% 떨어지게 된다.\[9\]\[10\] 성능 저하는 AES 셋을 지원하지 않는 오래된 [인텔 코어](https://ko.wikipedia.org/wiki/인텔_코어_\(마이크로아키텍처\) "wikilink") CPU의 경우 더 큰 차이로 벌어지게 된다.

### 마스터 키와 복구 키

파일볼트를 구동했을시, macOS는 복구 키를 보여주거나, 혹은 애플의 [iCloud](https://ko.wikipedia.org/wiki/iCloud "wikilink") 계정으로 복구할수 있는 옵션을 보여준다. 이 120 비트의 복구 키는 모든 문자와 1\~9까지의 숫자로 구성되어 있으며, `/dev/random`에서 읽는다. 그리고, 이로 인해 macOS에서 사용하고 있는 [유사난수 생성기의](https://ko.wikipedia.org/wiki/유사난수 "wikilink") 보안 여부와 직결된다. 2012년 암호분석에 의하면, 이 메커니즘은 안전한걸로 드러났다.\[11\]
...
## 더 보기
  ...
  - [비트로커](https://ko.wikipedia.org/wiki/비트로커 "wikilink")
