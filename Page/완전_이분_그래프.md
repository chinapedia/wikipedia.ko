...
[그래프 이론에서](https://ko.wikipedia.org/wiki/그래프_이론 "wikilink") **완전 이분 그래프**(完全二分graph, )란 꼭짓점의 집합이 서로 겹치지 않는 두 집합 X와 Y의 [합집합](https://ko.wikipedia.org/wiki/합집합 "wikilink")이고 X의 모든 꼭짓점이 Y의 각각의 꼭짓점과 하나의 변으로 연결되어 있는 [이분 그래프이다](../Page/이분_그래프.md "wikilink").
...
\(V_1,\dotsc,V_k\)의 [집합의 크기가](https://ko.wikipedia.org/wiki/집합의_크기 "wikilink") 각각 \(n_1,\dotsc,n_k\)일 때, 이에 대응하는 완전 \(k\)분 그래프는 \(K_{n_1,\dotsc,n_k}\)로 표기한다.

\(k=1\)일 경우, 이는 [완전 그래프와](https://ko.wikipedia.org/wiki/완전_그래프 "wikilink") 같다. \(k=2\)일 경우 이를 **완전 이분 그래프**(完全二分graph, ), \(k=3\)일 경우 이를 **완전 삼분 그래프**(完全三分graph, )라고 한다.
...
[평면 그래프는](../Page/평면_그래프.md "wikilink") \(K_{3,3}\)를 [그래프 마이너로](https://ko.wikipedia.org/wiki/그래프_마이너 "wikilink") 가질 수 없다. 반대로, [평면 그래프가](../Page/평면_그래프.md "wikilink") 아닌 모든 그래프는 \(K_{3,3}\) 또는 \(K_5\)를 [그래프 마이너로](https://ko.wikipedia.org/wiki/그래프_마이너 "wikilink") 갖는다 (바그너 정리 )
