...
local span_open = '<span style="font-size:100%; font-weight:normal" class="error">'; local code_open = '<code style="color:inherit; border:inherit; padding:inherit;">'; local help_link = ' ([도움말](../Page/틀:London_Gazette.md "wikilink"))';

local supp_error = mw.ustring.format ('%sinvalid %s|supp=</code>%s</span>', span_open, code_open, help_link); local duplicate_page_error = mw.ustring.format (' %smore than one of %s|page=</code> and %s|pages=</code>%s</span>', span_open, code_open, code_open, help_link);

local p = {}

\--[--------------------------\< I S _ S E T \>------------------------------------------------------------------ Whether variable is set or not. A variable is set when it is not nil and not empty.](https://ko.wikipedia.org/wiki/--------------------------\<_I_S_S_E_T_\>------------------------------------------------------------------_Whether_variable_is_set_or_not._A_variable_is_set_when_it_is_not_nil_and_not_empty. "wikilink")
...
\--[--------------------------\< O R D I N A L _ S U F F I X \>-------------------------------------------------- render a numerical text string in ordinal form suitable for English language use. In this module, num_str is limited by calling functions to the integer values 1-99. The argument num_str must be known to be set before this function is called.](https://ko.wikipedia.org/wiki/--------------------------\<_O_R_D_I_N_A_L_S_U_F_F_I_X_\>--------------------------------------------------_render_a_numerical_text_string_in_ordinal_form_suitable_for_English_language_use._In_this_module,_num_str_is_limited_by_calling_functions_to_the_integer_values_1-99._The_argument_num_str_must_be_known_to_be_set_before_this_function_is_called. "wikilink")
...
Determine the value to be assigned to the specified cs1|2 |page= or |pages= parameter in . The arguments |param=page and |param=pages specify which of the cs1|2 parameters |page= or |pages= for which this function is to create a value.

This function inspects the content of the |page= and |pages= parameters, along with the |param= specifier. From this information it creates a value appropriate for the specified |page= or |pages= parameter. Only one will have a value, the other will get an empty string.

This function is called twice from ; once for each of cs1 |page= and |pages= parameters:

`   |page={{#invoke:Gazette util|make_page_param|param=page|page=``|pages=``}}`
`   |pages={{#invoke:Gazette util|make_page_param|param=pages|page=``|pages=``}}`

except for the lvalue and the rvalue assigned to |param=, the two calls must be identical else odd results.

|page= or |pages= without a comma, hyphen, or en dash separator → cs1 |page= |page= or |pages= with a separator → cs1 |pages=

Hyphen separator characters are converted to en dash characters. Any white space around hyphen and en dash separators is removed.

If both |page= and |pages= are set, this function mimics cs1|2 and chooses |page=.

Another function, page_error() is required for error messaging because we don't want to dump css markup into a parameter value that will be included in the cs1|2 metadata.
...
Inspect page number parameters and return an error message that will be appended to the end of the  template rendering. Error messages are handled this way so that the error message is not made part of the cs1|2 citation's metadata.

{{\#invoke:Gazette util|page_error|page=|pages=}}
...
set the value that is assigned to the cite magazine |type= parameter using the values of the London Gazette |supp= and |display-supp= parameters
...
|url={{\#invoke:Gazette util|url_param|city=|issue=|supp=|page=|pages=}}
