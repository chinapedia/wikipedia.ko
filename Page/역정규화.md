> This article is converted from Wikipedia: [역정규화](https://ko.wikipedia.org/wiki/역정규화).


**역정규화**()는 이전에 [정규화된](../Page/데이터베이스_정규화.md "wikilink") 데이터베이스에서 성능을 개선하기 위해 사용되는 전략이다. [컴퓨팅](../Page/컴퓨팅.md "wikilink")에서 역정규화는 일부 쓰기 성능의 손실을 감수하고 데이터를 묶거나 데이터의 복제 사본을 추가함으로써 [데이터베이스](../Page/데이터베이스.md "wikilink")의 읽기 성능을 개선하려고 시도하는 과정이다.\[1\]\[2\] 아주 많은 수의 읽기 작업을 처리할 필요가 있는 [관계형](../Page/관계형_모델.md "wikilink") [데이터베이스 소프트웨어의](../Page/데이터베이스.md "wikilink") [성능이나](https://ko.wikipedia.org/wiki/컴퓨터_성능 "wikilink") 스케일링에서 고려된다. 역정규화는 [비정규화](https://ko.wikipedia.org/wiki/비정규화 "wikilink")(unnormalized form)와는 구별한다. 데이터베이스/테이블은 이들을 효율적으로 역정규화하기 위해 우선 정규화되어야 한다.
...
## 같이 보기
  ...
  - [정규화](../Page/데이터베이스_정규화.md "wikilink")
