...
아래의 [C](https://ko.wikipedia.org/wiki/C_\(프로그래밍_언어\) "wikilink") 코드는 배열을 탐색하여 5 보다 큰 값을 찾는 일을 한다. 먼저 반복자를 사용한 코드를 살펴보자.
