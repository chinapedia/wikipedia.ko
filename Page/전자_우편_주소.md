...
[섬네일](https://ko.wikipedia.org/wiki/파일:Mail_in_foto.jpg "wikilink") **전자 우편 주소**(電子 郵便 住所) 또는 **이메일 주소**()는 [전자 우편](https://ko.wikipedia.org/wiki/전자_우편 "wikilink") 메시지가 전달될 전자 메일 상자를 식별한다. 초기 이메일 시스템에서는 여러 형태의 포맷이 사용되었지만 오늘날에는 1980년대부터 [인터넷](https://ko.wikipedia.org/wiki/인터넷 "wikilink") 메일 시스템을 위해 개발된 표준에 따라 오직 하나의 포맷만이 사용된다.
...
이메일 주소 중 도메인 이름은 명확한 지침을 따라야 한다. [호스트명](https://ko.wikipedia.org/wiki/호스트명 "wikilink")의 요구 사항, 점(.)으로 구분되는 [DNS](https://ko.wikipedia.org/wiki/DNS "wikilink") 레이블의 목록, 각 레이블이 63자를 넘지 않는 등의 요구 사항을 충족해야 하며, 다음으로 구성되어야 한다:\[2\]
...
### 예
  ...
  - 유효한 전자 우편 주소
    ...
    `example@s.solutions` ([인터넷 최상위 도메인 목록](https://ko.wikipedia.org/wiki/인터넷_최상위_도메인_목록 "wikilink") 참조)
    `user@localserver`
...
## 같이 보기
  ...
  - [이메일 클라이언트](https://ko.wikipedia.org/wiki/이메일_클라이언트 "wikilink")
