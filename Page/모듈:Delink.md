> This article is converted from Wikipedia: [모듈:Delink](https://ko.wikipedia.org/wiki/모듈:Delink).
...
`   -- We need to deal with colons, brackets, and commas, per `[`Help:Pipe``   ``trick`](https://ko.wikipedia.org/wiki/Help:Pipe_trick "wikilink")`.`
`   `
...
`   end`
`   `
`   -- Remove the colon if the link is using the `[`Help:Colon``   ``trick`](https://ko.wikipedia.org/wiki/Help:Colon_trick "wikilink")`.`
`   if result:match("%[%[:") then`
`       result = "[[" .. result:match("%[%[:(.*%]%])")`
`   end`
`   `
`   -- Deal with links using the `[`Help:Pipe``   ``trick`](https://ko.wikipedia.org/wiki/Help:Pipe_trick "wikilink")`.`
...
`       -- Remove any `[`Help:Strip``   ``markers`](https://ko.wikipedia.org/wiki/Help:Strip_markers "wikilink")` representing ref tags. In most situations `
