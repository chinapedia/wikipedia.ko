...
[컴퓨터 과학에서](https://ko.wikipedia.org/wiki/컴퓨터_과학 "wikilink"), **보고정렬**(bogosort\[1\]\[2\], 또한 **stupid sort**\[3\] 또는 **slowsort**\[4\]\[5\])은 [정렬 알고리즘](https://ko.wikipedia.org/wiki/정렬_알고리즘 "wikilink") 중 하나로 비효율적인 것에 속한다. 보고 정렬의 이름은 단어 "bogus"에서 유래된다. 보고정렬은 정렬하는데 유용하게 사용되지는 않지만, 실제로 사용되는 알고리즘과 비교를 통한 교육에서 사용될 수 있다. 또한 논리형 프로그래밍의 예시로서 사용되기도 한다. 만약 보고 정렬이 카드 뭉치를 정렬하는데 사용될 때 시행되는 과정은 다음과 같다.

## 알고리즘에 대한 설명

다음은 [의사코드](https://ko.wikipedia.org/wiki/의사코드 "wikilink") 로 되어있는 설명이다.
...
## [런타임](https://ko.wikipedia.org/wiki/런타임 "wikilink") 및 종결
...
한정된 개수의 숫자 배열에 대하여 이 알고리즘의 [런타임](https://ko.wikipedia.org/wiki/런타임 "wikilink")은 유한하다. 이는 [무한 원숭이 정리에](https://ko.wikipedia.org/wiki/무한_원숭이_정리 "wikilink") 기반하여 내릴 수 있는 결론이며, 섞음 후 정렬된 배열이 나올 수 있는 확률이 조금이라도 있기에, 여러번의 수행은 거의 확실히 정렬된 배열을 만들어 낼 수 있다. 하지만 랜덤한 섞음이 아닌 [유사난수](https://ko.wikipedia.org/wiki/유사난수 "wikilink") 생성기를 이용하여 섞음이 시행된다면 이 알고리즘에는 끝이 없을 수도 있다.
