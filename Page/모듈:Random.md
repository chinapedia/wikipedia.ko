...
`   return mw.ustring.format('`<b class="error">[`Module:Random`](https://ko.wikipedia.org/wiki/Module:Random "wikilink")` error: %s.`</b>`', msg)`
...
`   -- Randomizes a list and turns it into an HTML list. Uses `[`Module:List`](https://ko.wikipedia.org/wiki/Module:List "wikilink")`.`
`   listType = listType or 'bulleted'`
`   local listArgs = makeRandomList(args) -- Arguments for `[`Module:List`](https://ko.wikipedia.org/wiki/Module:List "wikilink")`.`
