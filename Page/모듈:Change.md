> This article is converted from Wikipedia: [모듈:Change](https://ko.wikipedia.org/wiki/모듈:Change).
...
`   end`
`   `
`   -- This code was copied from `[`Template:Convert`](https://ko.wikipedia.org/wiki/Template:Convert "wikilink")`  `
`   if outvalue == 0 then`
