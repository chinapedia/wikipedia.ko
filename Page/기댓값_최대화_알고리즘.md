...
기댓값 최대화 알고리즘은 1977년 아서 뎀스터, 낸 라드와 도널드 루빈에 의해 제시되었다. 하지만 특수한 경우에서의 기댓값 최대화 알고리즘은 이미 몇몇 저자들에 의해 연구되고 있었고, 특히 롤프 선드버그는 페르 마틴-뢰프, 앤더스 마틴-뢰트와 함께 [지수족](https://ko.wikipedia.org/wiki/지수족 "wikilink")에 한정된 기댓값 최대화 알고리즘에 관한 연구를 한 바 있다. 1977년 뎀스터-라드-루빈 논문은 이 방법을 일반화하여 [지수족](https://ko.wikipedia.org/wiki/지수족 "wikilink")뿐만 아니라 다양한 확률분포에 한해서도 알고리즘이 수렴함을 밝혔다. 왕립통계협회저널에 게재된 이 논문은 그 혁신성으로 인해 롤프 선드버그를 비롯한 여럿에게 극찬을 받았으며, 기댓값 최대화 알고리즘을 통계분석의 중요한 도구로 자리매김하게 하였다.

이후 뎀스터-라드-루빈 논문의 알고리즘 수렴성 분석에 오류가 발견되었고, 1983년 C.F. 제프 우에 의해 옳은 증명이 제시되었다. 제프 우의 증명은 기댓값 최대화 알고리즘이 [지수족](https://ko.wikipedia.org/wiki/지수족 "wikilink")뿐만 아니라 다른 확률분포에도 성립함을 보였다.
...
관측할 수 있는 확률변수 \(\mathbf{X}\)와 관측할 수 없는 확률변수 \(\mathbf{Z}\), 그리고 모수 \(\boldsymbol{\theta}\)가 있을 때, \((\mathbf{X}, \mathbf{Z})\)에 대한 확률분포는 \(L (\boldsymbol\theta;\mathbf{X},\mathbf{Z}) = p(\mathbf{X}, \mathbf{Z}|\boldsymbol{\theta})\)로 주어져 있다. 이 때 [최대가능도](https://ko.wikipedia.org/wiki/최대가능도 "wikilink")를 계산하고 싶은 경우, 최대화하려는 [가능도](https://ko.wikipedia.org/wiki/가능도 "wikilink") 함수는 \(L(\boldsymbol{\theta}; \mathbf{X}) = p(\mathbf{X} |\boldsymbol{\theta}) = \sum_{\mathbf{Z}} p(\mathbf{X}, \mathbf{Z}|\boldsymbol{\theta})\)로 정의할 수 있다.
...
비록 EM을 반복하는 것이 관측된 데이터(한계) 기능도 함수를 증가시키지만 그 순열이 [최대가능도로](https://ko.wikipedia.org/wiki/최대가능도방법 "wikilink") 수렴한다는 증거는 없다. [다항 분포에서는](https://ko.wikipedia.org/wiki/다항_분포 "wikilink"), EM 알고리즘이 초기값에 따라 관측된 데이터 가능도 함수의 [극대값](https://ko.wikipedia.org/wiki/극대값 "wikilink")으로 수렴할 수도 있다. [경사 하강법](https://ko.wikipedia.org/wiki/경사_하강법 "wikilink")(서로 다른 초기 추정량 \(\boldsymbol\theta^{(t)}\)에서 시작하는 방법)이나 [담금질 기법](../Page/담금질_기법.md "wikilink") 등 수 많은 체험적이거나 메타 발견적 학습 방법이 있다.

EM은 가능도가 [지수족](https://ko.wikipedia.org/wiki/지수족 "wikilink")일때 특히 유용하다: E 단계는 충분 통계량()의 기대값들의 합이 되며, M 단계는 선형 함수를 최대화하는 것을 포함한다. 이런 경우에는, 각각의 단계에서 선드버그 공식을 이용하여 닫힌 해를 도출하는 것이 가능하다.\[1\]\[2\]\[3\]\[4\]\[5\]\[6\]\[7\] \[8\]\[9\]

EM 방법은 뎀스터-라드-루빈 논문에서 [베이즈 추론에서](https://ko.wikipedia.org/wiki/베이즈_추론 "wikilink") [최대 사후 확률을](https://ko.wikipedia.org/wiki/최대_사후_확률 "wikilink") 계산하기 위해 변형되었다.

[경사 하강법](https://ko.wikipedia.org/wiki/경사_하강법 "wikilink"), [켤레기울기법](../Page/켤레기울기법.md "wikilink"), 또는 가우스-뉴턴 방법의 변형 등과 같은 [최대가능도를](https://ko.wikipedia.org/wiki/최대가능도방법 "wikilink") 찾는 여러 방법들이 존재한다. EM과 달리, 이러한 방법들은 일반적으로 가능도 함수의 첫 번째와 두 번째 미분의 계산을 필요로 한다.
...
상황에 따라서는, 기댓값 최대화 알고리즘을 다음의 두 최대화 단계로 보는 것이 편리하다.\[11\]\[12\] *q*는 미관측 데이터 *z*를 나타내는 임의의 확률 분포, *p*<sub>*Z*|*X*</sub>(· |*x*;*θ*)는 관측 데이터 *x*가 주어졌을 때의 조건부 확률 분포, *H*를 [엔트로피](https://ko.wikipedia.org/wiki/엔트로피 "wikilink"), 그리고 *D*<sub>KL</sub>를 [쿨백-라이블러 발산이라고](https://ko.wikipedia.org/wiki/쿨백-라이블러_발산 "wikilink") 할 때, 다음 식을 고려한다.
...
기댓값 최대화 알고리즘은 또한 [의학 영상](https://ko.wikipedia.org/wiki/의학_영상 "wikilink") 복원, 특히 [양전자 방출 단층촬영과](https://ko.wikipedia.org/wiki/양전자_방출_단층촬영 "wikilink") 단일광자방출단층활영()에 널리 쓰인다.
...
정보 기하학()에서, E 단계와 M 단계는 이중 [아핀 접속의](https://ko.wikipedia.org/wiki/아핀_접속 "wikilink") 사영()으로 해석되고, e-연결()과 m-연결()로 불린다. [쿨백-라이블러 발산](https://ko.wikipedia.org/wiki/쿨백-라이블러_발산 "wikilink") 또한 이 용어들을 사용해서 해석될 수 있다.
...
이제 [가능도](https://ko.wikipedia.org/wiki/가능도 "wikilink") 함수 \(L(\boldsymbol\theta;\mathbf{x},\mathbf{z})\)를 다음과 같이 표현할 수 있다: 불완전한 데이터 가능도 함수는
...
하나의 트레이닝 이미지(training image) \(\mathbf{x}\)에 관한 [가능도](https://ko.wikipedia.org/wiki/가능도 "wikilink") 함수는 다음과 같다.
...
\(\mathbb{E}_{\mathbf{Z} | \mathbf{X}, \theta^\text{old}} \left[ \mathcal{L}(\theta) \right]\)를 혼합 계수 \(\mathbf{\pi}\)와 조건 \(\sum_{k = 1}^K \pi_k = 1\)에 관해 최대화하기 위해 [라그랑주 승수법](https://ko.wikipedia.org/wiki/라그랑주_승수법 "wikilink")(Lagrange Multiplier)을 이용한다.
