...
\[B_1 \mapsto g^{-1}(L/2)B_0g(-l/2)\] 이를 사용하여, 게이지 퍼텐셜 \(T_0\)을 [상수 함수로](https://ko.wikipedia.org/wiki/상수_함수 "wikilink") 놓을 수 있다. 그렇다면, [남 방정식에](../Page/남_방정식.md "wikilink") 따라서 \(T_1,T_2,T_3\) 역시 [상수 함수가](https://ko.wikipedia.org/wiki/상수_함수 "wikilink") 된다.
...
\(L \to 0\) 극한을 취하면, 이 해들은 [점근 국소 유클리드 공간을](https://ko.wikipedia.org/wiki/점근_국소_유클리드_공간 "wikilink") 이룬다. 이 경우 마찬가지로 ALE 분류가 존재하며, 가장 간단한 경우는 [에구치-핸슨 공간이다](../Page/에구치-핸슨_공간.md "wikilink").
...
일반적 다중 토브-너트 공간의 대칭군은 \(N\ge3\)일 때 O(2)이며, 이에 대한 고정점들은 모두 [너트이다](../Page/너트와_볼트.md "wikilink"). 이는 [기번스-호킹 가설 풀이의](https://ko.wikipedia.org/wiki/기번스-호킹_가설_풀이 "wikilink") 퍼텐셜의 \(N\)개의 특이점들에 해당한다. \(N=2\)일 때는 대칭군은 U(1)×U(1)이며, 둘째 U(1)은 두 너트를 통과하는 축을 중심으로 하는 회전이다.
