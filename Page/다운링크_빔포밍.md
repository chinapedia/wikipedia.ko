...
빔포밍 메트릭스인 \(\mathbf{W}\)는 다운링크 채널 메트릭스에 맞게 구성된다. 다운링크 채널 메트릭스는 빔포밍 할 사용자의 채널 벡터들로 구성된다. 빔포밍 메트릭스를 구성하는 한 방법으로 사용자 채널 벡터들간의 간섭의 제거하는 방법이 있다. 간섭을 완전히 제거하는 방식으로 빔포밍 메트릭스를 구성하는 것을 [간섭 제거 빔포밍 (Zero forcing beamforming)이라](https://ko.wikipedia.org/wiki/간섭_제거_빔포밍 "wikilink") 한다. ZF 빔포밍에서 **W**는 채널 메트릭스를 인버스(inverse)한 메트릭스의 각 열(column)을 정규화한 벡터들로 구성된다.
...
\end{matrix}\] 여기서 \(\tilde{\mathbf{h}}_1 = \mathbf{h}_1/||\mathbf{h}_1||\)이고 \(\mathbf{w}_i, i \neq 1\)에 의한 결과는 *i*에 대한 identically distributed된 경우이므로 임의의 한 경우인 \(\mathbf{w}_2\)로 대체하였다. [베타 분포를](https://ko.wikipedia.org/wiki/베타_분포 "wikilink") 적용하면 다음과 같이 유도가 가능하다.
...
## 같이 보기
  ...
  - [간섭 제거 빔포밍](https://ko.wikipedia.org/wiki/간섭_제거_빔포밍 "wikilink")
