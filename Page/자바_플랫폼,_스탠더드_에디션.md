...
**자바 플랫폼 스탠더드 에디션**(, 약자 Java SE)는 [데스크톱](https://ko.wikipedia.org/wiki/데스크톱_환경 "wikilink") 및 [서버](https://ko.wikipedia.org/wiki/서버 "wikilink"), 최근의 고사양 [임베디드 시스템을](https://ko.wikipedia.org/wiki/임베디드_시스템 "wikilink") 위한 표준 [자바 플랫폼으로](https://ko.wikipedia.org/wiki/자바_플랫폼 "wikilink") 표준적인 컴퓨팅 환경을 지원하기 위한 [자바 가상 머신](https://ko.wikipedia.org/wiki/자바_가상_머신 "wikilink") 규격 및 API 집합을 포함한다. 따라서 [자바 EE](https://ko.wikipedia.org/wiki/자바_EE "wikilink"), [자바 ME](https://ko.wikipedia.org/wiki/자바_ME "wikilink") 등 다른 플랫폼은 구체적인 목적에 따라 [자바 SE를](https://ko.wikipedia.org/wiki/자바_SE "wikilink") 기반으로 API를 추가하거나 [자바 가상 머신](https://ko.wikipedia.org/wiki/자바_가상_머신 "wikilink") 규격 및 API의 일부를 택해서 정의된다.
...
자바 SE는 [자바 가상 머신](https://ko.wikipedia.org/wiki/자바_가상_머신 "wikilink") 규격\[1\] 전체를 지원한다. JVM 규격의 변경이 필요한 기능은 일반적으로 자바 SE에 제안되고 추후 자바 EE 및 ME에 포함된다. 엄밀히 JVM 규격의 일부는 아니지만 API를 통해 실행 시간에 클래스에 대한 정보를 얻고 메서드를 호출할 수 있는 리플렉션() API와 [가비지 콜렉션을](https://ko.wikipedia.org/wiki/가비지_콜렉션 "wikilink") 좀 더 세밀히 제어할 수 있는 참조 객체 API(java.lang.ref)도 JVM 구현에 영향을 미치는데 자바 SE에서는 이 들 API도 완전히 지원한다.

## API

자바 SE에 포함되는 API는 자바 EE에 그대로 포함되며 자바 ME에는 일부만 추려서 수용된다. JCP과 과거 [썬 마이크로시스템즈에](../Page/썬_마이크로시스템즈.md "wikilink") 의해 설계된 API는 java 혹은 javax로 시작되는 패키지 이름을 가지며 그 외 [W3C](https://ko.wikipedia.org/wiki/W3C "wikilink") 등 다른 기관에서 정의된 API의 경우 org.w3c.dom과 같이 다른 패키지 이름을 가지기도 한다. 다음 API 개요는 자바 6를 기준으로 한다.
...
## 같이 보기
  ...
  - [자바 플랫폼, 마이크로 에디션](https://ko.wikipedia.org/wiki/자바_플랫폼,_마이크로_에디션 "wikilink")
