...
[확률론](https://ko.wikipedia.org/wiki/확률론 "wikilink")과 [통계학](https://ko.wikipedia.org/wiki/통계학 "wikilink")에서, **베이즈 정리**()는 두 [확률 변수의](../Page/확률_변수.md "wikilink") [사전 확률과](https://ko.wikipedia.org/wiki/사전_확률 "wikilink") [사후 확률](https://ko.wikipedia.org/wiki/사후_확률 "wikilink") 사이의 관계를 나타내는 정리다. [베이즈 확률론](https://ko.wikipedia.org/wiki/베이즈_확률론 "wikilink") 해석에 따르면 베이즈 정리는 사전확률로부터 사후확률을 구할 수 있다.\[1\]
...
\[\Pr(A|B)=\frac{\Pr(B|A)\Pr(A)}{\Pr(B)}\propto\mathcal L(A|B)\Pr(A)\] 각각의 항은 다음과 같은 의미를 갖는다.
  ...
  - \(\Pr(B|A)\)는 *A*가 주어졌을 때 *B*의 [조건부 확률이다](https://ko.wikipedia.org/wiki/조건부_확률 "wikilink").
      - \(\mathcal L(A|B) = \Pr(B|A)\)는 \(B\)가 주어졌을 때 \(A\)의 [가능도](https://ko.wikipedia.org/wiki/가능도 "wikilink")이다.
...
\= P(B|A<sub>1</sub>)P(A<sub>1</sub>) + P(B|A<sub>2</sub>)P(A<sub>2</sub>)
  ...
  - [베이즈 정리](https://ko.wikipedia.org/wiki/베이즈_정리 "wikilink")
