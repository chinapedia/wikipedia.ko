> This article is converted from Wikipedia: [모듈:Internet Archive](https://ko.wikipedia.org/wiki/모듈:Internet_Archive).


\--[For functions related to Internet Archive Notes: 1. Internet Archive runs Elasticsearch search engine as of 4 Nov 2015 2. Program flowchart: Break name down into number of words Build a base URL based on number of words (1,2,3,4,5+), use of sopt=t switch, and availability of birth-death dates If any words contain extended-ascii characters append extra code for wildcards based on sopt=t or w return finished URL 3. URL length should not exceed 2000 characters or it will break certain popular browsers 4. Wildcard (\*) replacements should be avoided in the first letter of the first word, and with any single-letter words 5. Changing search formulations will have impacts on existing uses of the template and off-line tools which are optimized for these search recipes.](https://ko.wikipedia.org/wiki/For_functions_related_to_Internet_Archive_Notes:_1._Internet_Archive_runs_Elasticsearch_search_engine_as_of_4_Nov_2015_2._Program_flowchart:_Break_name_down_into_number_of_words_Build_a_base_URL_based_on_number_of_words_\(1,2,3,4,5+\),_use_of_sopt=t_switch,_and_availability_of_birth-death_dates_If_any_words_contain_extended-ascii_characters_append_extra_code_for_wildcards_based_on_sopt=t_or_w_return_finished_URL_3._URL_length_should_not_exceed_2000_characters_or_it_will_break_certain_popular_browsers_4._Wildcard_\(*\)_replacements_should_be_avoided_in_the_first_letter_of_the_first_word,_and_with_any_single-letter_words_5._Changing_search_formulations_will_have_impacts_on_existing_uses_of_the_template_and_off-line_tools_which_are_optimized_for_these_search_recipes. "wikilink")

local p = {}

\--[For Template:Internet Archive author](https://ko.wikipedia.org/wiki/For_Template:Internet_Archive_author "wikilink") function p.author(frame)
...
`   return "`[`:틀:"..tname.."`](https://ko.wikipedia.org/wiki/:틀:"..tname.." "wikilink")에` 오류가 있습니다: `[`"``   ``..name..``   ``"`](https://ko.wikipedia.org/wiki/"_..name.._" "wikilink")` 항목이 존재하지 않습니다."`
` end`
...
` if count == 0 then`
`   return "`[`:틀:"..tname.."`](https://ko.wikipedia.org/wiki/:틀:"..tname.." "wikilink")에` 오류가 있습니다: 제로 워드 이름입니다."`
` end `
...
` --`[`Format``   ``URL`](https://ko.wikipedia.org/wiki/Format_URL "wikilink")
...
` return "알 수 없는 오류 (1). `[`틀:"..tname.."`](https://ko.wikipedia.org/wiki/틀:"..tname.." "wikilink")의` 설명문서를 참고하십시오"`

end

\-- Build final output and render function p.IArender()
