...
충족 가능성 문제의 [결정 문제는](https://ko.wikipedia.org/wiki/결정_문제 "wikilink") [NP-완전](https://ko.wikipedia.org/wiki/NP-완전 "wikilink")에 속한다. 이것은 [스티븐 쿡이](https://ko.wikipedia.org/wiki/스티븐_쿡 "wikilink") 증명했으며([쿡-레빈 정리](../Page/쿡-레빈_정리.md "wikilink")), 이 문제는 NP-완전이라는 것이 증명된 최초의 문제이기도 하다. 또한, 논리식이 [논리곱 표준형으로](https://ko.wikipedia.org/wiki/논리곱_표준형 "wikilink") 이루어진 경우에도 역시 NP-완전에 속한다.

**k-충족 가능성 문제**, **k-SAT**는 각 클로저에 들어있는 리터럴의 개수가 k개 이하로 구성된 CNF 논리식만 입력으로 받는 문제이다. 예를 들어 3-SAT는 한 절에 들어가는 리터럴 개수를 3개 이하로 제한하는 문제이다. 3-SAT도 마찬가지로 [NP-완전](https://ko.wikipedia.org/wiki/NP-완전 "wikilink") 문제이다. 리터럴 개수를 정확히 3개로만 제한하는 문제는 **EXACT 3-SAT**이라고 하며, 모든 SAT 문제는 [다항 시간에](https://ko.wikipedia.org/wiki/다항_시간 "wikilink") 3-SAT 또는 EXACT 3-SAT로 [환산](https://ko.wikipedia.org/wiki/환산 "wikilink")(Reduction)될 수 있다.

반면, 2-SAT, CNF에서 한 절에 들어가는 리터럴 개수가 2개 이하인 문제는 [P에](https://ko.wikipedia.org/wiki/P_\(복잡도\) "wikilink") 속한다. 즉, 다항 시간에 풀 수 있다.
