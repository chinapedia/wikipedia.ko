> This article is converted from Wikipedia: [FM 알고리즘](https://ko.wikipedia.org/wiki/FM_알고리즘).
...
N: 회로에 주어진 Net의 개수이다. e.g., N= 4 는 즉 Net1, Net2, Net3, Net4가 있다는 의미이다.

P: Net과 Cell 사이에 연결된 총 Pin의 개수; P= p(1) + … + p(C) = n(1) + … + n(N) 즉 각각의 Cell의 연결된 Pin의 개수는 각 Net에 연결된 Pin의 개수와 당연히 일치한다.

r: Cutset을 위해 요구되는 Balance Factor이다. 0\< r\<1
