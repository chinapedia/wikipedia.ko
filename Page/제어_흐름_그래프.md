...
CFG는 많은 [컴파일러 최적화와](https://ko.wikipedia.org/wiki/컴파일러_최적화 "wikilink") [정적 프로그램 분석](https://ko.wikipedia.org/wiki/정적_프로그램_분석 "wikilink") 툴에서 핵심 요소이다.

## 정의

CFG에서 [그래프](https://ko.wikipedia.org/wiki/그래프 "wikilink")의 각 노드(node)는 [기본 블록](https://ko.wikipedia.org/wiki/기본_블록 "wikilink") (basic block)을 표현한다. 즉, 점프가 없는 코드의 직선 조각이다. (점프는 블록의 시작과 끝을 나눈다.) 방향성의 엣지(edge)들은 제어 흐름에서 점프를 나타낸다. 대부분의 경우에 흐름 그래프가 진입하는 엔트리 블록과 모든 제어 흐름이 떠나는 "종료 블록"이라는 두 특별히 고안된 블록을 지닌다.
...
위에서, 우리는 4개의 [기본 블록을](https://ko.wikipedia.org/wiki/기본_블록 "wikilink") 갖는다. A는 0부터 1, B는 2부터 3, C는 4 그리고 D는 5다. 특히, A의 경우 엔트리 블록이며, D의 경우 종료 블록이다. 그리고 라인 4와 5는 점프 대상이다. 이 코드의 그래프는 A부터 B, A부터 C, B부터 D 그리고 C부터 D까지의 엣지를 갖는다.
...
만약 종료 블록이 엔트리 블록에서 도달 불가능하다면, [무한 루프가](https://ko.wikipedia.org/wiki/무한_루프 "wikilink") 존재할 수 있다. [정지 문제](https://ko.wikipedia.org/wiki/정지_문제 "wikilink") 처럼, 모든 무한 루프가 탐지되는 것은 아니다. 또한 거기에 정지 순서도 존재할 수 있다.
...
백 엣지 (*back edge*)는 이미 [DFS](https://ko.wikipedia.org/wiki/깊이_우선_탐색 "wikilink") 동안 만났던 블록에 대한 포인터이다. 백 엣지는 대표적인 루프이다.
...
## 같이 보기
  ...
  - [순서도](https://ko.wikipedia.org/wiki/순서도 "wikilink")
  - [제어 흐름 다이어그램](https://ko.wikipedia.org/wiki/제어_흐름_다이어그램 "wikilink")
  - [제어 흐름 분석](https://ko.wikipedia.org/wiki/제어_흐름_분석 "wikilink")
  - [데이터 흐름 분석](https://ko.wikipedia.org/wiki/데이터_흐름_분석 "wikilink")
