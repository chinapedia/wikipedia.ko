...
**코시 주요값**(Cauchy主要-, ) 또는 **코시 주치**(Cauchy主値)는 일반적인 정적분으로 값을 구할 수 없는 일부 [이상적분](https://ko.wikipedia.org/wiki/이상적분 "wikilink")의 값을 구하는 방법 중 하나이다. [오귀스탱 루이 코시가](https://ko.wikipedia.org/wiki/오귀스탱_루이_코시 "wikilink") 도입하였다.
...
\[\mathcal P\int_a^b f(x)\;\mathrm dx\;\stackrel{\text{def}}=\;\lim_{\epsilon\to+0}\int_a^{x_0-\epsilon}f(x)\;\mathrm dx+\int_{x_0+\epsilon}^bf(x)\;\mathrm dx\] 이렇게 적분을 [규칙화](https://ko.wikipedia.org/wiki/규칙화 "wikilink")하여 얻는 값을 **코시 주요값**이라 한다. 실함수 뿐만 아니라, 복소 함수의 [선적분](https://ko.wikipedia.org/wiki/선적분 "wikilink")의 경우에도 유사한 방법으로 코시 주요값을 정의할 수 있다.
