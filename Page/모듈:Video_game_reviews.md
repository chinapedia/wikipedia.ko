...
`           end`
`           for _,v in ipairs(customAggregatorKeys) do`
`               if (args[v .. '점수'] == nil) then`
`                   renderRating(builder, args[v], args[v .. 'Score'])`
`               else`
`                   renderRating(builder, args[v], args[v .. '점수'])`
`               end`
`           end`
`       end`
`       if reviewerCount ~= 0 then`
...
`           :css('background', '#d1dbdf')`
`           :css('border-top', 'none')          `
`           :wikitext('위키데이터에서 편집하기 '.. vgwd.getUpdateLink('nosub'))`
`   end`
...
`   for k, v in pairs(args) do`
`       if string.match(k, "^표제$") then`
`           args.title = v`
`       elseif string.match(k, "^부제$") then`
`           args.subtitle = v`
`       elseif string.match(k, "^상태$") then`
`           args.status = v`
`       elseif string.match(k, "^폭$") then`
`           args.width = v`
`       elseif string.match(k, "^정렬$") then`
`           args.align = v`
`       end`
`   end`
