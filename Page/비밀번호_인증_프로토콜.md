...
PAP는 암호화되지 않은 ASCII 암호를 네트워크 상으로 전달하므로 안전하지 않은 것으로 간주된다. 원격 서버가 [CHAP](https://ko.wikipedia.org/wiki/챌린지-핸드셰이크_인증_프로토콜 "wikilink"), [EAP](../Page/확장_가능_인증_프로토콜.md "wikilink")(후자는 실제로 [프레임워크임](https://ko.wikipedia.org/wiki/소프트웨어_프레임워크 "wikilink").) 따위의 더 강력한 인증 프로토콜을 지원하지 않을 때 마지막으로 사용한다.
