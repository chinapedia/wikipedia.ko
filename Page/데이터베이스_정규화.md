...
데이터베이스 디자인 표준 가이드는 데이터베이스가 완전히 정규화되게 디자인되어야 한다; 그 뒤에 일부가 [성능상의](https://ko.wikipedia.org/wiki/컴퓨터_성능 "wikilink") 이유로 [비정규화](https://ko.wikipedia.org/wiki/비정규화 "wikilink")될 수는 있다.\[6\] 그러나, [데이터 웨어하우스](https://ko.wikipedia.org/wiki/데이터_웨어하우스 "wikilink") 디자인을 위한 [관점 모델링과](https://ko.wikipedia.org/wiki/관점_모델링 "wikilink") 같은 일부 모델링 규칙에서는 예외적으로 비 정규화된 디자인을 추천한다. 즉 대규모 부분에서의 디자인은 [3NF가](https://ko.wikipedia.org/wiki/제_3_정규형 "wikilink") 아니다.\[7\]

## 정규화의 목적

1970년 [에드거 F. 커드에](https://ko.wikipedia.org/wiki/에드거_F._커드 "wikilink") 의해 정의된 [제 1 정규형의](https://ko.wikipedia.org/wiki/제_1_정규형 "wikilink") 기본적의 목적은 [1차 논리에](../Page/1차_논리.md "wikilink") 기반을 둔 "보편적 데이터 부언어"에 의해 데이터가 질의되고 조작되게 하기 위해서였다.\[8\] ([SQL](https://ko.wikipedia.org/wiki/SQL "wikilink")이 이 데이터 부언어의 대표적인 예이지만, 정작 [에드거 F. 커드는](https://ko.wikipedia.org/wiki/에드거_F._커드 "wikilink") 이 언어는 심각한 결함을 가지고 있다고 생각하였다.)\[9\]

[에드거 F. 커드에](https://ko.wikipedia.org/wiki/에드거_F._커드 "wikilink") 의해 정의된 제 1 정규화(1NF) 의 목적은 아래와 같다:

  -
    1\. 고려되지 않은 삽입, 갱신, 삭제 의존에서부터 [관계의](https://ko.wikipedia.org/wiki/관계형_데이터베이스 "wikilink") 집합을 배제한다;
    2\. 새로운 [자료형](https://ko.wikipedia.org/wiki/자료형 "wikilink")이 나타날 때, 관계들의 집합의 재구성의 필요성을 낮추고, 그로 인하여 응용 프로그램의 생명주기를 연장한다;
...
[에드거 F. 커드는](https://ko.wikipedia.org/wiki/에드거_F._커드 "wikilink") 이러한 데이터 구조의 복잡성이 완전히 제거되었을 때 질의가 ([사용자와](https://ko.wikipedia.org/wiki/사용자_\(컴퓨팅\) "wikilink") [응용 프로그램에](https://ko.wikipedia.org/wiki/응용_프로그램 "wikilink") 의해서)표현되고 ([DBMS에](https://ko.wikipedia.org/wiki/데이터베이스_관리_시스템 "wikilink") 의해서) 수행됨에 있어서 더욱 강력하고 유연해진다고 보았다. 위 구조를 정규화하면 아래와 같다:
