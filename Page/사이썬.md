...
파이썬은 [스크립트 언어이다](https://ko.wikipedia.org/wiki/스크립트_언어 "wikilink"). 스크립트 언어는 작성된 [소스 코드를](https://ko.wikipedia.org/wiki/소스_코드 "wikilink") 실시간 해석하여 수행한다. 따라서 실시간 소스 코드 해석으로 인해 [C언어](https://ko.wikipedia.org/wiki/C언어 "wikilink")와 같은 컴파일러 언어에 비해 수행 속도가 느리다. 컴파일러 언어는 소스 코드를 미리 번역해둔 [실행 파일을](https://ko.wikipedia.org/wiki/실행_파일 "wikilink") 수행하기 때문에 실시간으로 코드를 해석할 필요가 없다. 따라서 스크립트 언어보다 수행 속도가 빠르다.
...
사이썬 코드는 [함수의](https://ko.wikipedia.org/wiki/함수_\(프로그래밍\) "wikilink") 인자와 내부 변수의 타입을 C언어와 유사한 형태로 선언한다.
...
사이썬은 C언어로된 함수를 파이썬 문법에서 부를 수 있도록 한다. C언어 함수를 부를 수 있는 다양한 방법들이 있지만 사이썬은 부가 코드를 작성하지 않고 바로 파이썬에서 호출이 가능한 장점이 있다. 예를 들어 일반화된 방법인 [Python-C-API](https://ko.wikipedia.org/wiki/Python-C-API "wikilink") 방식을 이용하는 경우는 래핑 코드를 별도로 짜주어야 한다. 이런 과정을 자동화하는 [SWIG](https://ko.wikipedia.org/wiki/SWIG "wikilink")이라는 방법이 있지만, SWIG를 통해 부가 코드를 생성하는 과정이 들어가는 불편함이 여전히 존재한다.
...
## 더 보기
  ...
  - 스크립트 컴파일러들에는 [Julia](https://ko.wikipedia.org/wiki/Julia "wikilink"), [Go](https://ko.wikipedia.org/wiki/Go_\(프로그래밍_언어\) "wikilink"), [CoffeeScript](https://ko.wikipedia.org/wiki/CoffeeScript "wikilink") 등이 있다. 여기서 Julia는 [Matlab](https://ko.wikipedia.org/wiki/Matlab "wikilink") 문법을 가진 컴파일러 언어로 수치 계산용이고, Go는 C언어로 컴파일이 가능한 시스템 언어이고, CoffeeScript는 [JavaScript](https://ko.wikipedia.org/wiki/JavaScript "wikilink")로 컴파일되는 웹 언어이다.
