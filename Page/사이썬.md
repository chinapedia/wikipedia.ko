...
C++ 파일을 연결한 경우, 사인썬 코드에 c++를 사용해서 컴파일 할 것을 요청해야 한다. 아래 link_cpp.pyx의 제일 상단에 있는 *\# distutils: language = c++*이 그 것을 의미한다. 이 때 setup.py는 상단에서 정의한 파일을 변화없이 그대로 사용하면 된다.
...
위 파일을 태스트 하기 위한 간단한 파이썬 코드는 다음과 같다.
...
C++과 연결하는 자세한 사용법은 사이썬 메뉴얼의 [C++ 래핑 부분](https://cython.readthedocs.io/en/latest/src/userguide/wrapping_CPlusPlus.html)을 참조하면 된다.
