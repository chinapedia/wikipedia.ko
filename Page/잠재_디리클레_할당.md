> This article is converted from Wikipedia: [잠재 디리클레 할당](https://ko.wikipedia.org/wiki/잠재_디리클레_할당).
...
생성 과정에서 각각의 변수는 다음과 같은 의미를 가진다.
  ...
  - \(\alpha\)는 \(k\) 차원 [디리클레 분포의](../Page/디리클레_분포.md "wikilink") 매개변수이다.
  - \(\theta\)는 \(k\) 차원 벡터이며, \(\theta^i\)는 문서가 \(i\)번째 주제에 속할 확률 분포를 나타낸다.
  - \(z_n\)는 \(k\) 차원 벡터이며, \(z_n^i\)는 단어 \(w_n\)이 \(i\)번째 주제에 속할 확률 분포를 나타낸다.
  - \(\beta\)는 \(k \times V\) 크기의 [행렬](../Page/행렬.md "wikilink") 매개변수로, \(\beta_{ij}\)는 \(i\)번째 주제가 단어집의 \(j\)번째 단어를 생성할 확률을 나타낸다.
...
잠재 변수 \(\alpha\), \(\beta\)가 주어졌을 때 \(\theta\), \(\mathbf{z} = \{ z_1, \dots, z_N \}\), \(\mathbf{w}\)에 대한 [결합 분포는](https://ko.wikipedia.org/wiki/결합_분포 "wikilink") 다음과 같이 구해진다.
...
볼록 기반 변분 알고리즘의 기본 아이디어는 [옌센 부등식을](../Page/옌센_부등식.md "wikilink") 기반으로 로그 가능도(log likelihood)의 하한값을 계산하는 것이다.\[11\] 로그 가능도의 하한값은 변분 매개 변수(varional paremeter)로 나타내지며, 이 매개변수는 최적화 과정을 통해 가장 좋은 하한값을 가지도록 정해진다.
...
위의 식에서 우변을 \(L( \gamma, \phi; \alpha, \beta )\)라 표기하자. 그러면 좌변과 우변의 차이가 [쿨백-라이블러 발산임을](../Page/쿨백-라이블러_발산.md "wikilink") 이용하여 다음과 같이 나타낼 수 있다.
...
위 식을 \(\phi_{ni}\)에 대해 [편미분](../Page/편미분.md "wikilink")하면 다음과 같은 식을 얻을 수 있다.
...
매개 변수 추정과 추론을 위해 [깁스 표집을](https://ko.wikipedia.org/wiki/깁스_표집 "wikilink") 이용한 잠재 디리클레 할당구현이다. [GNU 일반 공중 사용 허가서로](../Page/GNU_일반_공중_사용_허가서.md "wikilink") 배포하고 있다.
