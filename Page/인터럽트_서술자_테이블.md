...
**인터럽트 디스크립터 테이블 (Interrupt Descriptor Table** (**IDT**))은 [인터럽트 벡터](https://ko.wikipedia.org/wiki/인터럽트_벡터 "wikilink") 테이블을 구현하기 위해 [X86](https://ko.wikipedia.org/wiki/X86 "wikilink") 아키텍처에서 사용되는 데이터 구조체이다. IDT는 프로세서가 [인터럽트](https://ko.wikipedia.org/wiki/인터럽트 "wikilink")와 [예외에](https://ko.wikipedia.org/wiki/예외_처리 "wikilink") 대한 정확한 반응을 결정하기 위해 사용된다.

아래에서 설명할 자세한 부분들은 x86 아티텍처와 [AMD64](https://ko.wikipedia.org/wiki/X86-64 "wikilink") 아키텍처에서 적용되는 것이다. 다른 아키텍처들은 비슷한 구조체를 갖지만 행동은 조금 다를 수 있다.
...
| INT_NUM | 간단한 설명                                                          |
| -------- | --------------------------------------------------------------- |
| 0x00     | [0으로 나누기](https://ko.wikipedia.org/wiki/0으로_나누기 "wikilink")     |
| 0x01     | 디버거                                                             |
| 0x02     | NMI                                                             |
| 0x03     | 브레이크포인트                                                         |
| 0x04     | 오버플로우                                                           |
...
몇몇 [윈도우](https://ko.wikipedia.org/wiki/마이크로소프트_윈도우 "wikilink") 프로그램들은 IDT를 호출하기 위해 [후킹](https://ko.wikipedia.org/wiki/후킹 "wikilink")한다. 이것은 커널 모드 [장치 드라이버를](https://ko.wikipedia.org/wiki/장치_드라이버 "wikilink") 쓰는 것을 포함한다(IDT 호출을 가로채고 자신의 과정을 넣는다). 이것은 [마이크로소프트](https://ko.wikipedia.org/wiki/마이크로소프트 "wikilink")에서 정식으로 지원하지 않지만, [64비트](https://ko.wikipedia.org/wiki/64비트 "wikilink") 버전 까지는 프로그램적으로 막혀있지는 않다(커널 모드 후킹을 사용하려는 시도를 하는 드라이버는 시스템의 버그 체크를 유발할 것이다).\[1\]
