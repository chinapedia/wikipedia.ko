...
**선형 시간**(線型時間, Linear time)이란, [계산 복잡도 이론에서](https://ko.wikipedia.org/wiki/계산_복잡도_이론 "wikilink"), 입력의 길이 \(n\)에 대하여, 어떤 [알고리즘](https://ko.wikipedia.org/wiki/알고리즘 "wikilink")의 실행시간이 [선형](../Page/선형성.md "wikilink")([\({\color{Blue}O}(n)\)](https://ko.wikipedia.org/wiki/점근_표기법 "wikilink"))이 되는 것을 뜻한다. 예를 들면, 입력된 숫자열의 총합을 계산하는 순서는 숫자열의 길이에 비례하는 시간이 필요하다.

이상의 설명은 어디까지나 이론적인 것으로, 실제의 실행시간은(특히 \(n\)이 비교적 작은 경우) 입력의 길이에 정확하게 비례한다고 볼 수는 없다. 기술적으로 충분히 큰 n에 대하여, 알고리즘의 실행시간이 \(an\)에서 \(bn\)의 범위 안에 들 경우 (\(a\)와 \(b\)는 양의 정수), 이를 '선형 시간'이라고 부를 수 있다. 자세한 이론은 [점근 표기법을](https://ko.wikipedia.org/wiki/점근_표기법 "wikilink") 참조할 것.
...
예를 들어, [정렬](https://ko.wikipedia.org/wiki/정렬 "wikilink") 알고리즘의 경우, 입력이 되는 숫자열에 따라 선형시간으로 정렬이 가능한 경우도 있으나, 숫자열의 각 항목 사이의 비교를 기반으로 한 정렬 알고리즘의 경우는 일반적으로 \(O(n \log n)\)보다 시간을 단축시킬 수 없다. 이러한 복잡도 증명은, [점근 표기법이](https://ko.wikipedia.org/wiki/점근_표기법 "wikilink") 보는 대상이며, 일반적으로 정렬 알고리즘은 \(\Omega(n \log n)\)라고 말할 수 있다. 같은 식으로, 무작위로 생성된 길이 \(n\)의 숫자의 배열에서 최대치를 찾아내는 [선택 알고리즘](https://ko.wikipedia.org/wiki/선택_알고리즘 "wikilink")(Selection Algorithm)은, 최대치를 찾는데 적어도 (\(n-1\))회의 비교가 필요하다는 것을 논리적으로 볼때 알 수 있으며, \(\Omega(n)\)이 된다.
