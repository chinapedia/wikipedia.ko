...
**ADM 질량**() 또는 **ADM 에너지**()는 [ADM 형식에서](https://ko.wikipedia.org/wiki/ADM_형식 "wikilink") 자연스럽게 등장하는 일종의 [해밀토니언](https://ko.wikipedia.org/wiki/해밀토니언 "wikilink")의 값이다.\[1\]\[2\]
...
\[M=\frac1{16\pi G}\lim_{r\to\infty}\oint_r(\delta^{ij}\partial_ih_{jk}-\delta^{ij}\partial_kh_{ij})\hat n^k\;\sqrt{\gamma^{(2)}}\mathrm dx^2\] 여기서
  ...
  - 계수 \(1/(16\pi)\)는 [슈바르츠실트 계량의](https://ko.wikipedia.org/wiki/슈바르츠실트_계량 "wikilink") 질량과 비교하여 고정된 것이다.
...
ADM [에너지-운동량](https://ko.wikipedia.org/wiki/에너지-운동량 "wikilink") \(P^\mu\)는 점근적 [로런츠 변환에](https://ko.wikipedia.org/wiki/로런츠_변환 "wikilink") 대하여 [4차원 벡터로](https://ko.wikipedia.org/wiki/4차원_벡터 "wikilink") 변환한다.\[7\]\[8\]

### 존재 조건

만약 계량이 공간의 무한에서 점근적으로 평탄하지 않다면 (예를 들어, [우주 상수가](https://ko.wikipedia.org/wiki/우주_상수 "wikilink") 존재한다면), 위 적분들은 수렴하지 않을 수 있다.

구체적으로, \(D>2\)차원 [리만 다양체](https://ko.wikipedia.org/wiki/리만_다양체 "wikilink") \((\Sigma,g)\)가 주어졌으며, \(\Sigma\)는 [유클리드 공간의](https://ko.wikipedia.org/wiki/유클리드_공간 "wikilink") [부분 집합](https://ko.wikipedia.org/wiki/부분_집합 "wikilink")

\[\mathbb R^D\setminus\operatorname{ball}(0,1;\mathbb R^D)\] 와 [미분 동형이라고](https://ko.wikipedia.org/wiki/미분_동형 "wikilink") 하자 (\(\operatorname{ball}(0,1;\mathbb R^D)\)는 \(\mathbb R^D\) 속의 단위 [초구](https://ko.wikipedia.org/wiki/초구 "wikilink")). 즉, 이 경우 위 [미분 동형 사상에](https://ko.wikipedia.org/wiki/미분_동형_사상 "wikilink") 의하여 좌표계 \((x^i)_{i=1,\dots,D}\) 및 함수 \(r=\sqrt{\delta_{ij}x^ix^j}\)를 정의할 수 있다. \((\Sigma,g)\)의 ADM 질량이 유한하게 존재할 [충분 조건은](https://ko.wikipedia.org/wiki/충분_조건 "wikilink"), 다음 부등식들을 만족시키는 상수 \(\alpha\)가 존재하는 것이다.\[9\]
...
양 ADM 에너지 정리는 1979년에 리처드 멜빈 셰인()과 [야우싱퉁](https://ko.wikipedia.org/wiki/야우싱퉁 "wikilink")이 최초로 증명하였다.\[22\] 이후 1981년에 [에드워드 위튼이](https://ko.wikipedia.org/wiki/에드워드_위튼 "wikilink") [순간자](https://ko.wikipedia.org/wiki/순간자 "wikilink")를 통한 새 증명을 제시하였고,\[23\] 이듬해에 토머스 파커()와 클리퍼드 헨리 토브스()가 위튼의 증명을 수학적으로 엄밀하게 보완하였다.\[24\]

리만-펜로즈 부등식은 1973년에 [로저 펜로즈가](https://ko.wikipedia.org/wiki/로저_펜로즈 "wikilink") 물리학을 사용하여 추측하였으며,\[25\] 2001년에 휴버트 루이스 브레이() · 게르하르트 후이스켄(, 1958\~) · 톰 일매넌(, 1961\~)이 수학적으로 엄밀히 증명하였다.\[26\]\[27\]
