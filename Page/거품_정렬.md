...
[frame](https://ko.wikipedia.org/wiki/파일:Bubble_sort_animation.gif "wikilink") **거품 정렬**()은 두 인접한 원소를 검사하여 정렬하는 방법이다. [시간 복잡도가](../Page/시간_복잡도.md "wikilink") \(O(n^2)\)로 상당히 느리지만, 코드가 단순하기 때문에 자주 사용된다. 원소의 이동이 거품이 수면으로 올라오는 듯한 모습을 보이기 때문에 지어진 이름이다. __TOC__
