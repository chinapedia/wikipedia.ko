> This article is converted from Wikipedia: [거품 정렬](https://ko.wikipedia.org/wiki/거품_정렬).


[frame](https://ko.wikipedia.org/wiki/파일:Bubble_sort_animation.gif "wikilink") **거품 정렬**()은 두 인접한 원소를 검사하여 정렬하는 방법이다. [시간 복잡도가](../Page/시간_복잡도.md "wikilink") \(O(n^2)\)로 상당히 느리지만, 코드가 단순하기 때문에 자주 사용된다. 원소의 이동이 거품이 수면으로 올라오는 듯한 모습을 보이기 때문에 지어진 이름이다. 양방향으로 번갈아 수행하면 [칵테일 정렬이](../Page/칵테일_정렬.md "wikilink") 된다. __TOC__
...
## [의사 코드로](https://ko.wikipedia.org/wiki/의사_코드 "wikilink") 나타낸 [알고리즘](../Page/알고리즘.md "wikilink")
