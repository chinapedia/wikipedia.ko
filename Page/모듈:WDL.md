...
local getArgs = require('Module:Arguments').getArgs local roundAndPad = require('Module:Decimals')._main
...
`           return ''`
`       end`
`       return frame:expandTemplate{title = 'Nts', args = { (won or 0) + (drawn or 0) + (lost or 0) }}`
...
`   else`
`       return frame:expandTemplate{title = 'Nts', args = { played }}`
`   end`
...
`   end`
`   return retval .. frame:expandTemplate{title = 'Nts', args = { roundAndPad(winPercent, decimals or 2) }}`
...
`   retval = retval .. tableprefix .. frame:expandTemplate{title = 'Nts', args = { args[2] }} .. '\n'`
`   retval = retval .. tableprefix .. frame:expandTemplate{title = 'Nts', args = { args[3] }} .. '\n'`
`   retval = retval .. tableprefix .. frame:expandTemplate{title = 'Nts', args = { args[4] }} .. '\n'`
`   if args['for'] then`
`       retval = retval .. tableprefix .. frame:expandTemplate{title = 'Nts', args = { args['for'] }} .. '\n'`
`   end`
`   if args.against then`
`       retval = retval .. tableprefix .. frame:expandTemplate{title = 'Nts', args = { args.against }} .. '\n'`
`   end`
...
[Category:WDL_error](https://ko.wikipedia.org/wiki/Category:WDL_error "wikilink")