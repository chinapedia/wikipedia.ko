...
기본 데이터 형식은 [정수](https://ko.wikipedia.org/wiki/정수 "wikilink"), [부동 소수점 실수 타입](https://ko.wikipedia.org/wiki/부동_소수점 "wikilink"), [문자열](https://ko.wikipedia.org/wiki/문자열 "wikilink"), 키([UUID](https://ko.wikipedia.org/wiki/전역_고유_식별자 "wikilink")), [벡터](https://ko.wikipedia.org/wiki/벡터 "wikilink")(3차원 위치 및 [RGB](https://ko.wikipedia.org/wiki/RGB "wikilink") 색깔 표현), 회전([사원수](https://ko.wikipedia.org/wiki/사원수 "wikilink")(四元數, quaternion))가 있다. 또한 배열 형식이나 구조에 해당하는 것으로, 기본 데이터 형식을 요소로 하는 [리스트](https://ko.wikipedia.org/wiki/리스트_\(추상_데이터_형\) "wikilink") 형식이 있다. 내장 함수는 2010년 3월 현재 350개가 있고, 사용자는 필요에 따라 사용자 정의 함수를 정의할 수있다.
...
스크립트는 개체에 추가되는대로 시작된다. 개체가 가상 세계에 배포되는 한, 소유자가 로그인하지 않은 상태에서도 실행은 계속된다. 소유자가 개체를 철거하여 자신의 인벤토리로 옮겨서 가상 세계에 재배치하는 경우에도 스크립트 상태가 보존된다. [파일](https://ko.wikipedia.org/wiki/파일_\(컴퓨터\) "wikilink") 또는 [데이터베이스](https://ko.wikipedia.org/wiki/데이터베이스 "wikilink") 같은 영속적인 데이터 저장 장치는 제공되지 않지만, HTTP 요청 통신을 이용하여 Second Life 외부에 데이터를 저장할 수 있다.

개체는 여러 스크립트를 포함할 수 있고, 그들을 동시에 실행할 수 있다. 독립형 스크립트에서 사용할 수있는 메모리 공간은 64[KiB](https://ko.wikipedia.org/wiki/키비바이트 "wikilink") 이하로 제한된다. 각 스크립트는 [바이트 코드](https://ko.wikipedia.org/wiki/바이트_코드 "wikilink") 실행 파일에 [컴파일되고](https://ko.wikipedia.org/wiki/컴파일러 "wikilink") 린든 랩의 서버에서 [Mono을](../Page/모노_\(소프트웨어\).md "wikilink") 이용한 [가상 머신에서](../Page/가상_머신.md "wikilink") 실행 된다.
...
## 외부 링크 및 관련 자료
  ...
  - [Second Life LSL 포털](http://wiki.secondlife.com/wiki/LSL_Portal/ko) — 린든 랩이 운영하는 공식 Wiki

  - [Mono for Second Life](http://wiki.secondlife.com/wiki/Mono) — Information regarding the move to Mono
  ...
  - [Linden Scripting Language Problems](http://xahlee.org/sl/ls-prob.html) — Criticism by Xah Lee
