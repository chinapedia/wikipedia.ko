...
[양자역학](https://ko.wikipedia.org/wiki/양자역학 "wikilink")에서, **리프먼-슈윙거 방정식**()은 입자의 [산란](https://ko.wikipedia.org/wiki/산란 "wikilink")을 다루는 방정식이다. 미국의 버너드 리프먼()과 [줄리언 슈윙거가](https://ko.wikipedia.org/wiki/줄리언_슈윙거 "wikilink") 1950년에 유도하였다.\[1\]
...
\[|\psi\rangle\stackrel{?}=|\phi\rangle+(E-H_0)^{-1}V|\psi\rangle\]. 하지만 \((E-H_0)^{-1}\)은 \(H_0\)에 대한 고윳값이 \(E\)일 경우에는 정의할 수 없다. [섭동 이론에서는](https://ko.wikipedia.org/wiki/섭동_이론_\(양자역학\) "wikilink") 고윳값이 문제가 되는 경우를 적절한 사영 연산자로 사영해 없앨 수 있지만, 산란 이론에서 다루는 해밀토니언은 연속 스펙트럼을 가지므로 이렇게 할 수 없다. 대신, \(E\)를 [복소수](https://ko.wikipedia.org/wiki/복소수 "wikilink")로 잡고, 그 허수 부분이 매우 작다고 하자.

\[|\psi^\pm\rangle=|\phi\rangle+(E\pm i\epsilon-H_0)^{-1}V|\psi^\pm\rangle\] (\(\epsilon\ll1\)). 이렇게 하여 \(|\psi^\pm\rangle\)을 나타낼 수 있다. 이 식을 **리프먼-슈윙거 방정식**이라고 한다. 리프먼-슈윙거 방정식은 [경로적분법](https://ko.wikipedia.org/wiki/경로적분법 "wikilink")을 통해 풀 수 있으며, 그 해 가운데 \(|\psi^+\rangle\)는 초기 상태, \(|\psi^-\rangle\)는 나중 상태의 [파동 함수이다](../Page/파동_함수.md "wikilink").
...
\[\langle\mathbf r|\psi^\pm\rangle
=\langle\mathbf r|\phi\rangle
+\int\langle\mathbf r|(E\pm i\epsilon+\nabla^2/2m)^{-1}|\mathbf r'\rangle V(\mathbf r')\langle\mathbf r'|\psi^\pm\rangle\,d\mathbf r'\]. 여기서 다음과 같이 [그린 함수](https://ko.wikipedia.org/wiki/그린_함수 "wikilink") \(G^\pm(\mathbf r,\mathbf r')\)를 정의하자.

\[G^\pm(\mathbf r,\mathbf r')=\langle\mathbf r|(E\pm i\epsilon+\nabla^2/2m)^{-1}|\mathbf r'\rangle\]. 이를 대입하고, [브라-켓 표기법을](https://ko.wikipedia.org/wiki/브라-켓_표기법 "wikilink") 일반 함수 표기법으로 바꾸면 다음과 같은 [적분 방정식을](https://ko.wikipedia.org/wiki/적분_방정식 "wikilink") 얻는다.
...
\[H_0=\mathbf p^2/2m=-\hbar^2\nabla^2/2m\]. 이에 해당하는 그린 함수 \(G^\pm(\mathbf r,\mathbf r')\)는 [경로적분법](https://ko.wikipedia.org/wiki/경로적분법 "wikilink")을 통해 다음과 같이 계산할 수 있다.
