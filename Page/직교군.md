...
\[M(+1)=\begin{pmatrix}1&0\\0&1\end{pmatrix}\qquad M(-1)=\begin{pmatrix}0&1\\1&0\end{pmatrix}\] 이며, \(\operatorname{Sym}(k)\)의 원소는 2×2 단위 행렬 블록의 \(2k\times 2k\) [치환행렬](https://ko.wikipedia.org/wiki/치환행렬 "wikilink")에 \((2k+1,2k+1)\)번째 성분 +1을 추가한 행렬이다.
