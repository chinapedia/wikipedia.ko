> This article is converted from Wikipedia: [바벤코-베크너 부등식](https://ko.wikipedia.org/wiki/바벤코-베크너_부등식).
...
n차원 [푸리에 변환의](../Page/푸리에_변환.md "wikilink") (q, p norm)은 다음과 같이 정의된다.
...
엄밀히 말하자면, 이 부등식은, 사용되는 [푸리에 변환의](../Page/푸리에_변환.md "wikilink") 정의가, 1차원에서는 아래와 같이 정의되고 n차원에서는 seperable kernel n개를 곱한 것을 kernel로 하는 방식으로 정의되어야만 성립한다.
...
`아래의 증명은 `[`르베그``   ``측도와`](../Page/르베그_측도.md "wikilink")` `[`Bernoulli``   ``trial`](https://ko.wikipedia.org/wiki/Bernoulli_trial "wikilink")` 및 `[`중심``   ``극한``   ``정리를`](../Page/중심_극한_정리.md "wikilink")` 이용한다:`
