...
**로봇 배제 표준**(robots exclusion standard), **로봇 배제 프로토콜**(robots exclusion protocol)은 [웹 사이트에](https://ko.wikipedia.org/wiki/웹_사이트 "wikilink") 로봇이 접근하는 것을 방지하기 위한 규약으로, 일반적으로 접근 제한에 대한 설명을 **robots.txt**에 기술한다.
