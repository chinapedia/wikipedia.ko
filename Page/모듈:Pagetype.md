> This article is converted from Wikipedia: [모듈:Pagetype](https://ko.wikipedia.org/wiki/모듈:Pagetype).


-----

\-- -- -- PAGETYPE -- -- -- -- This is a meta-module intended to replace  and similar -- -- templates. It automatically detects namespaces, and allows for a -- -- great deal of customisation. It can easily be ported to other -- -- wikis by changing the values in the [Module:Pagetype/config](https://ko.wikipedia.org/wiki/Module:Pagetype/config "wikilink"). -- -- --
...
`   -- Builds the arguments to pass to `[`Module:Namespace``   ``detect`](https://ko.wikipedia.org/wiki/Module:Namespace_detect "wikilink")` and returns`
`   -- the result.`
...
`               -- `[`Module:Namespace``   ``detect`](https://ko.wikipedia.org/wiki/Module:Namespace_detect "wikilink")`.`
