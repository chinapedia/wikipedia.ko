...
[해석학에서](https://ko.wikipedia.org/wiki/해석학_\(수학\) "wikilink"), **횔더 부등식**()은 [르베그 적분과](https://ko.wikipedia.org/wiki/르베그_적분 "wikilink") [L<sup>*p*</sup> 공간을](https://ko.wikipedia.org/wiki/Lp_공간 "wikilink") 연구하기 위해 사용하는 매우 중요한 [부등식](https://ko.wikipedia.org/wiki/부등식 "wikilink")이다. 부등식의 이름은 [오토 횔더의](../Page/오토_횔더.md "wikilink") 이름을 따서 지은 것이다.

## 정의

*S*,*Σ*,*μ*가 [측도 공간이고](https://ko.wikipedia.org/wiki/측도_공간 "wikilink"), *p*, *q* ∈ **R**가 1 ≤ *p*, *q* ≤ ∞ 이고 1/*p* + 1/*q* = 1 을 만족한다고 하자. 이때 모든 [실함수](https://ko.wikipedia.org/wiki/실함수 "wikilink"), [복소함수](https://ko.wikipedia.org/wiki/복소함수 "wikilink") 가운데 *S*에서 [가측 함수](https://ko.wikipedia.org/wiki/가측_함수 "wikilink") *f*, *g*에 대하여 다음과 같은 관계가 성립한다.
...
이때 *p*와 *q*는 **횔더 켤레**(**Hölder conjugates**)이다. *p* = *q* = 2인 경우, 이 부등식은 [코시-슈바르츠 부등식이](https://ko.wikipedia.org/wiki/코시-슈바르츠_부등식 "wikilink") 된다.

횔더 부등식은 L<sup>*p*</sup>(*μ*)에서 [삼각 부등식과](https://ko.wikipedia.org/wiki/삼각_부등식 "wikilink") [민코프스키 부등식을](https://ko.wikipedia.org/wiki/민코프스키_부등식 "wikilink") 증명하기 위해 사용하며, L<sup>*p*</sup>(*μ*)의 [쌍대공간](https://ko.wikipedia.org/wiki/쌍대공간 "wikilink") L<sup>*q*</sup>(*μ*)를 구성하기 위해 사용한다. (1 ≤ *q* \< ∞)
...
\((\Omega,\mathcal{F},\mathbb{P})\)를 [확률 공간이라](https://ko.wikipedia.org/wiki/확률_공간 "wikilink") 하고, \(\mathcal{G}\subset\mathcal{F}\)를 부분 [시그마 대수라](../Page/시그마_대수.md "wikilink") 하고, 횔더 켤레 *p*, *q* ∈ (1,∞)라 하면, 모든 실수, 복소수 값을 갖는 Ω에서의 확률 변수 *X*, *Y*에 대하여 다음이 성립한다.
...
*p*와 *q*가 (1,∞) [열린구간](https://ko.wikipedia.org/wiki/열린구간 "wikilink")에 속한다고 가정하자.
  ...
  - *n*차원 [유클리드 공간의](https://ko.wikipedia.org/wiki/유클리드_공간 "wikilink") 집합 *S* = {1, …, *n*} 가 [셈측도](https://ko.wikipedia.org/wiki/셈측도 "wikilink")를 측도로 가질 때, 다음 부등식이 성립한다.
...
\text{ for all }(x_1,\ldots,x_n),(y_1,\ldots.y_n)\in\mathbb{R}^n\text{ or }\mathbb{C}^n.\]
  ...
  - 만약 \(S=\mathbb N\)이 [셈측도](https://ko.wikipedia.org/wiki/셈측도 "wikilink")를 측도로 가질 때, [수열 공간에서의](https://ko.wikipedia.org/wiki/수열_공간 "wikilink") 횔더 부등식을 얻을 수 있다.
...
\[\int_S \bigl| f(x)g(x)\bigr| \,\mathrm{d}x \le\biggl(\int_S |f(x)|^p\,\mathrm{d}x \biggr)^{\!1/p\;} \biggl(\int_S |g(x)|^q\,\mathrm{d}x\biggr)^{\!1/q}.\]
  ...
  - [확률 공간](https://ko.wikipedia.org/wiki/확률_공간 "wikilink") \((\Omega,\mathcal{F},\mathbb{P})\)에서 , \(\operatorname{E}\)를 [기댓값](../Page/기댓값.md "wikilink") 연산으로 정의하자. Ω에서 실수, 복소수값을 갖는 [확률 변수](../Page/확률_변수.md "wikilink") *X*와 *Y*에 대해, 횔더 부등식은 다음과 같이 성립한다.
...
\[\operatorname{E}|X|^r\le\bigl(\operatorname{E}|X|^s\bigr)^{r/s}.\]
  ...
  -
    *s*의 절대 [모멘트](https://ko.wikipedia.org/wiki/모멘트 "wikilink")가 유한할 때, *r*의 절대 모멘트도 유한하다. (이 결과는 [옌센 부등식을](https://ko.wikipedia.org/wiki/옌센_부등식 "wikilink") 통해서도 얻을 수 있다.)
...
횔더 부등식은 L.J 로저스가 [1888년](https://ko.wikipedia.org/wiki/1888년 "wikilink")에 처음 찾아내었고, 이와는 독립적으로 횔더가 [1889년](https://ko.wikipedia.org/wiki/1889년 "wikilink")에 발견하였다.
