...
**컴퓨터 구조**(computer architecture)는 [컴퓨터 공학에서](https://ko.wikipedia.org/wiki/컴퓨터_공학 "wikilink") 컴퓨터 시스템의 기능(functionality), 조직(organization), 구현(implementation)에 대한 법칙과 방법을 통칭한다. 또 다른 정의로는 컴퓨터 구조는 [명령어 집합 구조](https://ko.wikipedia.org/wiki/명령어_집합 "wikilink")(Instruction set architecture, ISA), [마이크로아키텍처](../Page/마이크로아키텍처.md "wikilink")(Microarchitecture) 설계, 논리 설계 및 [구현](https://ko.wikipedia.org/wiki/구현 "wikilink")을 의미한다.\[1\]

## 역사

컴퓨터 구조의 첫번째 문서는 1837년 [찰스 배비지가](https://ko.wikipedia.org/wiki/찰스_배비지 "wikilink") 고안한 [해석기관](../Page/해석기관.md "wikilink")으로 실제 제작되지는 않았지만, 논리적 설계는 범용 컴퓨터의 모습을 예측한 중요한 모델로 평가된다. 1936년 [불 논리](https://ko.wikipedia.org/wiki/불_논리 "wikilink") 체계와 프로그래밍이 가능한 세계 최초의 기계식 컴퓨터 [Z1을](../Page/Z1_\(컴퓨터\).md "wikilink") 만들 때, [콘라트 추제는](https://ko.wikipedia.org/wiki/콘라트_추제 "wikilink") 향후 자신의 프로젝트에 대한 두 가지 특허 출원서에 명령어와 데이터가 동일한 저장장치에 저장하는 [프로그램 내장식 컴퓨터](https://ko.wikipedia.org/wiki/프로그램_내장식_컴퓨터 "wikilink") 개념을 최초로 기술하였다.\[2\]\[3\] 이 개념은 1945년에 컴퓨터 구조의 두 가지 중요한 발자취로 이어진다.

  - [존 폰 노이만은](https://ko.wikipedia.org/wiki/존_폰_노이만 "wikilink") 1945년 논문 "EDVAC에 대한 보고서의 첫 번째 초안(First Draft of a Report on the EDVAC)"에서 논리적 요소에 대한 구성을 기술\[4\]
  - [앨런 튜링은](https://ko.wikipedia.org/wiki/앨런_튜링 "wikilink") [존 폰 노이만의](https://ko.wikipedia.org/wiki/존_폰_노이만 "wikilink") 논문을 인용하여 1945년에 더 자세하게 제안된 ACE(Automatic Computing Engine)\[5\]

[컴퓨터 공학에서](https://ko.wikipedia.org/wiki/컴퓨터_공학 "wikilink") 컴퓨터 아키텍처라는 용어는 1959년 IBM 연구소의 라일 존슨(Lyle R. Johnson)과 [프레더릭 브룩스](https://ko.wikipedia.org/wiki/프레더릭_브룩스 "wikilink")(Frederick P. Brooks, Jr.)에 의해 사용되었다. 존슨은 [로스앨러모스 국립 연구소](https://ko.wikipedia.org/wiki/로스앨러모스_국립_연구소 "wikilink")(당시 로스앨러모스 과학 연구소)용 슈퍼컴퓨터인 스트레치(Stretch)를 세부 수준까지 상세히 기술하기 위해서 형식, 명령 종류, 하드웨어 매개변수, 속도 향상 등에 관한 설명은 "기계 구성"보다는 "시스템 아키텍처" 수준의 용어가 더 적합하다고 언급하였다.\[6\] 그 후, 스트레치 설계자인 [프레더릭 브룩스는](https://ko.wikipedia.org/wiki/프레더릭_브룩스 "wikilink") 1962년 컴퓨터 시스템 계획: 프로젝트 스트레치(Planning a Computer System: Project Stretch)에서 다음과 같이 언급했다.\[7\]
...
컴퓨터 구조는 기본적으로 3가지 하부 분류가 있다.\[8\]
  ...
  - [명령어 집합 구조](https://ko.wikipedia.org/wiki/명령어_집합 "wikilink")(Instruction set architecture, ISA) : 프로세서가 읽고 실행할 수 있는 [기계어](https://ko.wikipedia.org/wiki/기계어 "wikilink")
  - [마이크로아키텍처](../Page/마이크로아키텍처.md "wikilink")(혹은 컴퓨터 조직) : 특정 프로세서가 ISA를 어떻게 구현할 것인지를 설명.
  - 시스템 디자인 : 컴퓨팅 시스템 내 모든 하드웨어 구성품을 포함하는 것을 의미
      - 데이터 경로
      - [메모리 컨트롤러](https://ko.wikipedia.org/wiki/메모리_컨트롤러 "wikilink") 및 계층
...
## 같이 보기
  ...
  - [추상화 계층](https://ko.wikipedia.org/wiki/추상화_계층 "wikilink")
