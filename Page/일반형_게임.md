...
**일반형 게임** 또는 보수 행렬(Normal-form game 또는 pay-off matrix)이란, [게임 이론에](../Page/게임_이론.md "wikilink") 등장하는 게임의 종류 중 하나로, **전략형 게임**이라고도 한다. [전개형 게임과](https://ko.wikipedia.org/wiki/전개형_게임 "wikilink") 달리 [그래프](https://ko.wikipedia.org/wiki/그래프 "wikilink") 모양을 취하지 않고, [행렬](../Page/행렬.md "wikilink")의 형태를 취한다. 이 형태는 강우열전략과 [내쉬 균형을](https://ko.wikipedia.org/wiki/내쉬_균형 "wikilink") 찾는데 유용하나, 전개형 게임에 비해 몇 가지 정보를 표시할 수 없다는 단점이 있다. 일반형 게임은 각 경기자의 모든 선택 가능한 전략과 보수를 표현할 수 있고 특히 행렬로 표현할수있으며 이때 이러한 행렬을 '보수행렬'(payoff matrix)이라고 한다.
