...
**변환 색인 버퍼**(Translation Lookaside Buffer, TLB)는 [가상 메모리 주소를](https://ko.wikipedia.org/wiki/가상_메모리_주소 "wikilink") [물리적인 주소로](https://ko.wikipedia.org/wiki/물리_주소 "wikilink") 변환하는 속도를 높이기 위해 사용되는 [캐시](https://ko.wikipedia.org/wiki/캐시 "wikilink")로, 약칭은 **TLB**이다.\[1\] TLB는 최근에 일어난 가상 메모리 주소와 물리 주소의 변환 테이블을 저장하기 때문에 일종의 주소 변환 캐시라고 할 수 있다. TLB는 [CPU](https://ko.wikipedia.org/wiki/CPU "wikilink")와 [CPU 캐시](https://ko.wikipedia.org/wiki/CPU_캐시 "wikilink") 사이, CPU 캐시와 메인 메모리 사이 등 여러가지 다른 레벨의 캐시들 사이에서 주소를 변환하는데 사용할 수 있다. 현재 모든 [데스크탑](https://ko.wikipedia.org/wiki/데스크탑 "wikilink") 및 [서버](https://ko.wikipedia.org/wiki/서버 "wikilink")용 프로세서는 하나 또는 그 이상의 TLB를 메모리 관리 하드웨어에 가지고 있다. [페이지](../Page/페이지.md "wikilink") 단위나 [세그먼트](https://ko.wikipedia.org/wiki/세그먼트 "wikilink") 단위로 처리하는 가상 메모리를 사용하는 거의 모든 하드웨어는 TLB를 사용한다. [CPU는](https://ko.wikipedia.org/wiki/중앙_처리_장치 "wikilink") 1차적으로 TLB에 접근하여 원하는 페이지가 존재하는지 탐색하고, TLB에 존재하지 않을 경우 [MMU의](../Page/메모리_관리_장치.md "wikilink") [페이지 테이블을](../Page/페이지_테이블.md "wikilink") 참조한다.
