...
**x86 메모리 분할**(x86 memory segmentation)은 인텔 [x86](https://ko.wikipedia.org/wiki/x86 "wikilink") 명령어 구조를 사용하는 컴퓨터에서 구현된 [메모리 분할기법을](https://ko.wikipedia.org/wiki/메모리_세그먼트 "wikilink") 말한다.

메모리 분할은 1978년 발표된 [인텔 8086](https://ko.wikipedia.org/wiki/인텔_8086 "wikilink") 에서 도입되어 프로그램이 메모리상의 640 KiB 이상을 사용할 수 있도록 하였다. 1982년 출시된 [인텔 80286](https://ko.wikipedia.org/wiki/인텔_80286 "wikilink") 에서는 개량된 메모리 분할 기법이 도입되어, [가상 메모리와](https://ko.wikipedia.org/wiki/가상_메모리 "wikilink") [메모리 보호를](https://ko.wikipedia.org/wiki/메모리_보호 "wikilink") 지원할수 있게 되었고, 이 시점부터 기존 8086 에서 사용되던 기본 운영 모드는 **[리얼 모드](https://ko.wikipedia.org/wiki/리얼_모드 "wikilink")**로 이름이 바뀌고, 새롭게 **[보호 모드](https://ko.wikipedia.org/wiki/보호_모드 "wikilink")**가 추가되었다.

이후에도 8086 프로세서의 하위 호환을 위해 [리얼 모드에서](https://ko.wikipedia.org/wiki/리얼_모드 "wikilink") 분할 기능을 제공하다가, 2003년에 발표된 인텔 [x86-64](https://ko.wikipedia.org/wiki/x86-64 "wikilink") 구조부터는 64 비트 모드에서 더이상 분할 기법을 지원하지 않게 되었다.

[리얼 모드와](https://ko.wikipedia.org/wiki/리얼_모드 "wikilink") [보호 모드](https://ko.wikipedia.org/wiki/보호_모드 "wikilink") 둘다 실제 메모리 주소를 계산하기 위해 16 비트의 [세그먼트 레지스터를](https://ko.wikipedia.org/wiki/세그먼트_레지스터 "wikilink") 사용하였다.

[리얼 모드](https://ko.wikipedia.org/wiki/리얼_모드 "wikilink") 에서는 메모리상에 저장된 데이터의 종류에 따라 CS, DS, SS, ES 로 나뉘는데, [코드 세그먼트](https://ko.wikipedia.org/wiki/코드_세그먼트 "wikilink") (CS)는 현재 사용 중인 프로그램의 코드가 저장된 세그먼트의 주소를 가리키고, [데이터 세그먼트](https://ko.wikipedia.org/wiki/데이터_세그먼트 "wikilink") (DS)는 현재 프로그램이 사용 중인 데이터가 저장된 세그먼트의 주소를 가리키며, [스택 세그먼트](https://ko.wikipedia.org/wiki/스택_세그먼트 "wikilink") (SS)는 현재 프로그램에서 사용 중인 스택이 저장된 세그먼트의 주소를 가리키고, [확장 세그먼트](https://ko.wikipedia.org/wiki/확장_세그먼트 "wikilink")(EXTRA segment) (ES)는 프로그래머에 의해 결정된다. 1985년 발표된 인텔 [80386](https://ko.wikipedia.org/wiki/80386 "wikilink")에는 FS 와 GS 라는 사용처가 정해지지 않은 두 개의 세그먼트 레지스터가 추가되었다. 세그먼트 레지스터들은 운영 모드에 따라 사용 방법이 달라진다.\[1\]
...
[리얼 모드나](https://ko.wikipedia.org/wiki/리얼_모드 "wikilink") [가상 86모드에서는](https://ko.wikipedia.org/wiki/가상_86모드 "wikilink") 세그먼트의 크기가 언제나 65,536 바이트이다. (16 비트 오프셋을 사용)
