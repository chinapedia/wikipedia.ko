...
### 확정된 것
  ...
  - 3D 트라이 게이트
  - [고급 벡터 확장](https://ko.wikipedia.org/wiki/고급_벡터_확장 "wikilink")2 (AVX2) 사용\[4\]
  - 듀얼 채널 DDR3 기본적 지원\[5\]
  - 14구간의 파이프라인\[6\]
  - 코어당 L1 캐시메모리는 64kb(명령 32kb+데이터 32kb), L2 캐시메모리는 256kb\[7\]
  - 새로운 소켓 (LGA 1150)\[8\]
  - [단일 곱셈-누산기](https://ko.wikipedia.org/wiki/단일_곱셈-누산기 "wikilink")3() 명령어 지원\[9\]
...
브로드웰은 인텔의 [틱-톡](https://ko.wikipedia.org/wiki/인텔_틱-톡 "wikilink") 전략에 따라 하스웰 출시 후 1년 뒤에 14 nm 공정으로 다이 축소를 하여 제조될 마이크로아키텍처의 코드네임이다. 한때 데스크탑용은 나오지 않고 모바일용으로만 출시될 것으로 알려지기도 하였으나\[29\], 차후에 코어 클럭 배율 잠금이 해제된 제품(Broadwell-K 시리즈)에 한해 데스크탑용으로도 출시되는 것으로 공식 확인되었다.\[30\]
...
## 같이 보기
  ...
  - [샌디 브리지 마이크로아키텍처](https://ko.wikipedia.org/wiki/샌디브리지 "wikilink")
