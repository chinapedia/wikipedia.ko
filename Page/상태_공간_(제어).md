...
[제어에서](https://ko.wikipedia.org/wiki/제어_이론 "wikilink") **상태 공간 표현식**(State space representation)이란 물리적 계를 입력, 출력, 상태 변수의 1차 [미분 방정식으로](https://ko.wikipedia.org/wiki/미분_방정식 "wikilink") 표현하는 수학적 모델이다. 다수의 입력, 출력, 상태를 간결하게 표현하기 위하여 변수를 [벡터](https://ko.wikipedia.org/wiki/벡터 "wikilink")로 표시하며, [동역학계](https://ko.wikipedia.org/wiki/동역학계 "wikilink")가 선형이고 시간에 따라 변하지 않을 경우, 미분 방정식과 수식은 [행렬](https://ko.wikipedia.org/wiki/행렬 "wikilink") 형태로 쓰인다. 상태 공간 표현식 (또는 시간-영역 접근 방식) 은 편리하고 간결하게 입력과 출력이 다수인 시스템을 모델하고 분석할 수 있다. 달리 \(p\)개의 입력과 \(q\)개의 출력을 가지고 있는 시스템을 표현하려면 \(q \times p\)개의 [라플라스 변환식을](https://ko.wikipedia.org/wiki/라플라스_변환 "wikilink") 사용해야만 시스템 전체의 정보를 부호화할 수 있을 것이다. 주파수 영역 접근 방식과는 달리 상태 공간 표현식은 선형 요소와 0-초기 조건에만 적용 가능한 것은 아니다. "상태 공간"은 공간의 각 축이 상태 변수인 공간을 가리킨다. 시스템의 상태는 그 공간 안의 벡터에 의해 표시될 수 있다.

## 상태변수

[framed](https://ko.wikipedia.org/wiki/파일:Typical_State_Space_model.png "wikilink")
...
전달함수의 분모는 \(sI - A\)의 [행렬식](https://ko.wikipedia.org/wiki/행렬식 "wikilink")으로 얻어지는 [특성 다항식과](https://ko.wikipedia.org/wiki/특성_다항식 "wikilink") 같다.

  -
    \(\mathbf{\lambda}(s) = |sI - A|\)

이 다항식의 근 ([고윳값](https://ko.wikipedia.org/wiki/고윳값 "wikilink"))이 시스템 전달함수의 극(즉, 전달함수의 증폭비가 무한대가 되는 특이점)이다. 이 극은 시스템이 점근 안정인지 경계 안정인지 분석하는 데 사용될 수 있다. 안정성을 판정하기 위한 다른 접근 방법은, 고윳값을 계산하지 않는 방법으로, 시스템의 [랴푸노프 안정성을](https://ko.wikipedia.org/wiki/랴푸노프_안정성 "wikilink") 해석하는 것이다. 영은 \(\textbf{G}(s)\)의 분자에서 찾을 수 있는 것으로 비슷하게 시스템이 최소 위상 여부를 판정하는 데 사용될 수 있다. 시스템은 **입력–출력 안정** ([BIBO 안정](https://ko.wikipedia.org/wiki/BIBO_안정 "wikilink") 참조)이면서도 심지어 내적 불안정일 수도 있다. 이는 어떤 경우인가 하면, 불안정한 극이 영에 의해 상쇄되는 경우이다.
...
\[\operatorname{rank}\begin{bmatrix}B& AB& A^{2}B& ...& A^{n-1}B\end{bmatrix} = n\] (rank 는 [계수](https://ko.wikipedia.org/wiki/계수_\(선형대수학\) "wikilink") 로 행렬에서 선형 독립인 행의 수를 말한다.)
...
첫 번째로 다음 식의 [라플라스 변환을](https://ko.wikipedia.org/wiki/라플라스_변환 "wikilink") 취한다.
