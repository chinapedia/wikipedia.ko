...
[섬네일](https://ko.wikipedia.org/wiki/파일:Dirac_distribution_CDF.svg "wikilink") **단위 계단 함수**(unit step function) 또는 **헤비사이드 계단 함수**(Heaviside step function)은 0보다 작은 [실수](https://ko.wikipedia.org/wiki/실수 "wikilink")에 대해서 0, 0보다 큰 실수에 대해서 1, 0에 대해서 1/2의 값을 갖는 [함수](https://ko.wikipedia.org/wiki/함수 "wikilink")이다. 이 함수는 [신호처리](https://ko.wikipedia.org/wiki/신호처리 "wikilink") 분야에서 자주 사용된다.

단위 계단 함수는 [디랙 델타 함수의](https://ko.wikipedia.org/wiki/디랙_델타_함수 "wikilink") [부정적분](https://ko.wikipedia.org/wiki/부정적분 "wikilink")이다. 즉,
...
과 같이 되며 이 때 *n*은 [정수](https://ko.wikipedia.org/wiki/정수 "wikilink")이다. 주어진 문제가 이산적이지 않은 상황에서는 *H*\[0\]의 정의가 중요하다.

이산-시간 단위 충격량은 이산-시간 단계에서 첫 번째 차이값을 의미하는

\[\delta\left[ n \right] = H[n] - H[n-1]\]

으로 나타낼 수 있다. 이 함수는 [크로네커 델타의](https://ko.wikipedia.org/wiki/크로네커_델타 "wikilink") 합
...
때때로 [복소 적분의](https://ko.wikipedia.org/wiki/복소해석학 "wikilink") 형태로도 나타낼 수 있다:
