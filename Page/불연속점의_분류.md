...
[연속 함수의](https://ko.wikipedia.org/wiki/연속_함수 "wikilink") 이론에서, [함수](https://ko.wikipedia.org/wiki/함수 "wikilink")의 **불연속점**(不連續點, )은 연속점이 아닌, [정의역](https://ko.wikipedia.org/wiki/정의역 "wikilink") 속의 점이다. 함수의 불연속점의 집합은 [이산 집합이거나](https://ko.wikipedia.org/wiki/이산_집합 "wikilink") [조밀 집합일](https://ko.wikipedia.org/wiki/조밀_집합 "wikilink") 수 있으며, 함수의 정의역 전체일 수 있다. 불연속점을 연속이 실패하는 원인이 무엇인지에 따라 분류할 수 있다. 일부 종류의 불연속점은 자연스럽게 연속점이 되게 메워줄 수 있으며, 일부는 그럴 수 없다.
...
실변수 실숫값 함수의 제1종 불연속점의 집합은 [가산 집합이다](https://ko.wikipedia.org/wiki/가산_집합 "wikilink").  편의상, 실수 구간 \(I\subseteq\mathbb R\)에 정의된 실숫값 함수 \(f\colon I\to\mathbb R\)만을 생각하자. \(f\)의 불연속점 집합을 \(E\subseteq I\)라고 하자. 그렇다면, 다음이 성립한다.
...
## 같이 보기
  ...
  - [특이점 (해석학)](https://ko.wikipedia.org/wiki/특이점_\(해석학\) "wikilink")
