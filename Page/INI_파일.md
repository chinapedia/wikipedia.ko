...
가상의 프로그램(실제로 존재하지는 않음)을 위한 INI 파일의 예는 다음과 같다. 두 개의 섹션을 가지고 있으며, 그 가운데 한 섹션은 소프트웨어의 소유자에 대한 것이며, 나머지 하나는 종업원 명부 데이터베이스 연결에 대한 것이다. 또, 파일을 마지막으로 수정한 날짜, [도메인 이름](https://ko.wikipedia.org/wiki/도메인_이름_시스템 "wikilink") 대신 [IP 주소가](../Page/IP_주소.md "wikilink") 쓰인 까닭에 대해 주석을 적고 있다.
