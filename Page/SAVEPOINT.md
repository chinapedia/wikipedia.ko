...
**SAVEPOINT**는 하위 [트랜잭션](../Page/데이터베이스_트랜잭션.md "wikilink") (중첩 트랜잭션이라고도 함)을 실현하기 위한 [데이터베이스 언어](../Page/데이터베이스_언어.md "wikilink") SQL 구문 중 하나이다. 트랜잭션의 특정 지점에 이름을 지정하고, 그 지점 이전에 수행 한 작업에 영향을 주지 않고 그 지점 이후에 수행한 작업을 [롤백](https://ko.wikipedia.org/wiki/롤백 "wikilink")(ROLLBACK)할 수 있다. 단일 트랜잭션에서 여러 SAVEPOINT를 만들 수도 있다.
