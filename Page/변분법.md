...
**변분법**(變分法, )이란 [미적분학](https://ko.wikipedia.org/wiki/미적분학 "wikilink")의 한 분야로, 일반 [미적분학](https://ko.wikipedia.org/wiki/미적분학 "wikilink")과는 달리 [범함수](https://ko.wikipedia.org/wiki/범함수 "wikilink")를 다룬다. 이런 미적분학은 알려지지 않은 함수와 이 함수의 도함수를 다루는데, 주로, 어떠한 값을 최대화 하거나, 최소화 하는 함수 모양이 어떻게 되는가를 다룬다. [아이작 뉴턴이](https://ko.wikipedia.org/wiki/아이작_뉴턴 "wikilink") 베르누이가 도전형식으로 낸 난제를 풀기 위해 하룻밤사이에 만들어낸 분야라고 한다.

## 정류값

변분법은 범함수의 극댓값, 극솟값을 연구하는데, 이를 합쳐서 **정류값**이라한다. [함수](https://ko.wikipedia.org/wiki/함수 "wikilink")가 [변수](https://ko.wikipedia.org/wiki/변수 "wikilink")에 의존하듯이, 범함수는 [함수](https://ko.wikipedia.org/wiki/함수 "wikilink")에 의존하므로 흔히 함수의 함수로 설명한다. 범함수는 정의역의 원소인 \(y\)에 대해 정류값을 갖는다. 범함수 \(J[y]\)가 함수 \(f\)에서 정류값을 갖는다는 것은 \(\Delta J = J[y]-J[f]\)이 \(f\)의 미소근방에서 같은 [부호](https://ko.wikipedia.org/wiki/부호 "wikilink")를 갖는다는 것이다. 함수 \(f\)은 ‘’정류‘’함수 또는 정류점이라 한다. 정류값 \(J[f]\)은 \(f\)의 미소근방에서 \(\Delta J \le 0\)이면 극댓값이라 하고 \(\Delta J \ge 0\)이면 극솟값이라 한다.
...
[평균값 정리에](https://ko.wikipedia.org/wiki/평균값_정리 "wikilink") 의해 미분해서 0이되는 함수는 그 구간에선 상수함수이므로,
