> This article is converted from Wikipedia: [위키백과:위키상형 문법](https://ko.wikipedia.org/wiki/위키백과:위키상형_문법).


"위키상형"은 png 그림으로 [이집트 상형 문자를](https://ko.wikipedia.org/wiki/이집트_상형_문자 "wikilink") 렌더하기 위한 PHP 스크립트입니다. 이집트 상형 문자는 현재 부분적으로만 [유니코드](../Page/유니코드.md "wikilink")가 지원되므로 이는 올린 사용자 정의 그림의 짧은 문서 텍스트에 상형 문자를 렌더링할 수 있는 유일한 방법입니다.

스크립트는 \<hiero\> 태그에 의해 호출됩니다. 예를 들어 \<hiero\>A\</hiero\>는 "[Aleph](https://ko.wikipedia.org/wiki/Aleph "wikilink")"상형 문자를 얻습니다. <hiero>A</hiero>

[미디어위키에](https://ko.wikipedia.org/wiki/:mw: "wikilink") 있는 프로젝트 문서에 대해 [:mw:Extension:WikiHiero](https://ko.wikipedia.org/wiki/:mw:Extension:WikiHiero "wikilink")를 참고하십시오.

## 상형 문자

상형 문자를 인코딩하는 방법은 두 가지가 있습니다:

  - 가디의 기호 목록 코드를 사용 (e.g. `A12`)
  - 음소 빈도로 (e.g. `xA`)

### 가디의 기호 목록

상형문자 글자 하나는 다음과 같은 기호를 할당받습니다.

  - 분류를 나타내는 글자
  - 분류 내의 순서

소프트웨어가 활성화되지 않았다면 상형 문자 대신 이 기호가 대신 표시됩니다.
...
[고대 이집트의 음역](https://ko.wikipedia.org/wiki/고대_이집트의_음역 "wikilink") 문서를 참고하십시오.

## 코드

  - 코드는 문자입니다
  - 공간의 상형 문자 조직을 나타냅니다 (즉, 블록에있는 상형 문자의 위치입니다)
...
| 올바름 | `A1-B1` | <hiero>A1-B1</hiero> |
| --- | ------- | -------------------- |
| 허용  | `A1 B1` | <hiero>A1 B1</hiero> |
| 잘못됨 | `A1B1`  | <hiero>A1B1</hiero>  |
...
| 기본 레이아웃 | `p-t` | <hiero>p-t</hiero> |
| ------- | ----- | :----------------: |
| 위에 둠    | `p:t` | <hiero>p:t</hiero> |
...
<tr class="even">
<td><p>허용<br />
<em>although apparently<br />
</em>not'' fully functional''</p></td>
<td><p><code>A1-B1! C1-D1</code></p></td>
<td><p><hiero>A1-B1! C1-D1</hiero></p></td>
</tr>
</tbody>
