...
\-- Finds whether a Wikidata property is qualified as being in English. local function isEnglish(prop)
...
`           '`</strong>`'`
`       if qid then`
`           result = result.. ' `[`frameless`](https://ko.wikipedia.org/wiki/파일:OOjs_UI_icon_edit-ltr-progressive.svg "wikilink")`'`
`       end`
`       return result`
`   end`
`   local ret = {}`
...
`       local qid = entity.id`
`       if qid then`
`           ret[#ret + 1] = '`[`frameless`](https://ko.wikipedia.org/wiki/파일:OOjs_UI_icon_edit-ltr-progressive.svg "wikilink")`'`
`       end`
`   end`
...
`   end`
`   return category and string.format('`[`분류:%s`](https://ko.wikipedia.org/wiki/분류:%s "wikilink")`', category) or ''`
