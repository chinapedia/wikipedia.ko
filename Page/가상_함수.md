...
[객체 지향 프로그래밍에서](https://ko.wikipedia.org/wiki/객체_지향_프로그래밍 "wikilink"), **가상 함수**(virtual function 또는 **virtual method**)는 상속하는 클래스 내에서 같은 시그니처의 함수로 [오버라이딩](https://ko.wikipedia.org/wiki/함수_오버라이딩 "wikilink") 될 수 있는 [함수](https://ko.wikipedia.org/wiki/함수_\(프로그래밍\) "wikilink") 또는 [메소드이다](../Page/메소드_\(컴퓨터_프로그래밍\).md "wikilink"). 이 개념은 [객체 지향 프로그래밍](https://ko.wikipedia.org/wiki/객체_지향_프로그래밍 "wikilink") (OOP)의 [다형성](https://ko.wikipedia.org/wiki/다형성 "wikilink")에서 중요한 부분이다.
...
순수 가상 함수들은 또한 메소드 정의가 인터페이스를 정의하는데 쓰일 때 사용될 수 있다. 이것은 자바에서 인터페이스 키워드와 비슷하다. 이러한 사용에서, 유도 클래스들은 모든 구현을 제공할 것이다. [디자인 패턴](https://ko.wikipedia.org/wiki/디자인_패턴 "wikilink") 같은 경우, 인터페이스를 위한 추상 클래스는 오직 순수 가상 함수만 갖으며, 데이터 멤버나 메소드들은 갖지 않는다. C++에서는 이러한 순수 추상 클래스들을 인터페이스 방식으로 사용한다. 왜냐하면 C++은 [다중 상속을](https://ko.wikipedia.org/wiki/다중_상속 "wikilink") 지원하기 때문이다. 하지만 많은 OOP 언어들은 이것을 지원하지 않기 때문에, 종종 인터페이스 메커니즘을 따로 제공하여야 한다. 예를 들면 [자바가](https://ko.wikipedia.org/wiki/자바_\(프로그래밍_언어\) "wikilink") 있다.

## 생성과 소멸 사이의 행위

객체의 [생성자](https://ko.wikipedia.org/wiki/생성자 "wikilink")나 [소멸자의](https://ko.wikipedia.org/wiki/소멸자_\(컴퓨터_프로그래밍\) "wikilink") 실행 중에, 언어들은 각자의 행위를 한다. 몇몇 언어에서는 (C++ 같은) 가상 디스패칭 메커니즘이 객체의 생성과 소멸 중에 다른 의미를 갖는다. 생성자 안에서 가상 함수를 호출하는 것은 C++에서 피해야 하지만,\[1\] 다른 언어들에서는, 예를 들면 C\#과 Java, 유도 구현은 생성이나 design patterns시에 호출될 수 있다.

## 가상 소멸자

일반적으로 객체 지향 언어들은 객체들이 생성되거나 소멸될 때 자동적으로 메모리 할당과 회수를 관리한다. 그러나 몇몇 객체 지향 언어들은 커스텀하게 [소멸자](https://ko.wikipedia.org/wiki/소멸자_\(컴퓨터_프로그래밍\) "wikilink") 메소드를 구현할 수 있게 허락하기도 한다. 만약 언어가 자동 메모리 관리를 사용한다면 호출된 컴스텀 소멸자는 객체에 적절하다고 확신할 것이다. 예를 들면, 만약 Animal을 상속하는 Wolf 타입의 객체가 생성되고, 둘 다 커스텀 소멸자를 갖는다면, 호출되는 것은 Wolf에 정의된 것이 된다.
