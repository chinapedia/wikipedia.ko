...
\[E=(\mathrm TM)^{\otimes p}\otimes(\mathrm T^*\!M)^{\otimes q}\] 가 \((p,q)\)차 텐서장의 [벡터 다발일](../Page/벡터_다발.md "wikilink") 경우, \(E\) 위에는 [리만 계량에](https://ko.wikipedia.org/wiki/리만_계량 "wikilink") 의한 표준적인 [코쥘 접속인](../Page/코쥘_접속.md "wikilink") [레비치비타 접속이](https://ko.wikipedia.org/wiki/레비치비타_접속 "wikilink") 존재한다. 이 경우, 레비치비타 접속을 사용한 라플라스 연산자를 역시 **라플라스-벨트라미 연산자**라고 한다.
