> This article is converted from Wikipedia: [데이터 실행 방지](https://ko.wikipedia.org/wiki/데이터_실행_방지).


**데이터 실행 방지**(Data Execution Prevention, DEP)는 현대의 [마이크로소프트 윈도우](../Page/마이크로소프트_윈도우.md "wikilink") [운영 체제에](../Page/운영_체제.md "wikilink") 포함된 보안 기능이며, 실행 방지 메모리 영역의 실행 코드에서 [응용 프로그램이나](https://ko.wikipedia.org/wiki/응용_프로그램 "wikilink") 서비스가 실행되지 못하게 막기 위해 고안된 것이다. 이를테면 [버퍼 오버플로를](https://ko.wikipedia.org/wiki/버퍼_오버플로 "wikilink") 통한 코드를 저장하는 특정한 이용을 막을 수 있다. DEP는 두 가지 모드로 실행된다.:

  - [CPU를](../Page/중앙_처리_장치.md "wikilink") 위한 하드웨어 강화 DEP: 메모리 페이지를 실행 불가능 상태로 표시한다.
