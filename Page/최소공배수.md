...
두 정수 \(n,m\in\mathbb Z : n m\neq0\)의 **최소공배수** \(\operatorname{lcm}\{n,m\}\)는 다음과 같이 정의할 수 있으며, 이들 정의는 서로 [동치](../Page/동치.md "wikilink")이다.
  ...
  - \(n\), \(m\)의 음이 아닌 공배수이자, \(n\), \(m\)의 모든 공배수의 [약수](https://ko.wikipedia.org/wiki/약수 "wikilink")
...
## 성질
  ...
  - 여러 개의 정수들의 최소공배수는 [최대공약수](../Page/최대공약수.md "wikilink")와 다음과 같은 관계를 가진다.
    \[N_1 = \operatorname{gcd}\{N_1,N_2, \ldots, N_k \} \cdot n_1\]
    \[N_2 = \operatorname{gcd}\{N_1,N_2, \ldots, N_k \} \cdot n_2\]
    \[\ldots\]
    \[N_k = \operatorname{gcd}\{N_1,N_2, \ldots, N_k \} \cdot n_k\]일 때,
      -

        \(\operatorname{lcm}\{N_1,N_2,\ldots, N_k\}= \operatorname{gcd}\{N_1,N_2, \ldots, N_k \} \cdot n_1 n_2\ldots n_k\)
  ...
  - [소인수분해](https://ko.wikipedia.org/wiki/소인수분해 "wikilink")가 주어진 정수들의 최대공배수는 공통된 소인수의 최대 지수 거듭제곱의 곱이다. 두 정수의 경우, 소인수분해가
...
[통분](https://ko.wikipedia.org/wiki/통분 "wikilink")은 [분수끼리](https://ko.wikipedia.org/wiki/분수_\(수학\) "wikilink") 더하거나 뺄 때 사용되는 기법이다. 통분 과정에서 최소공분모(=분모의 최소공배수)를 공분모로서 사용하면, 분모의 곱을 사용하는 경우보다 더 쉽게 계산할 수 있다. 예를 들어,
