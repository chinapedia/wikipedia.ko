> This article is converted from Wikipedia: [후킹](https://ko.wikipedia.org/wiki/후킹).


**후킹**()은 [소프트웨어 공학](../Page/소프트웨어_공학.md "wikilink") 용어로, [운영 체제나](../Page/운영_체제.md "wikilink") [응용 소프트웨어](../Page/응용_소프트웨어.md "wikilink") 등의 각종 [컴퓨터 프로그램에서](../Page/컴퓨터_프로그램.md "wikilink") [소프트웨어 구성 요소](https://ko.wikipedia.org/wiki/소프트웨어_구성_요소 "wikilink") 간에 발생하는 [함수 호출](https://ko.wikipedia.org/wiki/함수_호출 "wikilink"), [메시지](../Page/메시지.md "wikilink"), [이벤트](https://ko.wikipedia.org/wiki/이벤트 "wikilink") 등을 중간에서 바꾸거나 가로채는 [명령](https://ko.wikipedia.org/wiki/명령어 "wikilink"), 방법, 기술이나 행위를 말한다. 이때 이러한 간섭된 [함수 호출](https://ko.wikipedia.org/wiki/함수_호출 "wikilink"), [이벤트](https://ko.wikipedia.org/wiki/이벤트 "wikilink") 또는 [메시지](../Page/메시지.md "wikilink")를 처리하는 코드를 **후크**()라고 한다.

[크래킹](https://ko.wikipedia.org/wiki/크래킹 "wikilink")(불법적인 [해킹](../Page/해킹.md "wikilink"))을 할 때 크래킹 대상 컴퓨터의 [메모리](../Page/기억_장치.md "wikilink") 정보, [키보드](../Page/컴퓨터_자판.md "wikilink") 입력 정보 등을 빼돌리기 위해서 사용되기도 한다.

예를 들어 특정한 API를 후킹하게 되면 해당 API의 리턴값을 조작하는 등의 동작을 수행할 수 있다.

## 방법

일반적으로 후크는 [소프트웨어](../Page/소프트웨어.md "wikilink")가 이미 실행중일때 삽입되지만, 후킹은 [응용 프로그램이](https://ko.wikipedia.org/wiki/응용_프로그램 "wikilink") 실행되기도 전에 쓰일 수 있는 전략이다. 이 두 기술은 아래에 더 자세히 설명되어 있다.
...
다음은 윈도에서 [DirectDraw](https://ko.wikipedia.org/wiki/DirectDraw "wikilink")(다이렉트드로) 함수 호출을 후킹한 예이다. 이것은 APIHijack(에이피아이 하이잭)이라는 후킹 라이브러리를 활용한 것이고, 소스는 DLL([동적 링크 라이브러리](../Page/동적_링크_라이브러리.md "wikilink"))로 [컴파일](https://ko.wikipedia.org/wiki/컴파일 "wikilink")되었다. 인스톨훅(InstallHook)을 호출할 추가적인 [응용 프로그램도](https://ko.wikipedia.org/wiki/응용_프로그램 "wikilink") 필요하다.\[1\]
...
다음 소스 코드는 [오픈 소스인](../Page/오픈_소스.md "wikilink") Chrom(크롬) 라이브러리를 사용하는 API/함수 후킹의 예시이다. 이는 새로운 함수에 대한 [JMP](../Page/JMP_\(x86_명령어\).md "wikilink") 명령으로 대상 함수의 처음 6바이트를 덮어써서 후크한다. 코드는 DLL로 컴파일 된 후, 코드 인젝션의 방법을 사용하여 대상 프로세스에 로드된다. 이 예제 코드는 "nspr4.dll\[2\]"의 "PR_Write" 함수와 "c:/log.txt"에 기록된 모든 [HTTP](../Page/HTTP.md "wikilink")/[HTTPS](../Page/HTTPS.md "wikilink") 요청을 후크한다.\[3\]
