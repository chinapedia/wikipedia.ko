...
**암호화 해시 함수**(cryptographic hash function)은 [해시 함수의](https://ko.wikipedia.org/wiki/해시_함수 "wikilink") 일종으로, 해시 값으로부터 원래의 입력값과의 관계를 찾기 어려운 성질을 가지는 경우를 의미한다. 암호화 해시 함수가 가져야 하는 성질은 다음과 같다.\[1\]

  - 역상 저항성(preimage resistance): 주어진 해시 값에 대해, 그 해시 값을 생성하는 입력값을 찾는 것이 계산상 어렵다. 즉, [제 1 역상 공격에](https://ko.wikipedia.org/wiki/역상_공격 "wikilink") 대해 안전해야 한다. 이 성질은 [일방향함수](https://ko.wikipedia.org/wiki/일방향함수 "wikilink")와 연관되어 있다.
  ...
  - 충돌 저항성(collision resistance): [해시 충돌에](https://ko.wikipedia.org/wiki/해시_충돌 "wikilink") 대해 안전해야 한다. 같은 해시 값을 생성하는 두 개의 입력값을 찾는 것이 계산상 어려워야 한다.
...
[순환 중복 검사](https://ko.wikipedia.org/wiki/순환_중복_검사 "wikilink")(CRC)와 같은 몇몇 해시 함수는 암호 안전성에 필요한 저항성을 가지지 않으며, 우연적인 손상을 검출할 수는 있지만 의도적인 손상의 경우 검출되지 않도록 하는 것이 가능하기도 하다. 가령, [유선 동등 프라이버시](https://ko.wikipedia.org/wiki/유선_동등_프라이버시 "wikilink")(WEP)의 경우 암호화 해시 함수로 CRC를 사용하나, CRC의 암호학적 취약점을 이용한 [암호공격](https://ko.wikipedia.org/wiki/암호공격 "wikilink")이 가능하다는 것이 밝혀져 있다.

## 암호화 해시 함수의 비교

가장 널리 사용되는 해시 함수에는 [MD5](../Page/MD5.md "wikilink")와 [SHA-1](https://ko.wikipedia.org/wiki/SHA-1 "wikilink")이 있으나, 이들은 안전하지 않다는 것이 알려져 있다. 미국 US-CERT에서는 2008년 MD5를 사용하지 말아야 한다고 발표했다.\[2\] NIST에서는 2008년 SHA-1의 사용을 중지하며 [SHA-2](https://ko.wikipedia.org/wiki/SHA-2 "wikilink")를 사용할 것이라고 발표했다.\[3\]
...
| **[SHA-2](https://ko.wikipedia.org/wiki/SHA-2 "wikilink")56/224**                           | 256/224                    | 256           | 512     | 64          | 32        | 64         | [2<sup>28.5</sup>:24](http://eprint.iacr.org/2008/270.pdf)                                                            |
| **[SHA-512/384](https://ko.wikipedia.org/wiki/SHA-2 "wikilink")**                           | 512/384                    | 512           | 1,024   | 128         | 64        | 80         | [2<sup>32.5</sup>:24](http://eprint.iacr.org/2008/270.pdf)                                                            |
