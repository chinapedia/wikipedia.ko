...
**배타적 논리합**(排他的論理合, )은 [수리 논리학에서](https://ko.wikipedia.org/wiki/수리_논리학 "wikilink") 주어진 2개의 명제 가운데 1개만 참일 경우를 판단하는 [논리 연산이다](https://ko.wikipedia.org/wiki/논리_연산 "wikilink"). 약칭으로 **XOR**, **EOR**, **EXOR**라고도 쓴다.

[연산자](https://ko.wikipedia.org/wiki/연산자 "wikilink")는 \(\veebar\) ([유니코드](https://ko.wikipedia.org/wiki/유니코드 "wikilink"): U+22BB ⊻), \(\dot\vee\)이다. 혼동이 되지 않을 경우, XOR, xor, \(\oplus\) (유니코드: U+2295 ⊕), [+](https://ko.wikipedia.org/wiki/+ "wikilink"), [≠](https://ko.wikipedia.org/wiki/≠ "wikilink")라고도 쓴다.

추가로 [컴퓨터 프로그래밍](https://ko.wikipedia.org/wiki/컴퓨터_프로그래밍 "wikilink") 등에서 [응용 수학으로](https://ko.wikipedia.org/wiki/응용_수학 "wikilink"), 비트간 배타적 논리합()을 간단히 배타적 논리합, XOR이라고 부르는 경우가 있다. 연산자는 XOR, xor, \(\oplus\), [^](https://ko.wikipedia.org/wiki/^ "wikilink") 등이 사용된다.
...
추가로, 두 명제 *A*,*B* 에 대한 [교집합](https://ko.wikipedia.org/wiki/교집합 "wikilink") (*A* ∧ *B*)가 [공집합](https://ko.wikipedia.org/wiki/공집합 "wikilink")이면, 배타적 논리합은 [논리합](https://ko.wikipedia.org/wiki/논리합 "wikilink")과 같게 된다. 예를 들어, *A* = “내 키는 160cm이다.”와 *B* = “내 키는 170cm이다.”는 동시에 성립할 수 없기 때문에(교집합이 없음), (*A* xor *B*)와 (*A* ∨ *B*)는 동일하게 “내 키는 160cm와 170cm 중 하나다.”가 된다.
...
2를 몫으로 하는 [잉여류체](https://ko.wikipedia.org/wiki/환_\(수학\) "wikilink") \(\mathbb{Z} / [2]\)의 가감산(이 체에서는 덧셈과 뺄셈이 같다)은 0을 거짓, 1을 참으로 생각하면 배타적 논리합이 된다. 같은 종류(거짓이나 참)를 더하면 거짓이 되고 다른 종류를 더하면 참이 된다.
...
[이진법](https://ko.wikipedia.org/wiki/이진법 "wikilink")으로 표현한 수의 각 [비트에](https://ko.wikipedia.org/wiki/비트_\(단위\) "wikilink") 대한 2진 집합체 \(\mathbb{Z} / [2]\) 에 가감산(0은 거짓, 1은 참으로 생각하고 배타적 논리합을 함)의 결과를 **비트간 배타적 논리합**, **배타적 비트화**라고 하며 단순하게 **배타적 논리합**이라고도 한다.

비트간 배타적 논리합은 이진 [유한체](https://ko.wikipedia.org/wiki/유한체 "wikilink") \(\mathrm{GF}(2^n), n \in \mathbb{N}\) 로 가감산(덧셈과 뺄셈이 같음)에 동일하다. 추가로, \(\mathbb{Z} / [2]\)는, 이진 유한체 \(\mathrm{GF}(2)\,\) 이다.
...
## 같이 보기
  ...
  - [XOR 스왑 알고리즘](https://ko.wikipedia.org/wiki/XOR_교체_알고리즘 "wikilink")
