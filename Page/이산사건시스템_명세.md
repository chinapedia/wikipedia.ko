...
**이산사건 시스템 명세**(Discrete Event System Specification, 이하 DEVS)는 [애리조나 대학교의](https://ko.wikipedia.org/wiki/애리조나_대학교 "wikilink") 교수인 [버나드 P. 지글러](https://ko.wikipedia.org/wiki/버나드_P._지글러 "wikilink")(Bernard P. Zeigler)\[1\] 가 고안한 [모형](https://ko.wikipedia.org/wiki/모형 "wikilink")화를 위한 계층적 형식론(形式論, formalism)으로 사건의 발생에 따라 상태가 변하는 관점에서 시스템의 동적인 변화를 기술한다.

## 역사

DEVS 형식론은 지글러가 자신의 박사학위 논문을 작성하던 중 [오토마타](https://ko.wikipedia.org/wiki/오토마타_이론 "wikilink")\[2\]\[3\]에서부터 (1) 입력 사건에 의해서 상태가 천이(遷移, transition)되거나 (2) 시간의 흐름에 따라서 내부적으로 상태 천이\[4\]가 가능한 방법론을 고안하게 된다. 그는 1976년에 출간된 그의 첫 번째 저서인 *Theory of Modeling and Simulation*\[5\]에서 이러한 방법론을 [미분방정식](https://ko.wikipedia.org/wiki/미분방정식 "wikilink")으로 표현되는 연속 시스템(continuous system)과 차분방정식(difference equation)으로 표현되는 이산시간시스템(discrete time system)과 비교하여 설명하였다.
...
1.
2.  [오토마타는](https://ko.wikipedia.org/wiki/오토마타_이론 "wikilink") 지글러의 박사학위 논문에서 수학모델로 사용되었다.
3.  \[Zeigler68\]
4.  이때 출력 사건을 발생시킴
5.  \[Zeigler76\]
6.