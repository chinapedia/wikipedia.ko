...
\[S(v_1\otimes\dots\otimes v_m) = (-1)^mv_m\otimes\cdots\otimes v_1\] 여기서 \(\operatorname{Sh}(p,m-p)\subseteq\operatorname{Sym}(p)\)는 \((p,m-p)\)-[셔플 순열의](https://ko.wikipedia.org/wiki/셔플_순열 "wikilink") 집합이다.
