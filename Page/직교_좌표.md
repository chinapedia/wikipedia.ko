...
일반화된 곡선 좌표계 대신 직교 좌표계를 선호하는 이유는 단순성에 있다: 대부분의 복잡성은 좌표가 직교하지 않을 때 발생한다. 예를 들면, 직교 좌표계에서는 많은 문제들이 “변수 분리법”이라 불리는 수학적 기법을 사용하여 쉽게 풀려질 수 있다. 변수 분리법을 사용하면 복잡한 d차원의 문제가 알려진 함수들을 통해 풀려질 수 있는 d개의 일차원 문제들로 변환될 수 있다. 많은 방정식들은 [라플라스 방정식](https://ko.wikipedia.org/wiki/라플라스_방정식 "wikilink") 또는 [헬름홀츠 방정식으로](https://ko.wikipedia.org/wiki/헬름홀츠_방정식 "wikilink") 환원될 수 있으며 라플라스 방정식은 13개의 직교 좌표계들에서 분리가능하고 헬름홀쯔 방정식은 11개의 직교 좌표계들에서 분리가능하다.\[1\]\[2\]
...
메트릭 텐서의 대각 성분들의 제곱근들 또는 국소 기저 벡터들 \(\mathbf e_k\) 의 길이들과 같다. 이러한 스케일 함수들 *h<sub>i</sub>* 은 새로운 좌표계에서 미분 연산자들(예를 들면, [기울기 연산자](../Page/기울기_\(벡터\).md "wikilink")(gradient), [라플라시안](../Page/라플라스_연산자.md "wikilink"), [발산 연산자](https://ko.wikipedia.org/wiki/발산_\(벡터\) "wikilink") 및 [회전 연산자](https://ko.wikipedia.org/wiki/회전_\(벡터\) "wikilink"))를 계산하는데 사용된다.

2차원에서 직교 좌표계들을 생성하기 위한 간단한 방법은 데카르트 좌표들(*x*, *y*)의 표준적인 2차원 그리드의 [등각 매핑을](https://ko.wikipedia.org/wiki/등각_매핑 "wikilink") 통해서이다. 복소수 *z* = *x* + *iy* 는 실수 좌표*x* 와 *y*로부터 형성될 수 있다(여기서 i는 허수 단위를 나타낸다). 영이 아닌 복소 도함수를 갖는 임의의 [정칙 함수](https://ko.wikipedia.org/wiki/정칙_함수 "wikilink") *w* = *f*(*z*)는 등각 매핑을 생성한다: 그렇게 만들어진 복소수가 *w* = *u* + *iv*로 쓰여진다면, u 및 v가 상수인 곡선들은 x 및 y가 상수인 원래의 선들과 마찬가지로 직각으로 교차한다.
...
위 식은 [크로네커 델타](https://ko.wikipedia.org/wiki/크로네커_델타 "wikilink")(Kronecker delta)를 사용하여 \(\mathbf e_i \cdot \mathbf e^j = \delta^j_i\)이라는 사실로부터 얻어진다. 또한 아래와 같다:
