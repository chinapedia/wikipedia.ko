...
최근에는 [노키아](https://ko.wikipedia.org/wiki/노키아 "wikilink")(Nokia) 같은 모바일 핸드셋 제작사들이 자신들의 장비에 LBS를 내장하기 위해 자발적으로 노력하고 있지만, 원래 LBS는 [이동통신](https://ko.wikipedia.org/wiki/이동통신 "wikilink")사와 모바일 콘텐츠 공급사들의 협력 아래에서 개발되었다.

LBS의 주요 장점은 [무선 인터넷](https://ko.wikipedia.org/wiki/무선_인터넷 "wikilink") 사용자가 여러 위치를 이동하면서도 직접 주소나 지역 구분자를 입력하지 않아도 된다는 점이며, [GPS](https://ko.wikipedia.org/wiki/GPS "wikilink") 측위 기술은 이를 가능하게 하여 무선 인터넷 서비스 접근을 용이하게 해주는 주요 요소기술 중 하나이다.
...
[PC](https://ko.wikipedia.org/wiki/개인용_컴퓨터 "wikilink") 기반의 유선 인터넷 서비스 역시, 무선 인터넷 영역으로 경계 넘어 확장을 계속하면서 위치 기반 서비스 기능과의 융합(Convergence) 요구에 직면하고 있다.
...
서비스 공급자는 무선 단말에 내장된 [GPS](https://ko.wikipedia.org/wiki/GPS "wikilink")칩을 통해 위치를 측정할 수 있다. 이 경우 무선 단말은 복수 GPS 위성으로부터의 신호를 수신하고 신호로부터 위치 좌표를 계산하는 측위 기능 전체를 직접 담당하며 이동통신망을 통해 그 좌표를 입력값으로 각종 정보를 조회할 수 있다.
...
한편, 이동통신망 이외의 영역에서 사용자 또는 사물의 위치를 파악하여 서비스에 반영하는 흐름도 계속 증가하고 있다. [무선랜](https://ko.wikipedia.org/wiki/무선랜 "wikilink")을 이용한 이동식 네트워크나 [Wibro](https://ko.wikipedia.org/wiki/Wibro "wikilink") 등 새 규격의 이동식 네트워크 환경, 또는 유선 인터넷 환경에서 [IP 주소](https://ko.wikipedia.org/wiki/IP_주소 "wikilink") 기준의 사용자 위치 인식 기술도 연구와 산업화가 진행되는 중이며, 이를 포괄적으로 위치 기반 서비스를 위한 측위 기술에 포함하는 것이 타당하다.
