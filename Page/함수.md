...
[섬네일](https://ko.wikipedia.org/wiki/파일:Codomain2.SVG "wikilink") **함수** \(f\)는 다음과 같은 [튜플](../Page/튜플.md "wikilink") \((X,Y,\operatorname{graph}f)\)이다.
  ...
  - \(Y\)는 [집합](../Page/집합.md "wikilink")이며, \(f\)의 **[공역](https://ko.wikipedia.org/wiki/공역_\(수학\) "wikilink")**이라고 한다.
...
다시 말해, 함수는 정의역의 각 원소를 정확히 하나의 공역 원소에 대응시킨다. 이러한 \(y\)를 \(f(x)\)라고 쓰며, 이러한 \(y\)들의 집합을 **[치역](https://ko.wikipedia.org/wiki/치역 "wikilink")**이라고 한다. 치역은 공역의 부분 집합이나, 공역보다 작을 수 있다.

**사상**은 보통 함수의 동의어로 쓰이나, [추상대수학](../Page/추상대수학.md "wikilink")에서는 더 좁은, [범주론](https://ko.wikipedia.org/wiki/범주론 "wikilink")에서는 더 넓은 뜻을 갖는다.
...
[섬네일의](https://ko.wikipedia.org/wiki/파일:Complex_gamma.jpg "wikilink") 그래프\]\] 정의역이 [실수](https://ko.wikipedia.org/wiki/실수 "wikilink") 집합 \(\mathbb R\)인 함수 \(f\colon\mathbb R\to X\)를 **실변수 함수**(實變數函數, )라고 한다. 공역이 실수 집합 \(\mathbb R\)인 함수 \(f\colon X\to\mathbb R\)를 **실숫값 함수**(實數-函數, ) 또는 **실함수**(實函數, )라고 한다. 실변수 실숫값 함수에 대하여, [단조함수](../Page/단조함수.md "wikilink") · [홀함수와 짝함수](https://ko.wikipedia.org/wiki/홀함수와_짝함수 "wikilink") · [주기 함수](https://ko.wikipedia.org/wiki/주기_함수 "wikilink") 등의 개념을 정의할 수 있다. 단조함수는 대략 [그래프가](../Page/함수의_그래프.md "wikilink") 오른쪽으로 갈수록 줄곧 상승하거나 줄곧 하강하는 함수이다. 전자를 [단조 증가 함수](https://ko.wikipedia.org/wiki/단조_증가_함수 "wikilink"), 후자를 [단조 감소 함수라고](https://ko.wikipedia.org/wiki/단조_감소_함수 "wikilink") 한다.\[4\] 예를 들어, \(f(x)=2x\)는 단조 증가 함수이며, \(f(x)=-2x\)는 단조 감소 함수이다. 홀함수는 그래프가 원점에 의하여 중심 대칭인 함수, 짝함수는 그래프가 y축에 의하여 반사 대칭인 함수이다. 예를 들어, \(f(x)=x^3\)는 홀함수, \(f(x)=x^2\)은 짝함수이다.\[5\] 주기함수는 대략 그래프가 x축의 방향의 벡터에 의한 평행 이동 대칭을 갖는 함수이다. [삼각 함수가](https://ko.wikipedia.org/wiki/삼각_함수 "wikilink") 대표적이다.
...
\[f(x)=\begin{cases}x^2&x<1/2\\-x^2+2x&x\ge1/2\end{cases}\] 일반적으로, [매끄러운 다양체](https://ko.wikipedia.org/wiki/매끄러운_다양체 "wikilink") \(X,Y\) 사이의 함수 \(f\colon X\to Y\)에 대하여, 다음 두 조건을 만족시키는 정의역의 [분할](../Page/집합의_분할.md "wikilink") \(X=X_1\sqcup\cdots\sqcup X_n\)이 존재한다면, \(f\)를 **조각마다 \~ 함수**라고 한다.
...
| [선형 함수](https://ko.wikipedia.org/wiki/선형_함수 "wikilink") | 조각마다 선형 함수(-線型函數, )                                               | [절댓값](https://ko.wikipedia.org/wiki/절댓값 "wikilink") 함수  |
...
\[f^{-1}\colon Y\to X\] 가 존재한다. 이를 \(f\)의 **역함수**이라고 한다. 역이 존재하는 함수를 **가역 함수**라고 한다. 어떤 함수가 가역 함수일 필요 충분 조건은, [전단사 함수라는](https://ko.wikipedia.org/wiki/전단사_함수 "wikilink") 것이다.
