...
[조합론](https://ko.wikipedia.org/wiki/조합론 "wikilink")에서, **블록 설계**(block設計, )는 같은 크기의 일련의 [부분 집합들이](https://ko.wikipedia.org/wiki/부분_집합 "wikilink") 주어져 있는 [유한 집합이다](https://ko.wikipedia.org/wiki/유한_집합 "wikilink").\[1\]\[2\]\[3\]\[4\] 이 경우, 이러한 부분 집합을 **블록**()이라고 하며, 블록 설계는 (예를 들어) “\(k\)개의 원소들을 포함하는 블록의 수는 원소의 선택에 상관없이 \(\lambda\)개”와 같은 꼴의 조건을 만족시켜야 한다.

## 정의

### 존슨 결합 도식 속의 블록 설계

세 [자연수](https://ko.wikipedia.org/wiki/자연수 "wikilink") \(t,k,n\in\mathbb N\)가 주어졌다고 하자. **\((t,k,n)\)-블록 설계** \((X,\mathcal B)\)는 다음과 같은 데이터로 주어진다.

  - 크기 \(n\)의 [유한 집합](https://ko.wikipedia.org/wiki/유한_집합 "wikilink") \(X\). 그 원소를 **점**(點, )이라고 한다.
  - \(X\)의, 크기 \(k\)의 부분 집합들의 족 \(\mathcal B\subseteq\operatorname{Pow}_k(X)\). 그 원소를 **블록**()이라고 한다. (여기서 \(\operatorname{Pow}_k(X)\)는 \(X\)의 부분 집합들 가운데 크기가 \(k\)인 것들로 구성된, [멱집합](https://ko.wikipedia.org/wiki/멱집합 "wikilink")의 부분 집합이다.)
...
두 \((t,k,n)\)-블록 설계 \((X,\mathcal B)\), \((X',\mathcal B')\)가 주어졌다고 하자. 만약 이 둘 사이에 [전단사 함수](https://ko.wikipedia.org/wiki/전단사_함수 "wikilink")
...
구체적으로, [결합 도식](https://ko.wikipedia.org/wiki/결합_도식 "wikilink") \((X,\{D_i\in\operatorname{Mat}(|X|,|X|;\mathbb Z)\}_{i\in I})\)이 주어졌다고 하자. 그렇다면, 그 복소수 계수 보스-메스너 대수
...
[파노 평면](https://ko.wikipedia.org/wiki/파노_평면 "wikilink") ([유한체](https://ko.wikipedia.org/wiki/유한체 "wikilink") \(\mathbb F_2\) 위의 [사영 평면](../Page/사영_평면.md "wikilink")) \(\mathbb P_{\mathbb F_2}^2\)을 생각하자.
...
[섬네일](https://ko.wikipedia.org/wiki/파일:Wesley_Woollhouse.jpg "wikilink") [섬네일](https://ko.wikipedia.org/wiki/파일:JakobSteiner.jpg "wikilink") 1844년에 영국의 [보험계리인](https://ko.wikipedia.org/wiki/보험계리인 "wikilink") 웨슬리 스토커 바커 울하우스(, 1809\~1893)가 자신이 편집자로 있던 잡지 《레이디즈 앤드 젠틀먼즈 다이어리》()에서 블록 설계에 대한 퍼즐을 제시하였다.\[12\] 그 전문(全文)은 다음과 같다.  이는 현대적 용어로는 \((q,p,n)\)-슈타이너 계를 다루는 것이다.

이후 이 문제는 1847년에 영국의 [잉글랜드 성공회](https://ko.wikipedia.org/wiki/잉글랜드_성공회 "wikilink") [사제](https://ko.wikipedia.org/wiki/사제 "wikilink") 토머스 페닝턴 커크먼(, 1806\~1895)이 해결하였다.\[13\] 그러나 이들의 논문은 크게 관심을 불러일으키지 못했다.

이후 울하우스와 커크먼과 독자적으로 [야코프 슈타이너가](https://ko.wikipedia.org/wiki/야코프_슈타이너 "wikilink") 1853년에 블록 설계에 대한 논문을 출판하였다.\[14\] 이후 그의 이름을 따 \(\lambda_t=1\)인 \(t\)-블록 설계는 “슈타이너 계”로 불리게 되었다.

비트 설계는 1931년에 [로버트 대니얼 카마이클이](https://ko.wikipedia.org/wiki/로버트_대니얼_카마이클 "wikilink") 최초로 발견하였으며,\[15\] [에른스트 비트가](../Page/에른스트_비트.md "wikilink") 1938년에 [마티외 군을](../Page/마티외_군.md "wikilink") 연구하던 도중 재발견하였다.\[16\]

피셔 부등식은 [로널드 피셔가](https://ko.wikipedia.org/wiki/로널드_피셔 "wikilink") 1940년에 증명하였다.\[17\]
