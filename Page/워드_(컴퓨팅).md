...
[인텔 80386](https://ko.wikipedia.org/wiki/인텔_80386 "wikilink") 컴파일러에서 제공되는 17개 데이터형 중에 워드는 다음과 같다.\[2\]
...
인텔의 8085 8비트 [마이크로프로세서](https://ko.wikipedia.org/wiki/마이크로프로세서 "wikilink")에서 기본 처리 단위가 한 바이트이지만, 8086은 16비트 프로세서이므로 '워드'는 16비트가 된다. 이때 'word'가 사용되었다. 이 16비트는 [마이크로소프트](https://ko.wikipedia.org/wiki/마이크로소프트 "wikilink") 사에서 소프트웨어 개발도구([비주얼스튜디오](https://ko.wikipedia.org/wiki/비주얼스튜디오 "wikilink"))와 결합하면서 WORD(소프트웨어에서는 주로 대문자로 표시)가 16비트를 의미한다. i386에서 데이터형은 byte, word, doubleword로 구분하였다.\[4\] i386의 32비트 CPU는 한 워드가 32비트 이어야 함에도 소프트웨어(어셈블리 표현, MFC 등의 정의 비트수) 상에서 WORD는 여전히 16비트를 의미한다.\[5\] 소프트웨어 상에서 32비트는 DWORD로 표현한다.\[6\] 따라서 소프트웨어를 작성할 때와 CPU의 워드 처리단위는 혼돈의 소지가 있다.
