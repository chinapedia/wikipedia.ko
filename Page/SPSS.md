...
현재의 기본적 앱 기능은 아래와 같다.

  - 기술 통계: 교차표, [빈도](https://ko.wikipedia.org/wiki/빈도 "wikilink"), 기술 자료, 탐색, 기술 비율 통계
  - 이변량 통계: 평균, t-검정, [분산 분석](../Page/분산_분석.md "wikilink"), 상관 관계(이변량, 부분, 거리), 비모수 검정, 베이지안 분석
  - 수치 결과 예측: 선형 [회귀 분석](../Page/회귀_분석.md "wikilink")
  - 집단 식별 예측: [요인 분석](https://ko.wikipedia.org/wiki/요인_분석 "wikilink"), 클러스터 분석(단계, K-평균, 위계적), 차별성 예측
  - 지리적 공간분석, 시뮬레이션
  - R 확장(GUI), 파이썬(Python)

이외 다양한 기능을 추가 확장할 수 있다. 과거 SPSS가 대형자료를 분석하기 어려워 데이터 마이닝이나 빅데이터 분석에 한계 있다는 단점이 거론되었으나, 현재는 IBM AMOS와 연동하여 [구조방정식](https://ko.wikipedia.org/wiki/구조방정식 "wikilink") 모형을 사용하며 문제가 되지 않으며, 대부분의 마케팅과 사회 조사기관에서 활용하고 있다.

## 개발 및 성장

1968년 [스탠퍼드 대학교](../Page/스탠퍼드_대학교.md "wikilink") [정치학](../Page/정치학.md "wikilink") [박사](../Page/박사.md "wikilink")과정에 있었던 노먼 니(Norman H. Nie) 박사가 통계 경로와 구조를 구상하고, 컴퓨터 공학자였던 데일 벤트(Dale H. Bent)와 해들레이 헐(C. Hadlai Hull)과 개발하였다. 개발 초기에는 펀칭카드를 이용한 데이터 관리 방식을 사용하였고, 메이프레임에 접속하여 사용하고 명령어를 입력하는 방식이었다. 노먼 니 박사는 대학교에서 연구교수로 재직하며 개발자들과 함께 SPSS주식회사를 설립하여 운영하였고, 지속적인 기능 향상과 통계적 기법을 개량하였다. 10판 SPSS 이후에는 점차 PC, 맥, 윈도우에 사용할 수 있는 프로그램으로 발전하였고, 그래픽 기능을 추가하였다.
