...
강우관측소에서 측정한 강우량은 점 강우량이며, 강우-유출모형은 입력자료로써 [유역](https://ko.wikipedia.org/wiki/유역 "wikilink")의 평균강우량을 이용하기 때문에 점 강우량을 면적평균 강우량으로 바꾸어주어야 한다. 여기에는 산술평균법(arithmetic average method), 티센다각형법(Thiessen polygon method), 등우선법(isohyetal method)이 있다.
