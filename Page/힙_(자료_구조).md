> This article is converted from Wikipedia: [힙 \(자료 구조\)](https://ko.wikipedia.org/wiki/힙_\(자료_구조\)).
...
이진 힙은 내부노드(internal node)에 키(key)와 요소(element)를 저장한 [이진 트리로](../Page/이진_트리.md "wikilink") 다음과 같은 두 가지 특징을 갖는 것을 말한다.
...
최대 힙을 이용한 [힙 정렬을](../Page/힙_정렬.md "wikilink") 다음과 같이 구현할 수 있다.
