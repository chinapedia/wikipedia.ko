...
Nginx는 요청에 응답하기 위해 비동기 [이벤트 기반](../Page/이벤트_\(컴퓨팅\).md "wikilink") 구조를 가진다. 이것은 [아파치 HTTP 서버의](https://ko.wikipedia.org/wiki/아파치_HTTP_서버 "wikilink") 스레드/프로세스 기반 구조를 가지는 것과는 대조적이다. 이러한 구조는 서버에 많은 부하가 생길 경우의 성능을 예측하기 쉽게 해준다.

## 설명

[위키백과](https://ko.wikipedia.org/wiki/위키백과 "wikilink")에서는 nginx를 [SSL 터미네이션 프록시로](https://ko.wikipedia.org/wiki/SSL_터미네이션_프록시 "wikilink") 사용한다.\[4\]
