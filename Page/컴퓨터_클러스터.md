...
[섬네일에서](https://ko.wikipedia.org/wiki/파일:MEGWARE.CLIC.jpg "wikilink") 기술자들이 대규모 [리눅스](../Page/리눅스.md "wikilink") 클러스터에서 작업하고 있다\]\] **컴퓨터 클러스터**(, )는 여러 대의 [컴퓨터](../Page/컴퓨터.md "wikilink")들이 연결되어 하나의 시스템처럼 동작하는 컴퓨터들의 집합을 말한다. 클러스터의 구성 요소들은 일반적으로 고속의 [근거리 통신망으로](../Page/근거리_통신망.md "wikilink") 연결된다. 서버로 사용되는 [노드](https://ko.wikipedia.org/wiki/노드 "wikilink")에는 각각의 [운영 체제가](../Page/운영_체제.md "wikilink") 실행된다. 컴퓨터 클러스터는 저렴한 [마이크로프로세서](https://ko.wikipedia.org/wiki/마이크로프로세서 "wikilink")와 고속의 [네트워크](https://ko.wikipedia.org/wiki/네트워크 "wikilink"), 그리고 고성능 [분산 컴퓨팅용](https://ko.wikipedia.org/wiki/분산_컴퓨팅 "wikilink") 소프트웨어들의 조합 결과로 태어났다. 클러스터는 일반적으로 단일 컴퓨터보다 더 뛰어난 성능과 안정성을 제공하며, 비슷한 성능과 안정성을 제공하는 단일 컴퓨터보다 비용 면에서 훨씬 더 효율적이다.\[1\] 따라서 열 개 안팎의 중소 규모의 클러스터부터 수천 개로 이루어진 대형 [슈퍼컴퓨터](https://ko.wikipedia.org/wiki/슈퍼컴퓨터 "wikilink")에 이르기까지 널리 사용되고 있다.
...
[섬네일의](https://ko.wikipedia.org/wiki/파일:Nec-cluster.jpg "wikilink") [인텔](https://ko.wikipedia.org/wiki/인텔 "wikilink") [네할렘](https://ko.wikipedia.org/wiki/네할렘 "wikilink") 프로세서 기반 클러스터\]\] 1980년대에 컴퓨터 클러스터가 나오면서 [슈퍼컴퓨터](https://ko.wikipedia.org/wiki/슈퍼컴퓨터 "wikilink")도 나왔다. 그 당시 초창기 슈퍼컴퓨터들은 [공유 메모리를](../Page/공유_메모리.md "wikilink") 많이 사용하였지만 지금의 클러스터들은 공유 메모리를 사용하지 않으며 많은 슈퍼컴퓨터들도 사용하지 않는다. 그렇지만 클러스터 파일 시스템은 현대 컴퓨터 클러스터에도 중요하게 사용된다. 예를 들면 공개 소프트웨어인 [러스터 파일 시스템은](https://ko.wikipedia.org/wiki/러스터_\(파일_시스템\) "wikilink") [TOP500](../Page/TOP500.md "wikilink")의 컴퓨터에서 가장 많이 사용되는 병렬 분산 공유 파일 시스템이다.
...
13.
14.
15. [*Grid and Cluster Computing* by Prabhu 2008 8120334280 pages 109-112](http://books.google.it/books?id=EIVdVtGHv-0C&pg=PA112&dq=%22message+passing%22+computer+cluster+MPI+PVM&hl=en&sa=X&ei=-dD7ToZCj_bhBOOxvI0I&redir_esc=y#v=onepage&q=%22message%20passing%22%20computer%20cluster%20MPI%20PVM&f=false)
16.
17.
