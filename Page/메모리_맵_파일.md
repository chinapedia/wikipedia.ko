...
**메모리 맵 파일**(Memory mapped file, **MMF**, 메모리 사상 파일)은 [운영 체제에서](https://ko.wikipedia.org/wiki/운영_체제 "wikilink") [파일](https://ko.wikipedia.org/wiki/파일 "wikilink")을 다루는 방법중 하나이다. 물리 디스크 파일, 장치, [공유 메모리 객체와](https://ko.wikipedia.org/wiki/공유_메모리_객체 "wikilink") 같이 운영 체제에서 [파일](https://ko.wikipedia.org/wiki/파일 "wikilink")로 다루는 모든 대상에 대해서 사용 가능하며, 메모리 맵 파일을 통해 [프로세스](../Page/프로세스.md "wikilink")의 [가상 메모리](https://ko.wikipedia.org/wiki/가상_메모리 "wikilink") 주소 공간에 파일을 매핑한 뒤 [가상 메모리](https://ko.wikipedia.org/wiki/가상_메모리 "wikilink") 주소에 직접 접근하는 것으로 파일 읽기/쓰기를 대신한다.

기본 파일 입출력 API에 비해서 저렴한 비용을 사용하여 파일의 내용에 접근할 수 있도록 해 주며, 메모리와 파일 간의 자료 전송은 [운영 체제가](https://ko.wikipedia.org/wiki/운영_체제 "wikilink") 자동으로 처리하므로 보다 간단하게 사용할 수 있다. 다른 프로세스에서 동일한 파일에 메모리 맵 파일을 이용하여 접근할 경우 [공유 메모리처럼](../Page/공유_메모리.md "wikilink") 사용하는 것도 가능하다.
...
[버퍼](https://ko.wikipedia.org/wiki/버퍼 "wikilink")나 파일 처리를 위한 추가적인 [자료 구조가](https://ko.wikipedia.org/wiki/자료_구조 "wikilink") 필요 없다. 운영 체제에서 [페이징](../Page/페이징.md "wikilink") 기법을 사용하여 파일의 내용을 관리하며, 페이지 크기(보통 4KiB)에 따라 적절히 파일의 내용을 읽고 쓸 수 있으며 파일 반영(flush) 작업도 수행해준다.
...
메모리 맵 파일을 이용한 접근은 최적의 파일 접근 방식은 아니다. 파일 입출력 API를 사용한 방법이 API의 오버헤드인 것에 비해 메모리 맵 파일에서는 [페이지 부재중의](https://ko.wikipedia.org/wiki/페이지_부재 "wikilink") 데이터 전송 시간이 오버헤드로 작용한다.\[4\] 따라서 [데이터베이스](https://ko.wikipedia.org/wiki/데이터베이스 "wikilink")와 같이 파일을 읽고 쓰는데 오버헤드를 최소로 줄인 자료구조와 알고리즘과 메모리 맵 파일을 비교할 때 오히려 메모리 맵 파일이 느릴 수도 있다.

크기가 지나치게 큰 파일을 처리하는 데 어려움이 있다. [IA-32](https://ko.wikipedia.org/wiki/IA-32 "wikilink") 기반 시스템에서 하나의 프로세스에서 [PAE](../Page/물리_주소_확장.md "wikilink") 기술을 사용하지 않고 사용 가능한 최대 크기는 4GB로 제한된다.\[5\] 따라서 프로세스의 메모리 주소를 사용하는 메모리 맵 파일이 한번에 다룰 수 있는 크기는 이보다 작아지며, 이 크기를 넘어가는 파일을 메모리 맵 파일로 다루려 할 때 계산이 복잡해질 수 있다.

## 용도

대부분 운영 체제에서는 [프로세스](../Page/프로세스.md "wikilink")를 실행할 때 [실행 파일의](../Page/실행_파일.md "wikilink") 각 [세그먼트를](https://ko.wikipedia.org/wiki/메모리_세그먼트 "wikilink") 메모리에 사상하기 위해 메모리 맵 파일을 이용한다. 이 경우 하나의 프로세스가 동시에 여러 번 동작할 때 실행 코드와 같은 읽기 전용 세그먼트를 공유할 수 있어 중복된 내용만큼의 메모리를 절약할 수 있다. 그리고 메모리 맵 파일이 가지는 [지연 적재](https://ko.wikipedia.org/wiki/지연_적재 "wikilink") 특징 덕분에 [실행 파일에서](../Page/실행_파일.md "wikilink") 사용하지 않는 부분이 있을 때도 메모리를 최적화하여 사용할 수 있다.

또한, 파일의 내용이 [프로그래밍 언어에서](https://ko.wikipedia.org/wiki/프로그래밍_언어 "wikilink") 직접 다룰 수 있는 [구조체](https://ko.wikipedia.org/wiki/구조체 "wikilink")와 같은 형식을 가지고 작성되어 있으면, 메모리 맵 파일을 이용해 자료에 바로 접근하는 방법을 사용하기도 한다. 이 경우 위에 서술한 바와 같이 많은 장점을 얻을 수 있다.

또 다른 사용 용도는 [프로세스 간 통신](../Page/프로세스_간_통신.md "wikilink") 수단이다. 현대의 [운영 체제에서는](https://ko.wikipedia.org/wiki/운영_체제 "wikilink") 하나의 [프로세스](../Page/프로세스.md "wikilink")는 고유의 메모리 주소만 갖게 되어 다른 [프로세스](../Page/프로세스.md "wikilink")의 메모리에 접근할 수 없다. 따라서 [프로세스 간 통신을](../Page/프로세스_간_통신.md "wikilink") 위해 [공유 메모리를](../Page/공유_메모리.md "wikilink") 사용하는 방법이 고안되어 있으며 메모리 맵 파일은 이 [공유 메모리를](../Page/공유_메모리.md "wikilink") 사용하는 방법이 될 수 있다.

## 지원 사항 및 예제

### POSIX

[POSIX](https://ko.wikipedia.org/wiki/POSIX "wikilink")에서는 [mmap](https://ko.wikipedia.org/wiki/mmap "wikilink")이라는 함수를 통해 메모리 맵 파일 기능을 제공하고 있다. `MAP_SHARED`라는 플래그를 통해 다른 프로세스에서 같은 파일에 대해 메모리 맵 파일 기능을 사용할 수 있도록 할 수 있어 공유 메모리로도 사용할 수 있다.\[6\]\[7\] [POSIX](https://ko.wikipedia.org/wiki/POSIX "wikilink")를 따르는 운영 체제들([리눅스](https://ko.wikipedia.org/wiki/리눅스 "wikilink"), [유닉스](https://ko.wikipedia.org/wiki/유닉스 "wikilink"), [솔라리스](https://ko.wikipedia.org/wiki/솔라리스_\(운영_체제\) "wikilink") 등)에서는 약간의 차이는 있지만 같은 기능과 특징을 가진 메모리 맵 파일 기능을 제공한다.\[8\]
...
### 언어별 지원
  ...
  - [자바에서는](https://ko.wikipedia.org/wiki/자바_\(프로그래밍_언어\) "wikilink") [`FileChannel` 클래스](http://docs.oracle.com/javase/7/docs/api/java/nio/channels/FileChannel.html)를 통해 메모리 맵 파일 기능을 제공한다.
  - [마이크로소프트](https://ko.wikipedia.org/wiki/마이크로소프트 "wikilink") [닷넷 프레임워크에서는](../Page/닷넷_프레임워크.md "wikilink") [`MemoryMappedFile` 클래스](http://msdn.microsoft.com/en-us/library/dd997372.aspx)를 통해 메모리 맵 파일을 제공하고 있다.
  - [파이썬](https://ko.wikipedia.org/wiki/파이썬 "wikilink")에서는 [`mmap` 클래스](http://docs.python.org/library/mmap.html)를 통해 메모리 맵 파일을 제공하고 있다. 다만, 윈도와 POSIX 기반 운영 체제에서 [클래스](https://ko.wikipedia.org/wiki/클래스_\(컴퓨터_과학\) "wikilink") 구조가 다르므로 주의가 필요하다.
  - [Boost](https://ko.wikipedia.org/wiki/Boost "wikilink")에서도 메모리 맵 파일기능을 제공한다.\[11\]\[12\]

## 같이 보기

  - [메모리 맵 입출력](../Page/메모리_맵_입출력.md "wikilink") - 메모리 맵 파일과 차이를 주목할 것
  - [mmap](https://ko.wikipedia.org/wiki/mmap "wikilink") - [POSIX](https://ko.wikipedia.org/wiki/POSIX "wikilink")에서 [메모리 맵 입출력과](../Page/메모리_맵_입출력.md "wikilink") 메모리 맵 파일 기능을 지원하는 함수
