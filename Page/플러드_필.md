...
**플러드 필**() 혹은 **시드 필**()은 다차원 [배열](https://ko.wikipedia.org/wiki/배열 "wikilink")의 어떤 칸과 [연결된](../Page/연결_그래프.md "wikilink") 영역을 찾는 [알고리즘](https://ko.wikipedia.org/wiki/알고리즘 "wikilink")이다. 이 알고리즘은 [그림 프로그램에서](https://ko.wikipedia.org/wiki/래스터_그래픽_편집기 "wikilink") 연결된 비슷한 색을 가지는 영역에 "채우기" 도구에 사용되며, [바둑](https://ko.wikipedia.org/wiki/바둑 "wikilink")이나 [지뢰 찾기](https://ko.wikipedia.org/wiki/지뢰_찾기 "wikilink") 같은 게임에서 어떤 비어 있는 칸을 표시 할 지를 결정할 때에도 사용된다.

## 알고리즘

[right적](https://ko.wikipedia.org/wiki/파일:Recursive_Flood_Fill_8_\(aka\).gif "wikilink") 플러드 필\]\] 플러드 필 알고리즘은 시작 칸, 목표 색, 대체 색의 세 개의 인자를 받는다. 이 알고리즘은 배열에 있는 시작 칸에서 목표 색으로 연결된 모든 칸을 방문해서 대체 색으로 바꾼다. 플러드 필 알고리즘을 구현하는 방법은 여러가지가 있지만, 대부분 [큐나](https://ko.wikipedia.org/wiki/큐_\(자료_구조\) "wikilink") [스택](https://ko.wikipedia.org/wiki/스택 "wikilink")과 같은 [자료구조](https://ko.wikipedia.org/wiki/자료구조 "wikilink")를 사용한다.
...
위에서 사용되는 알고리즘의 수행은 이해하기 쉽지만, 스택공간이 매우 제한된 언어나 환경(예: [자바 애플릿](https://ko.wikipedia.org/wiki/자바_애플릿 "wikilink"))에서는 실용적이지 않다.

### 큐 기반 수행

명시적인 큐 기반 수행(종종 "산불 알고리즘"이라고도 불린다\[1\])은 아래에 의사 코드로 나타나 있다. 이 수행은 단순한 재귀 해법과 비슷하지만 재귀 호출을 하지 않고 칸을 [큐에](https://ko.wikipedia.org/wiki/큐_\(자료_구조\) "wikilink") 넣는다:
...
이 알고리즘에서 영역의 모양을 저장하도록 추가로 배열을 사용하면 채워진 영역의 특정한 경계에서 칸이 다를 수 있는 "흐린" 플러드 필로 일반화 할 수 있다. 이 추가적인 배열을 [알파 채널로](https://ko.wikipedia.org/wiki/알파_채널 "wikilink") 사용하면 채워진 영역의 경계에서 채워지지 않은 영역과 부드럽게 섞일 수 있다.
...
[잉크스케이프](https://ko.wikipedia.org/wiki/잉크스케이프 "wikilink") 버전 0.46은 채우기 도구가 있어서 일반적인 비트맵 연산과 비슷한 결과를 내며 실제로 사용한다: 캔버스가 렌더 되고, 플러드 필 연산을 선택된 영역에 수행한 후, 그 결과를 가지고 경로를 되돌아간다. 이는 [경계 조건의](https://ko.wikipedia.org/wiki/경계값_문제 "wikilink") 개념을 사용한다.
...
## 같이 보기
  ...
  - [깊이 우선 탐색](https://ko.wikipedia.org/wiki/깊이_우선_탐색 "wikilink")
  - [그래프 순회](https://ko.wikipedia.org/wiki/그래프_순회 "wikilink")
  - [연결요소 번호 매김](https://ko.wikipedia.org/wiki/연결요소_번호_매김 "wikilink")
  - [데이크스트라 알고리즘](https://ko.wikipedia.org/wiki/데이크스트라_알고리즘 "wikilink")
