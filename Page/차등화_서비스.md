...
**차등화 서비스** 또는 **DiffServ**란 [컴퓨터 네트워킹의](https://ko.wikipedia.org/wiki/컴퓨터_네트워크 "wikilink") 한 구조로서, [IP 네트워크에서](https://ko.wikipedia.org/wiki/IP_네트워크 "wikilink") [IntServ](https://ko.wikipedia.org/wiki/IntServ "wikilink")와 같이 통신 흐름 마다 [QoS](https://ko.wikipedia.org/wiki/QoS "wikilink") 보장(통신 품질 보장)하는 것이 아니라 여러 흐름을 정리해(집계하고) 몇 개 정도의 클래스를 만들고 클래스마다 정해진 QoS 보장 방법의 조합을 적용하는 QoS 보장 방법이다. IETF 의 RFC 2474 등의 표준 문서에 의해 규정되어있다. 현대 IP 네트워크의 네트워크 트래픽을 분류 및 관리하고 [QoS](https://ko.wikipedia.org/wiki/QoS "wikilink")를 제공하는, 간단하고 [확장성](https://ko.wikipedia.org/wiki/확장성 "wikilink") 있는 대단위 매커니즘을 지정한다. 예를 들어, DiffServ는 웹 트래픽이나 파일 이동같은 중요도가 낮은 서비스에 단순한 최선형 서비스를 제공함으로써 [음성이나](https://ko.wikipedia.org/wiki/음성_인터넷_프로토콜 "wikilink") [스트리밍 미디어같은](https://ko.wikipedia.org/wiki/스트리밍 "wikilink") 중요한 네트워크 트래픽에는 저지연을 제공할 수 있다. DiffServ는 IP 헤더에서 패킷 분류의 목적으로 6비트의 차등화 서비스 [필드](https://ko.wikipedia.org/wiki/필드_\(컴퓨터_과학\) "wikilink") (DS 필드)를 사용한다. DS필드와 ECN필드는 기존 IPv4의 TOS필드를 대신한다.\[1\]

## 배경

현대의 데이터 네트워크는 음성, 비디오, 음악 스트리밍, 웹 페이지 및 [이메일](https://ko.wikipedia.org/wiki/전자_우편 "wikilink") 등의 매우 다양한 타입의 서비스를 가진다. 그렇기 때문에 이런 서비스들이 공존하는 대부분의 기존 QoS 매커니즘은 복잡할 뿐만아니라 [공공 인터넷의](https://ko.wikipedia.org/wiki/인터넷 "wikilink") 요구에 맞춰 확장되지 못했다. 1998년 12월, [IETF는](https://ko.wikipedia.org/wiki/국제_인터넷_표준화_기구 "wikilink") [IPv4](https://ko.wikipedia.org/wiki/IPv4 "wikilink"), [IPv6](https://ko.wikipedia.org/wiki/IPv6 "wikilink") 헤더에서의 차등화 서비스(DS필드)인 RFC 2474를 공개했고, 이것은 IPv4 TOS필드를 DS필드로 대체했다. DS필드에서, 8개 값의 범위(클래스 섹터)가 이전 TOS필드에서 IP 우선순위 분류와 함께 하위 호환성에 쓰인다. 현재는 주요 프로토콜 [라우터](https://ko.wikipedia.org/wiki/라우터 "wikilink")들이 차등 등급의 서비스를 제공하기 위해 DiffServ를 쓰게 되면서, TOS와 기타 통합 서비스(IntServ)와 같은 [네트워크 계층](../Page/네트워크_계층.md "wikilink") QoS 매커니즘을 대부분 대체했다.
