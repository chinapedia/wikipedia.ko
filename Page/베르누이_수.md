...
[수론](https://ko.wikipedia.org/wiki/수론 "wikilink")에서, **베르누이 수**(Bernoulli數, )는 거듭제곱수의 합,삼각함수의 멱급수 따위의 다양한 공식에 등장하는 [유리수](https://ko.wikipedia.org/wiki/유리수 "wikilink") [수열](https://ko.wikipedia.org/wiki/수열 "wikilink")이다.

[정수론](https://ko.wikipedia.org/wiki/정수론 "wikilink")과 깊은 관계가 있는 실수열로 [야코프 베르누이에](https://ko.wikipedia.org/wiki/야코프_베르누이 "wikilink") 의해 발견되고 그의 이름에서 명명됐다. 이와는 독립적으로 동시대에 [세키 다카카즈의](https://ko.wikipedia.org/wiki/세키_다카카즈 "wikilink") 발견도 이루어졌다.
...
베르누이 다항식의 [푸리에 급수는](https://ko.wikipedia.org/wiki/푸리에_급수 "wikilink") 다음과 같다.
...
[섬네일](https://ko.wikipedia.org/wiki/파일:BernoulliNumbersByZetaLowRes.png "wikilink") 베르누이 수는 [리만 제타 함수의](https://ko.wikipedia.org/wiki/리만_제타_함수 "wikilink") 특수한 값이다.

\[\tilde B_n=(-)^nB_n=-n\zeta(1-n)=-\frac 2{(2\pi)^n}\sin\left(\pi(1-n)/2\right)n!\zeta(n)\] 따라서, [리만 제타 함수는](https://ko.wikipedia.org/wiki/리만_제타_함수 "wikilink") 베르누이 수의 복소수 \(n\)에 대한 [해석적 연속으로](https://ko.wikipedia.org/wiki/해석적_연속 "wikilink") 볼 수 있다.

\(n=0\)일 경우 리만 제타 함수는 [극을](https://ko.wikipedia.org/wiki/극_\(복소해석학\) "wikilink") 갖지만, 이 경우 다음과 같이 [극한](https://ko.wikipedia.org/wiki/극한 "wikilink")을 취할 수 있다.

\[\tilde B_0=B_0=1=\lim_{n\to0}-n\zeta(1-n)\]

\[\tilde B_1=-B_1=-2\lim_{n\to1}\frac{\sin(\pi(1-n)/2)}{(2\pi)^n}n!\zeta(n)=1/2\] 따라서, [스털링 공식에](https://ko.wikipedia.org/wiki/스털링_공식 "wikilink") 따라 짝수차 베르누이 수의 [절댓값](https://ko.wikipedia.org/wiki/절댓값 "wikilink")에 대하여 다음과 같은 점근 공식이 성립한다.

\[|B_{2 n}| =\frac{2(2n)!}{(2\pi)^{2n}}\zeta(2n)\sim 4 \sqrt{\pi n} \left(\frac{n}{ \pi e} \right)^{2n}\qquad(n\gg1)\]

보다 일반적으로, 베르누이 다항식은 [후르비츠 제타 함수](https://ko.wikipedia.org/wiki/후르비츠_제타_함수 "wikilink") \(\zeta(s,q)\)의 특수한 값이다.

\[B_n(x) = -n \zeta(1-n,x)\]

\[\tilde B_n=B_n(1)=-n\zeta(1-n,1)=-n\zeta(1-n)\] 따라서, [후르비츠 제타 함수는](https://ko.wikipedia.org/wiki/후르비츠_제타_함수 "wikilink") 베르누이 다항식의 복소수 \(n\)에 대한 [해석적 연속으로](https://ko.wikipedia.org/wiki/해석적_연속 "wikilink") 볼 수 있다.
...
예를 들어, \(m=1\)일 경우, 1부터 \(n\)까지의 자연수의 합인 [삼각수 수)](https://ko.wikipedia.org/wiki/삼각수 "wikilink")(삼각형 공식을 얻을 수 있다.
...
\[1 + 2 +3+ \cdots + n = \frac12(B_0 n^2-2B_1 n^1) = \frac12(n^2+n)\] \(m=2\)일 경우, 제곱수의 합 공식을 얻을 수 있다.
  ...
  -
    ...
    베르누이 공식은 [음계산법](https://ko.wikipedia.org/wiki/음계산법 "wikilink")을 통하여 간단하게 적을 수 있다. 우선, 음변수 \(\mathsf b\)에 대하여 다음과 같은 선형 범함수를 정의하자.
    ...
    ### 알고리즘
    베르누이 수를 계산하는 효율적인 [알고리즘](https://ko.wikipedia.org/wiki/알고리즘 "wikilink")들이 알려져 있으며,\[1\]\[2\]\[3\]\[4\] \(B_n\)를 계산하는 가장 빠른 알려진 알고리즘의 시간 복잡도는
    ...
    **폰 슈타우트-클라우젠 정리**()에 따르면, 모든 양의 정수 \(n\)에 대하여, 만약 \(B_n\ne0\)이라면 다음이 성립한다.
    ...
    [소수](https://ko.wikipedia.org/wiki/소수_\(수론\) "wikilink") \(p\)에 대하여 다음 두 조건이 서로 [동치](https://ko.wikipedia.org/wiki/동치 "wikilink")이며, 이를 만족시키는 소수를 **정규 소수**()이라고 한다.
      - \(p\nmid B_{2n}\qquad\forall 2k\in\{2,4,6,\dots,p-3\}\)
      - \(p\nmid h_{\mathbb Q(\zeta_p))}\). 여기서 \(h_K\)는 \(K\)의 [유수이며](https://ko.wikipedia.org/wiki/유수_\(수론\) "wikilink"), \(\mathbb Q(\zeta_p)\)는 [원분체](https://ko.wikipedia.org/wiki/원분체 "wikilink")이다.
...
[세키 다카카즈](https://ko.wikipedia.org/wiki/세키_다카카즈 "wikilink")(1642\~1708)는 1712년 사후에 출판된 《괄요산법》()\[7\] 에 거듭제곱수의 합에 대한 일반 공식 및 베르누이 수를 제시하였다. 《괄요산법》에는 [산가지](https://ko.wikipedia.org/wiki/산가지 "wikilink")로 표기된 [파스칼 삼각형](https://ko.wikipedia.org/wiki/파스칼_삼각형 "wikilink") 밑에 다음과 같이 처음 12개의 베르누이 수가 수록돼 있다.

  -
    {| lang="ja" class=wikitable style="writing-mode: vertical-rl"

|- | 一級 || 全 |- | 二級 || 取㆓二分之一㆒爲㆑加 |- | 三級 || 取㆓六分之一㆒爲㆑加 |- | 四級 || 空 |- | 五級 || 取㆓三十分之一㆒爲㆑減 |- | 六級 || 空 |- | 七級 || 取㆓四十二分之一㆒爲㆑加 |- | 八級 || 空 |- | 九級 || 取㆓三十分之一㆒爲㆑減 |- | 十級 || 空 |- | 十一級 || 取㆓六十六分之五㆒爲㆑加 |- | 十二級 || 空 |} 즉, 분수 \(k/n\)은 *n**k*로 표시돼 있고, 부호는 양수일 경우 , 음수일 경우 로 표시돼 있다. (㆑, ㆒, ㆓와 같은 부호는 [간분의](https://ko.wikipedia.org/wiki/간분_\(일본_한문\) "wikilink") 가에리텐()이다.)

세키와 거의 동시에, [야코프 베르누이는](https://ko.wikipedia.org/wiki/야코프_베르누이 "wikilink") 1713년 사후에 출판된 저서 《추측술》()\[8\] 2부 3장 97쪽에 거듭제곱수의 합의 일반 공식을 제시하였지만, 증명하지 않았다. 이 책에서 베르누이는 다음과 같이 적었다.

1830년대에 [레온하르트 오일러와](https://ko.wikipedia.org/wiki/레온하르트_오일러 "wikilink") [콜린 매클로린은](https://ko.wikipedia.org/wiki/콜린_매클로린 "wikilink") [오일러-매클로린 공식을](https://ko.wikipedia.org/wiki/오일러-매클로린_공식 "wikilink") 발견하면서 베르누이 수를 재발견하였다. [아브라암 드무아브르와](https://ko.wikipedia.org/wiki/아브라암_드무아브르 "wikilink") [레온하르트 오일러는](https://ko.wikipedia.org/wiki/레온하르트_오일러 "wikilink") "베르누이 수"라는 표현을 최초로 사용하였다.\[9\] 1834년에 [카를 구스타프 야코프 야코비는](https://ko.wikipedia.org/wiki/카를_구스타프_야코프_야코비 "wikilink") 베르누이 공식을 엄밀하게 증명하였다.\[10\]

[에이다 러브레이스는](https://ko.wikipedia.org/wiki/에이다_러브레이스 "wikilink") 1843년에 [찰스 배비지의](https://ko.wikipedia.org/wiki/찰스_배비지 "wikilink") [해석기관](https://ko.wikipedia.org/wiki/해석기관 "wikilink")에 대한 책\[11\] 의 주석 G()에 베르누이 수를 계산하는 [알고리즘](https://ko.wikipedia.org/wiki/알고리즘 "wikilink")을 기술하였다. 이 주석 G는 세계 최초의 [컴퓨터 프로그램으로](https://ko.wikipedia.org/wiki/컴퓨터_프로그램 "wikilink") 여겨진다.

폰 슈타우트-클라우젠 정리는 카를 게오르크 크리스티안 폰 슈타우트()\[12\] 와 토마스 클라우젠()\[13\] 이 1840년에 독자적으로 발견하였다. [정규 소수의](https://ko.wikipedia.org/wiki/정규_소수 "wikilink") 쿠머 조건은 [에른스트 쿠머가](https://ko.wikipedia.org/wiki/에른스트_쿠머 "wikilink") 1850년에 증명하였다.\[14\]

## 같이 보기

  - [벨 수](https://ko.wikipedia.org/wiki/벨_수 "wikilink")
  - [오일러 수](https://ko.wikipedia.org/wiki/오일러_수 "wikilink")
