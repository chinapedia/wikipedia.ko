...
[다변수 미적분학에서](https://ko.wikipedia.org/wiki/다변수_미적분학 "wikilink"), **음함수 정리**(陰函數定理, )는 변수들에 대한 [방정식](https://ko.wikipedia.org/wiki/방정식 "wikilink")이 국소적으로 충분히 매끄러운 [함수](https://ko.wikipedia.org/wiki/음함수 "wikilink") 관계를 나타낼 충분 조건을 제시하는 정리이다.

## 도입

2차원 [유클리드 공간](https://ko.wikipedia.org/wiki/유클리드_공간 "wikilink") \(\mathbb R^2\) 위에서 원점 \((0,0)\)을 중심으로 하며 1을 반지름으로 하는 [원의](https://ko.wikipedia.org/wiki/원_\(기하학\) "wikilink") 방정식
...
\[y=-\sqrt{1-x^2}\qquad\forall x\in[-1,1]\] 이 방정식을 만족시키는 [연속 함수는](https://ko.wikipedia.org/wiki/연속_함수 "wikilink") \([-1,1]\times\mathbb R\)에서 위와 같은 두 가지가 있으므로 유일하지 않다. 그러나 \((0,1)\)을 지나는 연속 함수는 첫 번째 함수로 유일하다. 이 사실에 대한 기하학적 의미를 알아보기 위해, 다음과 같은 함수를 정의하자.
...
[열린 근방](https://ko.wikipedia.org/wiki/열린_근방 "wikilink") \(\mathbf a\in U\subseteq\mathbb R^n\)와 \(\mathbf b\in V\subseteq\mathbb R^m\) 및 [연속 함수](https://ko.wikipedia.org/wiki/연속_함수 "wikilink") \(\mathbf f\colon U\times V\to\mathbb R^m\), \((\mathbf x,\mathbf y)\mapsto\mathbf f(\mathbf x,\mathbf y)\) (\(\mathbf x\in U\), \(\mathbf y\in V\))가 다음을 만족시킨다고 하자.
...
\[\Delta y(\mathbf x',\mathbf x'+\Delta x_j\mathbf e_j)=g(\mathbf x'+\Delta x_j\mathbf e_j)-g(\mathbf x')\] 그러면 [평균값 정리에](https://ko.wikipedia.org/wiki/평균값_정리 "wikilink") 따라 다음을 만족시키는 \(0<\theta(\mathbf x',\mathbf x'+\Delta x_j\mathbf e_j)<1\)가 존재한다.
...
이제 임의의 \(\mathbf x'\in\operatorname B(\mathbf a,\delta_2)\)에 대하여, \(\mathbf F(\mathbf x',\cdot)\)가 \(\bar\operatorname B(\mathbf b,\delta_1)\) 위의 \(c\)-[립시츠 연속 함수임을](https://ko.wikipedia.org/wiki/립시츠_연속_함수 "wikilink") 증명하자. 우선 임의의 \(\mathbf y\in\bar\operatorname B(\mathbf b,\delta_1)\)에 대하여, 다음을 만족시키는 \(0<\theta(\mathbf x',\mathbf y)<1\)가 존재한다.
...
\end{align}\] 즉, \(\mathbf F(\mathbf x',\cdot)\)는 \(\bar\operatorname B(\mathbf b,\delta_1)\) 위의 \(c\)-[립시츠 연속 함수이다](https://ko.wikipedia.org/wiki/립시츠_연속_함수 "wikilink"). 또한 \((\bar\operatorname B(\mathbf b,\delta_1),\Vert\cdot\Vert)\)는 [바나흐 공간이므로](https://ko.wikipedia.org/wiki/바나흐_공간 "wikilink"), [바나흐 고정점 정리에](../Page/바나흐_고정점_정리.md "wikilink") 따라 \(\mathbf F(\mathbf x',\cdot)\)는 \(\bar\operatorname B(\mathbf b,\delta_1)\)에서 유일한 고정점 \(\mathbf g(\mathbf x')\in\bar\operatorname B(\mathbf b,\delta_1)\)를 가진다. 즉, 다음이 성립한다.
...
어떤 \(0<\epsilon<1\)에 대하여, [케플러 방정식](https://ko.wikipedia.org/wiki/케플러_방정식 "wikilink")
