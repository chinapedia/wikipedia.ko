...
[요구사항 분석](../Page/요구사항_분석.md "wikilink") 단계에서, 개발될 시스템에 대한 [요구사항](../Page/요구사항.md "wikilink")은 시스템 사용자의 필요를 분석함으로써 얻을 수 있다. 이 단계는 목표로 하는 이상적인 시스템이 수행해야할 기능에 대해 고민하는 단계이다. 그러나 이 단계에서는 소프트웨어가 어떻게 설계되고 구현될 것인지에 대한 사항은 다루지 않는다. 통상적으로 이 단계에서는 사용자를 만나 기능에 대한 의견을 청취하고, 이를 사용자 요구사항 문서로 생성하게 된다.
...
그러한 과정을 거쳐 개발 단계를 위한 청사진이 될 소프트웨어 기술서(software specification document)가 생성된다. 이 문서는 일반적인 시스템 구성과 메뉴 구조, [자료 구조](../Page/자료_구조.md "wikilink") 등을 기술한다. 또한 기술서는 비즈니스 시나리오에 대한 예나 샘플 윈도, 이해를 돕기 위한 보고서 등을 추가로 포함할 수 있다. 엔티티 다이어그램(entity diagrams)이나 데이터 목록(data dictionary)과 같은 기술자료 또한 설계 단계에서 산출된다. 그에 더하여 시스템 테스트를 위한 문서도 이 단계에서 준비된다.
