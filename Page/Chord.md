...
[컴퓨팅](https://ko.wikipedia.org/wiki/컴퓨팅 "wikilink")에서 **Chord**(코드)는 [peer-to-peer](https://ko.wikipedia.org/wiki/P2P "wikilink") [분산 해시 테이블에](https://ko.wikipedia.org/wiki/분산_해시_테이블 "wikilink") 대한 프로토콜 및 [알고리즘](https://ko.wikipedia.org/wiki/알고리즘 "wikilink")이다. 분산 해시 테이블은 다른 컴퓨터(노드)에 키를 할당하여 [키-값 쌍을](https://ko.wikipedia.org/wiki/연관_배열 "wikilink") 저장한다 (노드는 그것이 담당하는 모든 키에 대한 값을 저장한다). Chord는 노드에 키가 할당되는 방법과 주어진 키에 해당하는 첫번째 노드를 찾아 해당 키에 대한 값을 해당 노드가 찾을 수 있는 방법을 정한다.

Chord는 CAN, Tapestry 그리고 Pastry와 함께, 4개의 원조 [분산 해시 테이블](https://ko.wikipedia.org/wiki/분산_해시_테이블 "wikilink") 프로토콜 중 하나이다. 이 프로토콜은 Ion Stoica, Robert Morris, David  Karger, Frans Kaashoek, Hari Balakrishnan가 도입했으며, [MIT에서](https://ko.wikipedia.org/wiki/매사추세츠_공과대학교 "wikilink") 개발되었다.\[1\]

## 개요

[오른쪽](https://ko.wikipedia.org/wiki/파일:Chord_project.svg "wikilink") 노드와 키는 [일관된 해싱을](../Page/일관된_해싱.md "wikilink") 통해 -비트 식별자를 할당 받는다. SHA-1 알고리즘은 일관된 해싱을 위한 기반 [해싱 함수이다](https://ko.wikipedia.org/wiki/해시_함수 "wikilink"). 일관된 해싱은 Chord의 견고함과 성능을 위해 필수적인데, 이는 키와 노드(정확히는 노드의 [IP 주소](https://ko.wikipedia.org/wiki/IP_주소 "wikilink"))가 동일한 식별 공간 내에 무시할 만한 충돌 가능성과 함께 균일하게 분산되기 때문이다. 따라서, 방해 없이 노드를 네트워크에 참가시키거나 내보내는 것이 가능하다. 해당 프로토콜 내에서, 노드라는 용어는 모호할 것 없이 노드 자체와 그것의 식별자(ID) 모두를 가르킨다. '키'라는 용어도 마찬가지이다.
...
## 잠재적인 용도
  ...
  - 협력적인 미러링: 로컬 네트워크 외부의 컴퓨터에서 사용할 수 있는 정보를 호스팅하는 로컬 네트워크의 [로드 밸런싱](https://ko.wikipedia.org/wiki/부하분산 "wikilink") 매커니즘. 이 스킴으로 개발자들은 중앙 서버 대신 많은 서버 간의 부하량의 균형을 맞춰 제품의 고가용성을 보장할 수 있다.
