...
[함수 종속과](../Page/함수_종속.md "wikilink") 달리 **다치 종속**은 어떠한 조건을 만족하는 [투플](https://ko.wikipedia.org/wiki/투플 "wikilink")이 릴레이션 안에 있을 것을 요구한다. 그러므로 다치 종속은 [터플](https://ko.wikipedia.org/wiki/터플 "wikilink")이 만들어내는 "종속" 중의 한 종류로 분류할 수 있다.
...
다치 종속을 가진 데이터베이스는 결국 중복성을 보인다. [데이터베이스 정규화](https://ko.wikipedia.org/wiki/데이터베이스_정규화 "wikilink") 과정 중 [제 4 정규형이](https://ko.wikipedia.org/wiki/제_4_정규형 "wikilink") 다치 종속이 존재하지 않을 것을 요구한다.
