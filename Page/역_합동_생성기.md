...
여기서 역 합동 생성기의 각 인자들은 다음과 같은 성질을 만족해야 한다.
  ...
  - 나눔수 \(m\)은 [소수여야](https://ko.wikipedia.org/wiki/소수_\(수론\) "wikilink") 한다.
  - 곱함수 \(a\)와 더함수 \(c\)는 0보다 크고 나눔수보다 작아야 한다.
  - 초기값 \(X_0\)은 0보다 크거나 같고 나눔수보다 작아야 한다.

합동 곱셈에 대한 역원 \(X_n^{-1} ~\mbox{mod}~ m\)은 [유클리드 호제법의](https://ko.wikipedia.org/wiki/유클리드_호제법 "wikilink") 확장으로 계산할 수 있다. 또는 [페르마의 소정리에](https://ko.wikipedia.org/wiki/페르마의_소정리 "wikilink") 따라 \(X_n^{m-2} ~\mbox{mod}~ m\)을 대신 계산할 수 있다. 이 역원은 나눔수가 소수이기 때문에 \(X_n\)이 0이 아니면 항상 유일하게 존재하며, \(X_n\)이 0인 경우 후자와 같이 0을 역원으로 쓸 수 있다.

## 성질

역 합동 생성기의 난수열 주기는 최대 \(m\)이며, \(x^2 - cx - a\)가 [유한체](https://ko.wikipedia.org/wiki/유한체 "wikilink") \(\mathbb{Z}_m\) 상에서 [원시 다항식일](https://ko.wikipedia.org/wiki/원시_다항식 "wikilink") 때 최대 주기를 가진다. 이런 성질을 가지도록 인자를 선택하는 것은 자명하지는 않으나 효율적인 방법이 알려져 있다.\[2\]
...
역 합동 생성기는 나눔수가 소수가 아닌 경우로 일반화할 수 있다. 특히 컴퓨터 상의 빠른 연산을 위하여 \(m\)이 [2의 거듭제곱인](https://ko.wikipedia.org/wiki/2의_거듭제곱 "wikilink") 경우를 고려하면, 다음 조건이 만족될 때 최대 주기인 \(\frac{m}2\)을 얻을 수 있다
...
**명시적인 역 합동 생성기**(explicit inversive congruential generator, EICG)는 역 합동 생성기와 유사하나 약간 다른 [점화식](https://ko.wikipedia.org/wiki/점화식 "wikilink")을 사용한다.
