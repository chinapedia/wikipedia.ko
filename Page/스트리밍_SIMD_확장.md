...
SSE는 XMM 레지스터의 자료구조로 4개의 32비트 단정밀도 부동소수점을 사용했다. 즉, 하나의 레지스터에 4개의 값이 들어가는 형태였다. SSE에서는 정수 계산을 지원하지 않는다. 이것은 [MMX](../Page/MMX.md "wikilink") 명령어를 사용하는 방식으로 극복이 가능했다. 또한, SSE2부터는 SSE를 확장하여 다음의 자료구조를 지원한다.
