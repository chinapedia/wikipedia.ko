...
[크리스토퍼 J. 데이트에](https://ko.wikipedia.org/wiki/크리스토퍼_J._데이트 "wikilink") 의하면 테이블은 "어떤 관계와 동일 구조"이면 1NF 이다. 이는 아래의 5가지 조건을 충족한다는 의미이다:
...
이 테이블의 [키](https://ko.wikipedia.org/wiki/후보_키 "wikilink") 는 {Subscriber ID, Email Address} 이다.
