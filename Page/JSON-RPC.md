...
**JSON-RPC**는 [JSON](https://ko.wikipedia.org/wiki/JSON "wikilink")으로 인코딩된 [원격 프로시저 호출이다](https://ko.wikipedia.org/wiki/원격_프로시저_호출 "wikilink"). 매우 간단한 프로토콜([XML-RPC](https://ko.wikipedia.org/wiki/XML-RPC "wikilink")와 매우 흡사함)로서, 소량의 데이터 타입과 명령들만을 정의하고 있다. JSON-RPC는 알림(notification, 서버로 데이터가 전송되고 응답을 요구하지 않음)을 허용하며, 다수의 호출이 서버로 전송되고 순서없이 응답되는 것을 허용한다.
...
하나의 리모트 메소드는 [HTTP](https://ko.wikipedia.org/wiki/HTTP "wikilink") 혹은 [TCP/IP](https://ko.wikipedia.org/wiki/TCP/IP "wikilink") 소켓 (버전 2.0으로 시작하는)을 사용해 리모트 서비스로 요청을 보내는 것에 의해 호출된다. HTTP를 사용할 때는 [content-type이](https://ko.wikipedia.org/wiki/MIME#Content-Type "wikilink") `application/json`.\[1\]로 정의될 수도 있다.
...
## 같이 보기
  ...
  - [원격 프로시저 호출](https://ko.wikipedia.org/wiki/원격_프로시저_호출 "wikilink") (RPC)
