...
두 [가환환](https://ko.wikipedia.org/wiki/가환환 "wikilink") 사이의 [환 준동형](https://ko.wikipedia.org/wiki/환_준동형 "wikilink") \(f\colon R\to S\)가 주어졌을 때, \(S\)는 \(f\)를 통해 \(R\)-가환 [결합 대수를](../Page/결합_대수.md "wikilink") 이룬다. 만약 다음 조건이 성립한다면, \(f\)를 **유한 표시 준동형**(有限表示準同型, )이라고 한다.
