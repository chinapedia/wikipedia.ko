...
**인터페이스**(interface)는 [자바 프로그래밍 언어에서](../Page/자바_\(프로그래밍_언어\).md "wikilink") [클래스들이](https://ko.wikipedia.org/wiki/클래스_\(컴퓨터_과학\) "wikilink") 구현해야 하는 동작을 지정하는데 사용되는 [추상형](https://ko.wikipedia.org/wiki/추상형 "wikilink")이다. 이들은 [프로토콜](https://ko.wikipedia.org/wiki/프로토콜 "wikilink")과 비슷하다. 인터페이스는 `interface`라는 [키워드](https://ko.wikipedia.org/wiki/키워드 "wikilink")를 사용하여 선언하며, 메소드 시그너처와 상수 선언([static과](../Page/정적_변수.md "wikilink") [final이](https://ko.wikipedia.org/wiki/파이널_\(자바\) "wikilink") 둘 다 선언되는 변수 선언)만을 포함할 수 있다. 자바 8 미만의 모든 버전을 기준으로 인터페이스의 모든 메소드는 구현체(메소드 바디)를 포함하고 있지 않다. 자바 8부터, `default`와 `static` 메소드는 `interface` 정의에 구현체를 가지고 있을 수 있다.\[1\]
