...
**구체화 뷰**(), **구체화된 뷰**는 [쿼리](https://ko.wikipedia.org/wiki/쿼리 "wikilink")의 결과를 담고 있는 [데이터베이스](https://ko.wikipedia.org/wiki/데이터베이스 "wikilink") 오브젝트이다. 원거리에 위치한 데이터의 근거리 사본일 수도 있고, 테이블의 줄이나 열의 하부 집합 또는 [Join 결과일](../Page/Join_\(SQL\).md "wikilink") 수도 있으며 테이블 데이터 집합에 기반한 요약일 수도 있다. 원거리 테이블에 기반한 데이터를 저장하는 구체화 뷰는 스냅샷으로도 부른다. 스냅샷을 구체화 뷰로 재정의할 수 있다.

## 개요

구체화 뷰는 [오라클 데이터베이스에](https://ko.wikipedia.org/wiki/오라클_데이터베이스 "wikilink") 처음 도입된 것으로, 쿼리 다시 쓰기 기능은 버전 8i부터 추가되었다.\[1\] [사이베이스](https://ko.wikipedia.org/wiki/사이베이스 "wikilink") [SQL 애니웨어](https://ko.wikipedia.org/wiki/SQL_애니웨어 "wikilink")\[2\], [티베로](https://ko.wikipedia.org/wiki/티베로 "wikilink")에서도 지원한다. [IBM DB2에서는](https://ko.wikipedia.org/wiki/IBM_DB2 "wikilink") 구체화 쿼리 테이블(materialized query tables)로 부른다. [마이크로소프트 SQL 서버](https://ko.wikipedia.org/wiki/마이크로소프트_SQL_서버 "wikilink") 또한 색인화된 뷰(indexed view)라는 이름의 비슷한 기능이 있다.\[3\] [MySQL](https://ko.wikipedia.org/wiki/MySQL "wikilink")은 구체화 뷰를 네이티브로 지원하지는 않지만 트리거나 저장된 프로시저를 사용하거나\[4\] 오픈 소스 응용 프로그램인 플렉스뷰를 이용하여 일부 작업이 가능하다.\[5\] [PostgreSQL](https://ko.wikipedia.org/wiki/PostgreSQL "wikilink")에도 구체화 뷰를 적용할 수 있는데, 버전 9.3부터 이를 네이티브로 지원한다.\[6\]
