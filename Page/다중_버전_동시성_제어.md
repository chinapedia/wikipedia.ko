...
**다중 버전 동시성 제어**(, MCC, MVCC), **다중 버전 병행 수행 제어**는 [데이터베이스 관리 시스템이](https://ko.wikipedia.org/wiki/데이터베이스_관리_시스템 "wikilink") 일반적으로 사용하는 [동시성 제어](https://ko.wikipedia.org/wiki/동시성_제어 "wikilink") 방식으로, 데이터베이스로의 동시 접근을 제공하고 프로그래밍 언어에서 [트랜잭셔널 메모리를](https://ko.wikipedia.org/wiki/트랜잭셔널_메모리 "wikilink") 구현한다.\[1\]
