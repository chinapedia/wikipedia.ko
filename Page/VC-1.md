> This article is converted from Wikipedia: [VC-1](https://ko.wikipedia.org/wiki/VC-1).


**SMPTE 421M** 또는 **VC-1**는 초기에 [마이크로소프트](https://ko.wikipedia.org/wiki/마이크로소프트 "wikilink") 사의 사유 영상 포맷으로 개발되었으며, [SMPTE](../Page/SMPTE.md "wikilink") 영상 코덱 표준으로 2006년 4월 3일에 발표되었다. 현재 블루레이 디스크, 윈도 미디어, 실버라이트 프레임워크, 슬링박스 및 HD DVD(현재 생산중단)의 지원 코덱에 포함되어 있다.

## 외부 링크

  -
  -
  - [VC-1 reference software analysis](http://wiki.multimedia.cx/index.php?title=Understanding_VC-1) at the MultimediaWiki

  - RFC 4425 – IETF RTP Payload Format for Video Codec 1

  - for the Raspberry Pi

[분류:영상 코덱](https://ko.wikipedia.org/wiki/분류:영상_코덱 "wikilink") [분류:마이크로소프트 이니셔티브](https://ko.wikipedia.org/wiki/분류:마이크로소프트_이니셔티브 "wikilink") [분류:영상 압축](https://ko.wikipedia.org/wiki/분류:영상_압축 "wikilink")