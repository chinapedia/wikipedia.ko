...
**존스 행렬**(Jones Matrix) 또는 **존스 계산식**(Jones calculus)은 [빛](https://ko.wikipedia.org/wiki/빛 "wikilink")의 [편광](../Page/편광.md "wikilink")을 기술해 주는 이차원 벡터 **존스 벡터**(Jones Vector)를 다루기 위한 [행렬](https://ko.wikipedia.org/wiki/행렬 "wikilink") 표현식이다. 이 방법은 [1941년](https://ko.wikipedia.org/wiki/1941년 "wikilink") [미국](https://ko.wikipedia.org/wiki/미국 "wikilink")의 [물리학자](https://ko.wikipedia.org/wiki/물리학자 "wikilink") 존스(R. C. Jones)에 의해 고안되었다. 빛이 광학소자를 투과할 때 그 광학소자의 광학적 특성을 2×2 존스 행렬로 표현할 수 있는데, 빛의 존스 벡터에 이 존스 행렬를 곱하면 투과한 빛의 편광상태를 계산할 수 있다.

존스 벡터는 \(\begin{pmatrix} E_x(t) \\ E_y(t)\end{pmatrix}\)와 같이 정의되는데 \({E_x(t)}\,\)와 \({E_y(t)}\,\)는 각각 [전기장](https://ko.wikipedia.org/wiki/전기장 "wikilink")의 *x*축과 *y*축 방향 성분을 뜻한다. 일반적으로 두 성분의 제곱의 합이 1이 되도록 [규격화된](https://ko.wikipedia.org/wiki/정규화_\(행렬\) "wikilink") 존스 벡터(normalized Jones Vector)를 사용한다.
