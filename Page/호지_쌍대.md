...
[미분기하학](https://ko.wikipedia.org/wiki/미분기하학 "wikilink")에서, **호지 쌍대**(Hodge雙對, )는 [미분 형식을](../Page/미분_형식.md "wikilink") 그 여차원의 미분 형식으로 변환시키는 연산이다. 기호는 [별표](../Page/별표.md "wikilink")(\(*\)). **호지 별표**()로도 부른다.
...
\[\star\alpha = \alpha^\# \lrcorner \omega \in \Omega^{n-p}(M)\] 여기서
  ...
  - \(\omega \in\Omega^n(M)\)는 준 리만 계량 및 [방향으로](https://ko.wikipedia.org/wiki/방향_\(다양체\) "wikilink") 정의되는 [부피 형식이다](../Page/부피_형식.md "wikilink").

지표로 쓰면,

\[\omega_{i_1\dotso i_n} = \sqrt{|\det g|}\epsilon_{i_1\dotso i_n}\] 이므로,

\[(\star\alpha)_{i_{p+1}\dotso i_n} = \frac1{p!}\sqrt{|\det g|}\epsilon_{i_1\dotso i_n} g^{i_1j_1} \dotsm g^{i_pj_p}\alpha_{j_1\dotso j_p}\] 이다. 여기서 \(\epsilon^{i_1,\dots,i_n}\)은 \(n\)차원 [레비치비타 기호이다](https://ko.wikipedia.org/wiki/레비치비타_기호 "wikilink").
...
\[\forall \beta\in H^p\colon \beta\wedge\star\alpha=\langle\beta,\alpha\rangle_{H^p}\omega\in H^n\] 즉, 이 경우 호지 쌍대 연산은 [힐베르트 공간의](https://ko.wikipedia.org/wiki/힐베르트_공간 "wikilink") 내적과 [쐐기곱](https://ko.wikipedia.org/wiki/쐐기곱 "wikilink") 사이의 변환이다.
