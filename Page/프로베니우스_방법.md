...
\[0=r(r-1)\cdots(r-k+1)+p_{k-1}(0)r(r-1)\cdots(r-k+2)+\cdots+p_1(0)r+p_0(0)\] 임을 알 수 있다. \(r\)에 대한 이 *n*차 다항식을 **결정 다항식**()이라고 하며, \(r\)는 결정 다항식의 근이다.
  ...
  - 만약 결정 다항식의 근들의 차가 [정수](https://ko.wikipedia.org/wiki/정수 "wikilink")가 아니라면, 모든 해들은 위와 같은 [거듭제곱 급수로](https://ko.wikipedia.org/wiki/거듭제곱_급수 "wikilink") 전개될 수 있다.
...
\[r=\frac12\left(1-p_1(0)\pm\sqrt{(p_1(0)-1)^2-4p_2(0)}\right)\] 이 경우 두 근을 \(r_1,r_2\)라고 하자. 이 경우 다음과 같은 경우가 가능하다.
  ...
  - 만약 두 근이 서로 겹치지 않고, 또한 \(r_1-r_2\)가 [정수](https://ko.wikipedia.org/wiki/정수 "wikilink")가 아니라면
