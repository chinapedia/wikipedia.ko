...
**아마존 관계형 데이터베이스 서비스**(Amazon Relational Database Service) 또는 **아마존 RDS**(Amazon RDS)는 [아마존 웹 서비스](https://ko.wikipedia.org/wiki/아마존_웹_서비스 "wikilink")(AWS)가 서비스하는 분산 [관계형 데이터베이스이다](https://ko.wikipedia.org/wiki/관계형_데이터베이스 "wikilink").\[1\] 애플리케이션 내에서 관계형 데이터베이스의 설정, 운영, 스케일링을 단순케 하도록 설계된 [클라우드 내에서](https://ko.wikipedia.org/wiki/클라우드_컴퓨팅 "wikilink") 동작하는 [웹 서비스이다](https://ko.wikipedia.org/wiki/웹_서비스 "wikilink").\[2\] 데이터베이스 소프트웨어를 패치하거나 데이터베이스를 백업하거나 [시점 복구를](https://ko.wikipedia.org/wiki/시점_복구 "wikilink") 활성화하는 것과 같은 복잡한 관리 프로세스들은 자동으로 관리된다.\[3\] 스토리지와 연산 자원들을 스케일링하는 것은 하나의 [API](https://ko.wikipedia.org/wiki/API "wikilink") 호출로 수행할 수 있다.
