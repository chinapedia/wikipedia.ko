...
[섬네일](https://ko.wikipedia.org/wiki/파일:Sbus-das-fddi.jpg "wikilink") **파이버 분산형 데이터 인터페이스**(, FDDI)는 [근거리 통신망의](https://ko.wikipedia.org/wiki/근거리_통신망 "wikilink") [데이터 전송](https://ko.wikipedia.org/wiki/데이터_전송 "wikilink") 표준이다. **광섬유 분산 데이터 인터페이스**, **광섬유 분산 데이터 접속 방식**이라고도 한다.

## 개요

FDDI는 [근거리 통신망에서](https://ko.wikipedia.org/wiki/근거리_통신망 "wikilink") [데이터 전송을](https://ko.wikipedia.org/wiki/데이터_전송 "wikilink") 위한 100 [MBit/초의](https://ko.wikipedia.org/wiki/데이터_속도_단위 "wikilink") 광 표준을 제공하며, 최대 200킬로미터(120마일)까지 확장이 가능하다. FDDI 논리 토폴로지가 링 기반 토큰망이지만, IEEE 802.5 [토큰링](https://ko.wikipedia.org/wiki/토큰링 "wikilink") 프로토콜을 기초로 사용하지 않았다. 그 대신, 프로토콜은 IEEE 802.4 [토큰 버스](https://ko.wikipedia.org/wiki/토큰_버스 "wikilink") 타임드 토큰(timed token) 프로토콜에서 비롯되었다. 드넓은 지역을 커버할 수 있을 뿐 아니라, FDDI의 근거리 통신망은 수천 명의 사용자들을 지원할 수 있다. FDDI는 이중반전 토큰링 토폴로지인 DAS(Dual-Attached Station)와 토큰 버스 패싱 링 토폴로지인 SAS(Single-Attached Station)를 둘 다 제공한다.\[1\]
