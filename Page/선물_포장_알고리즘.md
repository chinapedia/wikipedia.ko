...
그러나, 실행 시간이 껍질의 정점에 선형으로 의존하기 때문에, 기 때문에, 껍질 정점의 개수 *h* 가 log *n* 보다 작을때에만 실행시간이 \(\) 인 [그레이엄 스캔](../Page/그레이엄_스캔.md "wikilink") 알고리즘보다 빠르게 작동할 수 있다. 볼록 껍질을 찾는 또 다른 알고리즘인 [찬의 알고리즘은](https://ko.wikipedia.org/wiki/찬의_알고리즘 "wikilink") 선물 포장 알고리즘에 그레이엄 스캔의 로그 의존성을 결합하여 두 알고리즘보다 향상된 점근 실행 시간 \(\)
