> This article is converted from Wikipedia: [모듈:Shortcut](https://ko.wikipedia.org/wiki/모듈:Shortcut).
...
`   return string.format('`[`%s:%s`](https://ko.wikipedia.org/wiki/%s:%s "wikilink")`', mw.site.namespaces[14].name, cat)`
...
`   local listItems = {}`
`   for i, shortcut in ipairs(shortcuts) do`
`       listItems[i] = string.format('`[`%s`](https://ko.wikipedia.org/wiki/%s "wikilink")`', shortcut)`
`   end`
...
`   root:newline() -- To match the old `[`Template:Shortcut`](https://ko.wikipedia.org/wiki/Template:Shortcut "wikilink")
