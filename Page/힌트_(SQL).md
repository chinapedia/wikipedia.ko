...
여러 데이터베이스 엔진들은 힌트 구현 시 각기 다른 접근 방식을 사용한다.
  ...
  - [MySQL](../Page/MySQL.md "wikilink")은 SQL 표준의 자체 확장을 사용하는데, [테이블](https://ko.wikipedia.org/wiki/테이블_\(데이터베이스\) "wikilink") 이름 뒤에 USE INDEX, FORCE INDEX , IGNORE INDEX 키워드가 올 수 있다.\[1\]
  - [오라클은](../Page/오라클_데이터베이스.md "wikilink") 특별히 제작된 코멘트(comment)를 + 기호로 시작하는 쿼리 안에 사용함으로써 SQL 호환성에 영향을 미치지 않음과 동시에 힌트를 구현한다.\[2\]
  - Postgres Plus® Advanced Server([엔터프라이즈DB](https://ko.wikipedia.org/wiki/엔터프라이즈DB "wikilink")에서 출시된 [PostgreSQL](../Page/PostgreSQL.md "wikilink")의 사유 버전)는 오라클과 동일하게 호환되는 힌트들을 제공한다.\[3\]\[4\]
