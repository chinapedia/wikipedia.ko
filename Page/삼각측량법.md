...
[섬네일|310px |삼각측량법은 해변에서 배까지의 거리를 측정하는 데 활용될 수 있다. 관측자 A가 해변과 배 사이의 [각도](https://ko.wikipedia.org/wiki/각도 "wikilink") α를 측정하고 관측자 B가 같은 식으로 [각도](https://ko.wikipedia.org/wiki/각도 "wikilink") β를 구한다. A와 B사이의 거리 l이 주어지거나, A, B 각각의 좌표가 주어진다면, [사인 법칙](https://ko.wikipedia.org/wiki/사인_법칙 "wikilink") 등을 이용하여 C에 위치한 배의 좌표를 알 수 있으며, 바닷가에서 배까지의 거리 d도 알아낼 수 있다.](https://ko.wikipedia.org/wiki/파일:Distance_by_triangulation.svg "wikilink")

**삼각측량법**은 측량 구역을 삼각형으로 분할하여 각 지점의 수평위치를 결정하는 측량법의 하나이다. 공공측량표준작업규정에서는 "1등, 2등, 3등, 4등 기본 삼각점 또는 기설 기준 삼각점을 기준으로 공공측량표준작업규정에서 정한 방법이나 정도로써 그 점의 위치를 결정하는 작업"이라고 정의한다. 그 점과 두 기준점이 주어졌으면, 그 점과 두 기준점이 이루는 삼각형에서 밑변과 다른 두 변이 이루는 각을 각각 측정하고, 그 변의 길이를 측정한 뒤, [사인 법칙](https://ko.wikipedia.org/wiki/사인_법칙 "wikilink") 등을 이용하여 일련의 계산을 수행함으로써, 그 점에 대해 [좌표](https://ko.wikipedia.org/wiki/좌표 "wikilink")와 거리를 알아내는 방법이다.

삼각측량법은 [측량](https://ko.wikipedia.org/wiki/측량 "wikilink"), [항해](https://ko.wikipedia.org/wiki/항해 "wikilink"), [측정](https://ko.wikipedia.org/wiki/측정 "wikilink"), [천체측량학](https://ko.wikipedia.org/wiki/천체측량학 "wikilink"), [로켓 공학](https://ko.wikipedia.org/wiki/로켓_공학 "wikilink") 등에 쓰이며, 무기(대포 등)의 방향 설정에도 쓰인다.
...
다음의 법칙들이 사용되었다. ([유클리드 기하에서만](https://ko.wikipedia.org/wiki/유클리드_기하학 "wikilink") 성립한다.)
...
<!-- end list -->
  ...
  - **MC**는 [피타고라스 정리를](https://ko.wikipedia.org/wiki/피타고라스_정리 "wikilink") 이용하여 구할 수 있다.
...
## 같이 보기
  ...
  - [시차 (천문학)](https://ko.wikipedia.org/wiki/시차_\(천문학\) "wikilink")
