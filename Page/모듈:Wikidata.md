...
`       [0] = "$10억년",  -- precision: billion years`
`       [1] = "$1억년",   -- precision: hundred million years`
`       [2] = "$1천만년",  -- precision: ten million years`
`       [3] = "$1백만년",  -- precision: million years`
`       [4] = "$100,000년",      -- precision: hundred thousand years`
`       [5] = "$10,000년",       -- precision: ten thousand years`
`       [6] = "$1천년",       -- precision: millennium`
`       [7] = "$1세기",           -- precision: century`
`       [8] = "$1년대",               -- precision: decade`
`       -- the following use the format of #time parser function`
`       [9]  = "Y년",                    -- precision: year, `
`       [10] = "Y년 n월",             -- precision: month`
`       [11] = "Y년 n월 j일",          -- precision: day`
`       [12] = "Y년 n월 j일 G시",           -- precision: hour`
`       [13] = "Y년 n월 j일 H:i",      -- precision: minute`
`       [14] = "Y년 n월 j일 H:i:s",        -- precision: second`
...
\-- this function needs to be internationalised along with the above: -- we need three exceptions in English for 1st, 2nd, 3rd -- takes cardinal numer as a numeric and returns the ordinal as a string local function makeOrdinal(cardinal)
...
`               return "`[`"``   ``..``   ``(linkName``   ``or``   ``linkTarget)``   ``..``   ``"`](https://ko.wikipedia.org/wiki/"_.._linkTarget_.._" "wikilink")`"`
`           else`
`               -- if there is no local Wikipedia article output the label or link to the Wikidata object to let the user input a proper label`
`               if linkName then return linkName else return "`[`"``   ``..``   ``id``   ``..``   ``"`](https://ko.wikipedia.org/wiki/:d:"_.._id_.._" "wikilink")`" end`
`           end`
`       else`
...
`                           `
`                       if sitelink then`
`                           out[#out + 1] = "`[`"``   ``..``   ``label``   ``..``   ``"`](https://ko.wikipedia.org/wiki/"_.._sitelink_.._" "wikilink")`"`
`                       else`
...
\-- This is used to get a value, or a comma separated list of them if multiple values exist -- from an arbitrary entry by using its QID. -- Use : {{\#invoke:Wikidata|getValueFromID|<ID>|<Property>|값찾기}} -- E.g.: {{\#invoke:Wikidata|getValueFromID|Q151973|P26|값찾기}} - to fetch value of 'spouse' (P26) from 'Richard Burton' (Q151973) -- Please use sparingly - this is an \*expensive call\*. p.getValueFromID = function(frame)
...
\-- This is used to fetch all of the images with a particular property, e.g. image (P18), Gene Atlas Image (P692), etc. -- Parameters are | propertyID | value / 값찾기 / nil | separator (default=space) | size (default=frameless) -- It will return a standard wiki-markup [size](https://ko.wikipedia.org/wiki/File:Filename "fig:size") for each image with a selectable size and separator (which may be html) -- e.g. {{\#invoke:Wikidata|getImages|P18|값찾기}} -- e.g. {{\#invoke:Wikidata|getImages|P18|값찾기|
...
`                   out[#out + 1] = "`[`"``   ``..``   ``imgsize``   ``..``   ``"`](https://ko.wikipedia.org/wiki/File:"_.._filename_.._" "fig:\" .. imgsize .. \"")`"`
`               end`
...
Call as {{\#invoke:Wikidata |getImageLegend | <PARAMETER> | lang=<ISO-639code> |id=<QID>}} Returns PARAMETER, unless it is equal to "값찾기", from Item QID (expensive call) If QID is omitted or blank, the current article is used (not an expensive call) If lang is omitted, it uses the local wiki language, otherwise it uses the provided ISO-639 language code ISO-639: <https://docs.oracle.com/cd/E13214_01/wli/docs92/xref/xqisocodes.html#wp1252447>
...
[Category:Called_function_'Dump'_from_module_Wikidata](https://ko.wikipedia.org/wiki/Category:Called_function_'Dump'_from_module_Wikidata "wikilink")