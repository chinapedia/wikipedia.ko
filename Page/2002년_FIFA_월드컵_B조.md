> This article is converted from Wikipedia: [2002 FIFA  B](https://ko.wikipedia.org/wiki/2002_FIFA__B).


[2002년 FIFA 월드컵은](https://ko.wikipedia.org/wiki/2002년_FIFA_월드컵 "wikilink") 32팀이 본선에 참가하였고 이들은 4팀씩 8개의 조로 나뉘었다. **[2002년 FIFA 월드컵](https://ko.wikipedia.org/wiki/2002년_FIFA_월드컵 "wikilink") B조**는 [스페인](../Page/스페인_축구_국가대표팀.md "wikilink"), [슬로베니아](../Page/슬로베니아_축구_국가대표팀.md "wikilink"), [파라과이](../Page/파라과이_축구_국가대표팀.md "wikilink"), 그리고 [남아프리카 공화국이](../Page/남아프리카_공화국_축구_국가대표팀.md "wikilink") 편성되었다. B조의 모든 일정은 2002년 6월 2일과 2002년 6월 12일사이에 치러졌다. [스페인은](../Page/스페인_축구_국가대표팀.md "wikilink") 이 조별 리그의 3경기에서 모두 승리하며 조 1위를 차지하였고, [파라과이와](../Page/파라과이_축구_국가대표팀.md "wikilink") 함께 16강에 진출하였다. [남아프리카 공화국은](../Page/남아프리카_공화국_축구_국가대표팀.md "wikilink") 파라과이와 승점과 골득실차에서 동률을 이루었으나, 다득점에서 1골차로 아깝게 탈락하였다. 이 대회에 첫 출전한 [슬로베니아는](../Page/슬로베니아_축구_국가대표팀.md "wikilink") 승점을 1점도 수확하지 못하고 탈락하였다.

## 순위

|}

  - *모든 시간은 [대한민국의](../Page/한국_표준시.md "wikilink") 동아시아 표준시를 따른다.* ([EAT/UTC+9](https://ko.wikipedia.org/wiki/UTC+9 "wikilink"))

## 파라과이 vs 남아프리카 공화국

|          |                        |                       |                        |               |               |                  |               |                   |                 |                |                 |          |                                   |                                  |                                   |                                   |                                    |                  |               |                   |                 |                |                |
| -------- | ---------------------- | --------------------- | ---------------------- | ------------- | ------------- | ---------------- | ------------- | ----------------- | --------------- | -------------- | --------------- | -------- | --------------------------------- | -------------------------------- | --------------------------------- | --------------------------------- | ---------------------------------- | ---------------- | ------------- | ----------------- | --------------- | -------------- | -------------- |
| {{축구 유니폼 | pattern_la = _par02h | pattern_b = _par02h | pattern_ra = _par02h | pattern_sh = | pattern_so = | leftarm = FF0000 | body = FF0000 | rightarm = FF0000 | shorts = 0033CC | socks = 0033CC | title = 파라과이 }} | {{축구 유니폼 | pattern_la = _adidasonwhite2002 | pattern_b = _adidasonwhite2002 | pattern_ra = _adidasonwhite2002 | pattern_sh = _adidasonwhite2002 | pattern_so = _3_stripes_yellow | leftarm = FFFF00 | body = FFFF00 | rightarm = FFFF00 | shorts = FFFF00 | socks = FFFFFF | title = 남아공 }} |

<table style="width:10%;">
<colgroup>
<col style="width: 5%" />
<col style="width: 0%" />
<col style="width: 5%" />
</colgroup>
<tbody>
<tr class="odd">
<td><table>
<tbody>
<tr class="odd">
<td><p>GK</p></td>
<td><p><strong>22</strong></p></td>
</tr>
<tr class="even">
<td><p>CB</p></td>
<td><p><strong>18</strong></p></td>
</tr>
<tr class="odd">
<td><p>CB</p></td>
<td><p><strong>5</strong></p></td>
</tr>
<tr class="even">
<td><p>CB</p></td>
<td><p><strong>4</strong></p></td>
</tr>
<tr class="odd">
<td><p>RWB</p></td>
<td><p><strong>2</strong></p></td>
</tr>
<tr class="even">
<td><p>LWB</p></td>
<td><p><strong>21</strong></p></td>
</tr>
<tr class="odd">
<td><p>CM</p></td>
<td><p><strong>10</strong></p></td>
</tr>
<tr class="even">
<td><p>CM</p></td>
<td><p><strong>6</strong></p></td>
</tr>
<tr class="odd">
<td><p>CM</p></td>
<td><p><strong>8</strong></p></td>
</tr>
<tr class="even">
<td><p>AM</p></td>
<td><p><strong>11</strong></p></td>
</tr>
<tr class="odd">
<td><p>CF</p></td>
<td><p><strong>9</strong></p></td>
</tr>
<tr class="even">
<td><p><strong>교체 선수:</strong></p></td>
<td></td>
</tr>
<tr class="odd">
<td><p>MF</p></td>
<td><p><strong>14</strong></p></td>
</tr>
<tr class="even">
<td><p>MF</p></td>
<td><p><strong>16</strong></p></td>
</tr>
<tr class="odd">
<td><p>DF</p></td>
<td><p><strong>17</strong></p></td>
</tr>
<tr class="even">
<td><p><strong>감독:</strong></p></td>
<td></td>
</tr>
<tr class="odd">
<td><p><a href="../Page/체사레_말디니.md" title="wikilink">체사레 말디니</a></p></td>
<td></td>
</tr>
</tbody>
</table></td>
<td></td>
<td><table>
<tbody>
<tr class="odd">
<td><p>GK</p></td>
<td><p><strong>16</strong></p></td>
</tr>
<tr class="even">
<td><p>RB</p></td>
<td><p><strong>4</strong></p></td>
</tr>
<tr class="odd">
<td><p>CB</p></td>
<td><p><strong>13</strong></p></td>
</tr>
<tr class="even">
<td><p>CB</p></td>
<td><p><strong>19</strong></p></td>
</tr>
<tr class="odd">
<td><p>LB</p></td>
<td><p><strong>3</strong></p></td>
</tr>
<tr class="even">
<td><p>RM</p></td>
<td><p><strong>2</strong></p></td>
</tr>
<tr class="odd">
<td><p>CM</p></td>
<td><p><strong>12</strong></p></td>
</tr>
<tr class="even">
<td><p>CM</p></td>
<td><p><strong>6</strong></p></td>
</tr>
<tr class="odd">
<td><p>LM</p></td>
<td><p><strong>7</strong></p></td>
</tr>
<tr class="even">
<td><p>CF</p></td>
<td><p><strong>17</strong></p></td>
</tr>
<tr class="odd">
<td><p>CF</p></td>
<td><p><strong>15</strong></p></td>
</tr>
<tr class="even">
<td><p><strong>교체 선수:</strong></p></td>
<td></td>
</tr>
<tr class="odd">
<td><p>MF</p></td>
<td><p><strong>9</strong></p></td>
</tr>
<tr class="even">
<td><p>FW</p></td>
<td><p><strong>23</strong></p></td>
</tr>
<tr class="odd">
<td><p><strong>감독:</strong></p></td>
<td></td>
</tr>
<tr class="even">
<td><p><a href="https://ko.wikipedia.org/wiki/조모_소노" title="wikilink">조모 소노</a></p></td>
<td></td>
</tr>
</tbody>
</table></td>
</tr>
</tbody>
</table>

<table>
<tbody>
<tr class="odd">
<td><p><strong>최우수 선수:</strong><br />
<a href="../Page/프란시스코_아르세.md" title="wikilink">프란시스코 아르세</a> (파라과이)</p>
<p><strong>부심:</strong><br />
이고르 슈람카 (<a href="https://ko.wikipedia.org/wiki/슬로바키아_축구_협회" title="wikilink">슬로바키아</a>)<br />
커티스 찰스 (<a href="https://ko.wikipedia.org/wiki/앤티가_바부다_축구_협회" title="wikilink">앤티가 바부다</a>)<br />
<strong>대기심:</strong><br />
<a href="https://ko.wikipedia.org/wiki/휴_댈러스" title="wikilink">휴 댈러스</a> (<a href="https://ko.wikipedia.org/wiki/스코틀랜드_축구_협회" title="wikilink">스코틀랜드</a>)</p></td>
</tr>
</tbody>
</table>

## 스페인 vs 슬로베니아

|          |                          |                         |                          |                          |                         |                  |               |                   |                 |                |                |          |              |                       |              |               |               |                  |               |                   |                 |                |                  |
| -------- | ------------------------ | ----------------------- | ------------------------ | ------------------------ | ----------------------- | ---------------- | ------------- | ----------------- | --------------- | -------------- | -------------- | -------- | ------------ | --------------------- | ------------ | ------------- | ------------- | ---------------- | ------------- | ----------------- | --------------- | -------------- | ---------------- |
| {{축구 유니폼 | pattern_la = _esp0204h | pattern_b = _esp0204h | pattern_ra = _esp0204h | pattern_sh = _esp0204h | pattern_so = _esp9496 | leftarm = e80113 | body = e80113 | rightarm = e80113 | shorts = 0000FF | socks = 003060 | title = 스페인 }} | {{축구 유니폼 | pattern_la= | pattern_b = _slo01h | pattern_ra= | pattern_sh = | pattern_so = | leftarm = FFFFFF | body = FFFFFF | rightarm = FFFFFF | shorts = FFFFFF | socks = FFFFFF | title = 슬로베니아 }} |

<table style="width:10%;">
<colgroup>
<col style="width: 5%" />
<col style="width: 0%" />
<col style="width: 5%" />
</colgroup>
<tbody>
<tr class="odd">
<td><table>
<tbody>
<tr class="odd">
<td><p>GK</p></td>
<td><p><strong>1</strong></p></td>
</tr>
<tr class="even">
<td><p>RB</p></td>
<td><p><strong>5</strong></p></td>
</tr>
<tr class="odd">
<td><p>CB</p></td>
<td><p><strong>6</strong></p></td>
</tr>
<tr class="even">
<td><p>CB</p></td>
<td><p><strong>20</strong></p></td>
</tr>
<tr class="odd">
<td><p>LB</p></td>
<td><p><strong>3</strong></p></td>
</tr>
<tr class="even">
<td><p>RM</p></td>
<td><p><strong>21</strong></p></td>
</tr>
<tr class="odd">
<td><p>CM</p></td>
<td><p><strong>17</strong></p></td>
</tr>
<tr class="even">
<td><p>CM</p></td>
<td><p><strong>8</strong></p></td>
</tr>
<tr class="odd">
<td><p>LM</p></td>
<td><p><strong>11</strong></p></td>
</tr>
<tr class="even">
<td><p>CF</p></td>
<td><p><strong>7</strong></p></td>
</tr>
<tr class="odd">
<td><p>CF</p></td>
<td><p><strong>10</strong></p></td>
</tr>
<tr class="even">
<td><p><strong>교체 선수:</strong></p></td>
<td></td>
</tr>
<tr class="odd">
<td><p>FW</p></td>
<td><p><strong>9</strong></p></td>
</tr>
<tr class="even">
<td><p>MF</p></td>
<td><p><strong>4</strong></p></td>
</tr>
<tr class="odd">
<td><p>DF</p></td>
<td><p><strong>15</strong></p></td>
</tr>
<tr class="even">
<td><p><strong>감독:</strong></p></td>
<td></td>
</tr>
<tr class="odd">
<td><p><a href="../Page/호세_안토니오_카마초.md" title="wikilink">호세 안토니오 카마초</a></p></td>
<td></td>
</tr>
</tbody>
</table></td>
<td></td>
<td><table>
<tbody>
<tr class="odd">
<td><p>GK</p></td>
<td><p><strong>1</strong></p></td>
</tr>
<tr class="even">
<td><p>CB</p></td>
<td><p><strong>3</strong></p></td>
</tr>
<tr class="odd">
<td><p>CB</p></td>
<td><p><strong>5</strong></p></td>
</tr>
<tr class="even">
<td><p>CB</p></td>
<td><p><strong>6</strong></p></td>
</tr>
<tr class="odd">
<td><p>RM</p></td>
<td><p><strong>7</strong></p></td>
</tr>
<tr class="even">
<td><p>CM</p></td>
<td><p><strong>8</strong></p></td>
</tr>
<tr class="odd">
<td><p>CM</p></td>
<td><p><strong>11</strong></p></td>
</tr>
<tr class="even">
<td><p>LM</p></td>
<td><p><strong>19</strong></p></td>
</tr>
<tr class="odd">
<td><p>AM</p></td>
<td><p><strong>10</strong></p></td>
</tr>
<tr class="even">
<td><p>AM</p></td>
<td><p><strong>13</strong></p></td>
</tr>
<tr class="odd">
<td><p>CF</p></td>
<td><p><strong>9</strong></p></td>
</tr>
<tr class="even">
<td><p><strong>교체 선수:</strong></p></td>
<td></td>
</tr>
<tr class="odd">
<td><p>FW</p></td>
<td><p><strong>21</strong></p></td>
</tr>
<tr class="even">
<td><p>MF</p></td>
<td><p><strong>18</strong></p></td>
</tr>
<tr class="odd">
<td><p>MF</p></td>
<td><p><strong>14</strong></p></td>
</tr>
<tr class="even">
<td><p><strong>감독:</strong></p></td>
<td></td>
</tr>
<tr class="odd">
<td><p><a href="https://ko.wikipedia.org/wiki/스레치코_카타네츠" title="wikilink">스레치코 카타네츠</a></p></td>
<td></td>
</tr>
</tbody>
</table></td>
</tr>
</tbody>
</table>

<table>
<tbody>
<tr class="odd">
<td><p><strong>최우수 선수:</strong><br />
<a href="https://ko.wikipedia.org/wiki/라울_곤살레스" title="wikilink">라울</a> (스페인)</p>
<p><strong>부심:</strong><br />
알리 토무상게 (<a href="https://ko.wikipedia.org/wiki/우간다_축구_협회" title="wikilink">우간다</a>)<br />
에곤 베로이터 (<a href="https://ko.wikipedia.org/wiki/오스트리아_축구_협회" title="wikilink">오스트리아</a>)<br />
<strong>대기심:</strong><br />
<a href="https://ko.wikipedia.org/wiki/코피_코지아" title="wikilink">코피 코지아</a> (<a href="https://ko.wikipedia.org/wiki/베냉_축구_협회" title="wikilink">베냉</a>)</p></td>
</tr>
</tbody>
</table>

## 스페인 vs 파라과이

|          |                          |                         |                          |                          |                         |                  |               |                   |                 |                |                |          |                        |                       |                        |               |               |                  |               |                   |                 |                |                 |
| -------- | ------------------------ | ----------------------- | ------------------------ | ------------------------ | ----------------------- | ---------------- | ------------- | ----------------- | --------------- | -------------- | -------------- | -------- | ---------------------- | --------------------- | ---------------------- | ------------- | ------------- | ---------------- | ------------- | ----------------- | --------------- | -------------- | --------------- |
| {{축구 유니폼 | pattern_la = _esp0204h | pattern_b = _esp0204h | pattern_ra = _esp0204h | pattern_sh = _esp0204h | pattern_so = _esp9496 | leftarm = e80113 | body = e80113 | rightarm = e80113 | shorts = 0000FF | socks = 003060 | title = 스페인 }} | {{축구 유니폼 | pattern_la = _par02a | pattern_b = _par02a | pattern_ra = _par02a | pattern_sh = | pattern_so = | leftarm = FAAC58 | body = FAAC58 | rightarm = FAAC58 | shorts = FFFFFF | socks = FFFFFF | title = 파라과이 }} |

<table style="width:10%;">
<colgroup>
<col style="width: 5%" />
<col style="width: 0%" />
<col style="width: 5%" />
</colgroup>
<tbody>
<tr class="odd">
<td><table>
<tbody>
<tr class="odd">
<td><p>GK</p></td>
<td><p><strong>1</strong></p></td>
</tr>
<tr class="even">
<td><p>RB</p></td>
<td><p><strong>5</strong></p></td>
</tr>
<tr class="odd">
<td><p>CB</p></td>
<td><p><strong>6</strong></p></td>
</tr>
<tr class="even">
<td><p>CB</p></td>
<td><p><strong>20</strong></p></td>
</tr>
<tr class="odd">
<td><p>LB</p></td>
<td><p><strong>3</strong></p></td>
</tr>
<tr class="even">
<td><p>RM</p></td>
<td><p><strong>21</strong></p></td>
</tr>
<tr class="odd">
<td><p>CM</p></td>
<td><p><strong>8</strong></p></td>
</tr>
<tr class="even">
<td><p>CM</p></td>
<td><p><strong>17</strong></p></td>
</tr>
<tr class="odd">
<td><p>LM</p></td>
<td><p><strong>11</strong></p></td>
</tr>
<tr class="even">
<td><p>CF</p></td>
<td><p><strong>7</strong></p></td>
</tr>
<tr class="odd">
<td><p>CF</p></td>
<td><p><strong>10</strong></p></td>
</tr>
<tr class="even">
<td><p><strong>교체 선수:</strong></p></td>
<td></td>
</tr>
<tr class="odd">
<td><p>MF</p></td>
<td><p><strong>4</strong></p></td>
</tr>
<tr class="even">
<td><p>FW</p></td>
<td><p><strong>9</strong></p></td>
</tr>
<tr class="odd">
<td><p>MF</p></td>
<td><p><strong>19</strong></p></td>
</tr>
<tr class="even">
<td><p><strong>감독:</strong></p></td>
<td></td>
</tr>
<tr class="odd">
<td><p><a href="../Page/호세_안토니오_카마초.md" title="wikilink">호세 안토니오 카마초</a></p></td>
<td></td>
</tr>
</tbody>
</table></td>
<td></td>
<td><table>
<tbody>
<tr class="odd">
<td><p>GK</p></td>
<td><p><strong>1</strong></p></td>
</tr>
<tr class="even">
<td><p>CB</p></td>
<td><p><strong>18</strong></p></td>
</tr>
<tr class="odd">
<td><p>CB</p></td>
<td><p><strong>5</strong></p></td>
</tr>
<tr class="even">
<td><p>CB</p></td>
<td><p><strong>4</strong></p></td>
</tr>
<tr class="odd">
<td><p>RWB</p></td>
<td><p><strong>2</strong></p></td>
</tr>
<tr class="even">
<td><p>LWB</p></td>
<td><p><strong>21</strong></p></td>
</tr>
<tr class="odd">
<td><p>CM</p></td>
<td><p><strong>14</strong></p></td>
</tr>
<tr class="even">
<td><p>CM</p></td>
<td><p><strong>10</strong></p></td>
</tr>
<tr class="odd">
<td><p>CM</p></td>
<td><p><strong>13</strong></p></td>
</tr>
<tr class="even">
<td><p>CF</p></td>
<td><p><strong>9</strong></p></td>
</tr>
<tr class="odd">
<td><p>CF</p></td>
<td><p><strong>20</strong></p></td>
</tr>
<tr class="even">
<td><p><strong>교체 선수:</strong></p></td>
<td></td>
</tr>
<tr class="odd">
<td><p>FW</p></td>
<td><p><strong>11</strong></p></td>
</tr>
<tr class="even">
<td><p>MF</p></td>
<td><p><strong>6</strong></p></td>
</tr>
<tr class="odd">
<td><p><strong>감독:</strong></p></td>
<td></td>
</tr>
<tr class="even">
<td><p><a href="../Page/체사레_말디니.md" title="wikilink">체사레 말디니</a></p></td>
<td></td>
</tr>
</tbody>
</table></td>
</tr>
</tbody>
</table>

<table>
<tbody>
<tr class="odd">
<td><p><strong>최우수 선수:</strong><br />
<a href="../Page/페르난도_모리엔테스.md" title="wikilink">페르난도 모리엔테스</a> (스페인)</p>
<p><strong>부심:</strong><br />
와기 파라그 (<a href="https://ko.wikipedia.org/wiki/이집트_축구_협회" title="wikilink">이집트</a>)<br />
브라이튼 무자미리 (<a href="https://ko.wikipedia.org/wiki/짐바브웨_축구_협회" title="wikilink">짐바브웨</a>)<br />
<strong>대기심:</strong><br />
<a href="https://ko.wikipedia.org/wiki/모하메드_게자즈" title="wikilink">모하메드 게자즈</a> (<a href="https://ko.wikipedia.org/wiki/모로코_왕립_축구_협회" title="wikilink">모로코</a>)</p></td>
</tr>
</tbody>
</table>

## 남아프리카 공화국 vs 슬로베니아

|          |                                   |                                  |                                   |                                   |                                    |                  |               |                   |                 |                |                |          |               |                       |               |               |               |                  |               |                   |                 |                |                  |
| -------- | --------------------------------- | -------------------------------- | --------------------------------- | --------------------------------- | ---------------------------------- | ---------------- | ------------- | ----------------- | --------------- | -------------- | -------------- | -------- | ------------- | --------------------- | ------------- | ------------- | ------------- | ---------------- | ------------- | ----------------- | --------------- | -------------- | ---------------- |
| {{축구 유니폼 | pattern_la = _adidasonwhite2002 | pattern_b = _adidasonwhite2002 | pattern_ra = _adidasonwhite2002 | pattern_sh = _adidasonwhite2002 | pattern_so = _3_stripes_yellow | leftarm = FFFF00 | body = FFFF00 | rightarm = FFFF00 | shorts = FFFF00 | socks = FFFFFF | title = 남아공 }} | {{축구 유니폼 | pattern_la = | pattern_b = _slo01a | pattern_ra = | pattern_sh = | pattern_so = | leftarm = 006060 | body = 006060 | rightarm = 006060 | shorts = 006060 | socks = 006060 | title = 슬로베니아 }} |

<table style="width:10%;">
<colgroup>
<col style="width: 5%" />
<col style="width: 0%" />
<col style="width: 5%" />
</colgroup>
<tbody>
<tr class="odd">
<td><table>
<tbody>
<tr class="odd">
<td><p>GK</p></td>
<td><p><strong>16</strong></p></td>
</tr>
<tr class="even">
<td><p>RB</p></td>
<td><p><strong>2</strong></p></td>
</tr>
<tr class="odd">
<td><p>CB</p></td>
<td><p><strong>4</strong></p></td>
</tr>
<tr class="even">
<td><p>CB</p></td>
<td><p><strong>19</strong></p></td>
</tr>
<tr class="odd">
<td><p>LB</p></td>
<td><p><strong>3</strong></p></td>
</tr>
<tr class="even">
<td><p>RM</p></td>
<td><p><strong>15</strong></p></td>
</tr>
<tr class="odd">
<td><p>CM</p></td>
<td><p><strong>6</strong></p></td>
</tr>
<tr class="even">
<td><p>CM</p></td>
<td><p><strong>12</strong></p></td>
</tr>
<tr class="odd">
<td><p>LM</p></td>
<td><p><strong>7</strong></p></td>
</tr>
<tr class="even">
<td><p>CF</p></td>
<td><p><strong>14</strong></p></td>
</tr>
<tr class="odd">
<td><p>CF</p></td>
<td><p><strong>17</strong></p></td>
</tr>
<tr class="even">
<td><p><strong>교체 선수:</strong></p></td>
<td></td>
</tr>
<tr class="odd">
<td><p>MF</p></td>
<td><p><strong>18</strong></p></td>
</tr>
<tr class="even">
<td><p>FW</p></td>
<td><p><strong>23</strong></p></td>
</tr>
<tr class="odd">
<td><p>MF</p></td>
<td><p><strong>11</strong></p></td>
</tr>
<tr class="even">
<td><p><strong>감독:</strong></p></td>
<td></td>
</tr>
<tr class="odd">
<td><p><a href="https://ko.wikipedia.org/wiki/조모_소노" title="wikilink">조모 소노</a></p></td>
<td></td>
</tr>
</tbody>
</table></td>
<td></td>
<td><table>
<tbody>
<tr class="odd">
<td><p>GK</p></td>
<td><p><strong>1</strong></p></td>
</tr>
<tr class="even">
<td><p>CB</p></td>
<td><p><strong>3</strong></p></td>
</tr>
<tr class="odd">
<td><p>CB</p></td>
<td><p><strong>4</strong></p></td>
</tr>
<tr class="even">
<td><p>CB</p></td>
<td><p><strong>6</strong></p></td>
</tr>
<tr class="odd">
<td><p>RM</p></td>
<td><p><strong>7</strong></p></td>
</tr>
<tr class="even">
<td><p>CM</p></td>
<td><p><strong>11</strong></p></td>
</tr>
<tr class="odd">
<td><p>CM</p></td>
<td><p><strong>8</strong></p></td>
</tr>
<tr class="even">
<td><p>LM</p></td>
<td><p><strong>19</strong></p></td>
</tr>
<tr class="odd">
<td><p>AM</p></td>
<td><p><strong>18</strong></p></td>
</tr>
<tr class="even">
<td><p>CF</p></td>
<td><p><strong>21</strong></p></td>
</tr>
<tr class="odd">
<td><p>CF</p></td>
<td><p><strong>13</strong></p></td>
</tr>
<tr class="even">
<td><p><strong>교체 선수:</strong></p></td>
<td></td>
</tr>
<tr class="odd">
<td><p>FW</p></td>
<td><p><strong>9</strong></p></td>
</tr>
<tr class="even">
<td><p>MF</p></td>
<td><p><strong>20</strong></p></td>
</tr>
<tr class="odd">
<td><p>DF</p></td>
<td><p><strong>23</strong></p></td>
</tr>
<tr class="even">
<td><p><strong>감독:</strong></p></td>
<td></td>
</tr>
<tr class="odd">
<td><p><a href="https://ko.wikipedia.org/wiki/스레치코_카타네츠" title="wikilink">스레치코 카타네츠</a></p></td>
<td></td>
</tr>
</tbody>
</table></td>
</tr>
</tbody>
</table>

<table>
<tbody>
<tr class="odd">
<td><p><strong>최우수 선수:</strong><br />
<a href="../Page/퀸턴_포춘.md" title="wikilink">퀸턴 포춘</a> (남아프리카 공화국)</p>
<p><strong>부심:</strong><br />
호르헤 라탈리노 (<a href="../Page/아르헨티나_축구_협회.md" title="wikilink">아르헨티나</a>)<br />
알리 알-트라이피 (<a href="https://ko.wikipedia.org/wiki/사우디아라비아_축구_연맹" title="wikilink">사우디아라비아</a>)<br />
<strong>대기심:</strong><br />
<a href="https://ko.wikipedia.org/wiki/얀_페허레이프" title="wikilink">얀 페허레이프</a> (<a href="../Page/네덜란드_왕립_축구_협회.md" title="wikilink">네덜란드</a>)</p></td>
</tr>
</tbody>
</table>

## 남아프리카 공화국 vs 스페인

|          |                                    |                            |                                    |                                      |                                   |                  |               |                   |                 |                |                |          |                          |                         |                          |                          |                         |                  |               |                   |                 |                |                |
| -------- | ---------------------------------- | -------------------------- | ---------------------------------- | ------------------------------------ | --------------------------------- | ---------------- | ------------- | ----------------- | --------------- | -------------- | -------------- | -------- | ------------------------ | ----------------------- | ------------------------ | ------------------------ | ----------------------- | ---------------- | ------------- | ----------------- | --------------- | -------------- | -------------- |
| {{축구 유니폼 | pattern_la = _adidasstripesgreen | pattern_b = _rsa02_away | pattern_ra = _adidasstripesgreen | pattern_sh = _adidasstripesongreen | pattern_so = _3_stripes_green | leftarm = ffff00 | body = ffff00 | rightarm = ffff00 | shorts = ffff00 | socks = ffff00 | title = 남아공 }} | {{축구 유니폼 | pattern_la = _esp0204h | pattern_b = _esp0204h | pattern_ra = _esp0204h | pattern_sh = _esp0204h | pattern_so = _esp9496 | leftarm = e80113 | body = e80113 | rightarm = e80113 | shorts = 0000FF | socks = 003060 | title = 스페인 }} |

<table style="width:10%;">
<colgroup>
<col style="width: 5%" />
<col style="width: 0%" />
<col style="width: 5%" />
</colgroup>
<tbody>
<tr class="odd">
<td><table>
<tbody>
<tr class="odd">
<td><p>GK</p></td>
<td><p><strong>16</strong></p></td>
</tr>
<tr class="even">
<td><p>RB</p></td>
<td><p><strong>2</strong></p></td>
</tr>
<tr class="odd">
<td><p>CB</p></td>
<td><p><strong>4</strong></p></td>
</tr>
<tr class="even">
<td><p>CB</p></td>
<td><p><strong>19</strong></p></td>
</tr>
<tr class="odd">
<td><p>LB</p></td>
<td><p><strong>3</strong></p></td>
</tr>
<tr class="even">
<td><p>RM</p></td>
<td><p><strong>15</strong></p></td>
</tr>
<tr class="odd">
<td><p>CM</p></td>
<td><p><strong>6</strong></p></td>
</tr>
<tr class="even">
<td><p>CM</p></td>
<td><p><strong>12</strong></p></td>
</tr>
<tr class="odd">
<td><p>LM</p></td>
<td><p><strong>7</strong></p></td>
</tr>
<tr class="even">
<td><p>CF</p></td>
<td><p><strong>17</strong></p></td>
</tr>
<tr class="odd">
<td><p>CF</p></td>
<td><p><strong>14</strong></p></td>
</tr>
<tr class="even">
<td><p><strong>교체 선수:</strong></p></td>
<td></td>
</tr>
<tr class="odd">
<td><p>FW</p></td>
<td><p><strong>23</strong></p></td>
</tr>
<tr class="even">
<td><p>DF</p></td>
<td><p><strong>22</strong></p></td>
</tr>
<tr class="odd">
<td><p>DF</p></td>
<td><p><strong>5</strong></p></td>
</tr>
<tr class="even">
<td><p><strong>감독:</strong></p></td>
<td></td>
</tr>
<tr class="odd">
<td><p><a href="https://ko.wikipedia.org/wiki/조모_소노" title="wikilink">조모 소노</a></p></td>
<td></td>
</tr>
</tbody>
</table></td>
<td></td>
<td><table>
<tbody>
<tr class="odd">
<td><p>GK</p></td>
<td><p><strong>1</strong></p></td>
</tr>
<tr class="even">
<td><p>RB</p></td>
<td><p><strong>2</strong></p></td>
</tr>
<tr class="odd">
<td><p>CB</p></td>
<td><p><strong>4</strong></p></td>
</tr>
<tr class="even">
<td><p>CB</p></td>
<td><p><strong>20</strong></p></td>
</tr>
<tr class="odd">
<td><p>LB</p></td>
<td><p><strong>15</strong></p></td>
</tr>
<tr class="even">
<td><p>RM</p></td>
<td><p><strong>22</strong></p></td>
</tr>
<tr class="odd">
<td><p>CM</p></td>
<td><p><strong>19</strong></p></td>
</tr>
<tr class="even">
<td><p>CM</p></td>
<td><p><strong>14</strong></p></td>
</tr>
<tr class="odd">
<td><p>LM</p></td>
<td><p><strong>16</strong></p></td>
</tr>
<tr class="even">
<td><p>CF</p></td>
<td><p><strong>7</strong></p></td>
</tr>
<tr class="odd">
<td><p>CF</p></td>
<td><p><strong>9</strong></p></td>
</tr>
<tr class="even">
<td><p><strong>교체 선수:</strong></p></td>
<td></td>
</tr>
<tr class="odd">
<td><p>MF</p></td>
<td><p><strong>18</strong></p></td>
</tr>
<tr class="even">
<td><p>FW</p></td>
<td><p><strong>12</strong></p></td>
</tr>
<tr class="odd">
<td><p>MF</p></td>
<td><p><strong>21</strong></p></td>
</tr>
<tr class="even">
<td><p><strong>감독:</strong></p></td>
<td></td>
</tr>
<tr class="odd">
<td><p><a href="../Page/호세_안토니오_카마초.md" title="wikilink">호세 안토니오 카마초</a></p></td>
<td></td>
</tr>
</tbody>
</table></td>
</tr>
</tbody>
</table>

<table>
<tbody>
<tr class="odd">
<td><p><strong>최우수 선수:</strong><br />
<a href="https://ko.wikipedia.org/wiki/라울_곤살레스" title="wikilink">라울</a> (스페인)</p>
<p><strong>부심:</strong><br />
호르헤 라탈리노 (<a href="../Page/아르헨티나_축구_협회.md" title="wikilink">아르헨티나</a>)<br />
아우니 하사우네 (<a href="https://ko.wikipedia.org/wiki/요르단_축구_협회" title="wikilink">요르단</a>)<br />
<strong>대기심:</strong><br />
<a href="https://ko.wikipedia.org/wiki/모하메드_게자즈" title="wikilink">모하메드 게자즈</a> (<a href="https://ko.wikipedia.org/wiki/모로코_왕립_축구_협회" title="wikilink">모로코</a>)</p></td>
</tr>
</tbody>
</table>

## 슬로베니아 vs 파라과이

|          |               |                       |               |               |               |                  |               |                   |                 |                |                  |          |                        |                       |                        |               |               |                  |               |                   |                 |                |                 |
| -------- | ------------- | --------------------- | ------------- | ------------- | ------------- | ---------------- | ------------- | ----------------- | --------------- | -------------- | ---------------- | -------- | ---------------------- | --------------------- | ---------------------- | ------------- | ------------- | ---------------- | ------------- | ----------------- | --------------- | -------------- | --------------- |
| {{축구 유니폼 | pattern_la = | pattern_b = _slo01h | pattern_ra = | pattern_sh = | pattern_so = | leftarm = FFFFFF | body = FFFFFF | rightarm = FFFFFF | shorts = FFFFFF | socks = FFFFFF | title = 슬로베니아 }} | {{축구 유니폼 | pattern_la = _par02h | pattern_b = _par02h | pattern_ra = _par02h | pattern_sh = | pattern_so = | leftarm = FF0000 | body = FF0000 | rightarm = FF0000 | shorts = 0033CC | socks = 0033CC | title = 파라과이 }} |

<table style="width:10%;">
<colgroup>
<col style="width: 5%" />
<col style="width: 0%" />
<col style="width: 5%" />
</colgroup>
<tbody>
<tr class="odd">
<td><table>
<tbody>
<tr class="odd">
<td><p>GK</p></td>
<td><p><strong>12</strong></p></td>
</tr>
<tr class="even">
<td><p>CB</p></td>
<td><p><strong>23</strong></p></td>
</tr>
<tr class="odd">
<td><p>CB</p></td>
<td><p><strong>3</strong></p></td>
</tr>
<tr class="even">
<td><p>CB</p></td>
<td><p><strong>15</strong></p></td>
</tr>
<tr class="odd">
<td><p>RM</p></td>
<td><p><strong>7</strong></p></td>
</tr>
<tr class="even">
<td><p>CM</p></td>
<td><p><strong>8</strong></p></td>
</tr>
<tr class="odd">
<td><p>CM</p></td>
<td><p><strong>11</strong></p></td>
</tr>
<tr class="even">
<td><p>LM</p></td>
<td><p><strong>19</strong></p></td>
</tr>
<tr class="odd">
<td><p>AM</p></td>
<td><p><strong>18</strong></p></td>
</tr>
<tr class="even">
<td><p>CF</p></td>
<td><p><strong>21</strong></p></td>
</tr>
<tr class="odd">
<td><p>CF</p></td>
<td><p><strong>9</strong></p></td>
</tr>
<tr class="even">
<td><p><strong>교체 선수:</strong></p></td>
<td></td>
</tr>
<tr class="odd">
<td><p>FW</p></td>
<td><p><strong>13</strong></p></td>
</tr>
<tr class="even">
<td><p>MF</p></td>
<td><p><strong>20</strong></p></td>
</tr>
<tr class="odd">
<td><p>FW</p></td>
<td><p><strong>16</strong></p></td>
</tr>
<tr class="even">
<td><p><strong>감독:</strong></p></td>
<td></td>
</tr>
<tr class="odd">
<td><p><a href="https://ko.wikipedia.org/wiki/스레치코_카타네츠" title="wikilink">스레치코 카타네츠</a></p></td>
<td></td>
</tr>
</tbody>
</table></td>
<td></td>
<td><table>
<tbody>
<tr class="odd">
<td><p>GK</p></td>
<td><p><strong>1</strong></p></td>
</tr>
<tr class="even">
<td><p>CB</p></td>
<td><p><strong>18</strong></p></td>
</tr>
<tr class="odd">
<td><p>CB</p></td>
<td><p><strong>5</strong></p></td>
</tr>
<tr class="even">
<td><p>CB</p></td>
<td><p><strong>4</strong></p></td>
</tr>
<tr class="odd">
<td><p>RWB</p></td>
<td><p><strong>2</strong></p></td>
</tr>
<tr class="even">
<td><p>LWB</p></td>
<td><p><strong>21</strong></p></td>
</tr>
<tr class="odd">
<td><p>CM</p></td>
<td><p><strong>13</strong></p></td>
</tr>
<tr class="even">
<td><p>CM</p></td>
<td><p><strong>10</strong></p></td>
</tr>
<tr class="odd">
<td><p>CM</p></td>
<td><p><strong>8</strong></p></td>
</tr>
<tr class="even">
<td><p>CF</p></td>
<td><p><strong>20</strong></p></td>
</tr>
<tr class="odd">
<td><p>CF</p></td>
<td><p><strong>9</strong></p></td>
</tr>
<tr class="even">
<td><p><strong>교체 선수:</strong></p></td>
<td></td>
</tr>
<tr class="odd">
<td><p>MF</p></td>
<td><p><strong>11</strong></p></td>
</tr>
<tr class="even">
<td><p>FW</p></td>
<td><p><strong>23</strong></p></td>
</tr>
<tr class="odd">
<td><p>MF</p></td>
<td><p><strong>17</strong></p></td>
</tr>
<tr class="even">
<td><p><strong>감독:</strong></p></td>
<td></td>
</tr>
<tr class="odd">
<td><p><a href="../Page/체사레_말디니.md" title="wikilink">체사레 말디니</a></p></td>
<td></td>
</tr>
</tbody>
</table></td>
</tr>
</tbody>
</table>

<table>
<tbody>
<tr class="odd">
<td><p><strong>최우수 선수:</strong><br />
<a href="../Page/넬손_쿠에바스.md" title="wikilink">넬손 쿠에바스</a> (파라과이)</p>
<p><strong>부심:</strong><br />
레이프 린베리 (<a href="https://ko.wikipedia.org/wiki/스웨덴_축구_협회" title="wikilink">스웨덴</a>)<br />
비스바 크리시난 (<a href="https://ko.wikipedia.org/wiki/싱가포르_축구_협회" title="wikilink">싱가포르</a>)<br />
<strong>대기심:</strong><br />
<a href="../Page/김영주_(축구인).md" title="wikilink">김영주</a> (<a href="../Page/대한축구협회.md" title="wikilink">대한민국</a>)</p></td>
</tr>
</tbody>
</table>

## 외부 링크

  - [rsssf 기록](http://www.rsssf.com/tables/2002full.html#grb)

[B조](https://ko.wikipedia.org/wiki/분류:2002년_FIFA_월드컵 "wikilink") [분류:스페인 축구 2001-02 시즌](https://ko.wikipedia.org/wiki/분류:스페인_축구_2001-02_시즌 "wikilink") [분류:슬로베니아 축구 2001-02 시즌](https://ko.wikipedia.org/wiki/분류:슬로베니아_축구_2001-02_시즌 "wikilink") [분류:남아프리카 공화국 축구 2001-02 시즌](https://ko.wikipedia.org/wiki/분류:남아프리카_공화국_축구_2001-02_시즌 "wikilink") [분류:파라과이 축구 2002년](https://ko.wikipedia.org/wiki/분류:파라과이_축구_2002년 "wikilink")