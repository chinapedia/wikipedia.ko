> This article is converted from Wikipedia: [다오 \(프로그래밍 언어\)](https://ko.wikipedia.org/wiki/다오_\(프로그래밍_언어\)).


**다오**()는 [객체 지향](https://ko.wikipedia.org/wiki/객체_지향 "wikilink") [스크립트 언어이다](../Page/스크립트_언어.md "wikilink"). 복잡합 [자료 구조를](../Page/자료_구조.md "wikilink") 지원하는 동적 타이핑 변수(dynamically typed variables)를 지원한다. [정규 표현식](https://ko.wikipedia.org/wiki/정규_표현식 "wikilink") 매칭 같은 텍스트 처리 기능이 들어가 있다. 복소수, 다차원 배열 및 그것에 맞는 연산 등 여러 다양한 자료형을 언어 자체에서 지원한다. 다오는 또한 자체적으로 [멀티 스레드](https://ko.wikipedia.org/wiki/멀티_스레드 "wikilink") 프로그래밍을 지원한다.

다오 인터프린터는 가볍고 효율적인 가상 레지스터(register) 머신 (Dao VM) 형태로 구현되어 있으며, [C로](../Page/C_\(프로그래밍_언어\).md "wikilink") 구현되어 있다. 간단하고 평이한 인터페이스를 통해 C 혹은 C++ 과 같이 확장되어 사용될 수 있다.
...
기본적으로, 변수는 변수를 선언하는 데 쓰인 익스프레션에서 다오 인터프린터가 유추해낸 형을 갖게 된다. 사람들은 이러한 타이핑을 암묵적 타이핑(implicit typing)이라고 부른다. 프로그래머는 변수의 형을 명시적으로 정의할 수 있는데, 다음과 같이 [파스칼 (프로그래밍 언어)](../Page/파스칼_\(프로그래밍_언어\).md "wikilink") 비슷한 문법을 쓰면 된다.
