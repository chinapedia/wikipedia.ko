> This article is converted from Wikipedia: [모듈:Sports table](https://ko.wikipedia.org/wiki/모듈:Sports_table).
...
`       gray1='#DFDFDF', black1='#BBBBBB', black2='#CCCCCC', black3='#DDDDDD', black4='#EEEEEE'}`
`   `
`   -- Declare results column header`
`   local results_header = {}`
`   results_header = {Q='통과', QR='참가 자격 및 강등',`
`       P='승격', PQR='승격, 참가 자격 및 강등 ',`
`       PR='승격 및 강등', R='강등'}`
...
`       pos_label = '`<abbr title="순위">`순위`</abbr>`'`
`   end`
...
`               text_field_result = '| style="font-size:92.5%;'..ii_fw..bg_col..'" rowspan="'..tostring(N_rows_res)..'" |'..text_result..note_res_string..'\n'`
...
`   `
`   -- Get info for footer`
`   local update = Args['update']           or '알려지지 않은 날짜'`
`   local start_date = Args['start_date']   or '알려지지 않은 날짜'`
`   local source = Args['source']           or frame:expandTemplate{ title = 'citation needed', args = { reason='No source parameter defined', ['날짜']=os.date('%Y-%m-%d') } }`
`   local class_rules = Args['class_rules'] or nil`
`   `
`   -- Create footer text`
`   -- Date updating`
`   local matches_text = Args['matches_text'] or '경기'`
`   if string.lower(update)=='complete' then`
`       -- Do nothing`
`   elseif update=='' then`
`       -- Empty parameter`
`       table.insert(t_footer,''..matches_text..' 결과 갱신 날짜가 불확실하다. ')`
`   elseif string.lower(update)=='future' then`
`       -- Future start date`
`       table.insert(t_footer,'첫 '..matches_text..'는 '..start_date..'부터 시작될 예정이다. ')`
`   else`
`       table.insert(t_footer,''..update..'까지의 '..matches_text..' 결과가 갱신되었다. ')`
`   end`
`   `
`   -- Stack footer or not`
