> This article is converted from Wikipedia: [코드 인젝션](https://ko.wikipedia.org/wiki/코드_인젝션).


**코드 인젝션**()은 유효하지 않은 데이터를 실행함으로써 야기되는 [소프트웨어 버그의](../Page/소프트웨어_버그.md "wikilink") 부당한 사용이다. 삽입은 공격자에 의해서 취약한 컴퓨터 프로그램 코드를 삽입하고 실행을 변경하는 방식으로 이용된다. 성공적인 코드 인젝션은 이것을 이용한 [웜](../Page/웜.md "wikilink")의 전파 같이 종종 처참한 결과를 보여준다.
...
코드 인젝션은 해킹이나 크래킹에서 정보를 얻거나 [권한 확대](../Page/권한_확대.md "wikilink") 또는 비인가 접근을 위한 인기있는 개념이다. 코드 인젝션은 아래와 같은 많은 악의적인 목적으로 사용될 수 있다.
  ...
  - [SQL 삽입은](../Page/SQL_삽입.md "wikilink") 데이터베이스에 독단적으로 수정된 값을 코드 인젝션하는 것이다. 이 결과는 [홈페이지 변조부터](https://ko.wikipedia.org/wiki/홈페이지_변조 "wikilink") 심각한 민감한 정보의 손상까지 다양하다.
  - 서버에 서버 스크립트 코드(PHP나 ASP 같은)를 삽입하여 [악성 소프트웨어를](../Page/악성_소프트웨어.md "wikilink") 설치하거나 악의적인 코드를 실행한다.
  ...
  - HTML/Script 삽입을 통해 웹 사용자를 공격한다.([사이트 간 스크립팅](../Page/사이트_간_스크립팅.md "wikilink")(XSS)).
...
코드 인젝션 문제를 예방하기 위하여, 아래와 같은 [안전한 입출력 처리를](../Page/안전한_입출력_처리.md "wikilink") 활용한다.
  ...
  - 정적 [자료형 체계를](../Page/자료형_체계.md "wikilink") 통한 언어 분리 강화\[2\]
  - 알려진 좋은 값만 받는 [화이트리스트](https://ko.wikipedia.org/wiki/화이트리스트 "wikilink") 같은 입력 확인
  - 위험한 문자들로부터 자유롭기 위한 입력 인코딩
  - [HTML 인젝션 (XSS)](../Page/사이트_간_스크립팅.md "wikilink") 공격을 예방하기 위한 출력 인코딩
  - 커널에서의 모듈과 셸 분리
...
위에 목록화된 해결법들은 주로 웹 관련된 것들을 다룬다. 그러나 사용자 머신에의 사용자 코드 인젝션 같은 접근들도 생각해봐야 한다. 관리되거나 관리되고 있지 않은 코드 인젝션을 탐지하고 분리하는 접근으로는 아래와 같다.
  ...
  - [NX 비트](../Page/NX_비트.md "wikilink") - 모든 유저 데이터는 실행 불가능한 특별한 섹션에 저장된다. 프로세서는 이 메모리에 코드가 없게 확인하고 실행이 불가능하도록 한다.
...
[PHP](../Page/PHP.md "wikilink")는 [객체](https://ko.wikipedia.org/wiki/객체 "wikilink")의 [직렬화](../Page/직렬화.md "wikilink")와 [역직렬화](https://ko.wikipedia.org/wiki/역직렬화 "wikilink")를 허용한다. 만약 신뢰되지 않은 입력이 역질렬화 함수에 허용된다면, 악의적인 공격이 가능해진다.\[4\]
...
| [Pipelines](../Page/파이프_\(유닉스\).md "wikilink") | `\| malicious_command`    | `/bin/funnytext \| malicious_command`    | Sends the output of `funnytext` as input to `malicious_command`.                    |
...
## 외부 링크
  ...
  - News article "[Flux spreads wider](https://web.archive.org/web/20050924080540/http://www.emsisoft.com/en/kb/articles/news041104/)" - First [Trojan horse](../Page/트로이_목마_\(컴퓨팅\).md "wikilink") to make use of code injection to prevent detection from a [firewall](../Page/방화벽_\(네트워킹\).md "wikilink")
