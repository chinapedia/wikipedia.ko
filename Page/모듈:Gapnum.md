> This article is converted from Wikipedia: [모듈:Gapnum](https://ko.wikipedia.org/wiki/모듈:Gapnum).
...
\-- Not named p._main so that it has a better function name when required by Module:Val function p.gaps(n,tbl)
...
\-- Creates tables where each element is a different group of the number function p.groups(num,precision)
