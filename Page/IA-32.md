...
처음에 [인텔 80386은](../Page/인텔_80386.md "wikilink") 32비트 일반 레지스터 8개와 [부동 소수점](https://ko.wikipedia.org/wiki/부동_소수점 "wikilink") 실수 레지스터 8개를 지원했다. 뒤에 나온 프로세서들은 [MMX](../Page/MMX.md "wikilink"), [3DNow\!](https://ko.wikipedia.org/wiki/3DNow! "wikilink"), [SSE](../Page/스트리밍_SIMD_확장.md "wikilink"), [SSE2](../Page/SSE2.md "wikilink"), [SSE3](../Page/SSE3.md "wikilink")과 같은 다양한 [SIMD](https://ko.wikipedia.org/wiki/SIMD "wikilink") 명령 집합들을 위한 레지스터들을 더 지원한다.
...
IA-32는 원래 [8087](https://ko.wikipedia.org/wiki/인텔_8087 "wikilink"), [80287](https://ko.wikipedia.org/wiki/인텔_80287 "wikilink"), [80387](https://ko.wikipedia.org/wiki/인텔_80387 "wikilink") 같은 별도의 프로세서로 부동 소수점 연산을 지원했다. [80486](../Page/인텔_80486.md "wikilink") 이후로 부동 소수점 연산이 프로세서에 통합되었으며 ST(0)부터 ST(7)까지의 8개의 부동 소수점 레지스터도 일반적으로 사용 가능하게 되었다. 이 레지스터들은 80비트이며 [IEEE 754의](../Page/IEEE_754.md "wikilink") 확장 배정도 형식을 사용한다.

부동 소수점 레지스터는 직접 접근할 수 없으며 [스택](../Page/스택.md "wikilink")을 사용하여 접근한다. 레지스터 번호는 고정되어 있지 않으며 스택 꼭대기에 상대적이다. 즉 ST(0)은 스택 꼭대기에 있는 값이고, ST(1)은 그 아래에 있는 값이다. 모든 부동 소수점 연산은 스택에서 몇 개의 값을 꺼내서 연산한 뒤 다시 스택에 집어 넣는 방식으로 이루어진다.

IA-32의 확장인 [MMX](../Page/MMX.md "wikilink")와 [3DNow\!](https://ko.wikipedia.org/wiki/3DNow! "wikilink")에서는 부동 소수점 레지스터를 다른 용도로 사용한다. 따라서 이들 기능과 부동 소수점 연산을 동시에 수행할 수 없다는 단점이 있지만, 한편으로는 별도의 레지스터를 추가하지 않으므로 레지스터 상태 등을 저장해야 하는 경우 이전과 같은 방법을 사용할 수 있다는 장점이 있다.

#### MMX 레지스터

MMX 확장에서는 MM0부터 MM7까지 8개의 레지스터가 추가되었다. 실제로 MMX 레지스터는 이미 존재하던 부동 소수점 레지스터들의 하위 비트들과 동일하며, 스택을 사용하지 않으므로 레지스터 번호가 상대적이지 않다는 차이점만 있다. MMX 레지스터는 64비트지만, 하나의 레지스터를 명령에 따라 각각 8비트, 16비트, 32비트, 64비트씩 해석하는 압축 자료형(packed data type)을 제공한다.

MMX 레지스터와 부동 소수점 레지스터가 실제로는 동일하기 때문에 둘을 함께 사용할 수는 없다. MMX 레지스터가 사용될 경우 상위 16비트는 모두 1로 채워지며, 이는 부동 소수점 형식으로는 [NaN](https://ko.wikipedia.org/wiki/NaN "wikilink") 또는 [무한대](https://ko.wikipedia.org/wiki/무한대 "wikilink")로 해석되기 때문에 현재 처리하는 데이터가 부동 소수점 데이터인지 MMX 데이터인지 판단하기 쉬워진다.

#### 3DNow\! 레지스터

3DNow\! 확장은 MMX를 정수에서 부동 소수점 실수로 확장한 것이다. 이 확장에서는 MMX와 똑같이 MM0부터 MM7까지의 64비트 레지스터를 사용하며, 단지 64비트 레지스터를 두 개의 32비트 단정도 부동 소수점 형식으로 취급하는 것만 다르다.

### SSE 레지스터

MMX와 3DNow\!는 부동 소수점 레지스터를 공유했기 때문에 크기에 제한이 있었다. [SSE](../Page/스트리밍_SIMD_확장.md "wikilink") 확장은 부동 소수점 레지스터를 더 이상 사용하지 않고, XMM0부터 XMM7까지의 128비트 레지스터 8개를 새로 추가했다. ([AMD64](https://ko.wikipedia.org/wiki/AMD64 "wikilink")에서는 XMM0부터 XMM15까지 16개의 레지스터를 지원한다.) SSE 확장이 등장하면서 인텔은 새 레지스터들의 상태를 저장하기 위한 명령들을 함께 추가했으며, 호환성을 위해 보호 모드와 같지만 SSE 관련 명령을 지원하는 강화 모드(enhanced mode)도 만들었다.

SSE는 3DNow\!와 같이 부동 소숫점을 다루는 확장이지만 레지스터 크기가 두 배기 때문에 단정도(32비트) 부동 소수점 실수 4개를 한꺼번에 다룰 수 있다. 원래 SSE는 배정도(64비트) 부동 소수점 실수를 다룰 수 없었지만 [SSE2](../Page/SSE2.md "wikilink") 확장에서는 배정도 부동 소수점 실수 두 개를 한꺼번에 다루는 명령들이 추가되었다.

## 64비트 확장

IA-32는 멀티미디어 관련 명령들을 추가하며 발전해 왔지만, 여전히 32비트 환경만을 지원한다. IA-32를 64비트 환경에서 쓸 수 있도록 한 확장으로는 인텔의 [IA-64](https://ko.wikipedia.org/wiki/IA-64 "wikilink")와 [AMD의](https://ko.wikipedia.org/wiki/어드밴스트_마이크로_디바이시스 "wikilink") [AMD64](https://ko.wikipedia.org/wiki/AMD64 "wikilink")가 있으며, [마이크로소프트](../Page/마이크로소프트.md "wikilink")는 업체 중립적인 IA-32의 64비트 확장의 이름으로 [x64](https://ko.wikipedia.org/wiki/x64 "wikilink")를 제안해서 사용하고 있다.
