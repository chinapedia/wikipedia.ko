...
`   page = page or ''`
`   return string.format('`[`%s`](https://ko.wikipedia.org/wiki/%s#%s "wikilink")`', page, section, display)`
...
`       if k == 1 then`
`           -- Doing this in the loop because of a bug in `[`Module:Arguments`](https://ko.wikipedia.org/wiki/Module:Arguments "wikilink")
