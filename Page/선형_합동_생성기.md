...
매우 나쁜 난수열을 생성해 내는 것으로 알려진 [RANDU](https://ko.wikipedia.org/wiki/RANDU "wikilink") 루틴은 다음 수식을 사용한다.

  -
    \(X_{n+1} = (65539 X_n) \mod 2^{31}\)

일반적으로 컴퓨터에서는 [나눗셈](https://ko.wikipedia.org/wiki/나눗셈 "wikilink") 및 [나머지](https://ko.wikipedia.org/wiki/나머지 "wikilink") 연산이 매우 느리기 때문에 \(m\)이 \(2^{16}\)이나 \(2^{32}\) 같이 정수의 크기에 맞는 나눔수를 흔히 사용한다. 자주 사용되는 다음 생성기는 그렇지 않은 한 예이다.

  -
    \(X_{n+1} = (279470273 X_n) \mod (2^{32} - 5)\)

위 생성기는 [C99](https://ko.wikipedia.org/wiki/C99 "wikilink") 코드로 다음과 같이 표현할 수 있다.
...
특정한 경우 선형 합동 생성기의 사용이 사실상 불가능할 수도 있다. 만약 선형 합동 생성기가 만들어 내는 숫자를 \(n\)개씩 짝을 지어 \(n\)차원 공간 상에 점을 찍으면, [마서글리아 효과에](https://ko.wikipedia.org/wiki/마서글리아_효과 "wikilink") 따라 그 점들은 최대 \(m^{\frac1n}\)개의 [초평면](https://ko.wikipedia.org/wiki/초평면 "wikilink") 상에 위치하게 된다. 또한 \(m\)이 [2의 거듭제곱일](https://ko.wikipedia.org/wiki/2의_거듭제곱 "wikilink") 경우 생성된 난수열의 하위 비트들은 상위 비트들에 비해 훨씬 강한 상관관계를 나타낸다. (예를 들어 최하위 비트는 0, 1, 0, 1 순으로 반복될 것이다.) 때문에 많은 선형 합동 생성기 구현에서는 하위 비트를 버리고 상위 비트만을 반환하곤 한다
...
## 참조
  ...
  - [D. E. Knuth](https://ko.wikipedia.org/wiki/도널드_커누스 "wikilink"). *[The Art of Computer Programming](https://ko.wikipedia.org/wiki/컴퓨터_프로그래밍의_예술 "wikilink")*, Volume 2: *Seminumerical Algorithms*, Third Edition. Addison-Wesley, 1997. . Section 3.2.1: The Linear Congruential Method, pp.10–26.

## 같이 보기

  - [역 합동 생성기](https://ko.wikipedia.org/wiki/역_합동_생성기 "wikilink")
