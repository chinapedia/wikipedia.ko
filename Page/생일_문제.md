...
[thumb](https://ko.wikipedia.org/wiki/파일:Birthday_Paradox.svg "wikilink") **생일 문제**()는 사람이 임의로 모였을 때 그 중에 생일이 같은 두 명이 존재할 [확률](https://ko.wikipedia.org/wiki/확률 "wikilink")을 구하는 문제이다. 생일의 가능한 가짓수는 366개이므로 366명 이상의 사람이 모인다면 [비둘기집 원리에](../Page/비둘기집_원리.md "wikilink") 따라 생일이 같은 두 명이 반드시 존재하며, 23명 이상이 모인다면 그 중 두 명이 생일이 같은 확률은 1/2를 넘는다.
