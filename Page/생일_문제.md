...
[thumb](https://ko.wikipedia.org/wiki/파일:Birthday_Paradox.svg "wikilink") **생일 문제**()는 사람이 임의로 모였을 때 그 중에 생일이 같은 두 명이 존재할 [확률](https://ko.wikipedia.org/wiki/확률 "wikilink")을 구하는 문제이다. 생일의 가능한 가짓수는 365개([2월 29일을](https://ko.wikipedia.org/wiki/2월_29일 "wikilink") 고려할 경우 366개)이므로 366명 이상의 사람이 모인다면 [비둘기집 원리에](https://ko.wikipedia.org/wiki/비둘기집_원리 "wikilink") 따라 생일이 같은 두 명이 반드시 존재하며, 23명 이상이 모인다면 그 중 두 명이 생일이 같은 확률은 1/2를 넘는다.
...
생일이 같은 두 사람을 찾는 것과 비슷하게, [암호학적 해시 결과가](https://ko.wikipedia.org/wiki/암호학적_해시_함수 "wikilink") 같은([해시 충돌](https://ko.wikipedia.org/wiki/해시_충돌 "wikilink")) 두 입력값을 찾는 것 역시 모든 입력값을 계산하지 않아도 충분히 높은 확률로 해시 충돌을 찾을 수 있다. 이러한 암호 공격을 [생일 공격](https://ko.wikipedia.org/wiki/생일_공격 "wikilink")(birthday attack)이라고 부른다.

## 확률 계산

만약 366명 이상의 사람이 있다면 [비둘기집 원리에](https://ko.wikipedia.org/wiki/비둘기집_원리 "wikilink") 따라 생일이 같은 두 사람이 존재해야 한다. 365명 이하의 사람이 있을 경우를 계산한다. \(n\)명의 사람이 있을 때 그 중 생일이 같은 사람이 둘 이상 있을 확률을 \(p(n)\)이라고 한다면, 반대로 모든 사람의 생일이 다를 확률 \(\bar p(n)\)은 \(1-p(n)\)이 된다. 먼저 \(\bar p(n)\)을 구해보면, 두 번째 사람의 생일은 첫 번째 사람과 다르고, 세 번째 사람의 생일은 첫 번째와 두 번째 모두와 달라야 하므로 다음과 같은 식을 얻을 수 있다.
...
\[\begin{align} p(n) &= 1 - { 365! \over 365^n (365-n)!} \end{align}\] 가 된다. 여기서, n≤365 인 자연수이고, \!는 [계승](https://ko.wikipedia.org/wiki/계승 "wikilink")을 의미한다.
