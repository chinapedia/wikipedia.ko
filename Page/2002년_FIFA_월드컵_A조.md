> This article is converted from Wikipedia: [2002 FIFA  A](https://ko.wikipedia.org/wiki/2002_FIFA__A).


[2002년 FIFA 월드컵은](https://ko.wikipedia.org/wiki/2002년_FIFA_월드컵 "wikilink") 32팀이 본선에 참가하였고 이들은 4팀씩 8개의 조로 나뉘었다. **[2002년 FIFA 월드컵](https://ko.wikipedia.org/wiki/2002년_FIFA_월드컵 "wikilink") A조**는 [1998년 FIFA 월드컵](https://ko.wikipedia.org/wiki/1998년_FIFA_월드컵 "wikilink") 우승국 [프랑스](https://ko.wikipedia.org/wiki/프랑스_축구_국가대표팀 "wikilink"), 2회 우승국 [우루과이](../Page/우루과이_축구_국가대표팀.md "wikilink"), [덴마크](../Page/덴마크_축구_국가대표팀.md "wikilink"), 그리고 [FIFA 월드컵](https://ko.wikipedia.org/wiki/FIFA_월드컵 "wikilink") 처녀 출전국 [세네갈이](../Page/세네갈_축구_국가대표팀.md "wikilink") 편성되었다. A조의 첫 경기는 2002년 5월 31일에 치러졌고, 최종전은 2002년 6월 11일에 거행되었다.

## 요약

A조는 시작부터 뜻밖의 결과들이 속출하였는데, 세네갈은 대회 개막전부터 프랑스를 1-0으로 쓰러뜨렸다. 프랑스는 이어지는 우루과이와의 경기에서 주축 스트라이커 [티에리 앙리가](../Page/티에리_앙리.md "wikilink") 퇴장당한 후 비겼고, 덴마크와의 최종전에서 또다시 패하여 탈락하였다. 세네갈은 남은 두번의 조별 리그 경기에서 2무를 추가하였는데, 덴마크전에서 선방하여 1점을 추가하였고, 12명이 경고를 받은 우루과이와의 격렬한 경기에서도 비겼다.

덴마크는 [울산에서](../Page/울산문수축구경기장.md "wikilink") 열린 우루과이전에서 첫 승리를 거두며 안정적으로 시작하였으나, [대구월드컵경기장에서](https://ko.wikipedia.org/wiki/대구_스타디움 "wikilink") 열린 세네갈과의 2차전에서 동점골을 허용해 2경기만에 16강 진출을 확정짓지 못하였다. 그 대신, 이들은 프랑스와의 최종전에서 비기기만 해도 다음 라운드 진출이 가능했다. 덴마크는 이 경기에서 2-0 완승을 거두었고, 세네갈과 우루과이가 비김에 따라 조 1위를 차지했다.

A조의 다크호스 우루과이도 기대 이하의 프랑스를 상대로 득점 없이 비기는 것 외에는 별다른 성과를 거두지 못했다. 그러나, 이들은 세네갈과의 다득점 무승부 경기에서 3골을 집어넣는데 성공했으나, 1차전에서 덴마크에 패한 것으로 발목이 잡혔고, 이는 16강 진출에 대한 희망의 불씨를 꺼뜨렸다.

## 순위

|}

  - *모든 시간은 [대한민국의](../Page/한국_표준시.md "wikilink") 동아시아 표준시를 따른다.* ([EAT/UTC+9](https://ko.wikipedia.org/wiki/UTC+9 "wikilink"))

## 프랑스 vs 세네갈

|          |                             |                            |                             |                             |                                   |                  |               |                   |                 |                |             |          |                             |                      |                             |                                 |               |                  |               |                   |                 |                |             |
| -------- | --------------------------- | -------------------------- | --------------------------- | --------------------------- | --------------------------------- | ---------------- | ------------- | ----------------- | --------------- | -------------- | ----------- | -------- | --------------------------- | -------------------- | --------------------------- | ------------------------------- | ------------- | ---------------- | ------------- | ----------------- | --------------- | -------------- | ----------- |
| {{축구 유니폼 | pattern_la = _france2002h | pattern_b = _france2002h | pattern_ra = _france2002h | pattern_sh = _france2002a | pattern_so = _3_stripes_white | leftarm = 0038D3 | body = 0038D3 | rightarm = 0038D3 | shorts = 0038D3 | socks = FF0000 | 이름 = 프랑스 }} | {{축구 유니폼 | pattern_la = _senegal2002 | pattern_b = _sen02 | pattern_ra = _senegal2002 | pattern_sh = _yellow_stripes | pattern_so = | leftarm = FFFFFF | body = FFFFFF | rightarm = FFFFFF | shorts = 008000 | socks = FFFFFF | 이름 = 세네갈 }} |

<table style="width:10%;">
<colgroup>
<col style="width: 5%" />
<col style="width: 0%" />
<col style="width: 5%" />
</colgroup>
<tbody>
<tr class="odd">
<td><table>
<tbody>
<tr class="odd">
<td><p>GK</p></td>
<td><p><strong>16</strong></p></td>
</tr>
<tr class="even">
<td><p>RB</p></td>
<td><p><strong>15</strong></p></td>
</tr>
<tr class="odd">
<td><p>CB</p></td>
<td><p><strong>18</strong></p></td>
</tr>
<tr class="even">
<td><p>CB</p></td>
<td><p><strong>8</strong></p></td>
</tr>
<tr class="odd">
<td><p>LB</p></td>
<td><p><strong>3</strong></p></td>
</tr>
<tr class="even">
<td><p>CM</p></td>
<td><p><strong>4</strong></p></td>
</tr>
<tr class="odd">
<td><p>CM</p></td>
<td><p><strong>17</strong></p></td>
</tr>
<tr class="even">
<td><p>RW</p></td>
<td><p><strong>11</strong></p></td>
</tr>
<tr class="odd">
<td><p>AM</p></td>
<td><p><strong>6</strong></p></td>
</tr>
<tr class="even">
<td><p>LW</p></td>
<td><p><strong>12</strong></p></td>
</tr>
<tr class="odd">
<td><p>CF</p></td>
<td><p><strong>20</strong></p></td>
</tr>
<tr class="even">
<td><p><strong>교체 선수:</strong></p></td>
<td></td>
</tr>
<tr class="odd">
<td><p>FW</p></td>
<td><p><strong>21</strong></p></td>
</tr>
<tr class="even">
<td><p>FW</p></td>
<td><p><strong>9</strong></p></td>
</tr>
<tr class="odd">
<td><p><strong>감독:</strong></p></td>
<td></td>
</tr>
<tr class="even">
<td><p><a href="../Page/로제_르메르.md" title="wikilink">로제 르메르</a></p></td>
<td></td>
</tr>
</tbody>
</table></td>
<td></td>
<td><table>
<tbody>
<tr class="odd">
<td><p>GK</p></td>
<td><p><strong>1</strong></p></td>
</tr>
<tr class="even">
<td><p>RB</p></td>
<td><p><strong>17</strong></p></td>
</tr>
<tr class="odd">
<td><p>CB</p></td>
<td><p><strong>13</strong></p></td>
</tr>
<tr class="even">
<td><p>CB</p></td>
<td><p><strong>4</strong></p></td>
</tr>
<tr class="odd">
<td><p>LB</p></td>
<td><p><strong>2</strong></p></td>
</tr>
<tr class="even">
<td><p>DM</p></td>
<td><p><strong>6</strong></p></td>
</tr>
<tr class="odd">
<td><p>RM</p></td>
<td><p><strong>14</strong></p></td>
</tr>
<tr class="even">
<td><p>CM</p></td>
<td><p><strong>19</strong></p></td>
</tr>
<tr class="odd">
<td><p>CM</p></td>
<td><p><strong>15</strong></p></td>
</tr>
<tr class="even">
<td><p>LM</p></td>
<td><p><strong>10</strong></p></td>
</tr>
<tr class="odd">
<td><p>CF</p></td>
<td><p><strong>11</strong></p></td>
</tr>
<tr class="even">
<td><p><strong>감독:</strong></p></td>
<td></td>
</tr>
<tr class="odd">
<td><p><a href="https://ko.wikipedia.org/wiki/브뤼노_메추" title="wikilink">브뤼노 메추</a></p></td>
<td></td>
</tr>
</tbody>
</table></td>
</tr>
</tbody>
</table>

<table>
<tbody>
<tr class="odd">
<td><p><strong>최우수 선수:</strong><br />
<a href="../Page/엘_하지_디우프.md" title="wikilink">엘 하지 디우프</a> (세네갈)</p>
<p><strong>부심:</strong><br />
알리 알-트라이피 (<a href="https://ko.wikipedia.org/wiki/사우디아라비아_축구_연맹" title="wikilink">사우디아라비아</a>)<br />
호르헤 라탈리노 (<a href="../Page/아르헨티나_축구_협회.md" title="wikilink">아르헨티나</a>)<br />
<strong>대기심:</strong><br />
<a href="https://ko.wikipedia.org/wiki/펠리페_라모스" title="wikilink">펠리페 라모스</a> (<a href="https://ko.wikipedia.org/wiki/멕시코_축구_협회" title="wikilink">멕시코</a>)</p></td>
</tr>
</tbody>
</table>

## 우루과이 vs 덴마크

|          |               |                          |               |               |               |                  |               |                   |                 |                |                 |          |                        |                       |                        |                         |               |                  |               |                   |                 |                |                |
| -------- | ------------- | ------------------------ | ------------- | ------------- | ------------- | ---------------- | ------------- | ----------------- | --------------- | -------------- | --------------- | -------- | ---------------------- | --------------------- | ---------------------- | ----------------------- | ------------- | ---------------- | ------------- | ----------------- | --------------- | -------------- | -------------- |
| {{축구 유니폼 | pattern_la = | pattern_b = _old laced | pattern_ra = | pattern_sh = | pattern_so = | leftarm = AACCFF | body = AACCFF | rightarm = AACCFF | shorts = 000000 | socks = 000000 | title = 우루과이 }} | {{축구 유니폼 | pattern_la = _den02h | pattern_b = _den02h | pattern_ra = _den02h | pattern_sh = _reddots | pattern_so = | leftarm = FF0000 | body = FF0000 | rightarm = FF0000 | shorts = FFFFFF | socks = FFFFFF | title = 덴마크 }} |

<table style="width:10%;">
<colgroup>
<col style="width: 5%" />
<col style="width: 0%" />
<col style="width: 5%" />
</colgroup>
<tbody>
<tr class="odd">
<td><table>
<tbody>
<tr class="odd">
<td><p>GK</p></td>
<td><p><strong>1</strong></p></td>
</tr>
<tr class="even">
<td><p>RB</p></td>
<td><p><strong>2</strong></p></td>
</tr>
<tr class="odd">
<td><p>CB</p></td>
<td><p><strong>14</strong></p></td>
</tr>
<tr class="even">
<td><p>CB</p></td>
<td><p><strong>4</strong></p></td>
</tr>
<tr class="odd">
<td><p>LB</p></td>
<td><p><strong>6</strong></p></td>
</tr>
<tr class="even">
<td><p>RM</p></td>
<td><p><strong>8</strong></p></td>
</tr>
<tr class="odd">
<td><p>CM</p></td>
<td><p><strong>5</strong></p></td>
</tr>
<tr class="even">
<td><p>LM</p></td>
<td><p><strong>7</strong></p></td>
</tr>
<tr class="odd">
<td><p>AM</p></td>
<td><p><strong>20</strong></p></td>
</tr>
<tr class="even">
<td><p>CF</p></td>
<td><p><strong>9</strong></p></td>
</tr>
<tr class="odd">
<td><p>CF</p></td>
<td><p><strong>13</strong></p></td>
</tr>
<tr class="even">
<td><p><strong>교체 선수:</strong></p></td>
<td></td>
</tr>
<tr class="odd">
<td><p>FW</p></td>
<td><p><strong>17</strong></p></td>
</tr>
<tr class="even">
<td><p>FW</p></td>
<td><p><strong>11</strong></p></td>
</tr>
<tr class="odd">
<td><p>FW</p></td>
<td><p><strong>18</strong></p></td>
</tr>
<tr class="even">
<td><p><strong>감독:</strong></p></td>
<td></td>
</tr>
<tr class="odd">
<td><p><a href="https://ko.wikipedia.org/wiki/빅토르_푸아" title="wikilink">빅토르 푸아</a></p></td>
<td></td>
</tr>
</tbody>
</table></td>
<td></td>
<td><table>
<tbody>
<tr class="odd">
<td><p>GK</p></td>
<td><p><strong>1</strong></p></td>
</tr>
<tr class="even">
<td><p>RB</p></td>
<td><p><strong>6</strong></p></td>
</tr>
<tr class="odd">
<td><p>CB</p></td>
<td><p><strong>4</strong></p></td>
</tr>
<tr class="even">
<td><p>CB</p></td>
<td><p><strong>3</strong></p></td>
</tr>
<tr class="odd">
<td><p>LB</p></td>
<td><p><strong>5</strong></p></td>
</tr>
<tr class="even">
<td><p>CM</p></td>
<td><p><strong>2</strong></p></td>
</tr>
<tr class="odd">
<td><p>CM</p></td>
<td><p><strong>7</strong></p></td>
</tr>
<tr class="even">
<td><p>RW</p></td>
<td><p><strong>19</strong></p></td>
</tr>
<tr class="odd">
<td><p>AM</p></td>
<td><p><strong>9</strong></p></td>
</tr>
<tr class="even">
<td><p>LW</p></td>
<td><p><strong>8</strong></p></td>
</tr>
<tr class="odd">
<td><p>CF</p></td>
<td><p><strong>11</strong></p></td>
</tr>
<tr class="even">
<td><p><strong>교체 선수:</strong></p></td>
<td></td>
</tr>
<tr class="odd">
<td><p>DF</p></td>
<td><p><strong>12</strong></p></td>
</tr>
<tr class="even">
<td><p>FW</p></td>
<td><p><strong>10</strong></p></td>
</tr>
<tr class="odd">
<td><p>MF</p></td>
<td><p><strong>17</strong></p></td>
</tr>
<tr class="even">
<td><p><strong>감독:</strong></p></td>
<td></td>
</tr>
<tr class="odd">
<td><p><a href="../Page/모르텐_올센.md" title="wikilink">모르텐 올센</a></p></td>
<td></td>
</tr>
</tbody>
</table></td>
</tr>
</tbody>
</table>

<table>
<tbody>
<tr class="odd">
<td><p><strong>최우수 선수:</strong><br />
<a href="../Page/욘_달_토마손.md" title="wikilink">욘 달 토마손</a> (덴마크)</p>
<p><strong>부심:</strong><br />
아우니 하사우네 (<a href="https://ko.wikipedia.org/wiki/요르단_축구_협회" title="wikilink">요르단</a>)<br />
드라만 단테 (<a href="https://ko.wikipedia.org/wiki/말리_축구_협회" title="wikilink">말리</a>)<br />
<strong>대기심:</strong><br />
<a href="../Page/비론_모레노.md" title="wikilink">비론 모레노</a> (<a href="https://ko.wikipedia.org/wiki/에콰도르_축구_협회" title="wikilink">에콰도르</a>)</p></td>
</tr>
</tbody>
</table>

## 덴마크 vs 세네갈

|          |                        |                       |                        |               |               |                  |               |                   |                 |                |                |          |                             |                      |                             |                                |               |                  |               |                   |                 |                |                |
| -------- | ---------------------- | --------------------- | ---------------------- | ------------- | ------------- | ---------------- | ------------- | ----------------- | --------------- | -------------- | -------------- | -------- | --------------------------- | -------------------- | --------------------------- | ------------------------------ | ------------- | ---------------- | ------------- | ----------------- | --------------- | -------------- | -------------- |
| {{축구 유니폼 | pattern_la = _den02h | pattern_b = _den02h | pattern_ra = _den02h | pattern_sh = | pattern_so = | leftarm = FF0000 | body = FF0000 | rightarm = FF0000 | shorts = FF0000 | socks = FF0000 | title = 덴마크 }} | {{축구 유니폼 | pattern_la = _senegal2002 | pattern_b = _sen02 | pattern_ra = _senegal2002 | pattern_sh = _green_stripes | pattern_so = | leftarm = FFFFFF | body = FFFFFF | rightarm = FFFFFF | shorts = FFFFFF | socks = FFFFFF | title = 세네갈 }} |

<table style="width:10%;">
<colgroup>
<col style="width: 5%" />
<col style="width: 0%" />
<col style="width: 5%" />
</colgroup>
<tbody>
<tr class="odd">
<td><table>
<tbody>
<tr class="odd">
<td><p>GK</p></td>
<td><p><strong>1</strong></p></td>
</tr>
<tr class="even">
<td><p>RB</p></td>
<td><p><strong>6</strong></p></td>
</tr>
<tr class="odd">
<td><p>CB</p></td>
<td><p><strong>4</strong></p></td>
</tr>
<tr class="even">
<td><p>CB</p></td>
<td><p><strong>3</strong></p></td>
</tr>
<tr class="odd">
<td><p>LB</p></td>
<td><p><strong>5</strong></p></td>
</tr>
<tr class="even">
<td><p>CM</p></td>
<td><p><strong>2</strong></p></td>
</tr>
<tr class="odd">
<td><p>CM</p></td>
<td><p><strong>7</strong></p></td>
</tr>
<tr class="even">
<td><p>RW</p></td>
<td><p><strong>19</strong></p></td>
</tr>
<tr class="odd">
<td><p>AM</p></td>
<td><p><strong>9</strong></p></td>
</tr>
<tr class="even">
<td><p>LW</p></td>
<td><p><strong>8</strong></p></td>
</tr>
<tr class="odd">
<td><p>CF</p></td>
<td><p><strong>11</strong></p></td>
</tr>
<tr class="even">
<td><p><strong>교체 선수:</strong></p></td>
<td></td>
</tr>
<tr class="odd">
<td><p>FW</p></td>
<td><p><strong>10</strong></p></td>
</tr>
<tr class="even">
<td><p>MF</p></td>
<td><p><strong>17</strong></p></td>
</tr>
<tr class="odd">
<td><p>FW</p></td>
<td><p><strong>18</strong></p></td>
</tr>
<tr class="even">
<td><p><strong>감독:</strong></p></td>
<td></td>
</tr>
<tr class="odd">
<td><p><a href="../Page/모르텐_올센.md" title="wikilink">모르텐 올센</a></p></td>
<td></td>
</tr>
</tbody>
</table></td>
<td></td>
<td><table>
<tbody>
<tr class="odd">
<td><p>GK</p></td>
<td><p><strong>1</strong></p></td>
</tr>
<tr class="even">
<td><p>RB</p></td>
<td><p><strong>17</strong></p></td>
</tr>
<tr class="odd">
<td><p>CB</p></td>
<td><p><strong>13</strong></p></td>
</tr>
<tr class="even">
<td><p>CB</p></td>
<td><p><strong>4</strong></p></td>
</tr>
<tr class="odd">
<td><p>LB</p></td>
<td><p><strong>2</strong></p></td>
</tr>
<tr class="even">
<td><p>RM</p></td>
<td><p><strong>14</strong></p></td>
</tr>
<tr class="odd">
<td><p>CM</p></td>
<td><p><strong>3</strong></p></td>
</tr>
<tr class="even">
<td><p>CM</p></td>
<td><p><strong>15</strong></p></td>
</tr>
<tr class="odd">
<td><p>CM</p></td>
<td><p><strong>19</strong></p></td>
</tr>
<tr class="even">
<td><p>LM</p></td>
<td><p><strong>10</strong></p></td>
</tr>
<tr class="odd">
<td><p>CF</p></td>
<td><p><strong>11</strong></p></td>
</tr>
<tr class="even">
<td><p><strong>교체 선수:</strong></p></td>
<td></td>
</tr>
<tr class="odd">
<td><p>FW</p></td>
<td><p><strong>7</strong></p></td>
</tr>
<tr class="even">
<td><p>FW</p></td>
<td><p><strong>9</strong></p></td>
</tr>
<tr class="odd">
<td><p>DF</p></td>
<td><p><strong>21</strong></p></td>
</tr>
<tr class="even">
<td><p><strong>감독:</strong></p></td>
<td></td>
</tr>
<tr class="odd">
<td><p><a href="https://ko.wikipedia.org/wiki/브뤼노_메추" title="wikilink">브뤼노 메추</a></p></td>
<td></td>
</tr>
</tbody>
</table></td>
</tr>
</tbody>
</table>

<table>
<tbody>
<tr class="odd">
<td><p><strong>최우수 선수:</strong><br />
<a href="../Page/칼릴루_파디가.md" title="wikilink">칼릴루 파디가</a> (세네갈)</p>
<p><strong>부심:</strong><br />
셰켈리 페렌츠 (<a href="https://ko.wikipedia.org/wiki/헝가리_축구_협회" title="wikilink">헝가리</a>)<br />
비스바 크리시난 (<a href="https://ko.wikipedia.org/wiki/싱가포르_축구_협회" title="wikilink">싱가포르</a>)<br />
<strong>대기심:</strong><br />
<a href="../Page/김영주_(축구인).md" title="wikilink">김영주</a> (<a href="../Page/대한축구협회.md" title="wikilink">대한민국</a>)</p></td>
</tr>
</tbody>
</table>

## 프랑스 vs 우루과이

|          |                             |                            |                             |                             |                             |                  |               |                   |                 |                |                |          |               |                          |               |               |               |                  |               |                   |                 |                |                 |
| -------- | --------------------------- | -------------------------- | --------------------------- | --------------------------- | --------------------------- | ---------------- | ------------- | ----------------- | --------------- | -------------- | -------------- | -------- | ------------- | ------------------------ | ------------- | ------------- | ------------- | ---------------- | ------------- | ----------------- | --------------- | -------------- | --------------- |
| {{축구 유니폼 | pattern_la = _france2002a | pattern_b = _france2002a | pattern_ra = _france2002a | pattern_sh = _france2002a | pattern_so = _francea0608 | leftarm = FFFFFF | body = FFFFFF | rightarm = FFFFFF | shorts = FFFFFF | socks = FFFFFF | title = 프랑스 }} | {{축구 유니폼 | pattern_la = | pattern_b = _old laced | pattern_ra = | pattern_sh = | pattern_so = | leftarm = AACCFF | body = AACCFF | rightarm = AACCFF | shorts = 000000 | socks = 000000 | title = 우루과이 }} |

<table style="width:10%;">
<colgroup>
<col style="width: 5%" />
<col style="width: 0%" />
<col style="width: 5%" />
</colgroup>
<tbody>
<tr class="odd">
<td><table>
<tbody>
<tr class="odd">
<td><p>GK</p></td>
<td><p><strong>16</strong></p></td>
</tr>
<tr class="even">
<td><p>RB</p></td>
<td><p><strong>15</strong></p></td>
</tr>
<tr class="odd">
<td><p>CB</p></td>
<td><p><strong>18</strong></p></td>
</tr>
<tr class="even">
<td><p>CB</p></td>
<td><p><strong>8</strong></p></td>
</tr>
<tr class="odd">
<td><p>LB</p></td>
<td><p><strong>3</strong></p></td>
</tr>
<tr class="even">
<td><p>CM</p></td>
<td><p><strong>4</strong></p></td>
</tr>
<tr class="odd">
<td><p>CM</p></td>
<td><p><strong>17</strong></p></td>
</tr>
<tr class="even">
<td><p>RW</p></td>
<td><p><strong>11</strong></p></td>
</tr>
<tr class="odd">
<td><p>AM</p></td>
<td><p><strong>22</strong></p></td>
</tr>
<tr class="even">
<td><p>LW</p></td>
<td><p><strong>12</strong></p></td>
</tr>
<tr class="odd">
<td><p>CF</p></td>
<td><p><strong>20</strong></p></td>
</tr>
<tr class="even">
<td><p><strong>교체 선수:</strong></p></td>
<td></td>
</tr>
<tr class="odd">
<td><p>DF</p></td>
<td><p><strong>2</strong></p></td>
</tr>
<tr class="even">
<td><p>FW</p></td>
<td><p><strong>9</strong></p></td>
</tr>
<tr class="odd">
<td><p>FW</p></td>
<td><p><strong>21</strong></p></td>
</tr>
<tr class="even">
<td><p><strong>감독:</strong></p></td>
<td></td>
</tr>
<tr class="odd">
<td><p><a href="../Page/로제_르메르.md" title="wikilink">로제 르메르</a></p></td>
<td></td>
</tr>
</tbody>
</table></td>
<td></td>
<td><table>
<tbody>
<tr class="odd">
<td><p>GK</p></td>
<td><p><strong>1</strong></p></td>
</tr>
<tr class="even">
<td><p>RB</p></td>
<td><p><strong>3</strong></p></td>
</tr>
<tr class="odd">
<td><p>CB</p></td>
<td><p><strong>4</strong></p></td>
</tr>
<tr class="even">
<td><p>CB</p></td>
<td><p><strong>14</strong></p></td>
</tr>
<tr class="odd">
<td><p>LB</p></td>
<td><p><strong>6</strong></p></td>
</tr>
<tr class="even">
<td><p>RM</p></td>
<td><p><strong>8</strong></p></td>
</tr>
<tr class="odd">
<td><p>CM</p></td>
<td><p><strong>5</strong></p></td>
</tr>
<tr class="even">
<td><p>LM</p></td>
<td><p><strong>16</strong></p></td>
</tr>
<tr class="odd">
<td><p>AM</p></td>
<td><p><strong>20</strong></p></td>
</tr>
<tr class="even">
<td><p>CF</p></td>
<td><p><strong>9</strong></p></td>
</tr>
<tr class="odd">
<td><p>CF</p></td>
<td><p><strong>13</strong></p></td>
</tr>
<tr class="even">
<td><p><strong>교체 선수:</strong></p></td>
<td></td>
</tr>
<tr class="odd">
<td><p>FW</p></td>
<td><p><strong>11</strong></p></td>
</tr>
<tr class="even">
<td><p>MF</p></td>
<td><p><strong>22</strong></p></td>
</tr>
<tr class="odd">
<td><p>MF</p></td>
<td><p><strong>7</strong></p></td>
</tr>
<tr class="even">
<td><p><strong>감독:</strong></p></td>
<td></td>
</tr>
<tr class="odd">
<td><p><a href="https://ko.wikipedia.org/wiki/빅토르_푸아" title="wikilink">빅토르 푸아</a></p></td>
<td></td>
</tr>
</tbody>
</table></td>
</tr>
</tbody>
</table>

<table>
<tbody>
<tr class="odd">
<td><p><strong>최우수 선수:</strong><br />
<a href="../Page/파비앵_바르테즈.md" title="wikilink">파비앵 바르테즈</a> (프랑스)</p>
<p><strong>부심:</strong><br />
블라디미르 페르난데스 (<a href="https://ko.wikipedia.org/wiki/엘살바도르_축구_협회" title="wikilink">엘살바도르</a>)<br />
커티스 찰스 (<a href="https://ko.wikipedia.org/wiki/앤티가_바부다_축구_협회" title="wikilink">앤티가 바부다</a>)<br />
<strong>대기심:</strong><br />
<a href="https://ko.wikipedia.org/wiki/우르스_마이어" title="wikilink">우르스 마이어</a> (<a href="https://ko.wikipedia.org/wiki/스위스_축구_협회" title="wikilink">스위스</a>)</p></td>
</tr>
</tbody>
</table>

## 덴마크 vs 프랑스

|          |                        |                       |                        |                         |               |                  |               |                   |                 |                |                |          |                             |                            |                             |                             |                             |                  |               |                   |                 |                |                |
| -------- | ---------------------- | --------------------- | ---------------------- | ----------------------- | ------------- | ---------------- | ------------- | ----------------- | --------------- | -------------- | -------------- | -------- | --------------------------- | -------------------------- | --------------------------- | --------------------------- | --------------------------- | ---------------- | ------------- | ----------------- | --------------- | -------------- | -------------- |
| {{축구 유니폼 | pattern_la = _den02h | pattern_b = _den02h | pattern_ra = _den02h | pattern_sh = _reddots | pattern_so = | leftarm = FF0000 | body = FF0000 | rightarm = FF0000 | shorts = FFFFFF | socks = FF0000 | title = 덴마크 }} | {{축구 유니폼 | pattern_la = _france2002a | pattern_b = _france2002a | pattern_ra = _france2002a | pattern_sh = _france2002h | pattern_so = _francea0608 | leftarm = FFFFFF | body = FFFFFF | rightarm = FFFFFF | shorts = 0000C0 | socks = FFFFFF | title = 프랑스 }} |

<table style="width:10%;">
<colgroup>
<col style="width: 5%" />
<col style="width: 0%" />
<col style="width: 5%" />
</colgroup>
<tbody>
<tr class="odd">
<td><table>
<tbody>
<tr class="odd">
<td><p>GK</p></td>
<td><p><strong>1</strong></p></td>
</tr>
<tr class="even">
<td><p>RB</p></td>
<td><p><strong>6</strong></p></td>
</tr>
<tr class="odd">
<td><p>CB</p></td>
<td><p><strong>4</strong></p></td>
</tr>
<tr class="even">
<td><p>CB</p></td>
<td><p><strong>3</strong></p></td>
</tr>
<tr class="odd">
<td><p>LB</p></td>
<td><p><strong>12</strong></p></td>
</tr>
<tr class="even">
<td><p>CM</p></td>
<td><p><strong>2</strong></p></td>
</tr>
<tr class="odd">
<td><p>CM</p></td>
<td><p><strong>17</strong></p></td>
</tr>
<tr class="even">
<td><p>CM</p></td>
<td><p><strong>7</strong></p></td>
</tr>
<tr class="odd">
<td><p>RW</p></td>
<td><p><strong>19</strong></p></td>
</tr>
<tr class="even">
<td><p>LW</p></td>
<td><p><strong>10</strong></p></td>
</tr>
<tr class="odd">
<td><p>CF</p></td>
<td><p><strong>9</strong></p></td>
</tr>
<tr class="even">
<td><p><strong>교체 선수:</strong></p></td>
<td></td>
</tr>
<tr class="odd">
<td><p>FW</p></td>
<td><p><strong>8</strong></p></td>
</tr>
<tr class="even">
<td><p>DF</p></td>
<td><p><strong>20</strong></p></td>
</tr>
<tr class="odd">
<td><p>MF</p></td>
<td><p><strong>23</strong></p></td>
</tr>
<tr class="even">
<td><p><strong>감독:</strong></p></td>
<td></td>
</tr>
<tr class="odd">
<td><p><a href="../Page/모르텐_올센.md" title="wikilink">모르텐 올센</a></p></td>
<td></td>
</tr>
</tbody>
</table></td>
<td></td>
<td><table>
<tbody>
<tr class="odd">
<td><p>GK</p></td>
<td><p><strong>16</strong></p></td>
</tr>
<tr class="even">
<td><p>RB</p></td>
<td><p><strong>2</strong></p></td>
</tr>
<tr class="odd">
<td><p>CB</p></td>
<td><p><strong>15</strong></p></td>
</tr>
<tr class="even">
<td><p>CB</p></td>
<td><p><strong>8</strong></p></td>
</tr>
<tr class="odd">
<td><p>LB</p></td>
<td><p><strong>3</strong></p></td>
</tr>
<tr class="even">
<td><p>CM</p></td>
<td><p><strong>4</strong></p></td>
</tr>
<tr class="odd">
<td><p>CM</p></td>
<td><p><strong>7</strong></p></td>
</tr>
<tr class="even">
<td><p>RW</p></td>
<td><p><strong>11</strong></p></td>
</tr>
<tr class="odd">
<td><p>AM</p></td>
<td><p><strong>10</strong></p></td>
</tr>
<tr class="even">
<td><p>LW</p></td>
<td><p><strong>21</strong></p></td>
</tr>
<tr class="odd">
<td><p>CF</p></td>
<td><p><strong>20</strong></p></td>
</tr>
<tr class="even">
<td><p><strong>교체 선수:</strong></p></td>
<td></td>
</tr>
<tr class="odd">
<td><p>FW</p></td>
<td><p><strong>9</strong></p></td>
</tr>
<tr class="even">
<td><p>MF</p></td>
<td><p><strong>22</strong></p></td>
</tr>
<tr class="odd">
<td><p>MF</p></td>
<td><p><strong>6</strong></p></td>
</tr>
<tr class="even">
<td><p><strong>감독:</strong></p></td>
<td></td>
</tr>
<tr class="odd">
<td><p><a href="../Page/로제_르메르.md" title="wikilink">로제 르메르</a></p></td>
<td></td>
</tr>
</tbody>
</table></td>
</tr>
</tbody>
</table>

<table>
<tbody>
<tr class="odd">
<td><p><strong>최우수 선수:</strong><br />
<a href="https://ko.wikipedia.org/wiki/지네딘_지단" title="wikilink">지네딘 지단</a> (프랑스)</p>
<p><strong>부심:</strong><br />
카를루스 마투스 (<a href="https://ko.wikipedia.org/wiki/포르투갈_축구_협회" title="wikilink">포르투갈</a>)<br />
엘리스 도리리 (<a href="https://ko.wikipedia.org/wiki/바누아투_축구_협회" title="wikilink">바누아투</a>)<br />
<strong>대기심:</strong><br />
<a href="https://ko.wikipedia.org/wiki/류보시_미헬" title="wikilink">류보시 미헬</a> (<a href="https://ko.wikipedia.org/wiki/슬로바키아_축구_협회" title="wikilink">슬로바키아</a>)</p></td>
</tr>
</tbody>
</table>

## 세네갈 vs 우루과이

|          |                            |                            |                            |                                 |               |                  |               |                   |                 |                |                |          |               |                          |               |               |               |                  |               |                   |                 |                |                 |
| -------- | -------------------------- | -------------------------- | -------------------------- | ------------------------------- | ------------- | ---------------- | ------------- | ----------------- | --------------- | -------------- | -------------- | -------- | ------------- | ------------------------ | ------------- | ------------- | ------------- | ---------------- | ------------- | ----------------- | --------------- | -------------- | --------------- |
| {{축구 유니폼 | pattern_la =_sen02_away | pattern_b = _sen02_away | pattern_ra =_sen02_away | pattern_sh = _yellow_stripes | pattern_so = | leftarm = 008000 | body = 009530 | rightarm = 008000 | shorts = 008000 | socks = 008000 | title = 세네갈 }} | {{축구 유니폼 | pattern_la = | pattern_b = _old laced | pattern_ra = | pattern_sh = | pattern_so = | leftarm = AACCFF | body = AACCFF | rightarm = AACCFF | shorts = 000000 | socks = 000000 | title = 우루과이 }} |

<table style="width:10%;">
<colgroup>
<col style="width: 5%" />
<col style="width: 0%" />
<col style="width: 5%" />
</colgroup>
<tbody>
<tr class="odd">
<td><table>
<tbody>
<tr class="odd">
<td><p>GK</p></td>
<td><p><strong>1</strong></p></td>
</tr>
<tr class="even">
<td><p>RB</p></td>
<td><p><strong>17</strong></p></td>
</tr>
<tr class="odd">
<td><p>CB</p></td>
<td><p><strong>13</strong></p></td>
</tr>
<tr class="even">
<td><p>CB</p></td>
<td><p><strong>4</strong></p></td>
</tr>
<tr class="odd">
<td><p>LB</p></td>
<td><p><strong>2</strong></p></td>
</tr>
<tr class="even">
<td><p>RM</p></td>
<td><p><strong>19</strong></p></td>
</tr>
<tr class="odd">
<td><p>CM</p></td>
<td><p><strong>6</strong></p></td>
</tr>
<tr class="even">
<td><p>CM</p></td>
<td><p><strong>5</strong></p></td>
</tr>
<tr class="odd">
<td><p>LM</p></td>
<td><p><strong>10</strong></p></td>
</tr>
<tr class="even">
<td><p>CF</p></td>
<td><p><strong>7</strong></p></td>
</tr>
<tr class="odd">
<td><p>CF</p></td>
<td><p><strong>11</strong></p></td>
</tr>
<tr class="even">
<td><p><strong>교체 선수:</strong></p></td>
<td></td>
</tr>
<tr class="odd">
<td><p>DF</p></td>
<td><p><strong>21</strong></p></td>
</tr>
<tr class="even">
<td><p>MF</p></td>
<td><p><strong>14</strong></p></td>
</tr>
<tr class="odd">
<td><p>MF</p></td>
<td><p><strong>12</strong></p></td>
</tr>
<tr class="even">
<td><p><strong>감독:</strong></p></td>
<td></td>
</tr>
<tr class="odd">
<td><p><a href="https://ko.wikipedia.org/wiki/브뤼노_메추" title="wikilink">브뤼노 메추</a></p></td>
<td></td>
</tr>
</tbody>
</table></td>
<td></td>
<td><table>
<tbody>
<tr class="odd">
<td><p>GK</p></td>
<td><p><strong>1</strong></p></td>
</tr>
<tr class="even">
<td><p>RB</p></td>
<td><p><strong>3</strong></p></td>
</tr>
<tr class="odd">
<td><p>CB</p></td>
<td><p><strong>4</strong></p></td>
</tr>
<tr class="even">
<td><p>CB</p></td>
<td><p><strong>14</strong></p></td>
</tr>
<tr class="odd">
<td><p>LB</p></td>
<td><p><strong>8</strong></p></td>
</tr>
<tr class="even">
<td><p>RM</p></td>
<td><p><strong>16</strong></p></td>
</tr>
<tr class="odd">
<td><p>CM</p></td>
<td><p><strong>5</strong></p></td>
</tr>
<tr class="even">
<td><p>LM</p></td>
<td><p><strong>6</strong></p></td>
</tr>
<tr class="odd">
<td><p>AM</p></td>
<td><p><strong>20</strong></p></td>
</tr>
<tr class="even">
<td><p>CF</p></td>
<td><p><strong>9</strong></p></td>
</tr>
<tr class="odd">
<td><p>CF</p></td>
<td><p><strong>13</strong></p></td>
</tr>
<tr class="even">
<td><p><strong>교체 선수:</strong></p></td>
<td></td>
</tr>
<tr class="odd">
<td><p>FW</p></td>
<td><p><strong>17</strong></p></td>
</tr>
<tr class="even">
<td><p>FW</p></td>
<td><p><strong>18</strong></p></td>
</tr>
<tr class="odd">
<td><p>FW</p></td>
<td><p><strong>21</strong></p></td>
</tr>
<tr class="even">
<td><p><strong>감독:</strong></p></td>
<td></td>
</tr>
<tr class="odd">
<td><p><a href="https://ko.wikipedia.org/wiki/빅토르_푸아" title="wikilink">빅토르 푸아</a></p></td>
<td></td>
</tr>
</tbody>
</table></td>
</tr>
</tbody>
</table>

<table>
<tbody>
<tr class="odd">
<td><p><strong>최우수 선수:</strong><br />
<a href="../Page/파파_부바_디오프.md" title="wikilink">파파 부바 디오프</a> (세네갈)</p>
<p><strong>부심:</strong><br />
야프 폴 (<a href="../Page/네덜란드_왕립_축구_협회.md" title="wikilink">네덜란드</a>)<br />
셰켈리 페렌츠 (<a href="https://ko.wikipedia.org/wiki/헝가리_축구_협회" title="wikilink">헝가리</a>)<br />
<strong>대기심:</strong><br />
<a href="https://ko.wikipedia.org/wiki/키로스_바사라스" title="wikilink">키로스 바사라스</a> (<a href="https://ko.wikipedia.org/wiki/그리스_축구_협회" title="wikilink">그리스</a>)</p></td>
</tr>
</tbody>
</table>

## 외부 링크

  - [rsssf 기록](http://www.rsssf.com/tables/2002full.html#gra)

[A조](https://ko.wikipedia.org/wiki/분류:2002년_FIFA_월드컵 "wikilink") [분류:프랑스 축구 2001-02 시즌](https://ko.wikipedia.org/wiki/분류:프랑스_축구_2001-02_시즌 "wikilink") [분류:덴마크 축구 2002년](https://ko.wikipedia.org/wiki/분류:덴마크_축구_2002년 "wikilink") [분류:우루과이 축구 2002년](https://ko.wikipedia.org/wiki/분류:우루과이_축구_2002년 "wikilink") [분류:세네갈 축구 2002년](https://ko.wikipedia.org/wiki/분류:세네갈_축구_2002년 "wikilink")