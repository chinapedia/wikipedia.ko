...
**마이크로서비스**()는 애플리케이션을 [느슨히 결합된](https://ko.wikipedia.org/wiki/결합 "wikilink") 서비스의 모임으로 구조화하는 [서비스 지향 아키텍처](../Page/서비스_지향_아키텍처.md "wikilink")(SOA) 스타일의 일종인 [소프트웨어 개발](https://ko.wikipedia.org/wiki/소프트웨어_개발 "wikilink") 기법이다. 마이크로서비스 아키텍처에서 서비스들은 [섬세](https://ko.wikipedia.org/wiki/서비스_입자성_원칙 "wikilink")(fine-grained)하고 [프로토콜](https://ko.wikipedia.org/wiki/프로토콜 "wikilink")은 가벼운 편이다. 애플리케이션을 더 조그마한 여러 서비스로 분해할 때의 장점은 [모듈성을](https://ko.wikipedia.org/wiki/모듈성_\(프로그래밍\) "wikilink") 개선시키고 애플리케이션의 이해, 개발, 테스트를 더 쉽게 해주고 애플리케이션 침식에 더 탄련적으로 만들어 준다.\[1\] 규모가 작은 자율적인 팀들이 팀별 서비스를 독립적으로 개발, [전개](../Page/소프트웨어_전개.md "wikilink"), 규모 확장을 할 수 있게 함으로써 병렬로 [개발할](https://ko.wikipedia.org/wiki/소프트웨어_개발 "wikilink") 수 있게 한다.\[2\] 또, 지속적인 [리팩터링](../Page/리팩터링.md "wikilink")을 통해 개개의 서비스 아키텍처가 하나로 병합될 수 있게 허용한다.\[3\] 마이크로서비스 기반 아키텍처는 [지속적 배포와](https://ko.wikipedia.org/wiki/지속적_배포 "wikilink") 전개(디플로이)를 가능케 한다.\[4\]\[5\]

## 개요

마이크로서비스의 속성과 관련하여 아직까지 산업적인 합의는 없으며 공식적인 정의 또한 부재하다. 자주 인용되어 정의되는 특징들 가운데 일부는 다음을 포함한다:

  - 마이크로서비스 아키텍처(microservice architecture, MSA)의 서비스들\[6\]은 HTTP와 같은 기술 불가지론적인 [프로토콜을](../Page/통신_프로토콜.md "wikilink") 사용하여 목표를 달성하기 위해 [네트워크를](../Page/컴퓨터_네트워크.md "wikilink") 통해 통신하는 [프로세스](../Page/프로세스.md "wikilink")들인 경우도 있다.\[7\]\[8\]\[9\] 그러나, 서비스들은 [공유 메모리와](../Page/공유_메모리.md "wikilink") 같은 다른 종류의 [프로세스 간 통신](../Page/프로세스_간_통신.md "wikilink") 매커니즘을 사용할 수도 있다.\[10\] 서비스들은 이를테면 [OSGI](../Page/OSGi.md "wikilink") 번들에서처럼 동일한 프로세스 내에서 실행할 수 있다.
...
마이크로서비스 아키텍처의 철학은 필연적으로 "한 가지 일을 하되 잘 하라"(Do one thing and do it well)는 [유닉스 철학과](../Page/유닉스_철학.md "wikilink") 상응한다. 다음과 같이 기술된다:\[23\]\[24\]\[25\]
  ...
  - 조직 문화는 [테스트](../Page/소프트웨어_테스트.md "wikilink") 및 [전개의](../Page/소프트웨어_전개.md "wikilink") [자동화](../Page/자동화.md "wikilink")를 받아들여야 한다. 이를 통해 관리 및 운용 상의 부담을 완화하며 다른 개발팀들이 코드의 전개 가능한 단위를 독립적으로 작업할 수 있게 한다.\[26\]
...
마이크로서비스의 접근은 수많은 문제의 비평에 종속된다:
  ...
  - 네트워크상의 서비스 간 호출은 [모놀리식](https://ko.wikipedia.org/wiki/모놀리식_시스템 "wikilink") 서비스 프로세스 내의 프로세스 내 [호출에](../Page/함수_\(프로그래밍\).md "wikilink") 비해 네트워크 레이턴시, 메시지 처리 면에서 비용이 더 많이 든다\[30\]
