...
3.  거리 함수는 [평행이동 불변이다](https://ko.wikipedia.org/wiki/평행이동_불변_거리_함수 "wikilink"); 즉, *V*의 모든 *x*, *y* 그리고 *a*에 대해서 *d*(*x* + *a*, *y* + *a*) = *d*(*x*, *y*)이 성립한다.
4.  거리공간 (*V*, *d*)는 [완비적이다](https://ko.wikipedia.org/wiki/완비성 "wikilink").
...
\(\scriptstyle L^\frac{1}{2}[0,\, 1]\)은 F-공간이다. 이것은 연속 반노름과 연속 선형 범함수를 인정하지 않는다 — 이것은 자명한 [쌍대 공간을](https://ko.wikipedia.org/wiki/쌍대_공간 "wikilink") 가진다.
...
이것은 단위 디스크 \(\scriptstyle \mathbb{D}\)에서 다음을 만족한다:

\[\sum_{n}|a_n|^p < \infty\]

그리고 (일 때) \(\scriptstyle W_p(\mathbb{D})\)는 [p-노름](https://ko.wikipedia.org/wiki/p-노름 "wikilink")을 가지는 F-공간이다:

\[\|f\|_p= \sum_{n}|a_n|^p \qquad (0 < p < 1)\]

사실, \(\scriptstyle W_p\)은 [준-바나흐 대수이다](https://ko.wikipedia.org/wiki/준-바나흐_대수 "wikilink"). 게다가, \(\scriptstyle |\zeta| \;\leq\; 1\)를 가지는 모든 \(\scriptstyle \zeta\)에 대해, 맵핑 \(\scriptstyle f \,\mapsto\, f(\zeta)\)은 \(\scriptstyle W_p(\mathbb{D})\)에서 유계 선형(곰셈의 범함수)이다.

## 같이 보기

[K-공간 (함수해석학)](https://ko.wikipedia.org/wiki/K-공간_\(함수해석학\) "wikilink")

## 참조

  -
[F-공간](https://ko.wikipedia.org/wiki/분류:F-공간 "wikilink")
