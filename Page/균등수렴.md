...
[해석학에서](https://ko.wikipedia.org/wiki/해석학_\(수학\) "wikilink"), **균등수렴**(均等收斂, )하는 함수열은, 주어진 함수로 일제히 '동일한 속도'로 수렴하는 함수열이다. 균등수렴은 [점마다 수렴보다](https://ko.wikipedia.org/wiki/점마다_수렴 "wikilink") 더 강한 개념이며, 점마다 수렴이 보존하지 않는 여러 성질(예: [연속성](https://ko.wikipedia.org/wiki/연속함수 "wikilink"))을 보존한다.

균등수렴은 **고른수렴**, **평등수렴**(平等收斂), **일양수렴**(一樣收斂)이라고도 불린다.

## 정의

\(S\)는 임의의 [집합](https://ko.wikipedia.org/wiki/집합 "wikilink"), \(M\)은 [거리공간](https://ko.wikipedia.org/wiki/거리공간 "wikilink"), \(f_n\colon S\to M\)을 함수열, \(f\colon S\to M\)을 또 하나의 함수라고 하자. 만약 임의의 \(\epsilon > 0\)에 대해, 어떤 \(N\)이 존재하여, 임의의 \(n > N\)과 \(x \in S\)에 대해, \(d(f_n(x), f(x)) < \epsilon\)이라면, 함수열 \(f_n\)이 (**균등극한**) \(f\)로 **균등수렴**한다고 하고, \(f_n \rightrightarrows f\)로 표기한다. 균등수렴은
...
다음은 실함수항급수의 균등수렴에 대한 판정법들이다. 대다수는 실수항급수의 수렴판정법의 전제 조건이 정의역의 모든 곳에서 '균등적'으로 성립하는 것을 전제 조건으로 한다.
  ...
  - ([코시 수렴판정법](https://ko.wikipedia.org/wiki/코시_수렴판정법 "wikilink"))실함수항급수가 수렴할 필요충분조건은, 임의의 \(\epsilon > 0\)에 대해, 어떤 \(N\)이 존재하여, 임의의 \(n,m > N\)과 \(x \in I\)에 대해, \(\textstyle \left|\sum_{k=n}^m f_k(x)\right| < \epsilon\)
  - ([바이어슈트라스 M-판정법](https://ko.wikipedia.org/wiki/바이어슈트라스_M-판정법 "wikilink"))항상 \(|f_n(x)| \le M_n\)이고, (양수항급수) \(\textstyle \sum_{n=1}^{\infty} M_n\)이 수렴하면, \(\textstyle \sum_{n=1}^{\infty}  f_n\)은 절대균등수렴한다.
  - ([디리클레 판정법](https://ko.wikipedia.org/wiki/디리클레_판정법 "wikilink"))만약 \(\textstyle \sum_{n=1}^{\infty} g_n\)의 부분합 \(S_n\)이 [균등유계](https://ko.wikipedia.org/wiki/균등유계 "wikilink")이고, \(h_n\)이 \(n\)에 대해 단조롭고, 0으로 균등수렴하면, \(\textstyle \sum_{n=1}^{\infty} g_nh_n\)은 균등수렴한다.
  - ([아벨 판정법](https://ko.wikipedia.org/wiki/아벨_판정법 "wikilink"))만약 \(\textstyle \sum_{n=1}^{\infty} g_n\)이 균등수렴하고, \(h_n\)이 \(n\)에 대해 단조롭고, 균등유계이면, \(\textstyle \sum_{n=1}^{\infty} g_nh_n\)은 균등수렴한다.\<\!--
...
[점마다 수렴은](https://ko.wikipedia.org/wiki/점마다_수렴 "wikilink") 연속성을 보존하지 않는다.
...
[르베그 적분에](https://ko.wikipedia.org/wiki/르베그_적분 "wikilink") 대해서는, 더 많이 약화된 전제 조건을 사용할 수 있다.
