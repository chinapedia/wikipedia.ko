...
다음은 실함수항급수의 균등수렴에 대한 판정법들이다. 대다수는 실수항급수의 수렴판정법의 전제 조건이 정의역의 모든 곳에서 '균등적'으로 성립하는 것을 전제 조건으로 한다.
  ...
  - ([코시 수렴 판정법](https://ko.wikipedia.org/wiki/코시_수렴_판정법 "wikilink"))실함수항급수가 수렴할 필요충분조건은 임의의 \(\epsilon > 0\)에 대해, 어떤 \(N\)이 존재하여, 임의의 \(n,m > N\)과 \(x \in I\)에 대해, \(\textstyle \left|\sum_{k=n}^m f_k(x)\right| < \epsilon\)
