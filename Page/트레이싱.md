...
**트레이싱 (tracing)**은 프로그램의 실행에 관한 정보를 기록하기 위한 로깅의 특별한 사용이다. 이 정보는 보통 프로그래머가 디버깅을 목적으로 사용된다. 또는 [시스템 관리자나](https://ko.wikipedia.org/wiki/시스템_관리자 "wikilink") 기술 지원 인원 그리고 소프트웨어 감시 툴이 소프트웨어의 일반적인 문제들을 진단하기 위해서, 추가적인 자세한 트레이스 로그를 포함하는 정보로서 사용된다. 트레이싱은 [횡단 관심사](https://ko.wikipedia.org/wiki/횡단_관심사 "wikilink")(cross-cutting concern)이다.
...
| "높은 수준"의 정보를 기록 (프로그램 설치의 실패 같은                                        | "낮은 수준"의 정보를 기록 (throw된 [예외](https://ko.wikipedia.org/wiki/예외_처리 "wikilink") 같은) |
...
이벤트 로깅은 시스템 관리자가 진단하고 [감사](https://ko.wikipedia.org/wiki/감사 "wikilink")하는데 유용한 정보를 제공한다. (이벤트 메시지들 중에서 기록될 것과 세부 사항에 대한) 이벤트들의 다른 클래스들은 개발 단계 이전에 고려되는 경향이 있다. 많은 이벤트 로깅 기술들은 각 이벤트의 클래스에 고유한 코드가 부여되는 것을 허용하거나 요구한다. 이것은 [지역화](https://ko.wikipedia.org/wiki/지역화 "wikilink")를 가능케 하고 시스템 관리자가 발생한 문제에 대한 정보를 얻는 것을 쉽게 한다.
...
소프트웨어 트레이싱의 다른 중요한 고려할 점은 성능이다. 소프트웨어 트레이싱이 낮은 수준에서 이루어지기 때문에 트레이스 메시지의 가능한 양은 훨씬 커질 수 있다. 그렇기 때문에 종종 [컴파일 타임이나](https://ko.wikipedia.org/wiki/컴파일_타임 "wikilink") 실행 시간에 꺼놓을 필요가 있다. 다른 고려 사항을 보자면
  ...
  - [사유 소프트웨어에서](https://ko.wikipedia.org/wiki/사유_소프트웨어 "wikilink"), 트레이싱 데이터는 소스 코드에 대한 민감한 정보를 가질 수 있다.
...
소프트웨어 트레이싱:
  ...
  - 트레이싱 매크로
  - 디버거에 출력
  - [관점 지향 프로그래밍과](https://ko.wikipedia.org/wiki/관점_지향_프로그래밍 "wikilink") related instrumentation 기법
...
이벤트 로깅:
  ...
  - [Windows Event Log](http://msdn2.microsoft.com/en-us/library/aa385780.aspx) ([윈도우 비스타](https://ko.wikipedia.org/wiki/윈도우_비스타 "wikilink"))
...
둘 다에서 적절한:
  ...
  - [Visual Basic logging](http://msdn2.microsoft.com/en-us/library/microsoft.visualbasic.logging.aspx) (for [Visual Basic .](https://ko.wikipedia.org/wiki/비주얼_베이직_닷넷 "wikilink")[NET](https://ko.wikipedia.org/wiki/비주얼_베이직_닷넷 "wikilink"))
