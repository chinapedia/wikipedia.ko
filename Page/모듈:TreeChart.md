...
`   local args = require('Module:Arguments').getArgs(frame, {wrappers = '틀:차트', trim = false, removeBlanks = false})`
`   local cell_args = {`
...
`           -- white space at the front counts for param_``, but not whitespace at the end, so remove it`
`           local rightTrimmedVal = val:gsub('%s+$','')`
`           table.insert(cell_args, {`
`               text = args[trimmedVal] or ('``'),`
