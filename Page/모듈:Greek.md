...
\-- used for math graph template local numeral_latin_transliteration = {
...
\-- return number corresponding to letter -- params: letter (string) -- return: number local function value(letter)
...
\-- letter value \* 1000 -- params: letter (string), total (number) -- return: number local function thousandth(letter, total)
...
\-- reverse table (index to values, values to index) -- params: tbl (table) -- return: table local function reverse_table(tbl)
...
\-- special notation for numbers \> 9999 local function mu_notation(greek_number, digits)
...
` end`
` `
` result = '`\(\\stackrel{' .. result .. '}{\\Mu}\)`' .. postfix`
` `
` return result `

end

\-- convert standard number to greek -- params: frame (obj) -- return: string function p.to_greek(frame)
