...
[섬네일로](https://ko.wikipedia.org/wiki/파일:FactoryMethod.svg "wikilink") 표현된 팩토리 메서드\]\] [섬네일로](https://ko.wikipedia.org/wiki/파일:Factory_Method_pattern_in_LePUS3.png "wikilink") 표현된 팩토리 메서드\]\] **팩토리 메서드 패턴**()은 [객체지향](https://ko.wikipedia.org/wiki/객체지향 "wikilink") [디자인 패턴이다](https://ko.wikipedia.org/wiki/디자인_패턴 "wikilink"). Factory method는 부모(상위) [클래스에](https://ko.wikipedia.org/wiki/클래스_\(전산학\) "wikilink") 알려지지 않은 구체 클래스를 생성하는 패턴이며. 자식(하위) 클래스가 어떤 객체를 생성할지를 결정하도록 하는 패턴이기도 하다. 부모(상위) 클래스 코드에 구체 클래스 이름을 감추기 위한 방법으로도 사용한다.
