...
[수학](https://ko.wikipedia.org/wiki/수학 "wikilink")에서, **이차 함수**(二次函數, )는 최고 차수가 2인 [다항 함수이다](https://ko.wikipedia.org/wiki/다항_함수 "wikilink").

## 정의

**이차 함수**는 다음과 같은 꼴의 [함수](https://ko.wikipedia.org/wiki/함수 "wikilink") \(f\colon\mathbb R\to\mathbb R\) (또는 \(f\colon\mathbb C\to\mathbb C\))이다.
...
\[\alpha,\beta=\frac{-b\pm\sqrt{b^2-4ac}}{2a}\] 구체적으로, 이차 함수 \(f\)의 [판별식](../Page/판별식.md "wikilink") \(b^2-4ac\)의 값에 따라 다음과 같은 경우로 나뉜다.
  ...
  - \(b^2-4ac<0\)이라면, \(f\)는 실근을 가지지 않지만, 서로 다른 두 [허근](https://ko.wikipedia.org/wiki/허수 "wikilink") \(\alpha\ne\beta\)를 가진다. 이때 그래프는 \(x\)축과 만나지 않는다.
