...
[섬네일](https://ko.wikipedia.org/wiki/파일:Relational_database_terms.svg "wikilink") [관계 모델에서](https://ko.wikipedia.org/wiki/관계_모델 "wikilink") **관계**(relation)는 동일한 구조로 이루어진 [튜플](https://ko.wikipedia.org/wiki/튜플 "wikilink")의 집합을 말한다. 값으로서의 관계를 ‘관계값’(relation value)라고 하며, 관계값을 값으로 가지는 변수를 ‘[관계변수](https://ko.wikipedia.org/wiki/관계변수 "wikilink")’(relvar, relation variable)라고 한다.
...
[관계 데이터베이스의](https://ko.wikipedia.org/wiki/관계_데이터베이스 "wikilink") [데이터베이스 언어인](../Page/데이터베이스_언어.md "wikilink") [SQL](https://ko.wikipedia.org/wiki/SQL "wikilink")에서는 관계 변수와 거의 같은 의미로 [테이블](https://ko.wikipedia.org/wiki/테이블 "wikilink")이라는 용어가 사용되고 있다. 문맥에 따라서 ‘관계변수’를 ‘관계’라고 부르기도 하고 ‘관계값’을 관계라고 부르기도 한다.
...
기타 데이터베이스의 관계는 다음과 같은 것을 말할 수 있다.
  ...
  - 관계값을 구성하는 튜플 수를 [농도](https://ko.wikipedia.org/wiki/집합의_크기 "wikilink") (cardinality)라고 한다.
