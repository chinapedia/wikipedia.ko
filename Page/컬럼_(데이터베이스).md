...
**컬럼**(column)이란 [관계형 데이터베이스](https://ko.wikipedia.org/wiki/관계형_데이터베이스 "wikilink") [테이블에서](https://ko.wikipedia.org/wiki/테이블_\(데이터베이스\) "wikilink") 특정한 단순 [자료형](https://ko.wikipedia.org/wiki/자료형 "wikilink")의 일련의 데이터값과 테이블에서의 각 열을 말한다. 컬럼은 열이 어떻게 구성되어야 할 지에 대한 구조를 제공한다.
