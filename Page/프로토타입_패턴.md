...
패턴을 구현하려면, 우선 `clone()` 메소드를 선언하는 추상 베이스 클래스를 하나 만든다. [다형적](https://ko.wikipedia.org/wiki/다형성_\(컴퓨터_과학\) "wikilink") [생성자](https://ko.wikipedia.org/wiki/생성자 "wikilink")(polymorphic constructor) 기능이 필요한 클래스가 있다면, 그것을 앞에서 만든 클래스를 상속받게 한 후, `clone()` 메소드 내의 코드를 구현한다.
...
[추상 팩토리 패턴](../Page/추상_팩토리_패턴.md "wikilink"), [빌더 패턴](https://ko.wikipedia.org/wiki/빌더_패턴 "wikilink"), 프로토타입 패턴은 각 구현에 있어서 [싱글턴 패턴을](https://ko.wikipedia.org/wiki/싱글턴_패턴 "wikilink") 활용할 수 있다.\[2\]

다시 말해 추상 팩토리 클래스는 종종 팩토리 메소드과 함께 구현하거나\[3\], 프로토타입을 이용해서 구현되기도 한다.\[4\]

보통 설계는 처음에는 팩토리 메소드로 출발한다. 다음에 설계자의 재량에 따라 추상 팩토리 패턴, [빌더 패턴](https://ko.wikipedia.org/wiki/빌더_패턴 "wikilink"), 프로토타입 패턴으로 바뀔 수 있다.\[5\]

프로토타입은 [서브클래싱을](https://ko.wikipedia.org/wiki/서브클래스 "wikilink") 필요로 하지 않는다. 하지만 "초기화" 동작을 필요로 한다. [팩토리 메서드 패턴은](https://ko.wikipedia.org/wiki/팩토리_메서드_패턴 "wikilink") 서브클래싱을 필요로 하나, "초기화" 동작은 필요로 하지 않는다.

설계자는 [컴포지트 패턴이나](https://ko.wikipedia.org/wiki/컴포지트_패턴 "wikilink") [데코레이터 패턴을](https://ko.wikipedia.org/wiki/데코레이터_패턴 "wikilink") 매우 많이 사용한 무거운 설계를 프로토타입을 사용하여 더 좋게 만들 수 있다.
...
[PHP](https://ko.wikipedia.org/wiki/PHP "wikilink") 5에서는, 객체는 레퍼런스 형태로 전달된다(pass by reference).
...
## 같이 보기
  ...
  - [프로토타입 기반 프로그래밍](https://ko.wikipedia.org/wiki/프로토타입_기반_프로그래밍 "wikilink")
