...
\[{\rm Li} (x) =   \int_2^x \frac{dt}{\ln t} \;\] 여기서 \({\ln} \;\)은 [자연로그](https://ko.wikipedia.org/wiki/자연로그 "wikilink")를 의미한다.
...
여기서 \(\gamma\) ≈ 0.57721 56649 01532 ... 는 [오일러-마스케로니 상수이다](https://ko.wikipedia.org/wiki/오일러-마스케로니_상수 "wikilink").

## 점근적 표기

x → ∞에서의 li(x)의 행동은 다음과 같다.\[3\]

\[{\rm li} (x) = O \left( {x\over \ln x} \right) \;\] 여기서 \(O\)는 [점근 표기법을](https://ko.wikipedia.org/wiki/점근_표기법 "wikilink") 의미한다.

## 소수와의 관계

로그 적분 함수는 수론에서 매우 중요한데 왜냐하면 어떤 수 이하의 소수의 개수를 어림하는데 쓰이기 때문이다. 즉, [소수 정리는](https://ko.wikipedia.org/wiki/소수_정리 "wikilink") 다음을 보장한다.

\[\pi(x)\sim\operatorname{li}(x)\] 여기서의 \(\pi(x)\)은 [소수 계량 함수이다](https://ko.wikipedia.org/wiki/소수_계량_함수 "wikilink"). 실제로 계산해 보면 작은 범위 안에서는 \(li(x)\)가 \(\pi(x)\)보다 항상 약간 더 큰 것 처럼 보이지만 실제로는 [스큐스 수에서](https://ko.wikipedia.org/wiki/스큐스_수 "wikilink") \(\pi(x)\)가 \(li(x)\)보다 더 커지고 이후에는 무한히 대소 순서가 바뀐다는 것이 알려져 있다.\[4\]
...
## 같이 보기
  ...
  - [소수 계량 함수](https://ko.wikipedia.org/wiki/소수_계량_함수 "wikilink")
  - [지수 적분 함수](../Page/지수_적분_함수.md "wikilink")
  - [소수 정리](https://ko.wikipedia.org/wiki/소수_정리 "wikilink")
  - [라마누잔-솔드너 상수](https://ko.wikipedia.org/wiki/라마누잔-솔드너_상수 "wikilink")
