...
**로컬 보안 인증 하위 시스템 서비스**(, **LSASS**)는 시스템의 [보안 정책을](https://ko.wikipedia.org/wiki/보안_정책 "wikilink") 강화를 위한 윈도우의 [프로세스](https://ko.wikipedia.org/wiki/프로세스 "wikilink")이다. 이것은 윈도우 컴퓨터나 서버에 접속하는 유저들의 로그인을 검사하며, 비밀번호 변경을 관리하고, 액세스 토큰(access tokens)을 생성한다.\[1\] 또한 Windows Security Log를 작성한다.
