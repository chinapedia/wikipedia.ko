...
한편 [에츠허르 데이크스트라는](https://ko.wikipedia.org/wiki/에츠허르_데이크스트라 "wikilink") [구조화 이론에서](https://ko.wikipedia.org/wiki/구조적_프로그래밍 "wikilink") 모든 프로그래밍은 [순차실행](https://ko.wikipedia.org/wiki/순차적_실행 "wikilink"),[조건분기](../Page/조건문.md "wikilink") 그리고 [반복실행의](https://ko.wikipedia.org/wiki/반복문 "wikilink") 3개의 제어문 구문 유형으로 기술될수있음을 언급한바있다.\[1\]\[2\] 조건분기 구문으로는 'if\~else\~' 조건문이 , 그리고 반복실행 구문으로는 'for(\~;\~;\~)' 반복문이 여러 프로그래밍 언어에서 주로 사용되고있으며 순차적 실행을 위해서 'main()'함수같은 특정 초기 실행 함수를 예약해서 제공하고있다.
...
1.  [에츠허르 데이크스트라](https://ko.wikipedia.org/wiki/에츠허르_데이크스트라 "wikilink"), [*Notes on Structured Programming*](http://www.cs.utexas.edu/users/EWD/ewd02xx/EWD249.PDF), pg. 6
