...
오늘날 가장 널리 쓰이는 계층형 모델은 [LDAP](https://ko.wikipedia.org/wiki/LDAP "wikilink") 모델이다. 이외에는 계층형 모델은 오늘날의 데이터베이스들에서 거의 쓰이지 않고 있다. 그러나, 정보를 기록하는 다른 매체 및 기법들, 예를 들어 [XML](https://ko.wikipedia.org/wiki/XML "wikilink") 문서라든지, [파일 시스템이라든지](https://ko.wikipedia.org/wiki/파일_시스템 "wikilink"), [윈도 레지스트리](https://ko.wikipedia.org/wiki/윈도_레지스트리 "wikilink") 등에서는 쓰이고 있다.
