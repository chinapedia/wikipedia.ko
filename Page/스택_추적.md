...
예를 들면, 아래의 [파이썬](https://ko.wikipedia.org/wiki/파이썬 "wikilink")은 에러를 포함한다.
...
많은 프로그래밍 언어 ([자바와](https://ko.wikipedia.org/wiki/자바_\(프로그래밍_언어\) "wikilink") [C\#을](https://ko.wikipedia.org/wiki/C_샤프 "wikilink") 포함한)는 시스템 호출을 통해 현재 스택 추적을 검색하는 빌트인된 지원을 갖는다. [C++](https://ko.wikipedia.org/wiki/C++ "wikilink")은 이것에 대한 빌트인 지원은 없지만, C++ 사용자들은 [stacktrace](http://stacktrace.sourceforge.net/) [라이브러리를](https://ko.wikipedia.org/wiki/라이브러리_\(컴퓨팅\) "wikilink") 통해 스택 추적을 검색할 수 있다. [자바스크립트](https://ko.wikipedia.org/wiki/자바스크립트 "wikilink")에서는 [예외 처리는](https://ko.wikipedia.org/wiki/예외_처리 "wikilink") thrown이 발생한 스택을 포함하는 `stack` 속성을 갖는다.
