...
[섬네일](https://ko.wikipedia.org/wiki/파일:Helenos_for_Apache_Cassandra.PNG "wikilink") **아파치 카산드라**(Apache Cassandra)는 [자유-오픈 소스](https://ko.wikipedia.org/wiki/자유-오픈_소스_소프트웨어 "wikilink") [분산형](../Page/분산_데이터베이스.md "wikilink") [NoSQL](../Page/NoSQL.md "wikilink") [데이터베이스](https://ko.wikipedia.org/wiki/데이터베이스 "wikilink") 관리 시스템의 하나로, [단일 장애점](https://ko.wikipedia.org/wiki/단일_장애점 "wikilink") 없이 고성능을 제공하면서 수많은 서버 간의 대용량의 데이터를 관리하기 위해 설계되었다. 카산드라는 여러 데이터센터에 걸쳐 [클러스터를](https://ko.wikipedia.org/wiki/컴퓨터_클러스터 "wikilink") 지원하며\[1\] 마스터리스(masterless) 비동기 레플리케이션을 통해 모든 클라이언트에 대한 낮은 레이턴시 운영을 허용한다.
...
## 같이 보기
  ...
  - [분산 해시 테이블](https://ko.wikipedia.org/wiki/분산_해시_테이블 "wikilink") (DHT)
