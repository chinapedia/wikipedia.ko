...
**소수 계량 함수**(素數計量函數, )는 주어진 양의 실수 \(x\)에 대해 그 값보다 작거나 같은 [소수의](https://ko.wikipedia.org/wiki/소수_\(수론\) "wikilink") 개수를 세는 함수이다. 보통 그리스 소문자 [π](https://ko.wikipedia.org/wiki/π "wikilink")를 이용해 π(*x*)로 표기하지만, [원주율](https://ko.wikipedia.org/wiki/원주율 "wikilink") π와는 관계가 없다. [섬네일](https://ko.wikipedia.org/wiki/파일:PrimePi.PNG "wikilink")

## 역사

[정수론](https://ko.wikipedia.org/wiki/정수론 "wikilink")에서 소수 개수의 증가 속도는 매우 중요한 관심 대상이다. 18세기 말 [카를 프리드리히 가우스와](https://ko.wikipedia.org/wiki/카를_프리드리히_가우스 "wikilink") [아드리앵마리 르장드르는](https://ko.wikipedia.org/wiki/아드리앵마리_르장드르 "wikilink") 소수 계량 함수가 \(x/\ln (x)\)에 근접함을 추측했다. 즉,

\[\lim_{x \to \infty} \frac{\pi(x)}{x/\ln (x)} = 1\] 라고 생각했고, 이는 [소수 정리에](https://ko.wikipedia.org/wiki/소수_정리 "wikilink") 해당한다. 이와 동치로서 다음 극한이 있다.

\[\lim_{x \to \infty} \frac{\pi(x)}{\text{li} (x)} = 1\] 여기서 li는 [로그 적분 함수를](../Page/로그_적분_함수.md "wikilink") 의미한다. 1859년 [베른하르트 리만이](https://ko.wikipedia.org/wiki/베른하르트_리만 "wikilink") 도입한 [리만 제타 함수의](https://ko.wikipedia.org/wiki/리만_제타_함수 "wikilink") 성질을 이용하여 1896년에 [자크 아다마르와](https://ko.wikipedia.org/wiki/자크_아다마르 "wikilink") [샤를장 드 라 발레푸생이](../Page/샤를장_드_라_발레푸생.md "wikilink") 각각 독립적으로 소수 정리를 증명하였다.
...
이는 [베른하르트 리만이](https://ko.wikipedia.org/wiki/베른하르트_리만 "wikilink") 1859년에 발표한 논문의 주 내용인데, 엄밀한 증명은 1895년에 와서야 수학자 폰 망골트에 의해서 이루어졌다.

폰 망골트는 이 공식을 증명하면서 밑의 (사실상 동치인) 공식도 증명하였는데, 이는 다음과 같다:

\[\psi(x) = x - \sum_{\rho\in S} \frac{x^\rho}{\rho} - \ln 2\pi - \frac12 \ln(1-x^{-2})\] 여기서

  - \(S\)는 [리만 제타 함수의](https://ko.wikipedia.org/wiki/리만_제타_함수 "wikilink") 임계구역(critical strip)에 있는 영점들이다.
...
## 같이 보기
  ...
  - [주어진 수보다 작은 소수의 개수에 관하여](https://ko.wikipedia.org/wiki/주어진_수보다_작은_소수의_개수에_관하여 "wikilink")
