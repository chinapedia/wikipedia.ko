...
**디리클레 급수**(Dirichlet series)는 [복소수](https://ko.wikipedia.org/wiki/복소수 "wikilink") \(s\), 복소 수열 \(\{a_n\}\)에 대하여
...
[리만 제타 함수는](https://ko.wikipedia.org/wiki/리만_제타_함수 "wikilink") 디리클레 급수의 한 예로, 다음과 같이 정의된다.

\[\zeta(s)=\sum_{n=1}^{\infty} \frac{1}{n^s}\] 리만 제타 함수의 역수는 다음의 디리클레 급수로 표현할 수 있다.

\[\frac{1}{\zeta(s)}=\sum_{n=1}^{\infty} \frac{\mu(n)}{n^s}\] 여기서 \(\mu(n)\)은 [뫼비우스 함수이다](https://ko.wikipedia.org/wiki/뫼비우스_함수 "wikilink"). 또한, 제타함수의 로그는 다음과 같이 표현할 수 있다.
...
\[\frac {\zeta^\prime(s)}{\zeta(s)} = -\sum_{n=1}^\infty \frac{\Lambda(n)}{n^s}.\] 이 식은 [소수 정리를](https://ko.wikipedia.org/wiki/소수_정리 "wikilink") 증명하는 과정에서 쓰인다.\[1\]
