...
이를테면 [관계형 데이터베이스](https://ko.wikipedia.org/wiki/관계형_데이터베이스 "wikilink") 시대 이전에, 관리를 위한 인쇄물 형태의 보고서를 만들어내는 잡은 다음의 스텝들을 이룰 수 있다:
  ...
  - 보통 일반 목적의 유틸리티를 이용하여 임시 파일을 필요한 순서에 따라 [정렬](https://ko.wikipedia.org/wiki/정렬_알고리즘 "wikilink")
...
[유닉스](https://ko.wikipedia.org/wiki/유닉스 "wikilink")에서의 [파일 복사는](https://ko.wikipedia.org/wiki/파일_복사 "wikilink") 다음과 같은 단순한 형태로 가능하다:
...
시스템/360과 PC 환경의 차이는 다음과 같다:
  ...
  - 시스템/360 머신은 하나의 조직에 속한 모든 사용자가 공유하도록 설계되었다. 그러므로 JOB 카드는 운영 체제에게 사용자의 계정 청구 방법(IS198T30500), 다른 사용자의 잡에 상대적으로 우선 순위가 어떻게 되는지(CLASS=L) 등을 알려준다. `//SYSPRINT DD SYSOUT=*`는 컴퓨터가 프로그램의 보고서를 기본 [프린터](https://ko.wikipedia.org/wiki/프린터 "wikilink")에 일반 문서로 출력하도록 지시한다. `DISP=SHR`는 운영 체제에게 다른 프로그램들이 `OLDFILE`을 동시에 읽을 수 있음을 지시한다.
...
OS JCL 프로시저는 처음부터 매개변수화되었으며 마치 [매크로나](https://ko.wikipedia.org/wiki/어셈블리어 "wikilink") 단순 [함수처럼](../Page/함수_\(프로그래밍\).md "wikilink") 만들어준다. 이로써 재사용성을 다양한 환경에서 증대시킨다.
...
상기의 예는 아래의 [의사코드](https://ko.wikipedia.org/wiki/의사코드 "wikilink")로 변환할 수 있다:
