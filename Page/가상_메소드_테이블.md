...
**가상 메소드 테이블**(virtual method table, virtual function table, virtual call table, [디스패치 테이블](../Page/디스패치_테이블.md "wikilink"), **vtable**, 또는 **vftable**)은 [동적 디스패치](https://ko.wikipedia.org/wiki/동적_디스패치 "wikilink")(또는 [런타임](https://ko.wikipedia.org/wiki/런타임 "wikilink") [메소드](../Page/메소드_\(컴퓨터_프로그래밍\).md "wikilink") [바인딩](https://ko.wikipedia.org/wiki/네임_바인딩 "wikilink"))를 지원하기 위해 [프로그래밍 언어에서](https://ko.wikipedia.org/wiki/프로그래밍_언어 "wikilink") 사용되는 메커니즘이다.

클래스가 [가상 함수](https://ko.wikipedia.org/wiki/가상_함수 "wikilink")(또는 가상 메소드)을 정의할 때마다, 대부분의 컴파일러들은 클래스에 숨겨진 멤버 변수를 추가하는데, 이것은 (가상) 함수들에 대한 포인터들의 배열들(가상 메소드 테이블(VMT 또는 Vtable)라고 불리는)을 가리킨다. 이 포인터들은 실행 기간 도중에 정확한 함수를 가리키게 되는데, 왜냐하면 [컴파일 타임에는](https://ko.wikipedia.org/wiki/컴파일_타임 "wikilink") 베이스 함수가 호출될 것인지 또는 베이스 클래스를 상속한 클래스에 의해서 구현될 지 알려져 있지 않기 때문이다.
...
이러한 동적 디스패치를 구현하는 여러 다른 방법들이 있지만, vtable (virtual table) 솔루션은 특히 [C++](https://ko.wikipedia.org/wiki/C++ "wikilink")과 관련된 언어들에서 흔하다. [비주얼 베이직](https://ko.wikipedia.org/wiki/비주얼_베이직 "wikilink") 또는 [델파이](https://ko.wikipedia.org/wiki/델파이 "wikilink")처럼 구현 시 객체들의 프로그래매틱 인터페이스를 분리하는 언어들은 또한 vtable 접근을 사용하는 경향이 있다. 왜냐하면 이것은 단순히 다른 메소드 포인터들의 집합을 사용함으로써 객체들에게 다른 구현을 사용할 수 있게 하기 때문이다. 
...
많은 컴파일러들은 vpointer를 객체의 마지막 멤버에 위치시킨다. 다른 컴파일러들은 객체의 첫 번째 멤버에 위치시키기도 한다. portable 소스 코드는 둘 중 한 방식으로 일한다.\[2\] 예를 들면, [g++은](https://ko.wikipedia.org/wiki/GNU_컴파일러_모음 "wikilink") 미리 vpointer를 객체의 마지막에 위치시킨다.\[3\]
...
[GCC의](https://ko.wikipedia.org/wiki/GNU_컴파일러_모음 "wikilink") g++ 3.4.6은 객체 `b2`를 위해 아래의 32비트 메모리 레이아웃을 생성한다.\[4\]
...
그들의 선언에서 `virtual` 키워드를 갖지 않는 (`f0()`와 `d()` 같은) 이러한 함수들은 보통 vtable 안에 없다. [기본 생성자에](https://ko.wikipedia.org/wiki/기본_생성자 "wikilink") 의한 생성 같은 특별한 경우를 위한 예외가 존재한다.
...
게다가 [JIT 컴파일이](https://ko.wikipedia.org/wiki/JIT_컴파일 "wikilink") 사용되지 않은 환경에서, 가상 함수 호출은 보통 인라인화될 수 없다. 
...
## 같이 보기
  ...
  - [가상 함수](https://ko.wikipedia.org/wiki/가상_함수 "wikilink")
  - [가상 상속](https://ko.wikipedia.org/wiki/가상_상속 "wikilink")
  - [분기 테이블](https://ko.wikipedia.org/wiki/분기_테이블 "wikilink")
...
## 각주
  ...
  - Margaret A. Ellis and [비야네 스트롭스트룹](https://ko.wikipedia.org/wiki/비야네_스트롭스트룹 "wikilink") (1990) The Annotated C++ Reference Manual. Reading, MA: Addison-Wesley. ([ISBN 0-201-51459-1](https://ko.wikipedia.org/wiki/:en:Special:BookSources/0201514591 "wikilink"))
...
2.  Danny Kalev.
3.  [C++ ABI Closed Issues](https://web.archive.org/web/20110725153606/http://www.codesourcery.com/public/cxx-abi/cxx-closed.html)<span> at the </span>[웨이백 머신](https://ko.wikipedia.org/wiki/웨이백_머신 "wikilink") <span>(archived July 25, 2011)</span>
