...
[조합론](https://ko.wikipedia.org/wiki/조합론 "wikilink")에서, **결합 도식**(結合圖式, ) 또는 **일관 구조**(一貫構造, )는 어떤 특별한 조건을 만족시키는 일련의 [이항 관계들이](https://ko.wikipedia.org/wiki/이항_관계 "wikilink") 주어진 [유한 집합이며](https://ko.wikipedia.org/wiki/유한_집합 "wikilink"), [변 색칠이](../Page/변_색칠.md "wikilink") 주어진 [완전 그래프로도](https://ko.wikipedia.org/wiki/완전_그래프 "wikilink") 간주될 수 있다.\[1\]\[2\]\[3\]\[4\] 주어진 결합 도식으로부터 그 구조를 나타내는 [결합 대수인](../Page/결합_대수.md "wikilink") **보스-메스너 대수**(बसु-Mesner代數, )가 존재한다.

## 정의

결합 도식의 개념은 여러 가지로 정의될 수 있으나, 이 정의들은 모두 서로 [동치](https://ko.wikipedia.org/wiki/동치 "wikilink")이다.

### 조합론적 정의

**결합 도식** \((X,\mathcal R)\)은 다음과 같은 데이터로 주어진다.\[5\]\[6\]

  - [유한 집합](https://ko.wikipedia.org/wiki/유한_집합 "wikilink") \(X\)
...
**결합 도식** \((X,n,\mathcal A)\)은 다음과 같은 데이터로 주어진다.\[7\]
  ...
  - [유한 집합](https://ko.wikipedia.org/wiki/유한_집합 "wikilink") \(X=\{0,1,\dotsc,q-1\}\)
...
이 두 정의는 서로 [동치](https://ko.wikipedia.org/wiki/동치 "wikilink")이다. 즉, 두 정의 사이를 번역하려면, 각 [이항 관계](https://ko.wikipedia.org/wiki/이항_관계 "wikilink") \(\sim_i\)를 대신 [정사각 행렬](https://ko.wikipedia.org/wiki/정사각_행렬 "wikilink")

\[(A_i)_{x,y}=x\sim_iy\qquad(x,y\in X)\] 로 치환하면 된다.

### 기하학적 정의

결합 도식의 개념은 [거리 공간과](https://ko.wikipedia.org/wiki/거리_공간 "wikilink") 유사하게 정의될 수도 있다.\[8\]

**결합 도식** \((X,D,\partial)\)은 다음과 같은 데이터로 주어진다.

  - [유한 집합](https://ko.wikipedia.org/wiki/유한_집합 "wikilink") \(X\)
  - [유한 집합](https://ko.wikipedia.org/wiki/유한_집합 "wikilink") \(D\)
  - [함수](https://ko.wikipedia.org/wiki/함수 "wikilink") \(\partial\colon X\times X\to D\). 이는 **거리 함수**(距離函數, )라고 한다.

이는 다음 조건들을 만족시켜야 한다.

  - ㉡ 임의의 \(x,y,z\in X\)에 대하여, 만약 \(\partial(x,x)=\partial(y,z)\)라면, \(y=z\)이다.
  - ㉢ \(\partial\)은 [전사 함수이다](https://ko.wikipedia.org/wiki/전사_함수 "wikilink").
  - ㉣ 임의의 \(x,y,x',y'\in X\)에 대하여, \(\partial(x,y)=\partial(x',y')\)라면 \(\partial(y,x)=\partial(y',x')\)
  - ㉤ 임의의 \(x,y,x',y'\in X\)에 대하여, 만약 \(\partial(x,y)=\partial(x',y')\)라면, \(\forall z\in X\colon \partial(x,z)=\partial(x',f(z))\)이자 \(\forall z\in X\colon\partial(z,y)=\partial(f(z),y')\)가 되는 [전단사 함수](https://ko.wikipedia.org/wiki/전단사_함수 "wikilink") \(f\colon X\to X\)가 존재한다.
...
다음이 주어졌다고 하자.
  ...
  - [유한 집합](https://ko.wikipedia.org/wiki/유한_집합 "wikilink") \(X\) 위의 결합 도식 \((X,\mathcal R)\)
  - [유한 집합](https://ko.wikipedia.org/wiki/유한_집합 "wikilink") \(X'\) 위의 결합 도식 \((X',\mathcal R')\)
...
이는 사실 [이항 관계가](https://ko.wikipedia.org/wiki/이항_관계 "wikilink") 주어진 [구조](../Page/구조_\(논리학\).md "wikilink") 사이의 [준동형](https://ko.wikipedia.org/wiki/준동형 "wikilink")의 특수한 경우이다.

## 종류

결합 도식 \(X\)에 대하여 다음 조건들이 서로 [동치](https://ko.wikipedia.org/wiki/동치 "wikilink")이며, 이를 만족시키는 결합 도식을 **대칭 결합 도식**(對稱結合圖式, )이라고 한다.\[10\]
...
결합 도식 \((X,\mathcal R)\)에 대하여 다음 두 조건이 서로 [동치](https://ko.wikipedia.org/wiki/동치 "wikilink")이며, 이를 만족시키는 결합 도식을 **가환 결합 도식**(可換結合圖式,)이라고 한다.

  - 그 보스-메스너 대수가 [가환환](https://ko.wikipedia.org/wiki/가환환 "wikilink")이다.
...
(\\partial(x,z),\\partial(z,y))=(\\partial(y,f(z)),\\partial(f(z),x))</math>가 되는 [자기](https://ko.wikipedia.org/wiki/자기_함수 "wikilink") [전단사 함수](https://ko.wikipedia.org/wiki/전단사_함수 "wikilink") \(f\colon X\to X\)가 존재한다.
...
임의의 \(x,y,y'\in X\)가 주어졌으며, [귀류법](https://ko.wikipedia.org/wiki/귀류법 "wikilink")을 사용하여
...
\[|I|<\aleph_0\] 이 주어졌다고 하자. 그렇다면, [곱집합](https://ko.wikipedia.org/wiki/곱집합 "wikilink")
...
이는 [군의](https://ko.wikipedia.org/wiki/군_\(수학\) "wikilink") [직접곱](../Page/직접곱.md "wikilink")의 개념의 일반화이다.
...
\[\sum_{i,j=0}^np_{ij}^0=\sum_{i=0}^np_{i\bar\imath}^0=|X|\] (여기서 \(\delta_i^j\)는 [크로네커 델타이다](https://ko.wikipedia.org/wiki/크로네커_델타 "wikilink").)

### 대칭 결합 도식에 대응되는 변 색칠

대칭 결합 도식 \((X,\mathcal R)\)이 주어졌다고 하자. 그렇다면, \(X\)의 원소를 꼭짓점들로 하는 [완전 그래프](https://ko.wikipedia.org/wiki/완전_그래프 "wikilink") \(K_X\) 위에, \(\mathcal R\setminus\{R_0\}\)의 원소를 색으로 하는 [변 색칠](../Page/변_색칠.md "wikilink") \(c\colon \operatorname E(K_X)\to \mathcal R\setminus\{R_0\}\)을 정의할 수 있다.

\[c\colon (x,y)\mapsto R\iff (x,y)\in R\qquad(R\in\mathcal R,\;R\ne R_0)\] 이에 따라, 대칭 결합 도식은 특별한 [변 색칠이](../Page/변_색칠.md "wikilink") 주어진 유한 [완전 그래프로](https://ko.wikipedia.org/wiki/완전_그래프 "wikilink") 여겨질 수 있다.\[13\]
...
\[E_aE_b=\delta_{ab}E_a\qquad(a,b\in\{0,\dotsc,k\})\] 를 만족시킨다 (\(\delta_{ab}\)는 [크로네커 델타](https://ko.wikipedia.org/wiki/크로네커_델타 "wikilink")). 또한, 편의상
...
가환 결합 도식 \(X\)의 경우, 최소 멱등원의 수는 \(|X|=n\)와 같으며, [멱영원](https://ko.wikipedia.org/wiki/멱영원 "wikilink")들 \((E_a)_{0\le a\le n}\)은 보스-메스너 대수 \(\mathcal A\)의 [기저를](https://ko.wikipedia.org/wiki/기저_\(선형대수학\) "wikilink") 이룬다. 이에 따라, 다음을 정의할 수 있다.
...
특히, [해밍 결합 도식은](https://ko.wikipedia.org/wiki/해밍_결합_도식 "wikilink") 스스로의 쌍대이다.\[16\]
...
크기가 2 이상인 임의의 [유한 집합](https://ko.wikipedia.org/wiki/유한_집합 "wikilink") \(X\) 위에
...
\[p_{11}^1=|X|-2\] 이는 사실 \(n=1\)인 [해밍 결합 도식과](https://ko.wikipedia.org/wiki/해밍_결합_도식 "wikilink") 같다.

### 이산 결합 도식

임의의 [유한 집합](https://ko.wikipedia.org/wiki/유한_집합 "wikilink") \(X\) 위에,
...
임의의 [곱집합](https://ko.wikipedia.org/wiki/곱집합 "wikilink") \(X=F^n\) 위에, 두 벡터 사이의 [해밍 거리가](https://ko.wikipedia.org/wiki/해밍_거리 "wikilink") \(0,1,\dotsc,n\)인 것을 각각 [이항 관계로](https://ko.wikipedia.org/wiki/이항_관계 "wikilink") 삼으면, 이는 결합 도식을 이룬다. 이를 **[해밍 결합 도식](https://ko.wikipedia.org/wiki/해밍_결합_도식 "wikilink")**이라고 한다.

특히, \(F=\{0,1\}\)일 때, 주어진 [해밍 무게의](https://ko.wikipedia.org/wiki/해밍_무게 "wikilink") 벡터들만을 취한 부분 결합 도식을 **[존슨 결합 도식](https://ko.wikipedia.org/wiki/존슨_결합_도식 "wikilink")**이라고 한다.

### 정추이적 작용

[유한군](https://ko.wikipedia.org/wiki/유한군 "wikilink") \(G\)의, [유한 집합](https://ko.wikipedia.org/wiki/유한_집합 "wikilink") \(X\) 위의 왼쪽 [정추이적 작용이](https://ko.wikipedia.org/wiki/정추이적_작용 "wikilink") 주어졌다고 하자. (특히, \(|G|=|X|\)이다.) 그렇다면,
...
이에 대응하는 보스-메스너 대수는 [군환](https://ko.wikipedia.org/wiki/군환 "wikilink")

\[\mathbb Z[G]\] 과 동형이다.

또한, \(G\)에 대응하는 결합 도식이 가환 결합 도식일 [필요 충분 조건은](https://ko.wikipedia.org/wiki/필요_충분_조건 "wikilink") \(G\)가 [아벨 군인](https://ko.wikipedia.org/wiki/아벨_군 "wikilink") 것이다.

### 일반적 군 작용

[유한군](https://ko.wikipedia.org/wiki/유한군 "wikilink") \(G\)의, [유한 집합](https://ko.wikipedia.org/wiki/유한_집합 "wikilink") \(X\) 위의 왼쪽 [작용이](https://ko.wikipedia.org/wiki/군의_작용 "wikilink") 주어졌다고 하자. 그렇다면, \(G\)는 \(X^2=X\times X\) 위에 다음과 같이 작용한다.
...
| 결합 도식    | [군의 작용](https://ko.wikipedia.org/wiki/군의_작용 "wikilink")                                    |
| -------- | ------------------------------------------------------------------------------------------ |
| 올        | \(X\) 위의 \(G\)-[작용의](https://ko.wikipedia.org/wiki/군의_작용 "wikilink") 궤도                    |
...
1952년에 [라지 찬드라 보스와](https://ko.wikipedia.org/wiki/라지_찬드라_보스 "wikilink") 시마모토(T. Shimamoto)가 [블록 설계의](../Page/블록_설계.md "wikilink") 이론에 대한 응용을 위해 결합 도식의 개념 및 용어를 도입하였다.\[21\] 보스와 시마모토의 논문에서, “결합 도식”()이라는 용어는 대칭 결합 대수를 뜻했다.

1959년에 [라지 찬드라 보스와](https://ko.wikipedia.org/wiki/라지_찬드라_보스 "wikilink") 데일 마시 메스너()는 보스-메스너 대수를 도입하였다.\[22\]
