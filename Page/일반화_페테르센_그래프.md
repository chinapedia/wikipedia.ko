...
[그래프 이론에서](https://ko.wikipedia.org/wiki/그래프_이론 "wikilink"), **일반화 페테르센 그래프**(一般化Petersen graph, )는 같은 수의 꼭짓점을 갖는 정다각형과 별 모양에서 대응하는 꼭짓점들을 이어 얻는 [그래프](https://ko.wikipedia.org/wiki/그래프 "wikilink")이다. 특히, 오각형과 오각 별을 이어 얻는 그래프를 **페테르센 그래프**()라고 한다.\[1\]
...
\[\operatorname E(\operatorname{GPG}(n,k))=\{u_i u_{i+1}, u_i v_i, v_i v_{i+k}\colon i\in\mathbb Z/n\}\] 여기서 첨자는 [법](https://ko.wikipedia.org/wiki/합동_산술 "wikilink") \(n\)으로 계산한다. 즉, \(u_{n+i}=u_i\) 및 \(v_{n+i}=u_i\)로 간주한다. 일반화 페테르센 그래프의 변 가운데, \(u_iv_i\) 꼴의 변을 **바큇살**()이라고 한다.
...
예를 들어, 나우루 그래프 \(\operatorname{GPG}(12,5)\)의 경우 \(5^2\equiv 1\pmod{12}\)이며, 이는 [대칭군](https://ko.wikipedia.org/wiki/대칭군_\(군론\) "wikilink") \(\operatorname{Sym}(4)\)의 [케일리 그래프이다](../Page/케일리_그래프.md "wikilink"). 마찬가지로, [각기둥](https://ko.wikipedia.org/wiki/각기둥 "wikilink") 그래프 \(\operatorname{GPG}(n,1)\)는 크기 \(2n\)의 [정이면체군](https://ko.wikipedia.org/wiki/정이면체군 "wikilink") \(\operatorname{Dih}(n)\)의 [케일리 그래프이다](../Page/케일리_그래프.md "wikilink"). 반면, 페테르센 그래프 \(\operatorname{GPG}(5,2)\)는 [케일리 그래프가](../Page/케일리_그래프.md "wikilink") 아니다.
...
일반화 페테르센 그래프 \(\operatorname{GPG}(9,2)\)는 (색들의 [순열](https://ko.wikipedia.org/wiki/순열 "wikilink")을 무시하면) 유일한 3색 [변 색칠을](../Page/변_색칠.md "wikilink") 갖는다.
...
[평면 그래프가](../Page/평면_그래프.md "wikilink") 아닌 모든 그래프는 [완전 그래프](https://ko.wikipedia.org/wiki/완전_그래프 "wikilink") \(K_5\) 또는 [완전 이분 그래프](../Page/완전_이분_그래프.md "wikilink") \(K_{3,3}\)를 [그래프 마이너로](../Page/그래프_마이너.md "wikilink") 가지는데, 페테르센 그래프 \(\operatorname{GPG}(5,2)\)는 이 둘 다를 [그래프 마이너로](../Page/그래프_마이너.md "wikilink") 갖는다.
...
일부 일반화 페테르센 그래프는 다음과 같은 특별한 이름을 갖는다.
  ...
  - **페테르센 그래프**는 일반화 페테르센 그래프 \(\operatorname{GPG}(5, 2)\)이다. 이는 [완전 그래프](https://ko.wikipedia.org/wiki/완전_그래프 "wikilink") \(K_5\)의 [선 그래프의](../Page/선_그래프.md "wikilink") [여 그래프와](https://ko.wikipedia.org/wiki/여_그래프 "wikilink") 동형이다.
  - **뒤러 그래프**()는 일반화 페테르센 그래프 \(\operatorname{GPG}(6,2)\)이다.
  - 일반화 페테르센 그래프 \(\operatorname{GPG}(10,2)\)는 [정십이면체](https://ko.wikipedia.org/wiki/정십이면체 "wikilink")의 꼭짓점 및 변들로 구성된 그래프와 같다.
...
Triangular prismatic graph.png|일반화 페테르센 그래프 \(\operatorname{GPG}(3, 1)\) Cubical graph.png|일반화 페테르센 그래프 (정육면체 그래프) \(\operatorname{GPG}(4, 1)\) Pentagonal prismatic graph.png|일반화 페테르센 그래프 \(\operatorname{GPG}(5, 1)\) Petersen graph.svg|페테르센 그래프 \(\operatorname{GPG}(5, 2)\) Hexagonal prismatic graph.png|일반화 페테르센 그래프 \(\operatorname{GPG}(6, 1)\) Dürer_graph.svg|뒤러 그래프 \(\operatorname{GPG}(6,2)\) Heptagonal prismatic graph.png|일반화 페테르센 그래프 \(\operatorname{GPG}(7, 1)\) Octagonal prismatic graph.png|일반화 페테르센 그래프 \(\operatorname{GPG}(8, 1)\) Dodecahedral graph.neato.svg|[정십이면체](https://ko.wikipedia.org/wiki/정십이면체 "wikilink") 그래프 \(\operatorname{GPG}(10,2)\) DesarguesGraph.svg|데자르그 그래프 \(\operatorname{GPG}(10,3)\) Independent set graph.svg|일반화 페테르센 그래프 \(\operatorname{GPG}(12,4)\) 파일:Symmetric group 4; Cayley graph 1,5,21 (Nauru Petersen); numbers.svg|나우루 그래프 \(\operatorname{GPG}(12,5)\)

## 역사

[섬네일의](https://ko.wikipedia.org/wiki/파일:Dürer_Melancholia_I.jpg "wikilink") 판화 《멜란콜리아 1》\]\] [섬네일](https://ko.wikipedia.org/wiki/파일:Desargues_theorem_ter.svg "wikilink") [섬네일](https://ko.wikipedia.org/wiki/파일:Flag_of_Nauru.svg "wikilink")\]\] 독일의 판화가 [알브레히트 뒤러의](https://ko.wikipedia.org/wiki/알브레히트_뒤러 "wikilink") 1514년 [판화](https://ko.wikipedia.org/wiki/판화 "wikilink") 《멜란콜리아 1》()에는 [다면체](https://ko.wikipedia.org/wiki/다면체 "wikilink")가 등장하는데, 그 꼭짓점과 변으로 구성된 [그래프](https://ko.wikipedia.org/wiki/그래프 "wikilink")는 뒤러 그래프이다.
...
“나우루 그래프”()라는 이름은 데이비드 아서 엡스타인()이 도입하였으며, 이 그래프의 구성에 등장하는 12각 별 모양이 [나우루의 국기에](https://ko.wikipedia.org/wiki/나우루의_국기 "wikilink") 등장하는 별과 흡사하기 때문에 붙여졌다.
