...
local data_module -- 단위를 지정하는 모듈의 이름

local function valerror(msg, nocat, iswarning)

`   -- 오류나 경고를 위해 형식 지정된 메세지를 반환한다.`
`   -- 문제를 찾기 위해 문서에 페이지의 URL에 "#FormattingError"를 추가할 수 있다.`
`   local anchor = '`<span id="FormattingError"></span>`'`
`   local body, category`
`   if nocat or mw.title.getCurrentTitle():inNamespaces(1, 2, 3, 5) then`
`       -- 토론, 사용자, 사용자_토론, 또는 위키백과_토론에 문서가 없을 때.`
`       category = ''`
`   else`
`       category = '`[`분류:잘못된``   ``틀``   ``사용을``   ``포함한``   ``문서`](https://ko.wikipedia.org/wiki/분류:잘못된_틀_사용을_포함한_문서 "wikilink")`'`
`   end`

\-- iswarning = false -- 문제가 잦지 않기 때문에 편집자가 알 수 있도록 큰 오류를 띄우려고 하는 것이다.
...
`           '{{`[`val`](https://ko.wikipedia.org/wiki/틀:val "wikilink")`}}에서 오류가 발생함: ' ..`
`           msg ..`
`           '`</strong>`'`
`   end`
...
`   -- 모든 출력에 개행금지가 적용되어 있기 때문에 ' '가 필요하다.`
`   [","]   = ", ",`
`   ["by"]  = " by ",`
`   ["-"]   = "–",`
`   ["–"]   = "–",`
...
`   -- arg에서 원소를 추출하고 그 결과를  numbers[index]에 저장한다.`
`   -- 만약 인자가 없거나, 유효하다면, nil (오류 없음)을 반환한다;`
`   -- 그렇지 않으면, 오류 메세지를 반환한다.`
`   -- 저장된 결과는 다음과 같다:`
`   -- * 숫자로 이루어진 표 (특정된 숫자가 없다면 비어있다); 또는`
`   -- * 범위를 지정하는 문자열`
`   -- 1e3과 같은 입력은 인수 1을 제외하고는 어느 곳에서든 유효하지 않은 것으로 간주된다.`
`   -- 왜냐하면 인수 1은 e 표기법을 인수 'e'로 입력된 것 처럼 받아들이기 때문이다.`
`   -- 입력의 쉼표는 생략되기 때문에 1,234는 1234와 동일하다.`
`   local which = index`
`   local function fail(msg)`
`       local description`
`       if which == 'e' then`
`           description = '지수 인수 (`<b>`e`</b>`)'`
`       else`
`           description = '인수 ' .. which`
`       end`
`       return description .. ' ' .. (msg or '은(는) 유효하지 않은 숫자입니다') .. '.'`
`   end`
`   local result = {}`
...
`                   return fail('은(는) 첫 번째 인수에 "e"가 포함되어 있을 경우 범위로 사용할 수 없습니다')`
`               end`
`               numbers.has_ranges = true`
`           else`
`               if not numbers.has_ranges then`
`                   return fail('은(는) 인수 2에서 범위가 필요합니다')`
`               end`
`           end`
`           numbers[index] = range`
...
`           return nil`
`       end`
`       return fail('은(는) 범위로 사용될 수 없습니다')`
`   end`
`   if numbers.has_ranges and type(index) == 'number' and (index % 2 == 0) then`
`       return fail('은(는) 범위여야 합니다')`
`   end`
`   if index == 'e' then`
`       local e = numbers[1] and numbers[1].exp`
`       if e then`
`           if arg then`
`               return fail('은(는) 첫 번째 인수에 "e"가 있을 경우 사용할 수 없습니다')`
`           end`
`           arg = e`
`           which = 1`
`       end`
...
`               result.exp = b`
`           else`
`               return fail('은(는) e 표기법으로 쓸 수 없습니다')`
`           end`
`       end`
...
`       return '마지막 인수 뒤에 범위가 될 숫자가 필요합니다.'`
`   end`
...
`       end`
`   end`
`   error('단위 "' .. ucode .. '" 은(는) 잘못된 단위입니다 "' .. text .. '"')`
...
`   local lookup = require('모듈:Convert')._unit`
`   return lookup(ucode, {`
`           value = value,`
...
`   else`
`       if want_link then`
`           result.text = '`[`'``   ``..``   ``result.symbol``   ``..``   ``'`](https://ko.wikipedia.org/wiki/'_.._result.link_.._' "wikilink")`'`
`       else`
...
`                   text = unit.symbol`
`               else`
`                   text = '`[`'``   ``..``   ``unit.display``   ``..``   ``'`](https://ko.wikipedia.org/wiki/'_.._unit.link_.._' "wikilink")`'`
`               end`
...
`')`
`           else`
`               add(line .. ' ◆ `<b>`invalid definition`</b>
`')`
`           end`
`       else`
`           add(line)`
...
local delimit_groups = require('모듈:Gapnum').groups local function delimit(sign, numstr, fmt)
...
`   data_module = '모듈:Val/단위' .. (options.sandbox and '/sandbox' or '')`
...
`       end`
`       return valerror('유효하지 않은 동작 "' .. action .. '".', options.nocat)`
`   end`
...
`           -- The 'if' avoids `` giving an invalid sort_value due to overflow.`
...
`   local args = getArgs(frame, {wrappers = { '틀:Val', '틀:Val/sandboxlua' }})`
...
`       return valerror('단위 (`<b>`u`</b>`)와 링크가 있는 단위 (`<b>`ul`</b>`)가 둘 다 지정되었습니다. 둘 중 하나만 사용할 수 있습니다.', nocat)`
`   end`
