...
**플라이웨이트 패턴**()는 동일하거나 유사한 객체들 사이에 가능한 많은 데이터를 서로 공유하여 사용하도록 하여 [메모리](https://ko.wikipedia.org/wiki/기억_장치 "wikilink") 사용량을 최소화하는 [소프트웨어](https://ko.wikipedia.org/wiki/컴퓨터_소프트웨어 "wikilink") [디자인 패턴이다](https://ko.wikipedia.org/wiki/디자인_패턴 "wikilink"). 종종 [오브젝트의](https://ko.wikipedia.org/wiki/객체_\(컴퓨터_과학\) "wikilink") 일부 상태 정보는 공유될 수 있는데, 플라이웨이트 패턴에서는 이와 같은 상태 정보를 외부 [자료 구조에](https://ko.wikipedia.org/wiki/자료_구조 "wikilink") 저장하여 플라이웨이트 오브젝트가 잠깐 동안 사용할 수 있도록 전달한다.
