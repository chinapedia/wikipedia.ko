...
`   local MINUS = '−'  -- Unicode U+2212 MINUS SIGN (may be in values from `[`Module:Convert`](https://ko.wikipedia.org/wiki/Module:Convert "wikilink")`)`
...
`           s = s .. '`[`'``   ``..``   ``group``   ``..``   ``'`](https://ko.wikipedia.org/wiki/Orders_of_magnitude_\(numbers\)#10'_.._group_num*3_.._' "wikilink")`'`
`       else`
`           s = s .. group`
`       end`
