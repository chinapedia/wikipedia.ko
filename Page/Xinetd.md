...
**xinetd**(*extended Internet daemon*)는 [오픈 소스](../Page/오픈_소스.md "wikilink") [슈퍼 서버](../Page/슈퍼_서버.md "wikilink") [데몬으로서](../Page/데몬_\(컴퓨팅\).md "wikilink") 많은 [유닉스 계열](../Page/유닉스_계열.md "wikilink") 시스템에서 돌아가며 인터넷 기반 연결을 관리한다.
...
xinetd의 기본 설정 파일은/etc/xinetd.conf이며, 이것이 제공하는 서비스들의 설정은 /etc/xinetd.d 디렉터리에 저장된다. 각 서비스들의 설정은 보통 xinetd가 서비스를 활성화할지 안할지에 대한 제어 변경을 포함한다.
