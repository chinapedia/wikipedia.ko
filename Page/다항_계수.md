...
[수학](https://ko.wikipedia.org/wiki/수학 "wikilink")에서, **다항 계수**(多項係數, )는 주어진 개수의 원소들을 주어진 크기의 상자들에 넣는 방법의 가짓수이다. **다항 정리**(多項定理, )는 [다항식](https://ko.wikipedia.org/wiki/다항식 "wikilink")의 거듭제곱을 전개하는 정리이며, 전개식의 계수는 다항 계수이다. 다항 계수와 다항 정리는 [이항 계수와](https://ko.wikipedia.org/wiki/이항_계수 "wikilink") [이항 정리의](https://ko.wikipedia.org/wiki/이항_정리 "wikilink") 일반화이다.

## 정의

음이 아닌 정수들의 합 \(n=k_1+k_2+\cdots+k_m\)이 주어졌을 때, **다항 계수** \(\textstyle\binom n{k_1,k_2,\dots,k_m}\)는 다음과 같다.

\[\binom n{k_1,k_2,\dots,k_m}=\frac{n!}{k_1!k_2!\cdots k_m!}=\prod_{i=1}^m\binom{k_1+k_2+\cdots+k_i}{k_i}=\prod_{i=1}^m\binom{k_i+k_{i+1}+\cdots+k_m}{k_i}\] 다항 계수를 [단체에](https://ko.wikipedia.org/wiki/단체_\(수학\) "wikilink") 나열한 표를 **[파스칼의 단체](https://ko.wikipedia.org/wiki/파스칼의_단체 "wikilink")**(Pascal의單體, )라고 한다.
...
다항 계수 \(\textstyle\binom n{k_1,k_2,\dots,k_m}\)은 조합론적으로 다음과 같이 해석할 수 있다.
  ...
  - [중복집합](https://ko.wikipedia.org/wiki/중복집합 "wikilink") \(k_1\{1\}+k_2\{2\}+\cdots+k_n\{n\}\)의 [순열](https://ko.wikipedia.org/wiki/순열 "wikilink")의 수
      - 즉, \(n\)글자 단어가 각각 \(k_i\)번 나오는 \(m\)가지 글자로 이루어졌을 때, 그 단어의 [어구전철](https://ko.wikipedia.org/wiki/어구전철 "wikilink")의 수
...
\[(x_1+x_2+\cdots+x_m)^n=\sum_{K\in\mathbb N^m}^{|K|=n}\binom nKx^K\] 전개식의 항의 개수는 다음과 같이 [이항 계수로](https://ko.wikipedia.org/wiki/이항_계수 "wikilink") 나타낼 수 있다.

\[\binom{n+m-1}{m-1}\]  [이항 정리와](https://ko.wikipedia.org/wiki/이항_정리 "wikilink") [수학적 귀납법을](https://ko.wikipedia.org/wiki/수학적_귀납법 "wikilink") 사용하여 증명할 수 있다. 우선, \(m=0,1\)의 경우는 자명하게 성립하며, \(m=2\)의 경우는 이항 정리에 따라 성립한다.
