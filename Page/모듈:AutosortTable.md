> This article is converted from Wikipedia: [모듈:AutosortTable](https://ko.wikipedia.org/wiki/모듈:AutosortTable).
...
| class = wikitable | style = width: 50%; | sep = -- | order = 2, 1 | nsort = 2 | header = -- Name -- Age

| -- Bob -- 20 | -- Peter -- 35 | -- John -- 35 | -- James -- 50 | background-color: \#FFDDDD -- Henry -- 45
...
`   --`[`Writes``   ``a``   ``row``   ``to``   ``the``   ``table.``   ``css:``   ``CSS``   ``to``   ``apply``   ``to``   ``the``   ``row.``   ``data:``   ``The``   ``data``   ``(cells)``   ``of``   ``the``   ``row``   ``_type:``   ``Can``   ``be``   ``'header',``   ``'footer'``   ``or``   ``nil.`](https://ko.wikipedia.org/wiki/Writes_a_row_to_the_table._css:_CSS_to_apply_to_the_row._data:_The_data_\(cells\)_of_the_row_type:_Can_be_'header',_'footer'_or_nil. "wikilink")
