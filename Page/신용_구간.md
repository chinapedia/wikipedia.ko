...
베이즈 통계에서의 **신용 구간**은, 값의 범위 내에는 관측 [매개 변수](https://ko.wikipedia.org/wiki/매개변수 "wikilink") 값으로 특정한 [주관적 확률입니다](https://ko.wikipedia.org/wiki/베이즈_확률론 "wikilink"). 그것은 간격에서 도메인의 후 확률 분포 또는 예측 배포합니다.\[1\] 의 일반화하여 다변량 문제를 **신뢰할 수있는 지역**입니다. 신뢰할 수있는 구간은 유사한 [신뢰 구간](https://ko.wikipedia.org/wiki/신뢰_구간 "wikilink") 에서 frequentist 통계,\[2\] 비록 그들과 다를 철학에 기초;\[3\] Bayesian 간격으로 치료를 그들의 범위 수정 및 예상 매개 변수로는 임의 변수는 반면,frequentist 신뢰 구간을 치료하는 그 경계를 임의로 변수를 매개 변수로 고정 값을 가집니다. 또한,베이지안 신뢰할 수 있는 간격을 사용(실제로는,필요로)기술의 특정 상황에 맞는 [사전에 배포하는](https://ko.wikipedia.org/wiki/사전_확률 "wikilink") 동안,frequentist 신뢰 간격지 않습니다.

예를 들어, 모수 \(\mu\)의 가능한 값의 분포를 정하는 실험에서 만약 \(\mu\)가 35와 45 사이에 있을 [주관적 확률이](https://ko.wikipedia.org/wiki/베이즈_확률론 "wikilink") 0.95라면,  95%신용 구간이 된다..\(\)
...
신뢰할 수 있는 간격지에서 고유한 뒤 배포합니다. 를 정의하는 방법은 적합한 신뢰할 수 있 간격을 포함한다:
  ...
  - 선택하는 간격을 확률은 아래되는 구간은 가능성으로되고있다. 이 간격이 포함됩니다 [중간입니다](https://ko.wikipedia.org/wiki/중앙값 "wikilink"). 이것은 때로는 **동일한 꼬리 간격**입니다.
