...
[섬네일는](https://ko.wikipedia.org/wiki/파일:MPEG_Transport_Stream_HL.svg "wikilink") TS를 디코딩하여 화면에 보여 준다. 세계 수많은 지역에서 하나 이상의 변종 모듈러 DVB 시스템을 통하여 전달이 이루어진다.\]\]  **MPEG 트랜스포트 스트림**(MPEG transport stream, TS, TP, MPEG-TS로 줄임)은 [오디오](https://ko.wikipedia.org/wiki/오디오 "wikilink"), [비디오](../Page/비디오.md "wikilink"), [데이터](https://ko.wikipedia.org/wiki/데이터 "wikilink") 전송을 위한 [통신 프로토콜이다](https://ko.wikipedia.org/wiki/통신_프로토콜 "wikilink"). PES(packetized elementary streams) 와 기타 데이터를 포함하는 디지털 컨테이너 포맷의 일종이다. TS는 [MPEG-2](https://ko.wikipedia.org/wiki/MPEG-2 "wikilink") 파트 1, 시스템([ISO](https://ko.wikipedia.org/wiki/국제_표준화_기구 "wikilink")/[IEC](https://ko.wikipedia.org/wiki/국제_전기_표준_회의 "wikilink") 표준 13818-1)에 규정되어 있다.\[1\] ITU-T Rec. H.222.0으로도 알려져 있다. 디지털 영상과 소리를 [다중화](https://ko.wikipedia.org/wiki/다중화 "wikilink")하고 출력을 동기화하는 것이 이 시스템의 목표이다. 트랜스포트 스트림은 신뢰할 수 없는 매체의 [오류 정정](https://ko.wikipedia.org/wiki/오류_정정 "wikilink") 기능을 제공하고 [DVB와](https://ko.wikipedia.org/wiki/디지털_비디오_방송 "wikilink") [ATSC](https://ko.wikipedia.org/wiki/ATSC "wikilink")와 같은 영상 응용에도 쓰인다. [DVD](https://ko.wikipedia.org/wiki/DVD "wikilink")와 같은 신뢰할 수 있는(데이터 손실 우려가 없는) 매체용으로 개발된 [MPEG 프로그램 스트림과는](../Page/MPEG_프로그램_스트림.md "wikilink") 대조된다.
...
6.  매크로블록 - 6\~12개의 [DCT](https://ko.wikipedia.org/wiki/이산_코사인_변환 "wikilink") 블록으로 이루어져 있음
7.  인코딩 블록 / 단순 블록 - [DCT](https://ko.wikipedia.org/wiki/이산_코사인_변환 "wikilink") 인코딩 블록, 8x8 화소
...
<tr class="header">
<th><p>이름</p></th>
<th><p><a href="https://ko.wikipedia.org/wiki/비트_(단위)" title="wikilink">비트</a> 수</p></th>
...
TS의 각 테이블이나 기초 스트림(ES)은 13비트 패킷 ID (PID)로 식별한다. 역다중화기(demultiplexer)는 같은 PID로 식별된 패킷을 검색하여 부분적으로 TS로부터 기초 스트림을 추출한다. [시분할 다중화는](https://ko.wikipedia.org/wiki/시분할_다중화 "wikilink") 얼마나 자주 특정한 PID가 TS에 나타나는지를 결정하는 데 쓰인다.
...
### 다중 운영 체제
  ...
  - 비디오랜 [VLC 미디어 플레이어](https://ko.wikipedia.org/wiki/VLC_미디어_플레이어 "wikilink")\[9\]
