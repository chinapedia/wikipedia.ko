...
</ref> | latest release version = 5.04 | operating_system = [크로스 플랫폼](https://ko.wikipedia.org/wiki/크로스_플랫폼 "wikilink") | language = [다국어](http://www.bugzilla.org/download/#localizations) | programming language = [펄](https://ko.wikipedia.org/wiki/펄 "wikilink") | genre = [버그 추적 시스템](../Page/버그_추적_시스템.md "wikilink") | license = [모질라 공용 허가서](../Page/모질라_공용_허가서.md "wikilink") | website = <http://www.bugzilla.org/> }}
...
1998년에 [넷스케이프 커뮤니케이션스가](https://ko.wikipedia.org/wiki/넷스케이프_커뮤니케이션스 "wikilink") [오픈 소스 소프트웨어로](../Page/오픈_소스_소프트웨어.md "wikilink") 공개하였으며 다양한 단체가 [버그 추적 시스템으로](../Page/버그_추적_시스템.md "wikilink") 채택하였다. 버그질라는 [자유 및 오픈 소스 소프트웨어와](https://ko.wikipedia.org/wiki/자유_및_오픈_소스_소프트웨어 "wikilink") [사유의](https://ko.wikipedia.org/wiki/사유_소프트웨어 "wikilink") 프로젝트 및 제품 전반에 모두에 쓰인다.

## 역사

버그질라는 테리(Terry Weissman)가 1998년에 [Mozilla.org](https://ko.wikipedia.org/wiki/모질라_재단 "wikilink") 프로젝트로 작성한 것이다. 원래 [Tcl](../Page/Tcl.md "wikilink")로 작성하였다가, 넷스케이프 초기 오픈 소스 코드의 일부가 떨어져나가면서 테리가 버그질라를 [펄](https://ko.wikipedia.org/wiki/펄 "wikilink")로 포팅하기로 결정하였다. 당시 펄이 더 유명한 언어가 될 것으로 보았고 사람들이 이를 계속 사용하면 좋겠다고 생각했다.\[1\]

버그질라 2.0은 펄로 포팅한 뒤의 결과물이며 익명 [CVS](https://ko.wikipedia.org/wiki/CVS "wikilink")를 통하여 대중에게 공개된 첫 버전이다.
...
<timeline> Define $now = 23/02/2018 Define $width = 556 Define $warning = 436 \# $width – 120 ImageSize = width:800 height:500 PlotArea = left:40 right:10 bottom:100 top:10 DateFormat = dd/mm/yyyy Period = from:01/01/1998 till:$now TimeAxis = orientation:hor Alignbars = justify Legend = orientation:vertical position:bottom columns:1
...
버그질라의 시스템 요구 사항은 다음과 같다:
  ...
  - [펄](https://ko.wikipedia.org/wiki/펄 "wikilink") 5
  - 펄 모듈
...
## 외부 링크
  ...
  - [버그질라 웹사이트](http://www.bugzilla.org/)
