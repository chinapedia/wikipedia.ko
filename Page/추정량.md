...
[통계학](https://ko.wikipedia.org/wiki/통계학 "wikilink")에서, **추정량**(推定量, )은 표집값들로부터 모수의 값을 추정하는 방법이다.

## 정의

[확률변수](https://ko.wikipedia.org/wiki/확률변수 "wikilink") \(X\colon P\to\mathcal X\)가 모수 \(\theta\in\Theta\)를 가지는 분포를 따른다고 하자. 그렇다면 모수 \(\theta\)의 **추정량** \(\hat\theta\colon\mathcal X\to\Theta\)은 임의의 [가측 함수이다](https://ko.wikipedia.org/wiki/가측_함수 "wikilink").

모수 공간 \(\Theta\)와 [표본 공간](https://ko.wikipedia.org/wiki/표본_공간 "wikilink") \(\mathcal X\) 둘 다 [유클리드 공간의](https://ko.wikipedia.org/wiki/유클리드_공간 "wikilink") 부분공간으로 간주하자.

### 오차와 편향

표본 \(x\in\mathcal X\)에 대한, 모수 \(\theta\)의 추정량 \(\hat\theta\)의 **오차**()는 다음과 같다.

\[\mathcal E_{\hat\theta}(x)=\hat\theta(x)-\theta\] 모수 \(\theta\)의 추정량 \(\hat\theta\)의 **편향**()은 그 오차의 [기댓값](https://ko.wikipedia.org/wiki/기댓값 "wikilink")이다.

\[B(\hat\theta) = \operatorname{E}(\hat\theta(X)-\theta)\] 모수 \(\theta\)의 **불편추정량**() \(\hat\theta\)은 편향이 0인 추정량이다. 즉, 다음 성질을 만족시키는 추정량이다.

\[\operatorname E(\hat\theta(X))=\theta\] 추정량 \(\hat\theta\)의 **누적평균제곱오차**()는 오차의 제곱들의 [기댓값](https://ko.wikipedia.org/wiki/기댓값 "wikilink")이다.
...
\[d(x) =\hat\theta(x) - \operatorname{E}(\hat\theta(X) )=\hat\theta(x) - \operatorname{E}(\hat\theta)\] 모수 \(\theta\)의 추정량 \(\hat\theta\)의 **분산**()은 표본편차의 제곱의 [기댓값](https://ko.wikipedia.org/wiki/기댓값 "wikilink")이다.
