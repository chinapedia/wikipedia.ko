...
**할당자**(割當子) 또는 **얼로케이터**()는 [C++](https://ko.wikipedia.org/wiki/C++ "wikilink")의 [표준 라이브러리에서](https://ko.wikipedia.org/wiki/표준_라이브러리 "wikilink") 메모리 할당을 관리하는 객체이다. C++표준 라이브러리에서 이용되는 [리스트](https://ko.wikipedia.org/wiki/리스트_\(컴퓨팅\) "wikilink"), [셋](https://ko.wikipedia.org/wiki/셋 "wikilink") 등의 다양한 [컨테이너](https://ko.wikipedia.org/wiki/컨테이너 "wikilink") [자료구조](https://ko.wikipedia.org/wiki/자료구조 "wikilink")들은 용도에 따라 [동적 메모리 할당을](https://ko.wikipedia.org/wiki/동적_메모리_할당 "wikilink") 요구한다. **할당자**는 동적 메모리 할당을 통합 관리하는데 필요한 기능이 정의된 객체이다.
