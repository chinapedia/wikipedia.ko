> This article is converted from Wikipedia: [모듈:Duration](https://ko.wikipedia.org/wiki/모듈:Duration).
...
`   return '`<strong class="error">`Error in Module:Duration: ' .. error_str .. '`</strong>`'`
...
`   `
`   if tmp and tmp ~= '' then`
`       if duration ~= '' then tmp = mw.ustring.gsub(tmp, '%d*:?%d+:%d+%.?%d*', duration, 1) else tmp = tmp .. ' ' end`
`   else`
...
[Category:Duration_with_input_error](https://ko.wikipedia.org/wiki/Category:Duration_with_input_error "wikilink") [Category:Duration_without_hAudio_microformat](https://ko.wikipedia.org/wiki/Category:Duration_without_hAudio_microformat "wikilink")