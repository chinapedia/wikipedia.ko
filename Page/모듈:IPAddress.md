...
Only [dotted decimal](https://ko.wikipedia.org/wiki/dotted_decimal "wikilink") notation for IPv4 supported. Does not support dotted hexadecimal, dotted octal, or single-number formats (see [IPv4\#Address_representations](https://ko.wikipedia.org/wiki/IPv4#Address_representations "wikilink")).

Unit tests at [en:Module:IPAddress/tests](https://ko.wikipedia.org/wiki/en:Module:IPAddress/tests "wikilink") \]=\]

function _isIpV6( s )

`   local dcolon, groups`

`   s = string.gsub(mw.text.trim(s, "%s"), "/(%d+)$", "") -- trim spaces and CIDR range`
...
`   s = string.gsub(mw.text.trim(s, "%s"), "/(%d+)$", "")`
