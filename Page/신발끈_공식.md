...
**신발끈 공식**(―公式)은 [좌표평면](https://ko.wikipedia.org/wiki/좌표평면 "wikilink") 상에서 꼭짓점의 좌표를 알 때 [다각형](https://ko.wikipedia.org/wiki/다각형 "wikilink")의 [면적](https://ko.wikipedia.org/wiki/면적 "wikilink")을 구할 수 있는 방법이다. 다각형의 각 꼭짓점의 좌푯값을 교차하여 곱하는 모습이 [신발끈](https://ko.wikipedia.org/wiki/신발끈 "wikilink")을 묶을 때와 같아 이러한 이름이 붙었다.\[1\] **[가우스의](https://ko.wikipedia.org/wiki/카를_프리드리히_가우스 "wikilink") 면적 공식**이나 **사선 공식**(斜線 公式)으로도 불린다. 이 공식은 측량이나 임업과 같은 여러 분야에도 응용될 수 있다.\[2\]

신발끈 공식은 1769년에 수학자 마이스터 알베르트 루드비히 프레드리히(Meister Albrecht Ludwig Friedrich, 1724-1788)가 발견했으며\[3\], 1795년에 가우스도 독립적으로 발견하였다. 이 공식은 다각형을 여러 개의 [삼각형](https://ko.wikipedia.org/wiki/삼각형 "wikilink")으로 나누는 방식으로 증명할 수 있으며, [그린 정리의](https://ko.wikipedia.org/wiki/그린_정리 "wikilink") 특수한 형태로 볼 수도 있다.

이 공식은 다각형에서 각각의 모서리마다 임의의 선분 AB를 잡고, 원점 O를 꼭짓점으로 하는 삼각형 ABO의 넓이를 계산하여, 평행사변형의 넓이와 같은 [벡터곱](https://ko.wikipedia.org/wiki/벡터곱 "wikilink")을 구하고 2로 나눔으로써 유도된다. 다각형 주변을 감싸게 되면, 양의 면적과 음의 면적을 지니는 이 삼각형들은 겹쳐지고 원점과 다각형 사이의 면적이 상쇄되어 0이 되는데, 이때 기준으로 한 삼각형의 면적만은 남게 된다. 원점에서 바라볼 때 반시계 방향으로 돈다면, 왼쪽에서 오른쪽으로 가면서 양의 면적이 더해지고, 오른쪽에서 왼쪽으로 가면서 음의 면적이 더해진다.
...
꼭짓점들에 반시계방향으로 차례로 번호를 매긴다면, 위의 [행렬식](https://ko.wikipedia.org/wiki/행렬식 "wikilink")은 양의 값을 지니므로 [절댓값](https://ko.wikipedia.org/wiki/절댓값 "wikilink") 부호를 생략할 수 있다.\[7\] 번호가 시계방향으로 차례로 매겨진다면 행렬식은 음의 값을 지니게 된다. 이는 신발끈 공식을 [그린 정리의](https://ko.wikipedia.org/wiki/그린_정리 "wikilink") 특수한 경우로 볼 수 있기 때문이다.

## 예시

이 공식으로 다각형의 넓이를 구하기 위해서는 [좌표평면](https://ko.wikipedia.org/wiki/직교_좌표계 "wikilink") 위에서의 좌푯값을 알아야 한다.

첫 번째 \(x\)좌표를 두 번째 \(y\)좌표와 곱하고, 두 번째 \(x\)좌표를 세 번째 \(y\)좌표와 곱하고, 세 번째 \(x\)좌표를 첫 번째 \(y\)좌표와 곱해 모두 더한 값과, 첫 번째 \(y\)좌표를 두 번째 \(x\)좌표와 곱하고, 두 번째 \(y\)좌표를 세 번째 \(x\)좌표와 곱하고, 세 번째 \(y\)좌표를 첫 번째 \(x\)좌표와 곱한 값을 모두 더한 값의 차를 구해 \(2\)로 나누면 삼각형의 넓이는 다음과 같은 공식으로 나타내어진다.\[8\]
...
\(i=1,2,3,4\)일 때 [사각형](https://ko.wikipedia.org/wiki/사각형 "wikilink")의 넓이는 아래와 같이 구한다.

  -
    \(\mathbf{A}_\text{4} = {1 \over 2}|x_1y_2 + x_2y_3 +x_3y_4 + x_4y_1 - x_2y_1 - x_3y_2 - x_4y_3 - x_1y_4|\)

\(i=1,2,\cdots,5\)일 때 [오각형](https://ko.wikipedia.org/wiki/오각형 "wikilink")의 넓이는 아래와 같이 구한다.
...
[thumb](https://ko.wikipedia.org/wiki/파일:polygon_area_formula_\(English\).svg "wikilink")
...
이 공식은 보통 [행렬](https://ko.wikipedia.org/wiki/행렬 "wikilink")의 형태를 이용하여 쓰이는데, 그 꼴이 신발끈을 묶는 모양과 비슷하여 신발끈 공식이라는 이름이 붙었다. 꼭짓점의 좌표가 차례로 \((2,4), (3,-8), (1,2)\)인 삼각형을 생각하자. 각 꼭짓점의 좌푯값을 \((2,4)\)에서부터 시작해 차례대로 적고, 그 아래에 다시 \((2,4)\)를 적어 행렬의 꼴로 나타내면 다음과 같다.\[9\]
...
마찬가지로 이어진 두 수를 곱해 모두 더한 값은 \(4\times3+(-8)\times1+2\times2=8\)이다. 여기서 두 값의 차의 절댓값에 \(\frac{1}{2}\)을 곱하면 \(\frac{1}{2}\left| -6-8 \right|=7\)이므로 이 삼각형의 넓이는 7이다.

이렇게 숫자를 모두 적어 직선으로 이은 꼴이 신발끈이 묶인 신발의 모양과 같아, 신발끈 공식이라는 이름이 붙었다.
