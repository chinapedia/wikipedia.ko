...
[섬네일](https://ko.wikipedia.org/wiki/파일:Mvt2.svg "wikilink")  [미적분학](https://ko.wikipedia.org/wiki/미적분학 "wikilink")에서, **평균값 정리**(平均-定理, )는 대략 구간에 정의된 [함수](https://ko.wikipedia.org/wiki/함수 "wikilink")는 평균 변화율과 같은 순간 변화율을 갖는다는 정리이다. [기하학](https://ko.wikipedia.org/wiki/기하학 "wikilink")적 관점에서, 이는 곡선이 두 끝점을 잇는 선과 평행하는 접선을 갖는다는 것과 같다.\[1\] [롤의 정리로부터](https://ko.wikipedia.org/wiki/롤의_정리 "wikilink") 유도되며, [테일러 정리를](../Page/테일러_정리.md "wikilink") 비롯한 많은 확장이 존재한다. [미적분학의 기본 정리를](https://ko.wikipedia.org/wiki/미적분학의_기본_정리 "wikilink") 증명하는 데 쓰이며, [극값](../Page/극값.md "wikilink") · [고계 도함수](https://ko.wikipedia.org/wiki/고계_도함수 "wikilink") · [볼록 함수](https://ko.wikipedia.org/wiki/볼록_함수 "wikilink") · [역함수](https://ko.wikipedia.org/wiki/역함수 "wikilink")의 취급에도 응용된다.

## 정의

### 롤의 정리

함수 \(f\colon[a,b]\to\mathbb R\)가 \([a,b]\)에서 [연속 함수](https://ko.wikipedia.org/wiki/연속_함수 "wikilink"), \((a,b)\)에서 [미분 가능 함수라고](https://ko.wikipedia.org/wiki/미분_가능_함수 "wikilink") 하자. 또한, \(f(a)=f(b)\)라고 하자. 그렇다면, \(f'(c)=0\)인 \(c\in(a,b)\)가 존재한다. 이를 **롤의 정리**라고 한다.

### 평균값 정리

함수 \(f\colon[a,b]\to\mathbb R\)가 \([a,b]\)에서 [연속 함수](https://ko.wikipedia.org/wiki/연속_함수 "wikilink"), \((a,b)\)에서 [미분 가능 함수라고](https://ko.wikipedia.org/wiki/미분_가능_함수 "wikilink") 하자. 그렇다면, 다음을 만족시키는 \(c\in(a,b)\)가 적어도 하나 존재한다.\[2\]\[3\]

\[f'(c)=\frac{f(b)-f(a)}{b-a}\] 이를 **평균값 정리**라고 한다. 롤의 정리는 평균값 정리에서 \(f(a)=f(b)\)인 특수한 경우이다.  함수 \(g\colon[a,b]\to\mathbb R\)를 다음과 같이 정의하자.

\[g(x)=f(x)-f(a)-\frac{f(b)-f(a)}{b-a}(x-a)\] 그렇다면, \(g\)는 [롤의 정리의](https://ko.wikipedia.org/wiki/롤의_정리 "wikilink") 조건을 다음과 같이 만족시킨다.
...
[섬네일](https://ko.wikipedia.org/wiki/파일:Cauchy.svg "wikilink") 함수 \(f,g\colon[a,b]\to\mathbb R\)가 \([a,b]\)에서 [연속 함수](https://ko.wikipedia.org/wiki/연속_함수 "wikilink"), \((a,b)\)에서 [미분 가능 함수라고](https://ko.wikipedia.org/wiki/미분_가능_함수 "wikilink") 하자. 또한, \(g'\ne0\)라고 하자. 그렇다면, 다음을 만족시키는 \(c\in(a,b)\)가 존재한다.

\[\frac{f'(c)}{g'(c)}=\frac{f(b)-f(a)}{g(b)-g(a)}\] 이를 **코시 평균값 정리**() 또는 **확장된 평균값 정리**()라고 한다. 평균값 정리는 코시 평균값 정리에서 \(g(x)=x\)인 특수한 경우이다.

기하학적 관점에서, 코시 평균값 정리는 평균값 정리를 함수의 그래프에서 미분 가능 [단순 곡선까지](https://ko.wikipedia.org/wiki/단순_곡선 "wikilink") 확장시킨 결과이다. 그러나 곡선의 [임계점이](https://ko.wikipedia.org/wiki/임계점_\(수학\) "wikilink") 존재하지 않는다는 조건을 제거하면 반례가 존재한다. 예를 들어, 곡선 \([-1,1]\to\mathbb R^2\), \(t\mapsto(t^3,1-t^2)\)의 경우, 양 끝점 \((-1,0),(1,0)\)의 연결선은 수평선이지만, 수평 접선은 존재하지 않는다.  함수 \(h\colon[a,b]\to\mathbb R\)를 다음과 같이 정의하자.

\[h(x)=f(x)-f(a)-\frac{f(b)-f(a)}{g(b)-g(a)}(g(x)-g(a))\] 그렇다면, \(h\)는 [롤의 정리의](https://ko.wikipedia.org/wiki/롤의_정리 "wikilink") 조건을 다음과 같이 만족시킨다.
...
함수 \(f,g,h\colon[a,b]\to\mathbb R\)가 \([a,b]\)에서 [연속 함수](https://ko.wikipedia.org/wiki/연속_함수 "wikilink"), \((a,b)\)에서 [미분 가능 함수라고](https://ko.wikipedia.org/wiki/미분_가능_함수 "wikilink") 하자. 그렇다면, 다음을 만족시키는 \(x\in(a,b)\)가 존재한다.
...
\[\min f\le\frac{\int_a^bf(x)g(x)dx}{\int_a^bg(x)dx}\le\max f\] 이므로, [중간값 정리에](https://ko.wikipedia.org/wiki/중간값_정리 "wikilink") 따라 성립한다. 추가로 \(g\)가 연속 함수라면, 엄격 부등식이 성립함을 보일 수 있으므로, \(c\)를 \((a,b)\)에서 취할 수 있다.
...
\[h(b) \le \int_a^b f(x)g(x)dx \le h(a)\] 와 같다. \(h(t)\)는 [미적분학의 기본정리에](https://ko.wikipedia.org/wiki/미적분학의_기본정리 "wikilink") 의하여 연속함수이다. 따라서 여기에 중간값 정리를 적용하면 증명이 끝난다.

### 복소 적분 형태

[복소평면](../Page/복소평면.md "wikilink") 상에서 어떤 점 \(z_0\)을 중심으로 하는 반지름 \(r\)인 [원](https://ko.wikipedia.org/wiki/원_\(기하학\) "wikilink") 내에서 [정칙인](https://ko.wikipedia.org/wiki/정칙함수 "wikilink") 함수 \(f\)에 대하여,
...
다음은 평균값 정리로부터 간단히 유도되는 몇 가지 명제들이다.
  ...
  - 구간 \(I\)에 정의된 실수값함수 \(f\)가 만약 \(I\)에서 연속, \(I\)의 [내부에서](https://ko.wikipedia.org/wiki/내부_\(위상수학\) "wikilink") 미분 가능하며 항상 \(f'(x) = 0\)이라면, \(f\)는 \(I\)에서 [상수함수](https://ko.wikipedia.org/wiki/상수함수 "wikilink")이다.
...
이 정리의 최초의 입안자는 [인도](https://ko.wikipedia.org/wiki/인도 "wikilink")의 [바타세리 파라메슈바라](https://ko.wikipedia.org/wiki/바타세리_파라메슈바라 "wikilink")(Vatasseri Parameshvara)로 기록되어 있으며\[4\] 처음으로 공식화한 사람은 [오귀스탱 루이 코시이다](https://ko.wikipedia.org/wiki/오귀스탱_루이_코시 "wikilink").
